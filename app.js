...
const APP_CACHE_VERSION = "v82-finance-reference-error-bottom-menu-fix-20260628";
...
(function installFinanceCleanV77(){
  window.APP_UI_VERSION_V77 = 'v82-finance-reference-error-bottom-menu-fix-20260628';
  ...
  function fRequireAdmin(){
    if (fAdminMode()) return true;
    if (fAdminUser()) alert('Финансовый помощник открыт только в режиме администрирования. Включите администрирование вручную в меню.');
    else alert('Финансовый помощник пока закрыт для учеников.');
  ...
  function slideError(slide){
    const t = (slide.title + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (t.includes('аванс')) return 'Считать полученный аванс свободной прибылью и не видеть обязательство перед клиентом.';
    if (t.includes('кредит')) return 'Называть кредит доходом и принимать долг за результат бизнеса.';
    if (t.includes('дебитор')) return 'Считать признанную выручку деньгами, хотя оплата ещё не поступила.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Списывать долгосрочный актив как обычный расход одного месяца.';
    if (t.includes('бухгалтер')) return 'Использовать бухгалтерию как единственную систему управления бизнесом.';
    if (t.includes('выручк')) return 'Путать факт оплаты с фактом заработанной выручки.';
    return 'Делать вывод по одной цифре без проверки ОПиУ, ОДДС и баланса.';
  }
  function slideConclusion(slide){
    const t = (slide.title + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (t.includes('аванс')) return 'Авансы нужно отдельно контролировать в деньгах и обязательствах, а выручку признавать по мере оказания услуги.';
    if (t.includes('кредит')) return 'Кредит читается через финансовый поток, долг и будущую нагрузку, а не через выручку.';
    if (t.includes('дебитор')) return 'Дебиторка объясняет, почему бизнес мог заработать, но деньги ещё не пришли.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Крупная покупка должна быть разделена на платёж, актив баланса и будущую амортизацию.';
    if (t.includes('бухгалтер')) return 'Бухгалтерский, налоговый и управленческий контуры нужно связывать, но не смешивать.';
    return 'Сначала определяется финансовый смысл операции, затем её влияние на ОПиУ, ОДДС и баланс.';
  }
  function slideHtml(slide, number, totalSlides, index, totalScreens){
    const paragraphs = (slide.text || []).map(p => `<p>${fEsc(p)}</p>`).join('') || '<p>Текст слайда будет добавлен после редакторской проверки.</p>';
    return fCard('finance-slide-card-v77', `<p class="eyebrow">слайд ${number} из ${totalSlides}</p><h1>${fEsc(slide.title)}</h1><div class="finance-image-placeholder-v53"><div><b>Место для изображения</b><p>Изображение будет подключено после генерации визуалов.</p></div></div><div class="finance-main-thought-v77"><span>Главная мысль</span><p>${fEsc(slideMainThought(slide))}</p></div><div class="finance-slide-body-v77"><h2>Объяснение</h2>${paragraphs}</div><div class="finance-callouts-v77"><div><span>Типовая ошибка</span><p>${fEsc(slideError(slide))}</p></div><div><span>Управленческий вывод</span><p>${fEsc(slideConclusion(slide))}</p></div></div><p class="small">Экран ${index + 1} из ${totalScreens}</p>`);
  }
  ...
  function renderFinancialAssistantV77(){
    ...
    fShell(`${fCard('blue-card-v2 finance-hero-v77', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения.</p>`)}${fCard('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v49"><button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHomeV77()"><b>Финансовый модуль</b><p>11 разделов финансовой архитектуры бизнеса.</p><em>открыть</em></button><button class="finance-hub-card-v49" onclick="renderFinanceAnalyticsV77()"><b>Моя аналитика</b><p>Будущий контур личных показателей и управленческих выводов.</p><em>каркас</em></button><button class="finance-hub-card-v49" onclick="renderFinanceTemplatesV77()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и финансовых инструментов.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← На главную</button>`)}`, 'finance');
  }
  ...
  function lessonRow(sec, les){
    const hasFull = Array.isArray(les.fullContent) && les.fullContent.length;
    const note = hasFull ? 'полный текст' : 'структура урока';
    return `<button class="lesson-row-v2 finance-lesson-row-v77" onclick="renderFinanceLessonV77(${Number(sec.id)}, ${Number(les.id)}, 0)"><div><b>${String(les.id).padStart(2,'0')}. ${fEsc(les.title)}</b><p>${note}</p></div><span>→</span></button>`;
  }
  ...
  function renderFinanceLessonV77(sectionId, lessonId, screenIndex){
    ...
    if (current.type === 'slide') body = slideHtml(current.slide, current.i + 1, parsed.slides.length, index, total);
  ...
  function renderFinanceAnalyticsV77(){ if (!fRequireAdmin()) return; fShell(`${fCard('blue-card-v2 finance-hero-v77', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Раздел пока оставлен как каркас.</p>`)}${fCard('', `<button class="btn secondary" onclick="renderFinancialAssistantV77()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function renderFinanceTemplatesV77(){ if (!fRequireAdmin()) return; fShell(`${fCard('blue-card-v2 finance-hero-v77', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и финансовых инструментов.</p>`)}${fCard('', `<button class="btn secondary" onclick="renderFinancialAssistantV77()">← К финансовому помощнику</button>`)}`, 'finance'); }

  function financeHomeCardV77(){
    const open = fAdminMode();
  ...
  window.myBusinessCardHtml = function(){
    if (fAdminMode()) return fCard('my-business-card', `<p class="eyebrow">мой бизнес</p><h2>Финансовый помощник</h2><p>Финансовый модуль, аналитика и готовые шаблоны.</p><button class="btn primary" onclick="renderFinancialAssistantV77()">Открыть финансовый помощник</button>`);
    return fCard('my-business-card my-business-card-locked', `<p class="eyebrow">мой бизнес</p><div class="my-business-card-head"><h2>Финансовый помощник</h2><span>закрыто</span></div><p class="small">Раздел пока закрыт для учеников.</p>`);
  };
...
(function installFinanceV81NavigationFix(){
  window.APP_UI_VERSION_V81 = 'v82-finance-reference-error-bottom-menu-fix-20260628';
  ...
  window.openFinanceAssistantV81 = function(){
    ...
    if (!adminModeV81()) {
      alert('Финансовый помощник пока закрыт для учеников. Для проверки включите режим администрирования вручную.');
      return;
    }
  ...
  window.bottomNav = function(active){
    ...
    const learningIcon = typeof architectureNavIconV35 === 'function' ? `<span class="arch-nav-icon">${architectureNavIconV35('learning')}</span>` : '<span>▣</span>';
    const homeworkIcon = typeof architectureNavIconV35 === 'function' ? `<span class="arch-nav-icon">${architectureNavIconV35('homework')}</span>` : '<span>✓</span>';
    ...
    const financeButton = adminModeV81()
      ? `<button class="bottom-item ${active==='finance'?'active':''}" onclick="openFinanceAssistantV81()">${financeIcon}<b>Финансы</b></button>`
      : `<button class="bottom-item is-disabled-v41" type="button" disabled aria-disabled="true" title="Раздел закрыт для учеников">${financeIcon}<b>Финансы</b><small>закрыто</small></button>`;
    return `<nav class="bottom-nav-v2 bottom-nav-v2-five v41-bottom-nav finance-bottom-nav-v81" aria-label="Основное меню">
      ${navItem('home','Главная','renderHome',homeIcon)}
      ${navItem('learning','Уроки','renderLearning',learningIcon)}
      ${financeButton}
      ${navItem('homework','ДЗ','renderHomeworkCenter',homeworkIcon)}
  ...
  function financeHomeCardV81(){
    const open = adminModeV81();
    return mainCardV81('Финансовый помощник','Финансовый модуль, аналитика и готовые шаблоны.', open ? 'доступно администратору' : 'закрыто', open ? 'openFinanceAssistantV81()' : '', (open ? 'active' : 'soon') + ' main-block-card v40-primary-card finance-home-card-v81');
  }
  window.primaryRoutesHtmlV40 = function(){
  ...
  function ensureFinanceDrawerItemV81(){
    ...
    btn.className = open ? 'drawer-finance-v81' : 'drawer-finance-v81 student-locked-v41';
    if (open) btn.setAttribute('onclick', 'closeAppDrawerV40(); openFinanceAssistantV81()');
    else { btn.disabled = true; btn.setAttribute('aria-disabled','true'); }
    btn.innerHTML = `<span class="app-drawer-number-v40">00</span><span class="app-drawer-copy-v40"><b>Финансовый помощник</b><small>${open ? 'доступно администратору' : 'закрыто'}</small></span><span class="app-drawer-arrow-v40">${open ? '›' : '•'}</span>`;
  ...
  window.myBusinessCardHtml = function(){
    if (adminModeV81()) return card('my-business-card', `<p class="eyebrow">мой бизнес</p><h2>Финансовый помощник</h2><p>Финансовый модуль, аналитика и готовые шаблоны.</p><button class="btn primary" onclick="openFinanceAssistantV81()">Открыть финансовый помощник</button>`);
    return card('my-business-card my-business-card-locked', `<p class="eyebrow">мой бизнес</p><div class="my-business-card-head"><h2>Финансовый помощник</h2><span>закрыто</span></div><p class="small">Раздел пока закрыт для учеников.</p>`);
  };
})();
...
(function installFinanceDrawerV82(){
  window.APP_UI_VERSION_V82 = 'v82-finance-drawer-binding-20260628';
  function isAdminModeV82(){ return typeof isAdminMode === 'function' && isAdminMode(); }
  function openFinanceV82(){
    if (!isAdminModeV82()) { alert('Финансовый помощник закрыт в режиме просмотра ученика. Включите администрирование вручную.'); return; }
    if (typeof window.renderFinancialAssistantV77 === 'function') return window.renderFinancialAssistantV77();
    if (typeof window.renderFinancialAssistant === 'function') return window.renderFinancialAssistant();
  ...
  function injectFinanceDrawerV82(){
    ...
    var open = isAdminModeV82();
    var btn = document.createElement('button');
    btn.className = open ? 'drawer-finance-v82' : 'drawer-finance-v82 student-locked-v41';
    if (open) btn.setAttribute('onclick', 'openFinanceFromDrawerV82()');
    else { btn.disabled = true; btn.setAttribute('aria-disabled','true'); }
    btn.innerHTML = '<span class="app-drawer-number-v40">00</span>'
      + '<span class="app-drawer-copy-v40"><b>Финансовый помощник</b><small>' + (open ? 'доступно администратору' : 'закрыто') + '</small></span>'
      + '<span class="app-drawer-arrow-v40">' + (open ? '›' : '•') + '</span>';
    list.appendChild(btn);
