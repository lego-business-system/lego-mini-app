...
/* =====================================================
   v110 — Дополнительные материалы: прямое внедрение двух уроков
   Важно: это не слой-заглушка, а основная рабочая функция renderAdditionalMaterials.
   ===================================================== */
const ADDITIONAL_MATERIALS_VERSION_V110 = "v110-additional-materials-direct-20260704";
const ADD_BEP_CODE_V110 = "ADD-BEP-01";
const ADD_BEP_TABLE_URL_V110 = "https://docs.google.com/spreadsheets/d/1z3rPU1YJhmfaGKZcXnKh4cEepwdmOP19/edit?gid=1853890209#gid=1853890209";
const ADD_BEP_SLIDES_V110 = [
  {
    "number": 1,
...
const ADD_EQ_CODE_V110 = "ADD-EQ-01";
const ADD_EQ_TABLE_URL_V110 = "https://docs.google.com/spreadsheets/d/14NophgCw5e8DhOFR3RzhQgqGg6yYZ1KZ/edit?gid=1795302417#gid=1795302417";
const ADD_EQ_SLIDES_V110 = [
  {
    "number": 1,
...
function additionalPadV110(value, size) { return String(Number(value || 0)).padStart(size || 3, "0"); }
function additionalVersionV110() { return window.APP_CACHE_VERSION || ADDITIONAL_MATERIALS_VERSION_V110; }
function additionalEscV110(value) { return typeof esc === "function" ? esc(value) : String(value ?? "").replace(/[&<>'"]/g, function(c){ return ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[c]; }); }
function additionalCardV110(cls, html) { return typeof card === "function" ? card(cls, html) : `<section class="card-v2 ${cls || ''}">${html || ''}</section>`; }
function additionalShellV110(html) { if (typeof shell === "function") return shell(html, "home"); var root = document.getElementById("app"); if (root) root.innerHTML = html; }
function additionalAwardV110(eventKey, eventType, payload) {
  try { if (typeof awardResearchPointOnceV91 === "function") return awardResearchPointOnceV91(eventKey, eventType || "research_event", payload || {}); } catch(e) { console.warn("ADDITIONAL_AWARD_V110", e); }
}
function additionalOpenUrlV110(url) {
  var target = String(url || "").trim();
  if (!target || target === "#") { alert("Ссылка будет добавлена позже."); return; }
  try { if (tg && typeof tg.openLink === "function") tg.openLink(target); else window.open(target, "_blank", "noopener,noreferrer"); }
  catch(e) { window.open(target, "_blank", "noopener,noreferrer"); }
}
function additionalAssertAccessV110() {
  if (state && state.access !== true) { if (typeof accessDenied === "function") accessDenied("OPEN_FROM_TELEGRAM_REQUIRED"); return false; }
  return true;
}
function additionalHeroCardV110() {
  return additionalCardV110("blue-card-v2 additional-materials-hero-v106 additional-materials-hero-v110", `<p class="eyebrow">библиотека бизнес-систем</p><h1>Дополнительные материалы</h1><p>Практические уроки, таблицы и инструменты, которые можно проходить отдельно от основного маршрута.</p>`);
}

function renderAdditionalMaterials() {
  if (!additionalAssertAccessV110()) return;
  additionalAwardV110("block:additional_materials", "block_open", { block:"additional_materials" });
  additionalShellV110(
    additionalHeroCardV110() +
    additionalCardV110("additional-section-v106 additional-section-v110", `<div class="section-heading-v35"><div><p class="eyebrow">финансы и учёт</p><h2>Практические инструменты</h2></div><p>Материалы можно открывать отдельно от основного маршрута. Каждый урок содержит теорию, страницы и рабочую таблицу.</p></div><div class="lesson-list-v2 additional-list-v106 additional-list-v110"><button class="lesson-row-v2 additional-lesson-row-v106" onclick="renderBreakEvenIntroV110()"><div><b>Расчёт точки безубыточности в моём бизнесе</b><p>18 слайдов теории · рабочая Google Таблица · инструкция для самостоятельного расчёта</p></div><span>→</span></button><button class="lesson-row-v2 additional-lesson-row-v106 business-equation-row-v107" onclick="renderBusinessEquationIntroV110()"><div><b>Единое уравнение бизнеса</b><p>20 слайдов теории · рабочая Google Таблица · свободный денежный поток и ограничения модели</p></div><span>→</span></button></div>`) +
    additionalCardV110("", `<button class="btn secondary" onclick="renderHome()">На главную</button>`)
  );
}

function renderAdditionalLessonIntroV110(kind) {
  if (!additionalAssertAccessV110()) return;
  var isEq = kind === "eq";
  var code = isEq ? ADD_EQ_CODE_V110 : ADD_BEP_CODE_V110;
  additionalAwardV110("lesson:" + code, "lesson_open", { lessonCode:code, module:"additional_materials" });
  var title = isEq ? "Единое уравнение бизнеса" : "Расчёт точки безубыточности в моём бизнесе";
  var subtitle = isEq
    ? "Как связать поток, конверсию, мощность, чек, маржу, расходы, оборотный капитал, инвестиции, риск и свободный денежный поток."
    : "Как посчитать минимальную выручку, объём продаж и денежный порог, при котором бизнес перестаёт работать в минус.";
  var slidesCount = isEq ? 20 : 18;
  var tableLabel = isEq ? "Открыть таблицу единого уравнения" : "Открыть таблицу точки безубыточности";
  var startFn = isEq ? "renderBusinessEquationSlideV110(0)" : "renderBreakEvenSlideV110(0)";
  var tableFn = isEq ? "renderBusinessEquationTableV110()" : "renderBreakEvenTableV110()";
  var preview = isEq
    ? `<div><span>01</span><section><b>От спроса до свободного потока</b><p>Почему бизнес нельзя оценивать только по выручке и деньгам на счёте.</p></section></div><div><span>02</span><section><b>Поток, конверсия, повторяемость и мощность</b><p>Как определить реальный объём, который бизнес способен выполнить.</p></section></div><div><span>03</span><section><b>Маржа, вычеты, риск и стоимость</b><p>Как выручка превращается в свободный денежный поток после всех ограничений.</p></section></div><div><span>04</span><section><b>Практическая таблица</b><p>Как разложить формулу на управленческий порядок действий.</p></section></div>`
    : `<div><span>01</span><section><b>Формула точки безубыточности</b><p>Как постоянные расходы, переменные расходы и маржинальность связаны между собой.</p></section></div><div><span>02</span><section><b>Точка в выручке и единицах</b><p>Как перевести расчёт в клиентов, продажи, процедуры, часы или заказы.</p></section></div><div><span>03</span><section><b>Денежная устойчивость</b><p>Почему прибыльная ТБ и денежная ТБ могут отличаться.</p></section></div><div><span>04</span><section><b>Практическая таблица</b><p>Как создать копию таблицы и пройти расчёт по инструкции.</p></section></div>`;
  additionalShellV110(
    additionalCardV110("blue-card-v2 additional-lesson-hero-v110", `<p class="eyebrow">финансы и учёт</p><h1>${additionalEscV110(title)}</h1><p>${additionalEscV110(subtitle)}</p>`) +
    additionalCardV110("additional-lesson-intro-v110", `<h2>Что вы разберёте</h2><div class="content-preview-list-v40 additional-preview-v110">${preview}</div><div class="profile-score-grid additional-mini-stats-v110"><div><span>Страницы</span><b>${slidesCount}</b></div><div><span>Рабочий материал</span><b>1</b></div></div><div class="grid-v2"><button class="btn primary" onclick="${startFn}">Начать урок</button><button class="btn secondary" onclick="${tableFn}">${additionalEscV110(tableLabel)}</button><button class="btn secondary" onclick="renderAdditionalMaterials()">К дополнительным материалам</button></div>`)
  );
}
function renderBreakEvenIntroV110() { return renderAdditionalLessonIntroV110("bep"); }
function renderBusinessEquationIntroV110() { return renderAdditionalLessonIntroV110("eq"); }

function additionalNavV110(kind, index, total) {
  var isEq = kind === "eq";
  var introFn = isEq ? "renderBusinessEquationIntroV110()" : "renderBreakEvenIntroV110()";
  var slideFn = isEq ? "renderBusinessEquationSlideV110" : "renderBreakEvenSlideV110";
  var tableFn = isEq ? "renderBusinessEquationTableV110()" : "renderBreakEvenTableV110()";
  var prevDisabled = index <= 0 ? "disabled" : "";
  var nextLabel = index >= total - 1 ? "К таблице" : "Далее";
  var nextAction = index >= total - 1 ? tableFn : slideFn + "(" + (index + 1) + ")";
  return `<div class="nav-panel-v2 nav-panel-v2-three"><button class="btn secondary" onclick="${introFn}">К уроку</button><button class="btn secondary" ${prevDisabled} onclick="${slideFn}(${index - 1})">Назад</button><button class="btn primary" onclick="${nextAction}">${nextLabel}</button></div>`;
}
function additionalMediaV110(slide, total, kind) {
  var src = slide.image || "";
  return `<div class="media-counter">Слайд: ${slide.number}/${total}</div><div class="media-box-v2 additional-media-v110"><img src="${additionalEscV110(src)}?v=${additionalVersionV110()}" alt="${additionalEscV110(slide.title || '')}" loading="lazy" onerror="this.style.display=&quot;none&quot;; this.nextElementSibling.style.display=&quot;flex&quot;;"><div class="image-missing-v2" style="display:none"><b>Слайд ${slide.number}</b><p>Иллюстрация будет добавлена после генерации изображения.</p><small>${additionalEscV110(src)}</small></div></div>`;
}
function renderAdditionalSlideV110(kind, index) {
  if (!additionalAssertAccessV110()) return;
  var slides = kind === "eq" ? ADD_EQ_SLIDES_V110 : ADD_BEP_SLIDES_V110;
  var code = kind === "eq" ? ADD_EQ_CODE_V110 : ADD_BEP_CODE_V110;
  var i = Math.max(0, Math.min(Number(index || 0), slides.length - 1));
  var slide = slides[i];
  additionalAwardV110("lesson:" + code, "lesson_open", { lessonCode:code, module:"additional_materials" });
  additionalAwardV110("slide:" + code + ":" + additionalPadV110(slide.number, 3), "slide_open", { lessonCode:code, slideNo:slide.number, module:"additional_materials" });
  var cls = kind === "eq" ? "business-equation-slide-text-v107" : "break-even-slide-text-v106";
  additionalShellV110(additionalNavV110(kind, i, slides.length) + additionalMediaV110(slide, slides.length, kind) + `<section class="slide-text-v2 ${cls} additional-slide-text-v110">${slide.descriptionHtml || ''}</section>`);
}
function renderBreakEvenSlideV110(index) { return renderAdditionalSlideV110("bep", index); }
function renderBusinessEquationSlideV110(index) { return renderAdditionalSlideV110("eq", index); }

function renderAdditionalTableV110(kind) {
  if (!additionalAssertAccessV110()) return;
  var isEq = kind === "eq";
  var code = isEq ? ADD_EQ_CODE_V110 : ADD_BEP_CODE_V110;
  var url = isEq ? ADD_EQ_TABLE_URL_V110 : ADD_BEP_TABLE_URL_V110;
  var materialId = isEq ? "business_equation_table" : "break_even_table";
  var tableTitle = isEq ? "Практическая таблица: единое уравнение бизнеса" : "Практическая таблица: точка безубыточности";
  var backSlide = isEq ? "renderBusinessEquationSlideV110(19)" : "renderBreakEvenSlideV110(17)";
  var openFn = isEq ? "openBusinessEquationTableV110()" : "openBreakEvenTableV110()";
  additionalAwardV110("work_material:" + code + ":" + materialId, "work_material_open", { lessonCode:code, materialId:materialId, url:url, source:"additional_materials" });
  additionalShellV110(
    additionalCardV110("blue-card-v2 additional-table-hero-v110", `<p class="eyebrow">практический инструмент</p><h1>${additionalEscV110(tableTitle)}</h1><p>Откройте Google Таблицу, создайте личную копию через <b>Файл → Создать копию</b> и работайте только в своей версии.</p>`) +
    additionalCardV110("additional-table-card-v110", `<h2>Порядок работы</h2><div class="break-even-steps-v106 additional-table-steps-v110"><div><b>1. Откройте Google Таблицу</b><p>Нажмите кнопку ниже и дождитесь загрузки таблицы.</p></div><div><b>2. Создайте личную копию</b><p>В Google Таблицах выберите: <b>Файл → Создать копию</b>. Исходный шаблон не редактируйте.</p></div><div><b>3. Идите по инструкции</b><p>Заполняйте вводные по порядку. Не меняйте расчётные формулы.</p></div><div><b>4. Сделайте управленческий вывод</b><p>Результат таблицы должен завершиться решением, а не просто красивой цифрой.</p></div></div><div class="grid-v2"><button class="btn primary" onclick="${openFn}">Открыть Google Таблицу</button><button class="btn secondary" onclick="${backSlide}">Вернуться к уроку</button><button class="btn secondary" onclick="renderAdditionalMaterials()">К дополнительным материалам</button></div>`)
  );
}
function renderBreakEvenTableV110() { return renderAdditionalTableV110("bep"); }
function renderBusinessEquationTableV110() { return renderAdditionalTableV110("eq"); }
function openAdditionalTableV110(kind) {
  var isEq = kind === "eq";
  var code = isEq ? ADD_EQ_CODE_V110 : ADD_BEP_CODE_V110;
  var url = isEq ? ADD_EQ_TABLE_URL_V110 : ADD_BEP_TABLE_URL_V110;
  var materialId = isEq ? "business_equation_table" : "break_even_table";
  additionalAwardV110("work_material:" + code + ":" + materialId, "work_material_open", { lessonCode:code, materialId:materialId, url:url, source:"additional_materials" });
  additionalOpenUrlV110(url);
}
function openBreakEvenTableV110() { return openAdditionalTableV110("bep"); }
function openBusinessEquationTableV110() { return openAdditionalTableV110("eq"); }

// Алиасы для старых onclick-имён, чтобы предыдущие карточки не ломались.
window.renderAdditionalMaterials = renderAdditionalMaterials;
window.renderBreakEvenIntroV106 = renderBreakEvenIntroV110;
window.renderBreakEvenSlideV106 = renderBreakEvenSlideV110;
window.renderBreakEvenTableV106 = renderBreakEvenTableV110;
window.openBreakEvenTableV106 = openBreakEvenTableV110;
window.renderBusinessEquationIntroV107 = renderBusinessEquationIntroV110;
window.renderBusinessEquationSlideV107 = renderBusinessEquationSlideV110;
window.renderBusinessEquationTableV107 = renderBusinessEquationTableV110;
window.openBusinessEquationTableV107 = openBusinessEquationTableV110;
window.__ADDITIONAL_MATERIALS_V110 = { version: ADDITIONAL_MATERIALS_VERSION_V110, lessons:[ADD_BEP_CODE_V110, ADD_EQ_CODE_V110] };
  ...
  const html = `
    ...
    ${card('', `<h2>Выбрать блок</h2><p>Выберите направление работы внутри платформы.</p>
      ...
      <div class="secondary-track-grid-v22">
        ...
        ${renderMainBlockCard('Дополнительные материалы','Точка безубыточности, единое уравнение бизнеса и практические инструменты вне основного маршрута.','доступно','renderAdditionalMaterials()','active compact-card additional-materials-entry-v110')}
  ...
  var html = `
    ${card('hero-dashboard main-dashboard-card architecture-dashboard', `
      ...
      <div class="architecture-metrics">
        <div><span>Баллы</span><b>${formatPoints(points)}</b></div>
        <div><span>Уровень</span><b>${titleInfo.current.level}</b></div>
    ...
    return function(){
      ...
      var html = `
        ${card('hero-dashboard main-dashboard-card architecture-dashboard v40-dashboard v41-dashboard v43-dashboard', `
          ...
          <div class="architecture-metrics"><div><span>Баллы</span><b>${formatPoints(points)}</b></div><div><span>Уровень</span><b>${titleInfo.current.level}</b></div><div><span>Достижение</span><b>${esc(achievement)}</b></div></div>
          ${globalInstructionPanelHtml()}
        `)}
  ...
  window.renderHome = function(){
    ...
    var html = `
      ${card('hero-dashboard main-dashboard-card architecture-dashboard v40-dashboard v41-dashboard v43-dashboard v44-dashboard v47-dashboard', `
        ...
        <div class="architecture-metrics v47-metrics"><div><span>Баллы</span><b>${formatPoints(points)}</b></div><div><span>Уровень</span><b>${titleInfo.current.level}</b></div><div><span>Достижение</span><b>${esc(achievement)}</b></div></div>
        ${globalInstructionPanelHtml()}
      `)}
      ${card('architecture-blocks-card v40-blocks-card', `<div class="section-heading-v35"><div><p class="eyebrow">структура библиотеки</p><h2>Выберите блок</h2></div><p>Архитектуры, системы, разборы и материалы собраны в единой структуре.</p></div>${primaryRoutesHtmlV40()}${secondaryBlocksHtmlV40()}`)}
      ${typeof safeActiveChallengeCardHtmlV24 === 'function' ? safeActiveChallengeCardHtmlV24() : ''}
...
(function installFinanceV81NavigationFix(){
  ...
  window.secondaryBlocksHtmlV40 = function(){
    ...
    return `<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40">
      ...
      ${mainCardV81('Дополнительные материалы','Точка безубыточности, единое уравнение бизнеса и практические инструменты вне основного маршрута.','доступно','renderAdditionalMaterials()','active compact-card additional-materials-entry-v110')}
...
(function installTop100BusinessBooksV96(){
  ...
  window.secondaryBlocksHtmlV40 = function(){
    ...
    return '<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40">'
      ...
      + v96MainCard('Дополнительные материалы','Точка безубыточности, единое уравнение бизнеса и практические инструменты вне основного маршрута.','доступно','renderAdditionalMaterials()','active compact-card additional-materials-entry-v110')
  ...
  if (typeof globalInstructionPanelHtml === 'function') {
    window.globalInstructionPanelHtml = function(){
      return `<div id="global-instruction-panel" class="global-instruction-panel v44-instruction-panel" style="display:none">
        <div class="instruction-head"><b>Как пользоваться АРХИТЕКТУРОЙ</b><button onclick="toggleGlobalInstruction(false)" aria-label="Закрыть инструкцию">×</button></div>
        <div class="instruction-steps">
          <div><b>1. Главная</b><p>Главная — это стартовая карта приложения. Здесь видно общий прогресс, количество баллов, текущий уровень и основные блоки библиотеки. Если не знаете, куда идти дальше, начинайте с главной: она показывает структуру приложения и доступные разделы.</p></div>
          <div><b>2. Общий прогресс</b><p>Общий прогресс показывает, сколько учебных этапов уже завершено. В расчёт входят завершённые части опубликованных уроков: презентация, тест, саммари и самостоятельная работа. Этот блок помогает видеть, сколько системы уже пройдено и сколько ещё осталось разобрать.</p></div>
          <div><b>3. Боковое меню</b><p>Боковое меню открывается через кнопку в шапке приложения. В нём собраны основные разделы: маршруты, книги, форум, дополнительные материалы, профиль и другие блоки. Если на главной не видно нужного раздела, сначала проверьте боковое меню.</p></div>
          <div><b>4. Блок «Я предприниматель»</b><p>Это основной маршрут приложения. Внутри него выбирается вид деятельности: торговля, услуги, производство, строительство и проекты, логистика или HoReCa. Каждый вид деятельности разбирается через систему показателей, ограничений, решений и практических действий.</p></div>
          <div><b>5. Уроки внутри направления</b><p>Каждый урок устроен по одному порядку: презентация, тест, саммари и самостоятельная работа. Презентация даёт логику темы, тест проверяет понимание, саммари усиливает урок идеями из книг, а самостоятельная работа переносит материал в вашу ситуацию. Лучше проходить уроки последовательно, даже если опубликованные материалы доступны сразу.</p></div>
          <div><b>6. Самостоятельная работа</b><p>Самостоятельная работа — это практическая часть урока. Здесь нужно открыть шаблон, применить материал к своему бизнесу, сформулировать вывод и выбрать действие для проверки. Задача — не просто закрыть этап, а навести порядок в своей системе.</p></div>
          <div><b>7. Топ-100 книг для бизнеса</b><p>В этом разделе собраны саммари бизнес-книг. Их можно читать отдельно от основного маршрута, но лучше связывать с текущими задачами бизнеса. Прочитанные книги и выполненные действия учитываются в общем образовательном прогрессе.</p></div>
          <div><b>8. Бизнес-форум</b><p>Форум нужен для практических вопросов, обсуждения ситуаций и обмена опытом между участниками. Темы лучше формулировать конкретно: вид деятельности, проблема, цифры, что уже пробовали и какой вопрос нужно решить. Форум не заменяет уроки, а помогает разобрать применение материалов на реальных ситуациях.</p></div>
          <div><b>9. Дополнительные материалы</b><p>Дополнительные материалы — это отдельные практические инструменты вне основного маршрута. Здесь находятся точка безубыточности, единое уравнение бизнеса и другие прикладные материалы. Их можно открывать отдельно, когда нужно быстро разобраться с конкретным финансовым или управленческим вопросом.</p></div>
          <div><b>10. Профиль, баллы и уровни</b><p>Профиль показывает накопленный результат: баллы, уровень, достижение, изученные этапы и сохранённые выводы. Баллы и уровни не являются самоцелью. Их задача — показать объём пройденной работы и поддерживать порядок в обучении.</p></div>
          <div><b>11. Финансовый помощник и блоки в подготовке</b><p>Финансовый помощник и часть дополнительных разделов могут открываться постепенно. Если блок отмечен как «скоро» или «в подготовке», значит он уже заложен в структуру приложения, но ещё не доступен для работы. Основной маршрут при этом остаётся на главной и в блоке «Я предприниматель».</p></div>
          <div><b>12. Главное правило работы</b><p>Не проходите материалы ради галочки. После каждого урока должен остаться один практический вывод: где в бизнесе ограничение, какой показатель это подтверждает и какое действие нужно проверить дальше.</p></div>
        </div>
      </div>`;
    };
  }
...
/* =====================================================
   v126 — verified source-level fix: баллы исследования + уровни
   Исправляет причину, а не индекс: старые текстовые подписи
   больше не создаются слоем libraryPositioningTextV43.
   ===================================================== */
(function installResearchPointsSourceFixV126(){
  window.APP_UI_VERSION_V126 = 'v126-verified-research-points-source-fix-20260706';
  ...
  /* Главная причина ошибки была здесь: старый слой переводил баллы в старую подпись.
...
/* =====================================================
   v127 — Что посмотреть: библиотека фильмов и сериалов
   Открытый блок для учеников. Логика приложения, доступ,
   Supabase, уроки, форум и финансовый помощник не меняются.
   ===================================================== */
(function installWatchLibraryV127(){
  window.APP_UI_VERSION_V127 = 'v127-watch-library-films-series-20260706';

  const WATCH_FILMS_V127 = [
  {
    "id": "the-founder",
    "title": "The Founder / Основатель",
    "year": "2016",
    "image": "assets/watch/films/watch_film_001.png",
    "tags": [
      "масштабирование",
      "франшиза",
      "стандарты"
    ],
    "description": "История роста McDonald’s показывает, что масштабирование держится не только на идее, а на стандартах, договорах и контроле повторяемости. Фильм полезен для понимания франчайзинга, партнёрских конфликтов и силы операционной системы. Главный вопрос после просмотра: что в вашем бизнесе можно повторить без ручного участия собственника?"
  },
  {
    "id": "the-social-network",
    "title": "The Social Network / Социальная сеть",
    "year": "2010",
    "image": "assets/watch/films/watch_film_002.png",
    "tags": [
      "стартап",
      "доли",
      "конфликт основателей"
    ],
    "description": "Фильм показывает запуск Facebook через конфликт основателей, быстрый рост и юридические последствия неоформленных договорённостей. Для предпринимателя здесь важны темы долей, партнёрства, права на идею и скорости захвата рынка. Главный вывод: в стартапе слабая юридическая архитектура может стать дорогой ошибкой."
  },
  {
    "id": "steve-jobs",
    "title": "Steve Jobs / Стив Джобс",
    "year": "2015",
    "image": "assets/watch/films/watch_film_003.png",
    "tags": [
      "продукт",
      "презентация",
      "лидерство"
    ],
    "description": "Фильм построен вокруг презентаций продуктов и показывает, как сильное видение сталкивается с командой, семьёй и корпоративной политикой. Он полезен для разбора продуктового фокуса, личного бренда руководителя и цены жёсткого управления. Смотреть стоит не как биографию, а как кейс о напряжении между гениальностью и управляемостью."
  },
  {
    "id": "pirates-silicon-valley",
    "title": "Pirates of Silicon Valley / Пираты Силиконовой долины",
    "year": "1999",
    "image": "assets/watch/films/watch_film_004.png",
    "tags": [
      "технологии",
      "конкуренция",
      "стратегия"
    ],
    "description": "Фильм показывает раннюю конкуренцию Apple и Microsoft, борьбу за интерфейсы, рынок и скорость внедрения. Для бизнеса важны темы копирования идей, стратегического окна возможностей и умения превращать технологию в массовый продукт. Главный вопрос: где в вашей отрасли сейчас формируется новый стандарт?"
  },
  {
    "id": "moneyball",
    "title": "Moneyball / Человек, который изменил всё",
    "year": "2011",
    "image": "assets/watch/films/watch_film_005.png",
    "tags": [
      "данные",
      "метрики",
      "сопротивление системы"
    ],
    "description": "Фильм показывает, как управление через данные ломает старые экспертные привычки. Он полезен предпринимателю тем, что учит искать недооценённые ресурсы и принимать решения не по вкусу, а по метрикам. Главный вывод: система с меньшим бюджетом может выиграть, если лучше понимает свои показатели."
  },
  {
    "id": "the-big-short",
    "title": "The Big Short / Игра на понижение",
    "year": "2015",
    "image": "assets/watch/films/watch_film_006.png",
    "tags": [
      "финансы",
      "риск",
      "кризис"
    ],
    "description": "Фильм разбирает финансовый пузырь и показывает, как рынок может массово не видеть очевидный риск. Для предпринимателя он полезен как тренировка критического мышления и проверки общепринятых допущений. Главный вопрос: где в вашем бизнесе все верят в цифру, которую никто не проверяет?"
  },
  {
    "id": "margin-call",
    "title": "Margin Call / Предел риска",
    "year": "2011",
    "image": "assets/watch/films/watch_film_007.png",
    "tags": [
      "кризис",
      "решения",
      "этика"
    ],
    "description": "Фильм показывает одну ночь внутри финансовой компании, когда менеджмент понимает масштаб будущего краха. Это сильный материал о решениях под давлением, ответственности руководителей и границе между защитой компании и разрушением рынка. Смотреть полезно как кейс кризисного управления."
  },
  {
    "id": "wall-street",
    "title": "Wall Street / Уолл-стрит",
    "year": "1987",
    "image": "assets/watch/films/watch_film_008.png",
    "tags": [
      "сделки",
      "жадность",
      "корпоративная власть"
    ],
    "description": "Фильм показывает культуру быстрых денег, давления и корпоративных сделок. Для предпринимателя он полезен не как пример для подражания, а как предупреждение о риске жадности и короткого горизонта. Главный вывод: деньги без ограничителей быстро начинают управлять человеком."
  },
  {
    "id": "wolf-wall-street",
    "title": "The Wolf of Wall Street / Волк с Уолл-стрит",
    "year": "2013",
    "image": "assets/watch/films/watch_film_009.png",
    "tags": [
      "продажи",
      "манипуляции",
      "культура"
    ],
    "description": "Фильм показывает мощную, но разрушительную машину продаж, построенную на манипуляции, токсичной мотивации и отсутствии контроля. Его нельзя смотреть как вдохновение; это антипример корпоративной культуры. Главный вопрос: где в вашей системе продаж результат достигается ценой будущего разрушения?"
  },
  {
    "id": "boiler-room",
    "title": "Boiler Room / Бойлерная",
    "year": "2000",
    "image": "assets/watch/films/watch_film_010.png",
    "tags": [
      "продажи",
      "скрипты",
      "мошенничество"
    ],
    "description": "Фильм показывает агрессивные продажи, давление на клиента и работу псевдоинвестиционного отдела. Для предпринимателя он полезен как разбор опасной мотивационной среды и серых схем продаж. Главный вывод: высокий темп продаж не равен здоровой бизнес-модели."
  },
  {
    "id": "glengarry-glen-ross",
    "title": "Glengarry Glen Ross / Американцы",
    "year": "1992",
    "image": "assets/watch/films/watch_film_011.png",
    "tags": [
      "лиды",
      "план продаж",
      "мотивация"
    ],
    "description": "Фильм показывает отдел продаж под давлением плана, страха и неравного доступа к лидам. Он полезен для разбора того, как система мотивации может разрушать команду вместо роста результата. Главный вопрос: ваша система продаж создаёт дисциплину или отчаяние?"
  },
  {
    "id": "air",
    "title": "Air / Air: Большой прыжок",
    "year": "2023",
    "image": "assets/watch/films/watch_film_012.png",
    "tags": [
      "бренд",
      "переговоры",
      "ставка на талант"
    ],
    "description": "Фильм показывает сделку Nike с Майклом Джорданом как пример стратегической ставки на человека, бренд и новую категорию. Для предпринимателя здесь важны переговоры, вера в инсайт и способность предложить партнёру не деньги, а долю в будущем. Главный вывод: сильная сделка создаёт новую игру, а не просто покупает рекламу."
  },
  {
    "id": "joy",
    "title": "Joy / Джой",
    "year": "2015",
    "image": "assets/watch/films/watch_film_013.png",
    "tags": [
      "изобретение",
      "продукт",
      "продажи"
    ],
    "description": "Фильм показывает путь изобретателя от идеи до производства, продаж и защиты своих прав. Он полезен для малого бизнеса: здесь есть продукт, семья, поставщики, телевидение и проблемы контроля качества. Главный вопрос: что мешает вашей идее превратиться в продаваемый продукт?"
  },
  {
    "id": "tucker",
    "title": "Tucker: The Man and His Dream / Такер",
    "year": "1988",
    "image": "assets/watch/films/watch_film_014.png",
    "tags": [
      "инновация",
      "производство",
      "индустрия"
    ],
    "description": "Фильм показывает предпринимателя, который пытается создать автомобильную компанию против устоявшейся индустрии. Для бизнеса важны темы производства, регуляторного давления и борьбы маленького игрока с крупными интересами. Главный вывод: инновация должна выдержать не только рынок, но и систему сопротивления."
  },
  {
    "id": "the-aviator",
    "title": "The Aviator / Авиатор",
    "year": "2004",
    "image": "assets/watch/films/watch_film_015.png",
    "tags": [
      "масштаб",
      "риск",
      "индустрии"
    ],
    "description": "Фильм показывает предпринимателя, который строит бизнес на стыке кино, авиации и личной одержимости. Он полезен для понимания масштаба, риска, капиталоёмких проектов и цены контроля. Главный вопрос: где личная энергия основателя усиливает бизнес, а где начинает угрожать системе?"
  },
  {
    "id": "ford-v-ferrari",
    "title": "Ford v Ferrari / Ford против Ferrari",
    "year": "2019",
    "image": "assets/watch/films/watch_film_016.png",
    "tags": [
      "корпорация",
      "инженерия",
      "команда"
    ],
    "description": "Фильм показывает конфликт между корпоративной бюрократией и инженерной практикой. Для предпринимателя он полезен как разбор скорости решений, роли команды и опасности менеджмента, который мешает результату. Главный вывод: сильные исполнители должны иметь пространство для работы, иначе система сама тормозит победу."
  },
  {
    "id": "current-war",
    "title": "The Current War / Война токов",
    "year": "2017",
    "image": "assets/watch/films/watch_film_017.png",
    "tags": [
      "технологии",
      "стандарты",
      "патенты"
    ],
    "description": "Фильм показывает борьбу за энергетический стандарт и рынок будущей инфраструктуры. Здесь важны патенты, PR, партнёрства и выбор технологии, которая станет нормой для отрасли. Главный вопрос: какой стандарт в вашей нише может определить победителя на годы вперёд?"
  },
  {
    "id": "flash-of-genius",
    "title": "Flash of Genius / Проблеск гения",
    "year": "2008",
    "image": "assets/watch/films/watch_film_018.png",
    "tags": [
      "патенты",
      "изобретатель",
      "корпорация"
    ],
    "description": "Фильм показывает борьбу изобретателя с крупной корпорацией за признание права на разработку. Он полезен предпринимателю как предупреждение о важности юридической защиты продукта. Главный вывод: идея без защиты и договоров может стать чужим активом."
  },
  {
    "id": "pursuit-of-happyness",
    "title": "The Pursuit of Happyness / В погоне за счастьем",
    "year": "2006",
    "image": "assets/watch/films/watch_film_019.png",
    "tags": [
      "дисциплина",
      "продажи",
      "устойчивость"
    ],
    "description": "Фильм показывает личную устойчивость, дисциплину и вход в профессию через длительный период неопределённости. Для предпринимателя он полезен как разбор ежедневного действия без немедленного результата. Главный вывод: мотивация ничего не стоит без режима и способности выдерживать длинный цикл."
  },
  {
    "id": "jerry-maguire",
    "title": "Jerry Maguire / Джерри Магуайер",
    "year": "1996",
    "image": "assets/watch/films/watch_film_020.png",
    "tags": [
      "клиенты",
      "личный бренд",
      "услуги"
    ],
    "description": "Фильм показывает специалиста, который выходит из большой системы и пытается построить работу вокруг качества отношений с клиентами. Для предпринимателя важны темы личного бренда, сервиса, доверия и этики в продажах услуг. Главный вопрос: за что клиент остаётся с вами, если убрать громкое имя компании?"
  },
  {
    "id": "chef",
    "title": "Chef / Повар на колёсах",
    "year": "2014",
    "image": "assets/watch/films/watch_film_021.png",
    "tags": [
      "малый бизнес",
      "бренд",
      "соцсети"
    ],
    "description": "Фильм показывает перезапуск профессионала через малый продуктовый бизнес, личный бренд и прямой контакт с клиентом. Он полезен для понимания клиентского опыта, соцсетей и простого предложения, которое люди хотят покупать. Главный вывод: сильный продукт и честная подача могут вернуть бизнесу энергию."
  },
  {
    "id": "jiro-dreams",
    "title": "Jiro Dreams of Sushi / Мечты Дзиро о суши",
    "year": "2011",
    "image": "assets/watch/films/watch_film_022.png",
    "tags": [
      "качество",
      "процесс",
      "ремесло"
    ],
    "description": "Документальный фильм о мастерстве, стандартах и многолетнем улучшении процесса. Для предпринимателя он полезен как пример операционной дисциплины, где качество рождается из повторяемых действий. Главный вопрос: какие стандарты в вашем бизнесе должны стать ежедневным ритуалом?"
  },
  {
    "id": "devil-wears-prada",
    "title": "The Devil Wears Prada / Дьявол носит Prada",
    "year": "2006",
    "image": "assets/watch/films/watch_film_023.png",
    "tags": [
      "бренд",
      "индустрия",
      "культура"
    ],
    "description": "Фильм показывает индустрию моды через власть бренда, скорость, требования и корпоративную культуру. Для предпринимателя важен разбор того, как статус, вкус и внимание к деталям превращаются в экономическую силу. Главный вопрос: какие детали клиент воспринимает как показатель уровня вашего бренда?"
  },
  {
    "id": "thank-you-for-smoking",
    "title": "Thank You for Smoking / Здесь курят",
    "year": "2005",
    "image": "assets/watch/films/watch_film_024.png",
    "tags": [
      "PR",
      "риторика",
      "этика"
    ],
    "description": "Фильм показывает управление восприятием, аргументацию и PR в спорной индустрии. Для бизнеса он полезен как учебник риторики и одновременно как предупреждение об этических границах коммуникации. Главный вывод: умение убеждать должно быть связано с ответственностью за последствия."
  },
  {
    "id": "enron",
    "title": "Enron: The Smartest Guys in the Room / Enron",
    "year": "2005",
    "image": "assets/watch/films/watch_film_025.png",
    "tags": [
      "мошенничество",
      "отчётность",
      "контроль"
    ],
    "description": "Документальный фильм показывает корпоративное мошенничество, искажённую отчётность и культуру жадности. Для предпринимателя это важный антипример: рост без контроля превращается в системный риск. Главный вопрос: какие показатели в вашей компании можно красиво показать, но неправильно понять?"
  },
  {
    "id": "inside-job",
    "title": "Inside Job / Инсайдеры",
    "year": "2010",
    "image": "assets/watch/films/watch_film_026.png",
    "tags": [
      "кризис",
      "стимулы",
      "финансы"
    ],
    "description": "Документальный фильм разбирает финансовый кризис и системные стимулы, которые привели к катастрофе. Для предпринимателя он полезен как материал о конфликтах интересов, слабом контроле и риске сложных продуктов. Главный вывод: система поощрений определяет поведение сильнее лозунгов."
  },
  {
    "id": "startup-com",
    "title": "Startup.com",
    "year": "2001",
    "image": "assets/watch/films/watch_film_027.png",
    "tags": [
      "стартап",
      "инвестиции",
      "партнёрство"
    ],
    "description": "Документальный фильм показывает интернет-стартап изнутри: рост, инвестиции, конфликт основателей и столкновение ожиданий с реальностью. Он полезен для понимания того, как быстро команда может разрушиться при давлении денег и рынка. Главный вывод: рост без устойчивой основы усиливает слабые места."
  },
  {
    "id": "the-corporation",
    "title": "The Corporation / Корпорация",
    "year": "2003",
    "image": "assets/watch/films/watch_film_028.png",
    "tags": [
      "корпорации",
      "ответственность",
      "власть"
    ],
    "description": "Документальный фильм разбирает логику крупных компаний, их влияние и внешние эффекты решений. Для предпринимателя это не инструкция, а повод увидеть, как бизнес-модель влияет на общество, рынок и ответственность. Главный вопрос: какие последствия вашего бизнеса не видны в выручке?"
  },
  {
    "id": "becoming-warren-buffett",
    "title": "Becoming Warren Buffett / Стать Уорреном Баффетом",
    "year": "2017",
    "image": "assets/watch/films/watch_film_029.png",
    "tags": [
      "капитал",
      "долгий горизонт",
      "мышление"
    ],
    "description": "Фильм показывает инвестиционное мышление, дисциплину, простоту решений и силу долгого горизонта. Для предпринимателя он полезен тем, что учит думать о капитале спокойно, без суеты и моды. Главный вывод: устойчивость часто создаётся не гениальным ходом, а последовательностью правильных принципов."
  },
  {
    "id": "there-will-be-blood",
    "title": "There Will Be Blood / Нефть",
    "year": "2007",
    "image": "assets/watch/films/watch_film_030.png",
    "tags": [
      "ресурсы",
      "власть",
      "одержимость"
    ],
    "description": "Фильм показывает жёсткое предпринимательство в ресурсной отрасли, где деньги, власть и личная одержимость сплетены в одну систему. Для бизнес-разбора он полезен как история о цене роста без внутренних ограничителей. Главный вопрос: где стремление победить начинает разрушать предпринимателя и его окружение?"
  }
];
  const WATCH_SERIES_V127 = [
  {
    "id": "silicon-valley",
    "title": "Silicon Valley / Кремниевая долина",
    "year": "2014–2019",
    "image": "assets/watch/series/watch_series_001.png",
    "tags": [
      "стартап",
      "инвесторы",
      "продукт"
    ],
    "description": "Сериал показывает стартап-среду через продукт, доли, инвесторов, переговоры и абсурд венчурной культуры. Он полезен для предпринимателя как разбор ошибок основателей и давления роста. Главный вывод: технология сама по себе не заменяет бизнес-модель, команду и договорённости."
  },
  {
    "id": "succession",
    "title": "Succession / Наследники",
    "year": "2018–2023",
    "image": "assets/watch/series/watch_series_002.png",
    "tags": [
      "власть",
      "семейный бизнес",
      "сделки"
    ],
    "description": "Сериал показывает семейную корпорацию, борьбу за власть, преемственность и сделки на уровне совета директоров. Для предпринимателя он полезен как разбор собственнического контроля и токсичной культуры управления. Главный вопрос: что произойдёт с бизнесом, если власть не отделена от семейных эмоций?"
  },
  {
    "id": "billions",
    "title": "Billions / Миллиарды",
    "year": "2016–2023",
    "image": "assets/watch/series/watch_series_003.png",
    "tags": [
      "финансы",
      "стратегия",
      "переговоры"
    ],
    "description": "Сериал показывает мир хедж-фондов, юридического давления, стратегических ходов и борьбы влияния. Он полезен для разбора переговоров, информации, рисков и личных мотивов в больших деньгах. Главный вывод: стратегия без этических границ быстро превращается в войну всех против всех."
  },
  {
    "id": "mad-men",
    "title": "Mad Men / Безумцы",
    "year": "2007–2015",
    "image": "assets/watch/series/watch_series_004.png",
    "tags": [
      "реклама",
      "бренд",
      "клиенты"
    ],
    "description": "Сериал показывает рекламный бизнес, клиентские презентации, брендовые идеи и внутреннюю культуру агентства. Для предпринимателя он полезен как материал о восприятии, позиционировании и продаже смысла. Главный вопрос: какую эмоцию и обещание на самом деле продаёт ваш продукт?"
  },
  {
    "id": "halt-catch-fire",
    "title": "Halt and Catch Fire / Остановись и гори",
    "year": "2014–2017",
    "image": "assets/watch/series/watch_series_005.png",
    "tags": [
      "технологии",
      "команда",
      "рынок"
    ],
    "description": "Сериал показывает развитие технологического бизнеса через компьютеры, игры, интернет и смену рынков. Он полезен для понимания продуктового цикла, конкуренции и роли команды в переходные периоды. Главный вывод: рынок меняется быстрее, чем компания успевает привыкнуть к своей победе."
  },
  {
    "id": "the-playlist",
    "title": "The Playlist / Плейлист",
    "year": "2022",
    "image": "assets/watch/series/watch_series_006.png",
    "tags": [
      "платформа",
      "лицензии",
      "продукт"
    ],
    "description": "Сериал показывает создание Spotify через продукт, музыку, правообладателей и платформенную модель. Для предпринимателя важны темы лицензий, переговоров и смены экономики индустрии. Главный вопрос: чьи интересы должна соединить ваша платформа, чтобы рынок принял новый формат?"
  },
  {
    "id": "super-pumped",
    "title": "Super Pumped / Суперкачанные",
    "year": "2022",
    "image": "assets/watch/series/watch_series_007.png",
    "tags": [
      "агрессивный рост",
      "фаундер",
      "регуляторы"
    ],
    "description": "Сериал показывает Uber как пример быстрого роста, давления основателя, конфликта с регуляторами и токсичной культуры. Он полезен как антипример масштабирования без зрелой системы управления. Главный вывод: скорость роста может стать угрозой, если культура не выдерживает масштаб."
  },
  {
    "id": "wecrashed",
    "title": "WeCrashed / Не сработало",
    "year": "2022",
    "image": "assets/watch/series/watch_series_008.png",
    "tags": [
      "оценка",
      "инвестиции",
      "харизма"
    ],
    "description": "Сериал показывает WeWork, харизму основателя, завышенные ожидания и разрыв между историей для инвесторов и реальной экономикой. Для предпринимателя он полезен как предупреждение о мифе масштаба. Главный вопрос: ваша оценка бизнеса опирается на факты или на красивый рассказ?"
  },
  {
    "id": "the-dropout",
    "title": "The Dropout / Выбывшая",
    "year": "2022",
    "image": "assets/watch/series/watch_series_009.png",
    "tags": [
      "мошенничество",
      "продукт",
      "инвесторы"
    ],
    "description": "Сериал показывает Theranos и опасность фаундерского мифа, который заменяет работающий продукт. Он полезен для понимания проверки гипотез, медицинских рисков и слепой веры инвесторов. Главный вывод: нельзя масштабировать обещание, если технология не выдерживает проверку."
  },
  {
    "id": "startup",
    "title": "StartUp / Стартап",
    "year": "2016–2018",
    "image": "assets/watch/series/watch_series_010.png",
    "tags": [
      "финтех",
      "риск",
      "партнёрство"
    ],
    "description": "Сериал показывает запуск технологического проекта на стыке денег, криминала, инвестиций и личных интересов. Для бизнеса он полезен как разбор хаотичного партнёрства и риска нечистого капитала. Главный вопрос: какие источники денег могут испортить модель ещё до роста?"
  },
  {
    "id": "industry",
    "title": "Industry / Индустрия",
    "year": "2020–",
    "image": "assets/watch/series/watch_series_011.png",
    "tags": [
      "банк",
      "карьера",
      "давление"
    ],
    "description": "Сериал показывает молодых сотрудников инвестиционного банка, давление результата, продажи и внутреннюю конкуренцию. Он полезен предпринимателю для понимания корпоративной среды, где люди становятся частью системы стимулов. Главный вывод: культура компании проявляется не в лозунгах, а в том, какое поведение вознаграждается."
  },
  {
    "id": "shark-tank",
    "title": "Shark Tank / Акулы бизнеса",
    "year": "2009–",
    "image": "assets/watch/series/watch_series_012.png",
    "tags": [
      "питч",
      "инвесторы",
      "оценка"
    ],
    "description": "Шоу полезно как библиотека предпринимательских презентаций, ошибок и переговоров с инвесторами. Здесь видно, как слабая экономика, непонятный рынок или плохая упаковка ломают даже интересную идею. Главный вопрос: смогли бы вы объяснить свой бизнес за две минуты цифрами и смыслом?"
  },
  {
    "id": "dragons-den",
    "title": "Dragons’ Den / Логово драконов",
    "year": "2005–",
    "image": "assets/watch/series/watch_series_013.png",
    "tags": [
      "питчинг",
      "инвестиции",
      "переговоры"
    ],
    "description": "Формат похож на Shark Tank и показывает предпринимателей, которые защищают свои продукты перед инвесторами. Для приложения это хороший материал о презентации, оценке и слабых местах бизнес-модели. Главный вывод: инвестор покупает не идею, а доказанную возможность роста."
  },
  {
    "id": "the-profit",
    "title": "The Profit / Прибыль",
    "year": "2013–2021",
    "image": "assets/watch/series/watch_series_014.png",
    "tags": [
      "малый бизнес",
      "процессы",
      "финансы"
    ],
    "description": "Сериал показывает диагностику малого бизнеса через людей, продукт и процессы. Он очень прикладной: в сериях видно, где ломаются деньги, управление и предложение. Главный вопрос: что в вашем бизнесе слабее всего — продукт, команда или процесс?"
  },
  {
    "id": "undercover-billionaire",
    "title": "Undercover Billionaire / Миллиардер под прикрытием",
    "year": "2019–2021",
    "image": "assets/watch/series/watch_series_015.png",
    "tags": [
      "запуск",
      "ресурсы",
      "локальный рынок"
    ],
    "description": "Шоу показывает запуск бизнеса почти с нуля в незнакомой среде. Оно полезно для предпринимателя тем, что заставляет смотреть на ресурсы, связи, продажи и локальный спрос. Главный вывод: старт бизнеса — это не вдохновение, а быстрый сбор фактов и людей вокруг возможности."
  },
  {
    "id": "kitchen-nightmares",
    "title": "Kitchen Nightmares / Кошмары на кухне",
    "year": "2004–",
    "image": "assets/watch/series/watch_series_016.png",
    "tags": [
      "HoReCa",
      "операционка",
      "сервис"
    ],
    "description": "Сериал показывает ресторанный бизнес через хаос кухни, сервиса, меню, себестоимости и команды. Для предпринимателя это практический разбор операционных ошибок. Главный вопрос: какие проблемы вашего бизнеса клиент видит раньше, чем вы признаёте?"
  },
  {
    "id": "restaurant-impossible",
    "title": "Restaurant: Impossible / Ресторан: невозможное",
    "year": "2011–",
    "image": "assets/watch/series/watch_series_017.png",
    "tags": [
      "ресторан",
      "процессы",
      "финансы"
    ],
    "description": "Шоу разбирает рестораны через интерьер, меню, команду, операционную дисциплину и деньги. Оно полезно не только для HoReCa, потому что показывает типовые управленческие провалы малого бизнеса. Главный вывод: красивый ремонт не спасает бизнес без понятной модели и процесса."
  },
  {
    "id": "men-built-america",
    "title": "The Men Who Built America / Люди, построившие Америку",
    "year": "2012",
    "image": "assets/watch/series/watch_series_018.png",
    "tags": [
      "история",
      "монополии",
      "капитал"
    ],
    "description": "Сериал показывает индустриальных предпринимателей, которые строили железные дороги, нефть, сталь и финансы. Он полезен для понимания капитала, инфраструктуры, монополий и силы масштаба. Главный вопрос: какая инфраструктура делает ваш рынок возможным?"
  },
  {
    "id": "food-built-america",
    "title": "The Food That Built America / Еда, построившая Америку",
    "year": "2019–",
    "image": "assets/watch/series/watch_series_019.png",
    "tags": [
      "FMCG",
      "бренды",
      "упаковка"
    ],
    "description": "Сериал показывает продуктовые бренды, упаковку, франчайзинг и войны за массового потребителя. Он полезен для понимания того, как обычный продукт превращается в категорию и привычку. Главный вопрос: что делает ваш продукт повторяемым выбором клиента?"
  },
  {
    "id": "toys-built-america",
    "title": "The Toys That Built America / Игрушки, построившие Америку",
    "year": "2021–",
    "image": "assets/watch/series/watch_series_020.png",
    "tags": [
      "товары",
      "лицензии",
      "бренд"
    ],
    "description": "Сериал показывает рынок игрушек через спрос, лицензии, производство и брендовые войны. Для предпринимателя это разбор товарного бизнеса, где эмоция клиента связана с упаковкой и дистрибуцией. Главный вывод: продукт для массового рынка должен жить в системе каналов и истории."
  },
  {
    "id": "dirty-money",
    "title": "Dirty Money / Грязные деньги",
    "year": "2018–2020",
    "image": "assets/watch/series/watch_series_021.png",
    "tags": [
      "скандалы",
      "контроль",
      "мошенничество"
    ],
    "description": "Документальный сериал разбирает корпоративные скандалы, финансовые манипуляции и слабые места контроля. Для бизнеса это важный антипример того, как прибыль и отчётность могут скрывать системный риск. Главный вопрос: какой риск у вас сейчас замаскирован красивым показателем?"
  },
  {
    "id": "mcmillions",
    "title": "McMillions / МакМиллионы",
    "year": "2020",
    "image": "assets/watch/series/watch_series_022.png",
    "tags": [
      "франчайзинг",
      "промо",
      "мошенничество"
    ],
    "description": "Сериал показывает мошенничество вокруг промоакции McDonald’s и слабые места контроля в большой системе. Он полезен как кейс о процессах, доверии и проверке подрядчиков. Главный вывод: масштабная акция требует такой же масштабной системы контроля."
  },
  {
    "id": "inside-bills-brain",
    "title": "Inside Bill’s Brain / Внутри мозга Билла Гейтса",
    "year": "2019",
    "image": "assets/watch/series/watch_series_023.png",
    "tags": [
      "мышление",
      "системы",
      "сложные задачи"
    ],
    "description": "Документальный сериал показывает системное мышление Билла Гейтса и работу со сложными проблемами. Для предпринимателя он полезен как материал о фокусе, чтении, анализе и долгом горизонте. Главный вывод: сложная задача требует структуры, а не только желания её решить."
  },
  {
    "id": "abstract-design",
    "title": "Abstract: The Art of Design / Абстракция: искусство дизайна",
    "year": "2017–2019",
    "image": "assets/watch/series/watch_series_024.png",
    "tags": [
      "дизайн",
      "продукт",
      "ценность"
    ],
    "description": "Сериал показывает дизайнеров из разных областей и процесс создания ценности через форму, функцию и смысл. Для предпринимателя он полезен как тренировка продуктового мышления и внимания к деталям. Главный вопрос: как дизайн вашего продукта помогает клиенту принять решение?"
  },
  {
    "id": "the-office",
    "title": "The Office / Офис",
    "year": "2005–2013",
    "image": "assets/watch/series/watch_series_025.png",
    "tags": [
      "команда",
      "культура",
      "менеджмент"
    ],
    "description": "Комедийный сериал полезен как разбор слабого менеджмента, коммуникаций и офисной культуры. Под юмором хорошо видны ошибки руководителя, мотивации и командного взаимодействия. Главный вопрос: какие управленческие странности у вас стали нормой, потому что к ним привыкли?"
  },
  {
    "id": "better-call-saul",
    "title": "Better Call Saul / Лучше звоните Солу",
    "year": "2015–2022",
    "image": "assets/watch/series/watch_series_026.png",
    "tags": [
      "услуги",
      "личный бренд",
      "этика"
    ],
    "description": "Сериал показывает развитие личного бренда юриста, работу с нишей, продажу услуг и этические компромиссы. Для предпринимателя он полезен как материал о позиционировании и репутации. Главный вывод: личный бренд может быстро расти, но его фундаментом остаётся доверие."
  },
  {
    "id": "suits",
    "title": "Suits / Форс-мажоры",
    "year": "2011–2019",
    "image": "assets/watch/series/watch_series_027.png",
    "tags": [
      "переговоры",
      "клиенты",
      "юридический бизнес"
    ],
    "description": "Сериал показывает юридический бизнес, партнёрство, переговоры и обслуживание крупных клиентов. Для предпринимателя он полезен как разбор репутации, уверенности и клиентского сервиса в профессиональных услугах. Главный вопрос: насколько ваша услуга выглядит надёжной до того, как клиент увидел результат?"
  },
  {
    "id": "the-bear",
    "title": "The Bear / Медведь",
    "year": "2022–",
    "image": "assets/watch/series/watch_series_028.png",
    "tags": [
      "HoReCa",
      "операционка",
      "стандарты"
    ],
    "description": "Сериал показывает ресторан изнутри: стресс, команду, стандарты, сервис и операционные процессы. Он полезен как современный разбор того, как хаос превращается в систему. Главный вопрос: какой один процесс у вас должен быть стандартизирован первым?"
  },
  {
    "id": "mr-selfridge",
    "title": "Mr Selfridge / Мистер Селфридж",
    "year": "2013–2016",
    "image": "assets/watch/series/watch_series_029.png",
    "tags": [
      "ритейл",
      "витрина",
      "клиентский опыт"
    ],
    "description": "Сериал показывает развитие универмага, клиентского опыта, витрин, рекламы и розничной торговли. Для предпринимателя это хороший материал о том, как магазин становится не просто местом продажи, а событием для клиента. Главный вопрос: что в вашем бизнесе превращает покупку в опыт?"
  },
  {
    "id": "self-made",
    "title": "Self Made / Мадам Си Джей Уокер",
    "year": "2020",
    "image": "assets/watch/series/watch_series_030.png",
    "tags": [
      "женское предпринимательство",
      "продукт",
      "рынок красоты"
    ],
    "description": "Мини-сериал показывает предпринимательницу, которая строит продукт, продажи, личный бренд и сеть в индустрии красоты. Для бизнеса важны темы доступа к рынку, преодоления барьеров и создания предложения под конкретную аудиторию. Главный вывод: сильный продукт растёт быстрее, когда его история совпадает с болью клиента."
  }
];

  function watchAssertAccessV127(){
    if (typeof hasVerifiedAccessV32 === 'function' && !hasVerifiedAccessV32()) {
      if (typeof accessDenied === 'function') accessDenied('OPEN_FROM_TELEGRAM_REQUIRED');
      return false;
    }
  ...
  function watchUserSuffixV127(){
    try {
      var ids = typeof possibleIds === 'function' ? possibleIds() : [];
      if (ids && ids[0]) return String(ids[0]);
      var user = typeof getTelegramUser === 'function' ? getTelegramUser() : {};
      var username = typeof normalizeUsername === 'function' ? normalizeUsername((state.user && state.user.username) || user.username || '') : '';
      return username || 'local';
    } catch(e) { return 'local'; }
  }

  function watchStorageKeyV127(){ return 'architecture_watch_library_v127_' + watchUserSuffixV127(); }
  function loadWatchStateV127(){
    try {
      var parsed = JSON.parse(localStorage.getItem(watchStorageKeyV127()) || '{}');
      if (!parsed || typeof parsed !== 'object') parsed = {};
      if (!parsed.films || typeof parsed.films !== 'object') parsed.films = {};
      if (!parsed.series || typeof parsed.series !== 'object') parsed.series = {};
      if (!parsed.videos || typeof parsed.videos !== 'object') parsed.videos = {};
      if (!parsed.lectures || typeof parsed.lectures !== 'object') parsed.lectures = {};
      return parsed;
    } catch(e) { return { films:{}, series:{}, videos:{}, lectures:{} }; }
  }
  function saveWatchStateV127(data){
    try { localStorage.setItem(watchStorageKeyV127(), JSON.stringify(data || {})); } catch(e) {}
  }
  function watchItemsV127(type){
    if (type === 'films') return WATCH_FILMS_V127;
    if (type === 'series') return WATCH_SERIES_V127;
    return [];
  }
  function watchTypeTitleV127(type){
    if (type === 'films') return 'Фильмы';
    if (type === 'series') return 'Сериалы';
    if (type === 'videos') return 'Видео';
    if (type === 'lectures') return 'Лекции';
    return 'Что посмотреть';
  }
  function watchTypeNounV127(type){ return type === 'series' ? 'сериал' : 'фильм'; }
  function isWatchedV127(type, id){
    var stateData = loadWatchStateV127();
    return Boolean(stateData[type] && stateData[type][id]);
  }
  function watchStatsV127(type){
    var items = watchItemsV127(type);
    var stateData = loadWatchStateV127();
    var watched = items.filter(function(item){ return stateData[type] && stateData[type][item.id]; }).length;
    return { watched: watched, total: items.length, percent: items.length ? safePercent(watched / items.length * 100) : 0 };
  }
  function watchAwardV127(eventKey, eventType, payload){
    try { if (typeof awardResearchPointOnceV91 === 'function') awardResearchPointOnceV91(eventKey, eventType || 'watch_event', payload || {}); } catch(e) {}
  }
  function watchVersionV127(){
  ...
  function watchSectionCardV127(type, title, text, status, action, disabled){
    var stats = watchStatsV127(type);
    var progress = stats.total ? `<small>${formatPoints(stats.watched)} / ${formatPoints(stats.total)} посмотрено</small>` : `<small>${esc(status)}</small>`;
    return `<button class="watch-section-card-v127 ${disabled ? 'disabled' : ''}" ${disabled ? 'disabled' : `onclick="${action}"`}>
      <div><b>${esc(title)}</b><p>${esc(text)}</p>${progress}</div><span>${disabled ? '•' : '→'}</span>
    </button>`;
  }

  function installWatchStylesV127(){
    if (document.getElementById('watch-library-styles-v127')) return;
    var style = document.createElement('style');
    style.id = 'watch-library-styles-v127';
    style.textContent = `
      .watch-hero-v127 h1{margin-bottom:8px}
      .watch-section-grid-v127{display:grid;gap:10px}
      .watch-section-card-v127{width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;text-align:left;padding:15px;border-radius:18px;background:#fff;border:1px solid var(--lego-border,#DDE5EF);box-shadow:0 7px 18px rgba(16,36,58,.055)}
      .watch-section-card-v127 b{display:block;color:var(--lego-navy,#10243A);font-size:17px;margin-bottom:5px}
      .watch-section-card-v127 p{margin:0 0 8px;color:var(--lego-gray-700,#4D5B6B);font-size:13px;line-height:1.4}
      .watch-section-card-v127 small{display:inline-flex;padding:6px 9px;border-radius:999px;background:var(--lego-steel-light,#E7F0FA);color:var(--lego-steel,#2F5F9F);font-size:11px;font-weight:900}
      .watch-section-card-v127 span{font-size:24px;color:var(--lego-steel,#2F5F9F)}
      .watch-section-card-v127.disabled{opacity:.58;box-shadow:none;cursor:not-allowed;background:#F4F6F8}
      .watch-list-v127{display:grid;gap:11px}
      .watch-card-v127{display:grid;grid-template-columns:96px minmax(0,1fr);gap:12px;align-items:stretch;padding:10px;border-radius:18px;background:#fff;border:1px solid var(--lego-border,#DDE5EF);box-shadow:0 7px 18px rgba(16,36,58,.045)}
      .watch-card-v127.watched{background:linear-gradient(135deg,#EAF8F4,#FFFFFF);border-color:rgba(36,122,118,.35)}
      .watch-open-v127{width:100%;display:grid;grid-template-columns:96px minmax(0,1fr);gap:12px;grid-column:1 / -1;text-align:left;background:transparent;padding:0;border:0}
      .watch-card-copy-v127 b{display:block;color:var(--lego-navy,#10243A);font-size:15px;line-height:1.25;margin-bottom:4px}
      .watch-card-copy-v127 p{margin:0;color:var(--lego-gray-700,#4D5B6B);font-size:12.5px;line-height:1.38;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
      .watch-tags-v127{display:flex;flex-wrap:wrap;gap:5px;margin-top:8px}
      .watch-tags-v127 span{font-size:10px;font-weight:850;color:#247A76;background:#EAF7F4;border-radius:999px;padding:4px 7px}
      .watch-check-v127{grid-column:2;display:flex;align-items:center;gap:8px;margin-top:8px;color:var(--lego-navy,#10243A);font-size:13px;font-weight:850}
      .watch-check-v127 input{width:18px;height:18px;accent-color:#247A76;margin:0;flex:0 0 auto}
      .watch-status-v127{display:inline-flex;margin-top:8px;padding:5px 8px;border-radius:999px;background:#F1F4F7;color:#697586;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}
      .watch-card-v127.watched .watch-status-v127{background:#247A76;color:#fff}
      .watch-image-box-v127,.watch-image-fallback-v127{width:96px;min-height:122px;border-radius:14px;overflow:hidden;background:#EEF4FA;border:1px solid rgba(47,95,159,.18)}
      .watch-image-box-v127 img{width:100%;height:100%;min-height:122px;object-fit:cover;display:block}
      .watch-image-fallback-v127{display:grid;place-items:center;text-align:center;padding:8px;background:linear-gradient(135deg,#10243A,#2F5F9F);color:#fff}
      .watch-image-fallback-v127 b{display:block;color:#fff;font-size:13px;line-height:1.15}
      .watch-image-fallback-v127 span{display:block;margin-top:5px;color:#F8BA72;font-size:9px;font-weight:900;letter-spacing:.08em}
      .watch-image-box-v127.big,.watch-image-fallback-v127.big{width:100%;min-height:280px;border-radius:18px}
      .watch-image-box-v127.big img{min-height:280px;max-height:520px;object-fit:cover}
      .watch-detail-meta-v127{display:flex;flex-wrap:wrap;gap:7px;margin:10px 0}
      .watch-detail-meta-v127 span{padding:6px 9px;border-radius:999px;background:#F3F6F8;color:#697586;font-size:11px;font-weight:850}
      .watch-detail-text-v127 p{font-size:15px;line-height:1.62}
      .watch-detail-actions-v127{display:grid;gap:9px;margin-top:12px}
      .watch-progress-v127{margin-top:12px}
      .watch-progress-v127>div{height:9px;border-radius:999px;background:rgba(255,255,255,.18);overflow:hidden}
      .card-v2:not(.blue-card-v2) .watch-progress-v127>div{background:#E7ECF1}
      .watch-progress-v127 span{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,#2F5F9F,#247A76,#F28C28)}
      .watch-progress-v127 p{margin:6px 0 0!important;font-size:12px!important;color:inherit!important;opacity:.78}
      @media(max-width:420px){.watch-card-v127,.watch-open-v127{grid-template-columns:78px minmax(0,1fr)}.watch-image-box-v127,.watch-image-fallback-v127{width:78px;min-height:104px}.watch-image-box-v127 img{min-height:104px}.watch-check-v127{grid-column:1 / -1}}
    `;
    document.head.appendChild(style);
  }

  window.renderWatchLibraryV127 = function(){
    if (!watchAssertAccessV127()) return;
    installWatchStylesV127();
    watchAwardV127('watch:block', 'watch_block_open', { block:'watch' });
    var filmStats = watchStatsV127('films');
    var seriesStats = watchStatsV127('series');
    shell(`${card('blue-card-v2 watch-hero-v127', `<p class="eyebrow">что посмотреть</p><h1>Библиотека просмотра</h1><p>Фильмы, сериалы, видео и лекции для управленческого мышления. Отмечайте просмотренное — карточка будет подсвечиваться зелёным.</p>${watchProgressBarV127({ watched: filmStats.watched + seriesStats.watched, total: filmStats.total + seriesStats.total, percent: (filmStats.total + seriesStats.total) ? safePercent((filmStats.watched + seriesStats.watched)/(filmStats.total + seriesStats.total)*100) : 0 })}`)}
      ${card('watch-sections-v127', `<h2>Разделы</h2><div class="watch-section-grid-v127">
        ${watchSectionCardV127('films','Фильмы','30 фильмов о предпринимательстве, продажах, финансах, бренде, риске и масштабировании.','доступно',"renderWatchCategoryV127(\'films\')",false)}
        ${watchSectionCardV127('series','Сериалы','30 сериалов о стартапах, корпоративной власти, команде, сервисе и операционной системе.','доступно',"renderWatchCategoryV127(\'series\')",false)}
        ${watchSectionCardV127('videos','Видео','Короткие видеоразборы и интервью будут добавлены отдельным этапом.','в подготовке','',true)}
        ${watchSectionCardV127('lectures','Лекции','Лекции и длинные выступления будут добавлены после фильмов и сериалов.','в подготовке','',true)}
      </div>`)}
      ${card('', `<button class="btn secondary" onclick="renderHome()">На главную</button>`)}`, 'home');
  };

  window.renderWatchCategoryV127 = function(type){
    if (!watchAssertAccessV127()) return;
    installWatchStylesV127();
    var items = watchItemsV127(type);
    if (!items.length) return renderWatchLibraryV127();
    var stats = watchStatsV127(type);
    watchAwardV127('watch:category:' + type, 'watch_category_open', { type:type });
    shell(`${card('blue-card-v2 watch-hero-v127', `<p class="eyebrow">что посмотреть</p><h1>${esc(watchTypeTitleV127(type))}</h1><p>Откройте карточку, прочитайте описание и отметьте, если уже посмотрели. Посмотренные материалы подсвечиваются зелёным.</p>${watchProgressBarV127(stats)}`)}
      ${card('', `<div class="watch-list-v127">${items.map(function(item,index){
        var watched = isWatchedV127(type,item.id);
        var tags = (item.tags || []).slice(0,3).map(function(t){ return `<span>${esc(t)}</span>`; }).join('');
        return `<section class="watch-card-v127 ${watched ? 'watched' : ''}" data-watch-card="${esc(type + ':' + item.id)}">
          <button class="watch-open-v127" onclick="renderWatchItemV127('${esc(type)}','${esc(item.id)}')">
            ${watchImageHtmlV127(item,index,type,false)}
            <div class="watch-card-copy-v127"><b>${String(index+1).padStart(2,'0')}. ${esc(item.title)}</b><p>${esc(item.description)}</p><div class="watch-tags-v127">${tags}</div><em class="watch-status-v127">${watched ? 'посмотрено' : 'не посмотрено'}</em></div>
          </button>
          <label class="watch-check-v127" onclick="event.stopPropagation()"><input type="checkbox" ${watched ? 'checked' : ''} onchange="toggleWatchItemV127('${esc(type)}','${esc(item.id)}',this.checked)"><span>Посмотрел</span></label>
        </section>`;
      }).join('')}</div>`)}
      ${card('', `<div class="grid-v2"><button class="btn secondary" onclick="renderWatchLibraryV127()">К разделам</button><button class="btn secondary" onclick="renderHome()">На главную</button></div>`)}`, 'home');
  };

  window.renderWatchItemV127 = function(type, id){
    if (!watchAssertAccessV127()) return;
    installWatchStylesV127();
    var items = watchItemsV127(type);
    var index = items.findIndex(function(x){ return x.id === id; });
    var item = items[index];
    if (!item) return renderWatchCategoryV127(type);
    var watched = isWatchedV127(type,item.id);
    watchAwardV127('watch:item:' + type + ':' + item.id, 'watch_item_open', { type:type, itemId:item.id, title:item.title });
    var tags = (item.tags || []).map(function(t){ return `<span>${esc(t)}</span>`; }).join('');
    shell(`${card('blue-card-v2 watch-hero-v127', `<p class="eyebrow">${esc(watchTypeTitleV127(type))}</p><h1>${esc(item.title)}</h1><p>${esc(item.year || '')}</p>`)}
      ${card('watch-detail-card-v127', `${watchImageHtmlV127(item,index,type,true)}<div class="watch-detail-meta-v127"><span>${esc(watchTypeNounV127(type))}</span><span>${esc(item.year || '')}</span>${tags}</div><div class="watch-detail-text-v127"><p>${esc(item.description)}</p></div><div class="watch-detail-actions-v127"><label class="watch-check-v127"><input type="checkbox" ${watched ? 'checked' : ''} onchange="toggleWatchItemV127('${esc(type)}','${esc(item.id)}',this.checked)"><span>Посмотрел</span></label><div class="grid-v2"><button class="btn primary" onclick="renderWatchCategoryV127('${esc(type)}')">К списку</button><button class="btn secondary" onclick="renderWatchLibraryV127()">К разделам</button></div></div>`)}`, 'home');
  };

  window.toggleWatchItemV127 = function(type, id, checked){
    var data = loadWatchStateV127();
    if (!data[type]) data[type] = {};
    data[type][id] = Boolean(checked);
    saveWatchStateV127(data);
    var item = watchItemsV127(type).find(function(x){ return x.id === id; });
    if (checked && item) watchAwardV127('watch:watched:' + type + ':' + id, 'watch_mark_watched', { type:type, itemId:id, title:item.title });
    document.querySelectorAll('[data-watch-card="' + type + ':' + id + '"]').forEach(function(card){
      card.classList.toggle('watched', Boolean(checked));
      var status = card.querySelector('.watch-status-v127');
      if (status) status.textContent = checked ? 'посмотрено' : 'не посмотрено';
      var input = card.querySelector('input[type="checkbox"]');
      if (input) input.checked = Boolean(checked);
    });
  };

  // Старое имя раздела теперь ведёт в новую библиотеку.
  window.renderWatchV40 = window.renderWatchLibraryV127;

  // Открываем раздел для учеников: удаляем из списка заблокированных будущих блоков.
  try {
    if (Array.isArray(window.studentLockedBlocksV41)) {
      var i = window.studentLockedBlocksV41.indexOf('Что посмотреть');
      if (i >= 0) window.studentLockedBlocksV41.splice(i, 1);
    }
  } catch(e) {}

  // Главная: карточка «Что посмотреть» становится активной.
  window.secondaryBlocksHtmlV40 = function(){
    var forumReady = Boolean(typeof forumVisibleInNavigationV38 === 'function' && forumVisibleInNavigationV38() && typeof window.renderBusinessForum === 'function');
    var forumStatus = forumReady ? (isAdminMode() ? 'тестирование' : 'доступно') : 'в подготовке';
    var forumClass = forumReady ? 'active' : 'soon';
    return `<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40">
      ...
      ${renderMainBlockCard('Предпринимательские статьи','Практические статьи о ситуациях, цифрах, решениях и последствиях.','скоро','renderEntrepreneurArticlesV40()','soon compact-card')}
      ${renderMainBlockCard('Прямые разборы','Гарвардские и другие бизнес-кейсы с разбором вариантов решения.','скоро','renderDirectReviewsV40()','soon compact-card')}
      ${renderMainBlockCard('Что посмотреть','Фильмы, сериалы, видео и лекции с управленческими выводами.','доступно','renderWatchLibraryV127()','active compact-card')}
      ${renderMainBlockCard('Дополнительные материалы','Точка безубыточности, единое уравнение бизнеса и практические инструменты вне основного маршрута.','доступно','renderAdditionalMaterials()','active compact-card additional-materials-entry-v110')}
      ${renderMainBlockCard('VIP уровень','Расширенные разборы, инструменты и закрытые возможности.','в разработке','renderVipV40()','soon compact-card')}
    </div>`;
  };

  function unlockWatchDrawerV127(){
    try {
      document.querySelectorAll('.app-drawer-list-v40 > button').forEach(function(button){
        var titleNode = button.querySelector('b');
        var title = titleNode ? titleNode.textContent.trim() : '';
        if (title !== 'Что посмотреть') return;
        button.disabled = false;
        button.classList.remove('student-locked-v41');
        button.setAttribute('onclick','closeAppDrawerV40(); renderWatchLibraryV127()');
        var status = button.querySelector('small');
  ...
  window.unlockWatchDrawerV127 = unlockWatchDrawerV127;

  var lockBeforeV127 = window.lockDrawerItemsV41;
  if (typeof lockBeforeV127 === 'function') {
    window.lockDrawerItemsV41 = function(){
      var result = lockBeforeV127.apply(this, arguments);
      unlockWatchDrawerV127();
      return result;
    };
  }

  var shellBeforeWatchV127 = window.shell;
  if (typeof shellBeforeWatchV127 === 'function' && !shellBeforeWatchV127.__watchLibraryV127) {
    var shellWatchV127 = function(content, activeTab){
      var result = shellBeforeWatchV127(content, activeTab);
      setTimeout(unlockWatchDrawerV127, 0);
      setTimeout(unlockWatchDrawerV127, 120);
      return result;
    };
    shellWatchV127.__watchLibraryV127 = true;
    window.shell = shellWatchV127;
