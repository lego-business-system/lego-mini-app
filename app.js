...
(function installFinancialAssistantV49(){
  ...
  function financeCanOpenSectionV49(sectionId){
    ...
    if (id === 1) return true;
    return financeSectionOnePassedV49();
  }
  ...
  function financeProgressPillV49(){
    ...
    return `<div class="finance-status-pill ${passed ? 'passed' : 'locked'}">
      <span>${passed ? 'Раздел 1 засчитан' : 'Разделы 2–11 закрыты'}</span>
      <b>${score}/${total} · проходной ${FINANCE_SECTION1_PASS_SCORE_V49}/${FINANCE_SECTION1_TEST_V49.length}</b>
    </div>`;
  }
  ...
  function financeSectionCardHtmlV49(section){
    ...
    const lessonsCount = (section.lessons || []).length + (Number(section.id) === 1 ? 1 : 0);
    const status = Number(section.id) === 1
      ? 'доступно'
      : (open ? 'открыто после теста' : 'закрыто до теста 80%');
    return `<button class="finance-section-card-v49 ${open ? '' : 'locked'}" ${open ? `onclick="renderFinanceSection(${Number(section.id)})"` : `onclick="financeLockedSectionNotice()"`}>
      ...
      <em>${lessonsCount} уроков · ${status}</em>
    </button>`;
  }

  function financeLockedSectionNoticeV49(){
    alert('Следующие разделы откроются после прохождения итогового теста первого раздела минимум на 80%.');
  }
  ...
  function renderFinanceModuleHomeV49(){
    ...
    const html = `
      ...
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К помощнику</button></div><h2>11 разделов модуля</h2><p>Раздел 1 доступен сразу. Разделы 2–11 открываются после результата 80% по итоговому тесту первого раздела.</p><div class="finance-section-list-v49">${FINANCE_MODULE_SECTIONS_V49.map(financeSectionCardHtmlV49).join('')}</div>`)}
    `;
    shell(html, 'profile');
  }
  ...
  function renderFinanceSectionV49(sectionId){
    ...
    const testRow = Number(section.id) === 1
      ? `<button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v49" onclick="startFinanceSection1Test(true)">
          <div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает следующие разделы</p></div><span>→</span>
        </button>`
      : '';

    const html = `
      ${card('blue-card-v2 finance-hero-v49', `<p class="eyebrow">раздел ${Number(section.id)}</p><h1>${esc(section.title)}</h1><p>${esc(section.description || '')}</p>${Number(section.id) === 1 ? financeProgressPillV49() : ''}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам</button></div><h2>Уроки раздела</h2><div class="lesson-list-v2">${(section.lessons || []).map(financeLessonRowHtmlV49).join('')}${testRow}</div>`)}
    `;
    shell(html, 'profile');
  }
  ...
  function finishFinanceSection1TestV49(){
    ...
    const statusText = passed
      ? 'Раздел 1 засчитан. Следующие разделы финансового модуля открыты.'
      : 'Результат ниже 80%. Следующие разделы пока остаются закрытыми. Повторите первые 4 урока и пройдите диагностику заново.';
    const html = `
      ${card(passed ? 'result-ok-v2' : 'result-bad-v2', `<h1>${statusTitle}</h1><p>Результат: <b>${score}/${total}</b>. Проходной уровень: <b>${FINANCE_SECTION1_PASS_SCORE_V49}/${total}</b>.</p><p>Критических ошибок: <b>${criticalErrors}</b>.</p><p>${statusText}</p><div class="grid-v2"><button class="btn primary" onclick="${passed ? 'renderFinanceModuleHome()' : 'startFinanceSection1Test(true)'}">${passed ? 'К финансовому модулю' : 'Пройти заново'}</button><button class="btn secondary" onclick="renderFinanceSection(1)">К разделу 1</button></div>`)}
...
(function installFinanceHardStudentLockV55(){
  ...
  function renderFinanceAssistantHubV55(){
    ...
    const html = `
      ${card('blue-card-v2 finance-hero-v49', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Административный предпросмотр материалов. Для учеников блок остаётся закрытым до отдельного запуска.</p>`)}
      ${card('', `<h2>Выберите раздел помощника</h2><div class="finance-hub-grid-v49">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>11 разделов, 62 урока, первые 4 урока полностью наполнены текстом, итоговая диагностика стоит в конце первого раздела.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Черновой контур для будущей аналитики бизнеса: ввод фактов, показатели и первичные выводы.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Место для будущих таблиц, шаблонов и инструментов финансового модуля.</p><em>скоро</em></button>
