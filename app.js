...
const APP_CACHE_VERSION = "v57-finance-content-audit-trainer-20260628";
...
(function installFinancialAssistantV49(){
  ...
  /* =====================================================
     v57 — Финансовый модуль: аудит текста, поэтапное открытие разделов,
     финтренажер раздела 1 и нормальная подача вводных экранов
     ===================================================== */
  window.APP_UI_VERSION_V57 = 'v57-finance-content-gating-trainer-20260628';

  const FINANCE_SECTION1_TRAINER_URL_V57 = (window.FINANCE_SECTION1_TRAINER_URL || '').trim() || '#';

  function financeCleanupGrammarV57(value){
    let out = String(value || '').trim();
    if (!out) return '';
    out = out.replace(/\s+/g, ' ');
    out = out.replace(/ОПУ/g, 'ОПиУ');
    ...
    out = out.replace(/предпринимательу/gi, 'предпринимателю');
    out = out.replace(/предпринимательом/gi, 'предпринимателем');
    out = out.replace(/предпринимателье/gi, 'предпринимателе');
    out = out.replace(/у предприниматель\b/gi, 'у предпринимателя');
    out = out.replace(/для предприниматель\b/gi, 'для предпринимателя');
    out = out.replace(/к предприниматель\b/gi, 'к предпринимателю');
    out = out.replace(/с предприниматель\b/gi, 'с предпринимателем');
    out = out.replace(/о предприниматель\b/gi, 'о предпринимателе');
    out = out.replace(/предприниматель должен понять/gi, 'формируется понимание');
    out = out.replace(/предприниматель должен уметь/gi, 'формируется навык');
    out = out.replace(/Этот урок должен полностью убрать у предпринимателя привычку/gi, 'Основной смысл урока — убрать привычку');
    out = out.replace(/Если предприниматель понял этот кейс, он уже не будет/gi, 'После разбора кейса уже нельзя');
  ...
  function financeCleanLearningCopyV57(value){
    const base = (typeof financeCleanLearningCopyV56 === 'function') ? financeCleanLearningCopyV56(value) : String(value || '');
    return financeCleanupGrammarV57(base);
  }
  ...
  function financeNextLockedReasonV57(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return '';
    return `Раздел ${id} откроется после прохождения итоговой диагностики раздела ${id - 1} минимум на 80%.`;
  }

  function financeLockedSectionNoticeV57(sectionId){
    alert(financeNextLockedReasonV57(sectionId || 2) || 'Раздел пока закрыт.');
  }

  financeSectionOnePassedV49 = function(){ return financeSectionPassedV57(1); };
  financeCanOpenSectionV49 = financeCanOpenSectionV57;
  financeLockedSectionNoticeV49 = financeLockedSectionNoticeV57;

  function financeAttemptedSection1V57(){
    const progress = loadFinanceProgressV49();
    return Boolean(progress.section1TestAt) || progress.section1TestScore !== undefined;
  }
  ...
  function financeProgressPillV57(){
    ...
    const score = Number(progress.section1TestScore || 0);
    const total = Number(progress.section1TestTotal || FINANCE_SECTION1_TEST_V49.length);
    const percent = total ? safePercent(score / total * 100) : 0;
    const attempted = financeAttemptedSection1V57();
    const passed = financeSectionPassedV57(1);
    if (!attempted) {
      return `<div class="finance-status-pill locked finance-status-pill-v57"><span>Диагностика раздела 1 ещё не пройдена</span><b>Для открытия раздела 2 нужно 80%: ${FINANCE_SECTION1_PASS_SCORE_V49}/${FINANCE_SECTION1_TEST_V49.length}</b></div>`;
    }
    return `<div class="finance-status-pill ${passed ? 'passed' : 'locked'} finance-status-pill-v57"><span>${passed ? 'Раздел 1 засчитан · раздел 2 открыт' : 'Раздел 1 пока не засчитан'}</span><b>${score}/${total} · ${percent}% · проходной уровень 80%</b></div>`;
  }

  financeProgressPillV49 = financeProgressPillV57;

  function financeSectionProgressPanelV57(sectionId){
    const id = Number(sectionId || 0);
    if (id !== 1) {
      const previousPassed = id <= 1 || financeSectionPassedV57(id - 1);
      return `<div class="finance-section-progress-v57 ${previousPassed ? 'open' : 'locked'}"><b>${previousPassed ? 'Раздел открыт' : 'Раздел закрыт'}</b><p>${previousPassed ? 'Этот раздел доступен после выполнения условия предыдущего раздела.' : esc(financeNextLockedReasonV57(id))}</p></div>`;
    }
    const progress = loadFinanceProgressV49();
    const score = Number(progress.section1TestScore || 0);
    const total = Number(progress.section1TestTotal || FINANCE_SECTION1_TEST_V49.length);
    const percent = total && financeAttemptedSection1V57() ? safePercent(score / total * 100) : 0;
    const attempted = financeAttemptedSection1V57();
    const passed = financeSectionPassedV57(1);
    return `<div class="finance-section-progress-v57 ${passed ? 'open' : 'locked'}">
      <div class="finance-section-progress-head-v57"><b>${passed ? 'Итоговая диагностика пройдена' : (attempted ? 'Итоговая диагностика пока не пройдена' : 'Итоговая диагностика ещё не пройдена')}</b><span>${attempted ? `${score}/${total} · ${percent}%` : '0% пока не показываем как результат'}</span></div>
      <div class="progress-line"><div style="width:${percent}%"></div></div>
      <p>${passed ? 'Раздел 2 открыт. Следующая логика: каждый новый раздел будет открываться после диагностики предыдущего.' : `Для открытия раздела 2 нужно пройти диагностику раздела 1 минимум на 80%: ${FINANCE_SECTION1_PASS_SCORE_V49}/${FINANCE_SECTION1_TEST_V49.length}.`}</p>
    </div>`;
  }
  ...
  function financeSectionStatusV57(section){
    ...
    if (financeCanOpenSectionV57(id)) return 'открыто';
    return `после теста раздела ${id - 1}`;
  }

  function financeSectionCardHtmlV57(section){
    const open = financeCanOpenSectionV57(section.id);
    const id = Number(section.id);
    const additional = id === 1 ? 2 : 0;
    const lessonsCount = (section.lessons || []).length + additional;
    return `<button class="finance-section-card-v49 finance-section-card-v57 ${open ? '' : 'locked'}" ${open ? `onclick="renderFinanceSection(${id})"` : `onclick="financeLockedSectionNotice(${id})"`}>
      <div><span>Раздел ${id}</span><b>${esc(section.title)}</b><p>${esc(section.description || '')}</p></div>
      <em>${lessonsCount} блоков · ${esc(financeSectionStatusV57(section))}</em>
    </button>`;
  }

  function financeModuleMapHtmlV57(){
    return `<div class="finance-module-map-v57">
      <div><b>01</b><p>Финансовое мышление: деньги, прибыль, выручка, ОПиУ, ОДДС, баланс.</p></div>
      ...
      <div><b>08–11</b><p>Планирование, метрики, отраслевые финансы и регулярная финансовая система управления.</p></div>
    </div>`;
  }

  function renderFinancialAssistantV57(){
    if (!requireFinanceAdminV49()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Это контур финансовой архитектуры бизнеса: обучение, будущая личная аналитика и готовые шаблоны. Его задача — научить видеть не одну кассу, а всю систему: операции, выручку, прибыль, деньги, остатки, баланс, метрики и решения.</p><p class="small">Каждый следующий раздел финансового модуля открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>${financeProgressPillV57()}`)}
      ${card('', `<h2>Что внутри</h2><div class="finance-hub-grid-v49 finance-hub-grid-v57">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>Пошаговая учебная система из 11 разделов: от базового финансового мышления до отчётов, баланса, метрик, отраслевой логики и регулярного управления.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий личный контур для фактов бизнеса, показателей, рисков и управленческих выводов. Пока открыт как каркас для проверки логики.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Место для таблиц, тренажёров и финансовых инструментов, которые будут применяться после изучения соответствующих разделов.</p><em>готовится</em></button>
  ...
  function renderFinanceModuleHomeV57(){
    if (!requireFinanceAdminV49()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Финансовая архитектура показывает, как бизнес зарабатывает, как движутся деньги, где появляются остатки, почему нужен баланс и как метрики превращаются в управленческие решения.</p><p class="small">Логика прохождения строгая: раздел 1 доступен сразу, каждый следующий раздел открывается только после итоговой диагностики предыдущего раздела на 80% и выше.</p>${financeProgressPillV57()}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>Карта финансового модуля</h2>${financeModuleMapHtmlV57()}<h2>11 разделов</h2><div class="finance-section-list-v49">${FINANCE_MODULE_SECTIONS_V49.map(financeSectionCardHtmlV57).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }
  ...
  function financeLessonRowHtmlV57(lesson){
    const full = Array.isArray(lesson.fullContent) && lesson.fullContent.length;
    const label = full ? 'учебные экраны с полным текстом' : 'структура из общей карты модуля';
    return `<button class="lesson-row-v2 finance-lesson-row-v49" onclick="renderFinanceLesson(${Number(lesson.id)})"><div><b>${String(lesson.id).padStart(2,'0')}. ${esc(lesson.title)}</b><p>${esc(label)}</p></div><span>→</span></button>`;
  }

  function financeSection1TestRowV57(){
    const progress = loadFinanceProgressV49();
    const attempted = financeAttemptedSection1V57();
    const passed = financeSectionPassedV57(1);
    const score = Number(progress.section1TestScore || 0);
    const total = Number(progress.section1TestTotal || FINANCE_SECTION1_TEST_V49.length);
    const label = passed ? `пройдено · ${score}/${total}` : (attempted ? `не пройдено · ${score}/${total}` : '30 вопросов · проходной уровень 80%');
    return `<button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v49" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>${esc(label)} · открывает раздел 2</p></div><span>→</span></button>`;
  }

  function financeSection1TrainerRowV57(){
    return `<button class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57" onclick="renderFinanceSection1Trainer()"><div><b>Финтренажер раздела 1</b><p>Таблица для копирования: дополнительное закрепление операций, ОПиУ, ОДДС, баланса и управленческого вывода.</p></div><span>→</span></button>`;
  }
  ...
  function renderFinanceSectionV57(sectionId){
    ...
    if (!financeCanOpenSectionV57(section.id)) return financeLockedSectionNoticeV57(section.id);
    const id = Number(section.id);
    const extraRows = id === 1 ? financeSection1TestRowV57() + financeSection1TrainerRowV57() : '';
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел ${id}</p><h1>${esc(section.title)}</h1><p>${esc(section.description || '')}</p>${id === 1 ? financeSectionProgressPanelV57(1) : ''}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам</button></div><h2>Блоки раздела</h2><div class="lesson-list-v2">${(section.lessons || []).map(financeLessonRowHtmlV57).join('')}${extraRows}</div>`)}
    `;
    shell(html, 'finance');
  }

  function renderFinanceSection1TrainerV57(){
    if (!requireFinanceAdminV49()) return;
    const url = FINANCE_SECTION1_TRAINER_URL_V57;
    const linkButton = url && url !== '#'
      ? `<button class="btn primary" onclick="openFinanceSection1TrainerTable()">Открыть таблицу для копирования</button>`
      : `<button class="btn primary" onclick="openFinanceSection1TrainerTable()">Открыть таблицу для копирования</button><p class="small finance-template-note-v57">Ссылка на таблицу ещё не подключена. После загрузки шаблона нужно вставить URL в FINANCE_SECTION1_TRAINER_URL.</p>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел 1 · дополнительное закрепление</p><h1>Финтренажер раздела 1</h1><p>Тренажер нужен, чтобы закрепить главный навык раздела: различать финансовый смысл операции и видеть её влияние на ОПиУ, ОДДС и баланс.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как пройти тренажер</h2><div class="list-clean finance-trainer-instruction-v57"><div><b>1. Открыть таблицу</b><p>Откройте шаблон тренажера по кнопке ниже.</p></div><div><b>2. Скопировать файл</b><p>Сделайте личную копию таблицы, чтобы работать в своей версии и не менять исходный шаблон.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>В таблице будет отдельный лист с правилами прохождения и пояснениями.</p></div><div><b>4. Пройти задания</b><p>Разберите операции, отметьте влияние на ОПиУ, ОДДС и баланс, затем проверьте управленческий вывод.</p></div></div>${linkButton}<button class="btn secondary" onclick="renderFinanceSection(1)">Вернуться к урокам раздела</button>`)}
    `;
    shell(html, 'finance');
  }

  function openFinanceSection1TrainerTableV57(){
    const url = FINANCE_SECTION1_TRAINER_URL_V57;
    if (!url || url === '#') {
      alert('Ссылка на таблицу тренажера ещё не подключена. После создания шаблона нужно вставить ссылку в FINANCE_SECTION1_TRAINER_URL.');
      return;
    }
  ...
  function financeLessonFormatV57(lesson, parsed){
    const id = Number(lesson.id);
    const formats = {
      1: 'Теоретическое объяснение, разбор финансовых понятий, сквозной кейс, сравнение влияния на ОПиУ, ОДДС и баланс.',
      2: 'Теоретическое объяснение, схема финансовой карты, сквозной кейс одного месяца, разбор через ОПиУ, ОДДС, баланс и метрики.',
      3: 'Теория, сравнительные схемы, разбор бухгалтерского, налогового и управленческого контуров, кейс расхождения прибыли и кассового риска.',
      4: 'Теория, отраслевые схемы, формулы драйверов, сравнительный кейс и диагностика экономического двигателя бизнеса.'
    };
    return formats[id] || financeCleanLearningCopyV57(lesson.content || 'Теория, кейс, типовые ошибки и управленческий вывод.');
  }

  function financeLessonMainTaskV57(lesson){
    const id = Number(lesson.id);
    const tasks = {
      1: 'Разрушить базовый финансовый миф предпринимателя: деньги на счёте не равны прибыли, а поступление денег не всегда является выручкой.',
      2: 'Показать бизнес как систему: операции превращаются в выручку, деньги — в денежный поток, остатки — в баланс, а метрики — в решения.',
      3: 'Отделить управленческую финансовую систему собственника от бухгалтерского и налогового учёта.',
      4: 'Показать, за счёт чего конкретный бизнес зарабатывает и где у него возникает главное финансовое ограничение.'
    };
    return tasks[id] || financeCleanLearningCopyV57(lesson.objective || 'Раскрыть финансовую логику темы через предпринимательский кейс.');
  }

  function financeLessonMainOutputV57(lesson){
    const id = Number(lesson.id);
    const outputs = {
      1: 'Предприниматель перестаёт оценивать бизнес только по остатку денег и разделяет три финансовые реальности: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.',
      2: 'Финансы бизнеса перестают выглядеть как одна таблица кассы. Одна операция начинает читаться через прибыльность, движение денег, остатки и управленческие метрики.',
    ...
    return outputs[id] || financeCleanLearningCopyV57(lesson.result || 'Формируется управленческий вывод по теме.');
  }

  function parseFinanceLessonScreensV57(lesson){
    const parsed = parseFinanceLessonScreensV56(lesson);
    parsed.introRaw = (parsed.introRaw || []).map(financeCleanLearningCopyV57).filter(Boolean);
    parsed.finalRaw = (parsed.finalRaw || []).map(financeCleanLearningCopyV57).filter(Boolean);
    parsed.slides = (parsed.slides || []).map(function(slide){
      return {
        title: financeCleanLearningCopyV57(slide.title),
        text: (slide.text || []).map(financeCleanLearningCopyV57).filter(Boolean),
        visualBrief: (slide.visualBrief || []).map(financeCleanLearningCopyV57).filter(Boolean)
      };
    });
    return parsed;
  }

  function financeLessonCoverScreenHtmlV57(lesson, parsed, totalScreens){
    const slidesCount = (parsed.slides || []).length;
    const rows = [
      ['Количество слайдов', `${slidesCount}.`],
      ['Формат урока', financeLessonFormatV57(lesson, parsed)],
      ['Главная задача', financeLessonMainTaskV57(lesson)],
      ['Основной вывод', financeLessonMainOutputV57(lesson)]
    ];
    const grid = rows.map(function(row){ return `<div class="finance-cover-line-v53 finance-cover-line-v56"><span>${esc(row[0])}</span><p>${esc(row[1])}</p></div>`; }).join('');
    return `${card('blue-card-v2 finance-hero-v49 finance-lesson-cover-v53 finance-lesson-cover-v56 finance-lesson-cover-v57', `<p class="eyebrow">раздел ${Number(lesson.sectionId)} · урок ${Number(lesson.id)}</p><h1>${esc(lesson.title)}</h1><p>Структура урока перед основной последовательностью слайдов.</p>`)}${card('finance-cover-card-v53 finance-cover-card-v56', `<h2>Титульный лист урока</h2><div class="finance-cover-grid-v53 finance-cover-grid-v56">${grid}</div><div class="finance-screen-counter-v53">Экран 1 из ${Number(totalScreens)}</div>`)}`;
  }

  function financeParagraphsHtmlV57(lines){
    return (lines || []).map(function(line){ return `<p>${esc(financeCleanLearningCopyV57(line))}</p>`; }).join('');
  }

  function financeExtraClarificationHtmlV57(lesson, slide){
    const id = Number(lesson.id);
    const title = financeSlideCleanTitleV56(slide.title).toLowerCase();
    const all = (title + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (id === 1 && (title.includes('правило собственника') || all.includes('сначала смысл'))) {
      return `<div class="finance-extra-clarification-v57"><span>Проверочный порядок</span><p>Сначала определяется источник денег. Затем проверяется, передана ли клиенту ценность. После этого фиксируется влияние на ОПиУ, ОДДС и баланс. Только после такой проверки можно делать вывод о прибыли, свободных деньгах и риске.</p></div>`;
    }
    if (id === 2 && all.includes('один месяц')) {
      return `<div class="finance-extra-clarification-v57"><span>Как читать кейс</span><p>Одна операция не должна терять смысл при переходе между отчётами. Оказанная услуга создаёт выручку, оплата создаёт денежный поток, аванс создаёт обязательство, дебиторка объясняет разрыв между прибылью и деньгами.</p></div>`;
    }
    if (id === 3 && all.includes('прибыль') && all.includes('риск')) {
      return `<div class="finance-extra-clarification-v57"><span>Правило разбора</span><p>Не нужно выбирать, кто прав: бухгалтерия или собственник. Нужно разделить вопросы: прибыльность — через ОПиУ, платёжеспособность — через ОДДС и платёжный календарь, финансовое положение — через баланс.</p></div>`;
    }
    if (id === 4 && (all.includes('одинаковая выручка') || all.includes('экономический двигатель'))) {
      return `<div class="finance-extra-clarification-v57"><span>Диагностический вопрос</span><p>Перед ростом нужно ответить: за счёт чего появляется выручка, где создаётся маржа, что ограничивает мощность, где деньги застревают и какая метрика первой покажет ухудшение модели.</p></div>`;
    }
    return '';
  }

  function financeLessonSlideScreenHtmlV57(lesson, slide, slideNumber, slidesTotal, screenIndex, totalScreens){
    const cleanSlide = { title: financeCleanLearningCopyV57(slide.title), text: (slide.text || []).map(financeCleanLearningCopyV57), visualBrief: slide.visualBrief || [] };
    const title = financeSlideCleanTitleV56(cleanSlide.title);
    const body = financeParagraphsHtmlV57(cleanSlide.text) || '<p>Текст слайда будет добавлен после редакторской проверки.</p>';
    const mainThought = financeCleanLearningCopyV57(financeSlideMainThoughtV56(lesson, cleanSlide));
    const trap = financeCleanLearningCopyV57(financeSlideTrapV56(lesson, cleanSlide));
    const takeaway = financeCleanLearningCopyV57(financeSlideTakeawayV56(lesson, cleanSlide));
    const matrix = financeImpactMatrixHtmlV56(lesson, cleanSlide);
    const extra = financeExtraClarificationHtmlV57(lesson, cleanSlide);
    return `${card('finance-slide-card-v53 finance-slide-card-v56 finance-slide-card-v57', `<p class="eyebrow">слайд ${slideNumber} из ${slidesTotal}</p><h1>${esc(title)}</h1><div class="finance-image-placeholder-v53 finance-image-placeholder-v56"><div><b>Место для изображения</b><p>Изображение будет подключено после генерации визуалов.</p></div></div><div class="finance-main-thought-v56"><span>Главная мысль</span><p>${esc(mainThought)}</p></div><div class="finance-slide-body-v53 finance-slide-body-v56"><h3>Объяснение</h3>${body}</div>${extra}<div class="finance-lesson-callouts-v56"><div><span>Типовая ошибка</span><p>${esc(trap)}</p></div><div><span>Управленческий вывод</span><p>${esc(takeaway)}</p></div></div>${matrix}<div class="finance-screen-counter-v53">Экран ${screenIndex + 1} из ${totalScreens}</div>`)}`;
  }

  function financeLessonFinalTakeawaysV57(lesson){
    const id = Number(lesson.id);
    if (id === 1) {
      return [
        ...
        'Любая сумма сначала получает финансовый смысл, а уже потом попадает в ОПиУ, ОДДС и баланс.',
        'Остаток денег — не диагноз бизнеса. Диагноз появляется только после связи прибыли, денег, активов и обязательств.'
      ];
    }
    return financeLessonFinalTakeawaysV56(lesson).map(financeCleanLearningCopyV57);
  }

  function financeLessonSituationsHtmlV57(lesson){
    if (Number(lesson.id) !== 1) return '';
    const situations = [
      ['01', 'Заработанная выручка', 'Клиент уже получил ценность. В ОПиУ появляется выручка, в ОДДС — операционное поступление, в балансе — рост денег.'],
      ['02', 'Аванс клиента', 'Деньги пришли раньше выполнения обязательства. В ОДДС есть поступление, в балансе появляется обязательство перед клиентом.'],
      ['03', 'Кредит', 'Деньги пришли как внешнее финансирование. В ОПиУ выручки нет, в ОДДС — финансовый поток, в балансе растёт долг.'],
      ['04', 'Возврат дебиторки', 'Клиент погасил старый долг. Деньги пришли сейчас, но выручка могла быть признана раньше.'],
      ['05', 'Вклад собственника', 'Касса усилилась за счёт владельца, но бизнес не заработал выручку от клиента.']
    ];
    return `<h2>Пять ситуаций одной суммы</h2><div class="finance-situation-grid-v57">${situations.map(function(row){ return `<div><b>${row[0]}</b><span>${esc(row[1])}</span><p>${esc(row[2])}</p></div>`; }).join('')}</div>`;
  }

  function financeLessonFinalScreenHtmlV57(lesson, parsed, screenIndex, totalScreens){
    const takeaways = financeLessonFinalTakeawaysV57(lesson);
    const cards = takeaways.map(function(text, idx){ return `<div><b>${String(idx + 1).padStart(2,'0')}</b><p>${esc(financeCleanLearningCopyV57(text))}</p></div>`; }).join('');
    return `${card('blue-card-v2 finance-final-card-v53 finance-final-card-v56 finance-final-card-v57', `<p class="eyebrow">итоговая логика</p><h1>Итоговая логика урока</h1><p>Финальный смысловой вывод без служебных формулировок и без обращения к ученику.</p>`)}${card('', `<h2>Что должно остаться после урока</h2><div class="finance-final-grid-v56">${cards}</div>${financeLessonSituationsHtmlV57(lesson)}<div class="finance-screen-counter-v53">Экран ${screenIndex + 1} из ${totalScreens}</div>`)}`;
  }

  function renderFinanceLessonV57(lessonId, screenIndex){
    if (!requireFinanceAdminV49()) return;
    const lesson = getFinanceLessonV49(lessonId);
    if (!lesson) return renderFinanceModuleHomeV57();
    if (!financeCanOpenSectionV57(lesson.sectionId)) return financeLockedSectionNoticeV57(lesson.sectionId);
    state.financeLessonId = Number(lesson.id);
    const hasFull = Array.isArray(lesson.fullContent) && lesson.fullContent.length;
    if (!hasFull) {
      const html = `${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел ${Number(lesson.sectionId)} · урок ${Number(lesson.id)}</p><h1>${esc(lesson.title)}</h1><p>${esc(financeCleanLearningCopyV57(lesson.objective || ''))}</p>`)}${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(${Number(lesson.sectionId)})">← К урокам раздела</button></div>${financeSummaryLessonHtmlV56(lesson)}`)}`;
      shell(html, 'finance');
      return;
    }
    const parsed = parseFinanceLessonScreensV57(lesson);
    const screens = financeBuildLessonScreensV56(lesson, parsed);
    const total = screens.length;
    const index = Math.max(0, Math.min(Number(screenIndex || 0), total - 1));
    state.financeLessonScreenIndex = index;
    const current = screens[index];
    let body = '';
    if (current.type === 'cover') body = financeLessonCoverScreenHtmlV57(lesson, parsed, total);
    if (current.type === 'chapter') body = financeLessonChapterScreenHtmlV56(lesson, current.chapter, index, total);
    if (current.type === 'slide') body = financeLessonSlideScreenHtmlV57(lesson, current.slide, current.slideIndex + 1, parsed.slides.length, index, total);
    if (current.type === 'final') body = financeLessonFinalScreenHtmlV57(lesson, parsed, index, total);
    shell(`${financeLessonNavV56(lesson, index, total)}${body}`, 'finance');
  }

  window.renderFinancialAssistantV57 = renderFinancialAssistantV57;
  window.renderFinanceModuleHome = renderFinanceModuleHomeV57;
  window.renderFinanceSection = renderFinanceSectionV57;
  window.renderFinanceLesson = renderFinanceLessonV57;
  window.renderFinanceSection1Trainer = renderFinanceSection1TrainerV57;
  window.openFinanceSection1TrainerTable = openFinanceSection1TrainerTableV57;
  ...
  try { renderFinanceSection = renderFinanceSectionV57; } catch(e) {}
  try { renderFinanceLesson = renderFinanceLessonV57; } catch(e) {}
  try { financeLockedSectionNotice = financeLockedSectionNoticeV57; } catch(e) {}
...
(function installFinanceEditorialAndGateV57(){
  ...
  function financeFixTextV57(value){
    let out = String(value || '');
    out = out.replace(/предпринимательа/g, 'предпринимателя');
...
(function installFinanceEditorialAndUnlockV57(){
  ...
  const FINANCE_SECTIONS_V57 = [
    { id:1, title:'Финансовое мышление собственника', description:'Базовая логика: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс, управленческий учёт и экономический двигатель бизнеса.', lessons:6 },
  ...
  function financeSectionCardHtmlV57(section){
    ...
    return `<button class="finance-section-card-v49 finance-section-card-v57 ${open ? '' : 'locked'}" onclick="${click}">
      ...
      <em>${Number(section.lessons)} ${Number(section.id) === 1 ? 'блоков' : 'уроков'} · ${financeEscV57(status)}</em>
    </button>`;
  }
...
(function installFinanceContentAuditV57(){
  window.APP_UI_VERSION_V57 = 'v57-finance-content-audit-trainer-20260628';
...
/* =====================================================
   v57 — Финансовый помощник: финальная навигационная фиксация после v55
   ===================================================== */
(function installFinanceNavigationV57(){
  window.APP_UI_VERSION_V57_NAV = 'v57-finance-navigation-final-20260628';

  function financeAdminModeV57(){
    return typeof isAdminMode === 'function' && isAdminMode();
  }

  function openFinanceAssistantV57(){
    if (!financeAdminModeV57()) {
      alert('Финансовый помощник пока закрыт для учеников. Проверка материалов доступна только в режиме администрирования.');
      if (typeof renderHome === 'function') renderHome();
      return;
    }
    if (typeof window.renderFinancialAssistantV57 === 'function') return window.renderFinancialAssistantV57();
    if (typeof window.renderFinancialAssistant === 'function' && window.renderFinancialAssistant !== openFinanceAssistantV57) return window.renderFinancialAssistant();
    alert('Финансовый помощник не найден в текущей сборке.');
  }

  window.openFinanceAssistantV57 = openFinanceAssistantV57;
  window.openFinanceAssistantV55 = openFinanceAssistantV57;
  window.openFinanceAssistantV54 = openFinanceAssistantV57;
  window.openFinanceAssistantV52 = openFinanceAssistantV57;
  window.openFinanceAssistantV50 = openFinanceAssistantV57;
  window.renderFinancialAssistant = openFinanceAssistantV57;
  window.renderMyBusiness = openFinanceAssistantV57;
  try { openFinanceAssistantV55 = openFinanceAssistantV57; } catch(e) {}
  try { openFinanceAssistantV54 = openFinanceAssistantV57; } catch(e) {}
  try { openFinanceAssistantV52 = openFinanceAssistantV57; } catch(e) {}
  try { openFinanceAssistantV50 = openFinanceAssistantV57; } catch(e) {}
  try { renderFinancialAssistant = openFinanceAssistantV57; } catch(e) {}
  try { renderMyBusiness = openFinanceAssistantV57; } catch(e) {}
})();

/* =====================================================
   v57 — Финансовый модуль: аудит текста, последовательный доступ, финтренажёр
   ===================================================== */
(function installFinanceV57(){
  window.APP_UI_VERSION_V57 = 'v57-finance-content-audit-trainer-20260628';

  const PASS_PERCENT = 80;
  const SECTION1_TOTAL = 30;
  const SECTION1_PASS = 24;
  window.FINANCE_SECTION1_TRAINER_URL_V57 = window.FINANCE_SECTION1_TRAINER_URL_V57 || '';
  ...
  function isAdminModeV57(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; } catch(err) { return false; }
  }
  function requireAdminV57(){
    if (isAdminModeV57()) return true;
    alert('Финансовый помощник пока закрыт для учеников. Для проверки материалов включите режим администрирования.');
    return false;
  }
  function userSuffixV57(){
    try {
      const ids = typeof possibleIds === 'function' ? possibleIds() : [];
  ...
  function progressKeyV57(){ return 'architecture_finance_module_progress_v49_' + userSuffixV57(); }
  function loadProgressV57(){
    try { return JSON.parse(localStorage.getItem(progressKeyV57()) || '{}'); } catch(err) { return {}; }
  }
  function sectionPassedV57(sectionId){
    const id = Number(sectionId);
    const p = loadProgressV57();
    if (id === 1) return Boolean(p.section1TestPassed) || Number(p.section1TestScore || 0) >= SECTION1_PASS;
    return Boolean(p['section' + id + 'TestPassed']);
  }
  function canOpenSectionV57(sectionId){
    const id = Number(sectionId);
    if (id <= 1) return true;
    return sectionPassedV57(id - 1);
  }
  window.financeCanOpenSectionV57 = canOpenSectionV57;
  window.financeLockedSectionNoticeV57 = function(sectionId){
    const id = Number(sectionId || 2);
    const prev = Math.max(1, id - 1);
    alert('Раздел ' + id + ' откроется после прохождения итоговой диагностики раздела ' + prev + ' минимум на 80%.');
  };
  window.financeLockedSectionNotice = window.financeLockedSectionNoticeV57;

  function progressWidgetV57(){
    const p = loadProgressV57();
    const score = Math.max(0, Number(p.section1TestScore || 0));
    const total = Number(p.section1TestTotal || SECTION1_TOTAL);
    const percent = total ? Math.max(0, Math.min(100, Math.round(score / total * 100))) : 0;
    const passed = sectionPassedV57(1);
    return `<div class="finance-progress-v57 ${passed ? 'passed' : ''}"><div class="finance-progress-head-v57"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : `${score}/${total} · ${percent}%`}</b></div><div class="finance-progress-bar-v57"><div style="width:${percent}%"></div></div><p>${passed ? 'Раздел 2 открыт. Далее каждый раздел открывается после теста предыдущего раздела.' : `Для открытия раздела 2 нужно не менее ${SECTION1_PASS}/${SECTION1_TOTAL} правильных ответов (${PASS_PERCENT}%).`}</p></div>`;
  }

  function renderFinancialAssistantHubV57(){
    if (!requireAdminV57()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Финансовая архитектура бизнеса показывает, как операции превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника. Это не просто набор таблиц, а учебная система, которая постепенно собирает финансовое мышление и готовит к работе с инструментами.</p>`)}
      ${card('', `<h2>Что внутри</h2><div class="finance-hub-grid-v49 finance-hub-grid-v57">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>11 разделов: от базового различения денег, выручки и прибыли до баланса, ликвидности, метрик, отраслевых финансов и регулярной системы управления.</p><em>открыть</em></button>
  ...
  window.renderFinancialAssistant = renderFinancialAssistantHubV57;
  window.renderMyBusiness = renderFinancialAssistantHubV57;
  try { openFinanceAssistantV55 = renderFinancialAssistantHubV57; } catch(err) {}
  try { renderFinancialAssistant = renderFinancialAssistantHubV57; } catch(err) {}
  try { renderMyBusiness = renderFinancialAssistantHubV57; } catch(err) {}

  function sectionCardV57(section){
    const open = canOpenSectionV57(section.id);
    const status = Number(section.id) === 1 ? 'доступно' : (open ? 'открыто после теста раздела ' + (Number(section.id)-1) : 'закрыто до теста раздела ' + (Number(section.id)-1));
    const action = open ? `renderFinanceSection(${Number(section.id)})` : `financeLockedSectionNoticeV57(${Number(section.id)})`;
    return `<button class="finance-section-card-v49 finance-section-card-v57 ${open ? '' : 'locked'}" onclick="${action}"><div><span>Раздел ${Number(section.id)}</span><b>${e(section.title)}</b><p>${e(section.description)}</p></div><em>${Number(section.lessons)} уроков · ${e(status)}</em></button>`;
  }

  function renderFinanceModuleHomeV57(){
    if (!requireAdminV57()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль выстроен как последовательная лестница финансового мышления. Сначала фиксируется базовая логика: деньги не равны прибыли, выручка не равна поступлению, баланс показывает активы и обязательства. Далее открываются разделы про выручку, прибыльность, безубыточность, деньги, баланс, долг, планирование, метрики, отраслевые финансы и регулярную систему управления.</p>${progressWidgetV57()}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>Карта финансового модуля</h2><p>Разделы идут последовательно. Следующий раздел становится доступен после итоговой диагностики предыдущего раздела минимум на 80%.</p><div class="finance-section-list-v49">${SECTIONS.map(sectionCardV57).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }
  window.renderFinanceModuleHome = renderFinanceModuleHomeV57;
  try { renderFinanceModuleHome = renderFinanceModuleHomeV57; } catch(err) {}

  function patchSectionV57(sectionId){
    const id = Number(sectionId);
    const oldPill = document.querySelector('.finance-status-pill');
    if (oldPill) oldPill.outerHTML = progressWidgetV57();
    if (id === 1) {
      const hero = document.querySelector('.finance-hero-v49');
      const para = hero ? hero.querySelector('p:not(.eyebrow)') : null;
      if (para) para.textContent = 'Раздел формирует базовую логику: предприниматель перестаёт смотреть только на кассу и начинает видеть бизнес как систему финансовых потоков, остатков, обязательств и решений.';
      const list = document.querySelector('.lesson-list-v2');
      if (list && !list.querySelector('[data-finance-trainer-v57="1"]')) {
        const test = list.querySelector('.finance-test-row-v49');
      ...
      const testRowText = document.querySelector('.finance-test-row-v49 p');
      if (testRowText) testRowText.textContent = '30 вопросов · проходной уровень 80% · открывает раздел 2';
    }
  }
  ...
  window.renderFinanceSection = function(sectionId){
    if (!requireAdminV57()) return;
    if (!canOpenSectionV57(sectionId)) return window.financeLockedSectionNoticeV57(sectionId);
    if (typeof renderFinanceSectionBeforeV57 === 'function') {
      const result = renderFinanceSectionBeforeV57.apply(this, arguments);
      setTimeout(function(){ patchSectionV57(sectionId); }, 0);
      setTimeout(function(){ patchSectionV57(sectionId); }, 140);
      return result;
    }
    return renderFinanceModuleHomeV57();
  };
  try { renderFinanceSection = window.renderFinanceSection; } catch(err) {}

  window.copyFinanceTrainerLinkV57 = function(){
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    if (!url || url === '#') { alert('Ссылка на таблицу финтренажёра ещё не подключена. После создания таблицы нужно вставить URL в FINANCE_SECTION1_TRAINER_URL_V57.'); return; }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url).then(function(){ alert('Ссылка на финтренажёр скопирована.'); }).catch(function(){ prompt('Скопируйте ссылку на финтренажёр:', url); });
    } else { prompt('Скопируйте ссылку на финтренажёр:', url); }
  };
  window.openFinanceTrainerLinkV57 = function(){
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    if (!url || url === '#') { alert('Ссылка на таблицу финтренажёра ещё не подключена.'); return; }
    window.open(url, '_blank');
  };
  window.renderFinanceSection1Trainer = function(){
    if (!requireAdminV57()) return;
    ...
    const hasUrl = Boolean(url && url !== '#');
    const linkBlock = hasUrl
      ? `<div class="finance-trainer-link-box-v57"><span>Ссылка на таблицу</span><p>${e(url)}</p></div><div class="grid-v2"><button class="btn primary" onclick="openFinanceTrainerLinkV57()">Открыть таблицу</button><button class="btn secondary" onclick="copyFinanceTrainerLinkV57()">Скопировать ссылку</button></div>`
      : `<div class="finance-trainer-link-box-v57 muted"><span>Ссылка на таблицу</span><p>Будет подключена после загрузки шаблона финтренажёра.</p></div><button class="btn secondary" onclick="copyFinanceTrainerLinkV57()">Проверить подключение ссылки</button>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-trainer-hero-v57', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление после уроков и итоговой диагностики. Тренажёр нужен для отработки главной логики: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean finance-trainer-instruction-v57"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями по цветам, статусам и проверке.</p></div><div><b>4. Пройти задания</b><p>Последовательно разберите операции, один месяц через три отчёта и итоговый кейс. Цель — не угадать, а закрепить финансовую классификацию.</p></div></div>${linkBlock}<button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  };

  function cleanTextV57(text){
    let out = String(text || '');
    const replacements = [
      [/предпринимательа/g, 'предпринимателя'], [/Предпринимательа/g, 'Предпринимателя'],
      [/предпринимательу/g, 'предпринимателю'], [/Предпринимательу/g, 'Предпринимателю'],
      [/предпринимательом/g, 'предпринимателем'], [/Предпринимательом/g, 'Предпринимателем'],
      ...
      [/Этот урок должен полностью убрать у предпринимателя/gi, 'Этот урок убирает у предпринимателя'],
      [/Этот урок должен убрать у предпринимателя/gi, 'Этот урок убирает у предпринимателя'],
      [/Финальный смысловой вывод без обращения к ученику\.?/gi, 'Короткая фиксация главной логики урока.'],
      [/Финальный смысловой вывод по теме без обращения к ученику\.?/gi, 'Короткая фиксация главной логики урока.'],
      [/Что должно остаться после урока/gi, 'Итоговые правила урока'],
      [/\bДДС\b/g, 'ОДДС'], [/ООДДС/g, 'ОДДС'], [/ОДОДДС/g, 'ОДДС'], [/ОПУ/g, 'ОПиУ']
    ];
    replacements.forEach(function(pair){ out = out.replace(pair[0], pair[1]); });
  ...
  function patchTextNodesV57(root){
    ...
    while (walker.nextNode()) nodes.push(walker.currentNode);
    nodes.forEach(function(node){
      const next = cleanTextV57(node.nodeValue);
      if (next !== node.nodeValue) node.nodeValue = next;
    });
  }
  ...
  function patchCoverV57(){
    const grid = document.querySelector('.finance-cover-grid-v56');
    if (!grid) return;
    const title = document.querySelector('.finance-lesson-cover-v56 h1')?.textContent || document.querySelector('.finance-hero-v49 h1')?.textContent || '';
    const meta = {
      'Деньги, прибыль и выручка: почему предприниматель ошибается': ['18.', 'Теоретическое объяснение, разбор финансовых понятий, сквозной кейс, сравнение влияния на ОПиУ, ОДДС и баланс.', 'Разрушить базовый финансовый миф предпринимателя: деньги на счёте не равны прибыли, а поступление денег не всегда является выручкой.', 'Предприниматель перестаёт оценивать бизнес только по остатку денег и разделяет три финансовые реальности: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.'],
      'Финансовая карта бизнеса': ['22.', 'Теоретическое объяснение, схема финансовой карты, сквозной кейс одного месяца, разбор через ОПиУ, ОДДС, баланс и метрики.', 'Показать бизнес как систему, где операции превращаются в выручку, деньги — в денежный поток, остатки — в баланс, а метрики — в решения.', 'Финансы бизнеса перестают выглядеть как одна таблица кассы. Одна операция начинает читаться через прибыльность, движение денег, остатки и управленческие метрики.'],
      'Управленческий учёт против бухгалтерии': ['22.', 'Сравнительные схемы, разбор бухгалтерского, налогового и управленческого контуров, кейс расхождения прибыли и кассового риска.', 'Отделить управленческую финансовую систему собственника от бухгалтерского и налогового учёта.', 'Бухгалтерия, налоги и управленческий учёт перестают смешиваться. Каждый контур получает свою задачу, а собственник видит, какие данные нужны для решений.'],
      'Экономика бизнес-модели': ['32.', 'Теория, отраслевые схемы, формулы драйверов, сравнительный кейс и диагностика экономического двигателя бизнеса.', 'Показать, за счёт чего конкретный бизнес зарабатывает и где у него возникает главное финансовое ограничение.', 'Бизнес начинает читаться через экономический двигатель: выручку, маржу, мощность, оборачиваемость, повторяемость и главное ограничение модели.']
    };
    const key = Object.keys(meta).find(function(k){ return title.indexOf(k) !== -1; });
    if (!key) return;
    const labels = ['Количество слайдов', 'Формат урока', 'Главная задача', 'Основной вывод'];
    grid.innerHTML = labels.map(function(label, i){ return `<div class="finance-cover-line-v53 finance-cover-line-v56"><span>${e(label)}</span><p>${e(meta[key][i])}</p></div>`; }).join('');
    const p = document.querySelector('.finance-lesson-cover-v56 > p:not(.eyebrow)');
    if (p) p.textContent = 'Структура урока перед основной последовательностью слайдов.';
  }

  function patchSituationSlideV57(){
    const cardNode = document.querySelector('.finance-slide-card-v56');
    if (!cardNode) return;
    const title = cardNode.querySelector('h1')?.textContent || '';
    const body = cardNode.querySelector('.finance-slide-body-v56');
    if (!body || !/Сквозной кейс/i.test(title) || body.querySelector('.finance-situation-list-v57')) return;
    body.innerHTML = `<h3>Объяснение</h3><p>На счёт бизнеса пришла одна и та же сумма. В банковской выписке это выглядит одинаково: деньги увеличились. Но для управления бизнесом важен не только факт поступления, а причина, по которой деньги пришли.</p><div class="finance-situation-list-v57"><div><b>1. Оплата за уже оказанную услугу или проданный товар</b><p>Это может быть выручкой текущего периода. В ОПиУ появляется выручка, в ОДДС — операционное поступление, в балансе увеличиваются деньги.</p></div><div><b>2. Аванс клиента</b><p>Деньги уже пришли, но выручка ещё не заработана полностью. В балансе появляется обязательство: бизнес должен оказать услугу, поставить товар или выполнить обещание.</p></div><div><b>3. Кредит</b><p>Деньги пришли как финансирование, а не как заработок. В ОПиУ выручка не возникает. В ОДДС появляется финансовое поступление, а в балансе одновременно растут деньги и долг.</p></div><div><b>4. Возврат дебиторки</b><p>Клиент оплатил старый долг. Деньги пришли сейчас, но выручка могла быть признана раньше. В балансе дебиторка уменьшается, деньги увеличиваются.</p></div><div><b>5. Вклад собственника</b><p>Касса усилилась за счёт владельца, а не за счёт клиента. Это поддерживает бизнес, но не является выручкой и не доказывает прибыльность модели.</p></div></div><p>Вывод простой: одно поступление нельзя называть выручкой, пока не определён его финансовый смысл и влияние на ОПиУ, ОДДС и баланс.</p>`;
  }

  function detailNoteV57(){
    const title = String(document.querySelector('.finance-slide-card-v56 h1')?.textContent || '').toLowerCase();
    if (!title) return '';
    if (title.includes('главный финансовый миф')) return 'Остаток на счёте является сигналом ликвидности, но не доказывает заработок. Перед выводом нужно понять источник денег и обязательства, которые появились вместе с поступлением.';
    if (title.includes('три финансовые реальности')) return 'ОПиУ отвечает за результат периода, ОДДС — за реальные поступления и платежи, баланс — за незавершённые финансовые процессы: долги, авансы, дебиторку, активы и капитал.';
    if (title.includes('аванс')) return 'Аванс нельзя считать свободной прибылью. Он должен быть связан с объёмом неоказанных услуг, будущей загрузкой и обязательствами перед клиентами.';
    if (title.includes('кредит')) return 'Кредит может закрыть кассовый разрыв, но переносит нагрузку в будущее через проценты и возврат тела долга. Его нельзя смешивать с операционной выручкой.';
    if (title.includes('дебитор')) return 'Дебиторка является активом, но не является деньгами. Чем дольше клиент платит, тем сильнее прибыль отрывается от кассы.';
    if (title.includes('актив') || title.includes('оборуд')) return 'Если покупка будет приносить пользу нескольким периодам, её нужно видеть как актив. Иначе месяц покупки станет искусственно слабым, а следующие месяцы — искусственно сильными.';
    if (title.includes('бухгалтер') || title.includes('кассовый риск')) return 'Корректная бухгалтерская прибыль не отменяет денежный риск. Для управления нужны сроки оплат, ближайшие платежи и остатки, а не только итоговая прибыль периода.';
    if (title.includes('экономический двигатель')) return 'Экономический двигатель нужен для поиска ограничения. Рост выручки без понимания маржи, мощности и денежного цикла может усилить проблему, а не решить её.';
    if (title.includes('услуг')) return 'В услугах важно считать не только визиты и чек, но и выручку на час, загрузку, ФОТ специалиста, отмены и повторяемость.';
    if (title.includes('торгов')) return 'В торговле высокий оборот опасен без контроля валовой маржи, скидок, списаний и скорости превращения товара обратно в деньги.';
    if (title.includes('производ')) return 'В производстве прибыль может застревать в сырье, незавершёнке, готовой продукции и дебиторке. Поэтому важны WIP, брак, мощность и себестоимость.';
    return '';
  }
  function patchDetailNoteV57(){
    const cardNode = document.querySelector('.finance-slide-card-v56');
    if (!cardNode || cardNode.querySelector('.finance-detail-note-v57')) return;
    const note = detailNoteV57();
    if (!note) return;
    const body = cardNode.querySelector('.finance-slide-body-v56');
    if (body) body.insertAdjacentHTML('afterend', `<div class="finance-detail-note-v57"><span>Уточнение</span><p>${e(note)}</p></div>`);
  }
  function patchLessonViewV57(){
    patchTextNodesV57(document.querySelector('.content-v2'));
    patchCoverV57();
    patchSituationSlideV57();
    patchDetailNoteV57();
  }

  const renderFinanceLessonBeforeV57 = window.renderFinanceLesson;
  window.renderFinanceLesson = function(lessonId, screenIndex){
    const result = typeof renderFinanceLessonBeforeV57 === 'function' ? renderFinanceLessonBeforeV57.apply(this, arguments) : undefined;
    setTimeout(patchLessonViewV57, 0);
    setTimeout(patchLessonViewV57, 140);
    return result;
  };
  try { renderFinanceLesson = window.renderFinanceLesson; } catch(err) {}

  const shellBeforeV57 = window.shell;
  if (typeof shellBeforeV57 === 'function') {
    window.shell = function(content, activeTab){
      const result = shellBeforeV57.call(this, content, activeTab);
      setTimeout(function(){ patchTextNodesV57(document.querySelector('.content-v2')); }, 0);
      return result;
    };
