...
const APP_CACHE_VERSION = "v77-finance-single-clean-module-20260628";
...
(function installFinancialAssistantV77(){
  ...
  function clean(value){
    let s = String(value ?? '');
    s = s.replace(/О\s*О\s*О\s*ДДС/gi, 'ОДДС');
    s = s.replace(/О\s*О\s*ДДС/gi, 'ОДДС');
    s = s.replace(/О{2,3}ДДС/gi, 'ОДДС');
    s = s.replace(/О\s+ДДС/gi, 'ОДДС');
    s = s.replace(/(^|[^А-Яа-яЁёA-Za-z0-9_])ДДС([^А-Яа-яЁёA-Za-z0-9_]|$)/g, '$1ОДДС$2');
    s = s.replace(/ОПУ/g, 'ОПиУ');
    s = s.replace(/предпринимательа/g, 'предпринимателя');
    s = s.replace(/предпринимательу/g, 'предпринимателю');
    s = s.replace(/предпринимательом/g, 'предпринимателем');
    s = s.replace(/у предприниматель\b/g, 'у предпринимателя');
    s = s.replace(/почему предпринимателя ошибается/g, 'почему предприниматель ошибается');
    return s;
  }
  function t(value){ return e(clean(value)); }
  function c(cls, html){ return typeof card === 'function' ? card(cls, html) : `<section class="card-v2 ${cls || ''}">${html}</section>`; }
  function show(html, tab){ if (typeof shell === 'function') shell(clean(html), tab || 'finance'); }
  function adminMode(){ return typeof isAdminMode === 'function' && isAdminMode(); }
  function requireAdmin(){
    if (adminMode()) return true;
    alert('Финансовый помощник пока закрыт для учеников. Для проверки материалов включите режим администрирования вручную.');
    if (typeof renderHome === 'function') renderHome();
    return false;
  }
  function userSuffix(){
    try { const ids = typeof possibleIds === 'function' ? possibleIds() : []; const names = typeof possibleUsernames === 'function' ? possibleUsernames() : []; return ids[0] || names[0] || 'local'; }
    catch(e){ return 'local'; }
  }
  function progressKey(){ return 'architecture_finance_progress_v77_' + userSuffix(); }
  function loadProgress(){ try { return JSON.parse(localStorage.getItem(progressKey()) || '{}'); } catch(e){ return {}; } }
  function saveProgress(patch){ const next = Object.assign({}, loadProgress(), patch || {}, { updatedAt: (typeof nowIso === 'function' ? nowIso() : new Date().toISOString()) }); localStorage.setItem(progressKey(), JSON.stringify(next)); return next; }
  function section1Passed(){ const p = loadProgress(); return Boolean(p.section1Passed) || Number(p.section1Score || 0) >= FINANCE_PASS_SCORE_V77; }
  function canOpenSection(id){ id = Number(id); if (id === 1) return true; if (id === 2) return section1Passed(); return false; }
  function getSection(id){ return FINANCE_SECTIONS_V77.find(function(s){ return Number(s.id) === Number(id); }); }
  function getLesson(lessonId){
    lessonId = Number(lessonId);
    for (const section of FINANCE_SECTIONS_V77){
      const lesson = (section.lessons || []).find(function(l){ return Number(l.id) === lessonId; });
  ...
  function renderFinancialAssistantV77(){
    ...
    show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения. Для учеников вход пока закрыт.</p>`)}${c('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v49 finance-hub-grid-v77"><button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHomeV77()"><b>Финансовый модуль</b><p>11 разделов финансовой архитектуры бизнеса.</p><em>открыть</em></button><button class="finance-hub-card-v49" onclick="renderFinanceAnalyticsV77()"><b>Моя аналитика</b><p>Будущий блок личных показателей и управленческой аналитики.</p><em>каркас</em></button><button class="finance-hub-card-v49" onclick="renderFinanceTemplatesV77()"><b>Готовые шаблоны</b><p>Будущие таблицы, шаблоны и финансовые инструменты.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← На главную</button>`)}`, 'finance');
  }

  function sectionCard(section){
    const id = Number(section.id);
    const open = canOpenSection(id);
    const count = (section.lessons || []).length + (id === 1 ? 2 : 0);
    return `<button class="finance-section-card-v77 ${open ? '' : 'locked'}" ${open ? `onclick="renderFinanceSectionV77(${id})"` : `onclick="lockedFinanceSectionV77(${id})"`}><div><span>Раздел ${id}</span><b>${t(section.title)}</b><p>${t(section.description || '')}</p></div><em>${count} блоков · ${open ? 'доступно' : 'закрыто'}</em></button>`;
  }
  function renderFinanceModuleHomeV77(){
    if (!requireAdmin()) return;
    show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">финансовый модуль</p><h1>Финансовый модуль</h1><p>Выберите один из 11 разделов.</p>`)}${c('', `<div class="finance-toolbar-v77"><button class="btn secondary" onclick="renderFinancialAssistantV77()">← К финансовому помощнику</button></div><h2>11 разделов</h2><div class="finance-section-list-v77">${FINANCE_SECTIONS_V77.map(sectionCard).join('')}</div>`)}`, 'finance');
  }
  function lockedFinanceSectionV77(id){ alert(Number(id) === 2 ? 'Раздел 2 откроется после итоговой диагностики Раздела 1 минимум на 80%.' : 'Этот раздел откроется после прохождения предыдущих разделов.'); }

  function lessonRow(lesson){ return `<button class="lesson-row-v2 finance-lesson-row-v77" onclick="renderFinanceLessonV77(${Number(lesson.id)},0)"><div><b>${String(lesson.id).padStart(2,'0')}. ${t(lesson.title)}</b><p>${Number(lesson.id) <= 4 ? 'полный урок' : 'будет раскрыто позже'}</p></div><span>→</span></button>`; }
  function renderFinanceSectionV77(sectionId){
    if (!requireAdmin()) return;
    const section = getSection(sectionId) || getSection(1);
    if (!canOpenSection(section.id)) return lockedFinanceSectionV77(section.id);
    if (Number(section.id) !== 1){
      show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">раздел ${Number(section.id)}</p><h1>${t(section.title)}</h1><p>${t(section.description || '')}</p>`)}${c('', `<div class="finance-toolbar-v77"><button class="btn secondary" onclick="renderFinanceModuleHomeV77()">← К разделам</button></div><p>Материалы этого раздела будут добавлены следующим этапом.</p>`)}`, 'finance');
      return;
    }
    const extras = `<button class="lesson-row-v2 finance-lesson-row-v77 finance-test-row-v77" onclick="startFinanceSection1TestV77(true)"><div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80%</p></div><span>→</span></button><button class="lesson-row-v2 finance-lesson-row-v77 finance-trainer-row-v77" onclick="renderFinanceTrainerSection1V77()"><div><b>Финтренажёр раздела 1</b><p>Google Таблица для закрепления материала</p></div><span>→</span></button>`;
    show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">раздел 1</p><h1>${t(section.title)}</h1><p>${t(section.description || '')}</p>${financeProgressBoxV77()}`)}${c('', `<div class="finance-toolbar-v77"><button class="btn secondary" onclick="renderFinanceModuleHomeV77()">← К разделам</button></div><h2>Уроки и закрепление</h2><div class="lesson-list-v2">${(section.lessons || []).slice(0,4).map(lessonRow).join('')}${extras}</div>`)}`, 'finance');
  }
  function financeProgressBoxV77(){ const p = loadProgress(); const score = Number(p.section1Score || 0); const total = Number(p.section1Total || FINANCE_TEST_V77.length); const percent = total ? Math.round(score/total*100) : 0; return `<div class="finance-status-pill ${section1Passed() ? 'passed' : 'locked'}"><span>${section1Passed() ? 'Раздел 1 засчитан' : 'Раздел 2 пока закрыт'}</span><b>${score}/${total} · ${percent}% · нужно ${FINANCE_PASS_SCORE_V77}/${FINANCE_TEST_V77.length}</b></div>`; }

  function parseScreens(lesson){
    const lines = (Array.isArray(lesson.fullContent) ? lesson.fullContent : []).map(function(x){ return clean(String(x || '').trim()); }).filter(Boolean);
    const slideCountLine = lines.find(function(x){ return /^Количество слайдов:/i.test(x); }) || '';
    let formatLine = lines.find(function(x){ return /^Формат урока:/i.test(x); }) || ('Формат урока: теоретическое объяснение, разбор понятий, кейсы и управленческие выводы.');
    formatLine = formatLine.replace(/сквозной кейс на 1 000 000 ₽,\s*/i, 'сквозной кейс, ');
    const taskLine = lines.find(function(x){ return /^Главная задача урока:/i.test(x); }) || ('Главная задача урока: ' + (lesson.objective || ''));
    let resultLine = lines.find(function(x){ return /^Результат ученика:/i.test(x); }) || '';
    resultLine = resultLine ? resultLine.replace(/^Результат ученика:/i, 'Основной вывод:') : 'Основной вывод: предприниматель перестаёт смотреть только на кассу и начинает разделять прибыль, деньги, активы и обязательства.';
    const screens = [{ type:'title', lessonTitle: lesson.title, blocks:[slideCountLine, formatLine, taskLine, resultLine].filter(Boolean) }];
    let current = null, mode = 'none';
    function push(){ if (current){ current.paragraphs = (current.paragraphs || []).filter(Boolean); screens.push(current); current = null; } }
    lines.forEach(function(line){
      const m = line.match(/^Слайд\s+(\d+)\.\s*(.+)$/i);
      if (m){ push(); current = { type:'slide', number:Number(m[1]), title:'Слайд ' + m[1] + '. ' + m[2], paragraphs:[] }; mode = 'skip'; return; }
      if (/^Итоговая логика урока$/i.test(line)){ push(); current = { type:'final', title:'Итоговая логика урока', paragraphs:[] }; mode = 'text'; return; }
      if (!current) return;
      if (/^Что показать на слайде$/i.test(line)){ mode = 'skip'; return; }
      if (/^Текст под слайдом$/i.test(line)){ mode = 'text'; return; }
      if (/^Общая структура урока$/i.test(line) || /^Рекомендуемая длительность/i.test(line) || /^Тест после урока/i.test(line) || /^Практическая таблица/i.test(line)) return;
      if (mode === 'text') current.paragraphs.push(line);
    });
    push();
    return screens;
  }
  function firstSentence(paragraphs){ const source = String((paragraphs || []).find(Boolean) || '').trim(); const m = source.match(/^(.+?[.!?])\s/); return m ? m[1] : (source || 'Ключевой смысл экрана — правильно определить финансовую логику события.'); }
  function mistake(title, paragraphs){
    const raw = (String(title||'') + ' ' + (paragraphs||[]).join(' ')).toLowerCase();
    if (raw.includes('аванс')) return 'Считать аванс свободной прибылью и потратить деньги до выполнения обязательства перед клиентом.';
    if (raw.includes('кредит')) return 'Записать кредит как выручку и принять долг за заработанные деньги.';
    if (raw.includes('дебитор')) return 'Принять признанную выручку за уже полученные деньги.';
    if (raw.includes('оборуд') || raw.includes('актив')) return 'Списать покупку актива целиком в расход одного месяца.';
    if (raw.includes('бухгалтер')) return 'Ждать от бухгалтерии управленческий дашборд собственника.';
    if (raw.includes('выручк') || raw.includes('модель')) return 'Сравнивать бизнесы только по выручке и не видеть маржу, мощность и денежный цикл.';
    return 'Сделать вывод по одной цифре без связки ОПиУ, ОДДС и баланса.';
  }
  function conclusion(title, paragraphs){
    const raw = (String(title||'') + ' ' + (paragraphs||[]).join(' ')).toLowerCase();
    if (raw.includes('аванс')) return 'Авансы нужно отделять от заработанной выручки и контролировать как будущую нагрузку.';
    if (raw.includes('кредит')) return 'Кредит улучшает кассу сегодня, но создаёт долг и будущую нагрузку.';
    if (raw.includes('дебитор')) return 'Дебиторка показывает: бизнес заработал, но деньги ещё не получил.';
    if (raw.includes('оборуд') || raw.includes('актив')) return 'Актив влияет на деньги сразу, а на прибыль — по логике использования.';
    if (raw.includes('услуг')) return 'В услугах нужно видеть часы, загрузку, повторные визиты, ФОТ и выручку на час.';
    if (raw.includes('торгов')) return 'В торговле важны товарный остаток, маржа, скидки, списания и оборачиваемость.';
    if (raw.includes('производ')) return 'В производстве нужно контролировать мощность, себестоимость, брак, сырьё, WIP и готовую продукцию.';
    return 'Смысл операции нужно читать через три вопроса: что заработано, какие деньги пришли или ушли, что изменилось в активах и обязательствах.';
  }
  function renderFinanceLessonV77(lessonId, screenIndex){
    if (!requireAdmin()) return;
    const lesson = getLesson(lessonId);
    if (!lesson) return renderFinanceSectionV77(1);
    const screens = parseScreens(lesson);
    screenIndex = Math.max(0, Math.min(Number(screenIndex || 0), screens.length - 1));
    const last = screenIndex >= screens.length - 1;
    const prevDisabled = screenIndex <= 0 ? 'disabled' : '';
    const nextAction = last ? `renderFinanceSectionV77(${lesson.sectionId})` : `renderFinanceLessonV77(${Number(lesson.id)},${screenIndex+1})`;
    show(`<div class="nav-panel-v2 nav-panel-v2-three finance-nav-v77"><button class="btn secondary" onclick="renderFinanceSectionV77(${lesson.sectionId})">К урокам</button><button class="btn secondary" ${prevDisabled} onclick="renderFinanceLessonV77(${Number(lesson.id)},${Math.max(0, screenIndex-1)})">Назад</button><button class="btn primary" onclick="${nextAction}">${last ? 'К разделу' : 'Далее'}</button></div>${renderLessonScreenV77(lesson, screens[screenIndex], screenIndex, screens.length)}`, 'finance');
  }
  function renderLessonScreenV77(lesson, screen, index, total){
    if (screen.type === 'title'){
      return c('blue-card-v2 finance-lesson-title-v77', `<p class="eyebrow">структура урока · экран ${index+1}/${total}</p><h1>${t(lesson.title)}</h1><div class="finance-title-grid-v77">${screen.blocks.map(function(b){ const label = String(b).split(':')[0]; const value = String(b).replace(/^.*?:\s*/, ''); return `<div><span>${t(label)}</span><p>${t(value)}</p></div>`; }).join('')}</div>`);
    }
    if (screen.type === 'final'){
      return c('blue-card-v2 finance-lesson-title-v77', `<p class="eyebrow">итоговая логика</p><h1>${t(screen.title)}</h1>${(screen.paragraphs || []).map(function(p){ return `<p>${t(p)}</p>`; }).join('')}<button class="btn primary" onclick="renderFinanceSectionV77(${lesson.sectionId})">Завершить урок</button>`);
    }
    const paragraphs = (screen.paragraphs || []);
    return c('finance-slide-card-v77', `<p class="eyebrow">экран ${index+1}/${total}</p><h1>${t(screen.title)}</h1><div class="finance-image-placeholder-v77"><b>Место для изображения</b><span>Визуал будет подключён после генерации.</span></div><div class="finance-main-v77"><b>Главная мысль</b><p>${t(firstSentence(paragraphs))}</p></div><div class="finance-text-v77"><h2>Объяснение</h2>${paragraphs.map(function(p){ return `<p>${t(p)}</p>`; }).join('')}</div><div class="finance-error-v77"><b>Типовая ошибка</b><p>${t(mistake(screen.title, paragraphs))}</p></div><div class="finance-result-v77"><b>Управленческий вывод</b><p>${t(conclusion(screen.title, paragraphs))}</p></div>`);
  }

  function renderFinanceTrainerSection1V77(){
    if (!requireAdmin()) return;
    show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Таблица для закрепления материала первого раздела.</p>`)}${c('', `<div class="finance-toolbar-v77"><button class="btn secondary" onclick="renderFinanceSectionV77(1)">← К разделу 1</button></div><h2>Как пройти финтренажёр</h2><div class="list-clean"><div><b>1. Открыть таблицу</b><p>Нажмите кнопку ниже и перейдите в Google Таблицу.</p></div><div><b>2. Сохранить себе рабочую версию</b><p>В самой Google Таблице выберите <b>Файл → Создать копию</b>.</p></div><div><b>3. Пройти задания</b><p>Следуйте инструкции внутри таблицы и выполняйте задания по материалу первого раздела.</p></div></div><div class="finance-trainer-link-box-v77"><span>Ссылка на финтренажёр</span><p>${e(FINANCE_TRAINER_URL_V77)}</p></div><a class="btn primary" href="${FINANCE_TRAINER_URL_V77}" target="_blank" rel="noopener">Открыть финтренажёр</a><button class="btn secondary" onclick="renderFinanceSectionV77(1)">← Вернуться к урокам раздела</button>`)}`, 'finance');
  }

  let testIndex = 0, testAnswers = {};
  function startFinanceSection1TestV77(reset){ if (!requireAdmin()) return; if (reset){ testIndex = 0; testAnswers = {}; } renderFinanceTestQuestionV77(); }
  function renderFinanceTestQuestionV77(){
    const q = FINANCE_TEST_V77[testIndex];
    if (!q) return finishFinanceSection1TestV77();
    show(c('finance-test-card-v77', `<p class="eyebrow">итоговая диагностика · вопрос ${testIndex+1}/${FINANCE_TEST_V77.length}</p>${q.case ? `<div class="finance-case-v77">${t(q.case)}</div>` : ''}<h1>${t(q.q)}</h1>${(q.options || []).map(function(opt,i){ return `<button class="option-v2 ${Number(testAnswers[testIndex])===i?'selected':''}" onclick="selectFinanceTestAnswerV77(${i})">${String.fromCharCode(65+i)}. ${t(opt)}</button>`; }).join('')}<button class="btn secondary" onclick="renderFinanceSectionV77(1)">← К разделу 1</button>`), 'finance');
  }
  function selectFinanceTestAnswerV77(i){ testAnswers[testIndex] = Number(i); if (testIndex < FINANCE_TEST_V77.length - 1){ testIndex++; renderFinanceTestQuestionV77(); } else finishFinanceSection1TestV77(); }
  function finishFinanceSection1TestV77(){
    let score = 0, critical = 0;
    FINANCE_TEST_V77.forEach(function(q,i){ const ok = Number(testAnswers[i]) === Number(q.correct); if (ok) score++; else if (q.critical) critical++; });
    const passed = score >= FINANCE_PASS_SCORE_V77;
    saveProgress({ section1Score: score, section1Total: FINANCE_TEST_V77.length, section1Passed: passed, section1CriticalErrors: critical });
    show(c(passed ? 'result-ok-v2' : 'result-bad-v2', `<h1>${passed ? 'Диагностика пройдена' : 'Диагностика не пройдена'}</h1><p>Результат: <b>${score}/${FINANCE_TEST_V77.length}</b>. Проходной уровень: <b>${FINANCE_PASS_SCORE_V77}/${FINANCE_TEST_V77.length}</b>.</p><p>Критических ошибок: <b>${critical}</b>.</p><div class="grid-v2"><button class="btn primary" onclick="${passed ? 'renderFinanceModuleHomeV77()' : 'startFinanceSection1TestV77(true)'}">${passed ? 'К финансовому модулю' : 'Пройти заново'}</button><button class="btn secondary" onclick="renderFinanceSectionV77(1)">К разделу 1</button></div>`), 'finance');
  }

  function renderFinanceAnalyticsV77(){ if (!requireAdmin()) return; show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Раздел пока оставлен как каркас.</p>`)}${c('', `<button class="btn secondary" onclick="renderFinancialAssistantV77()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function renderFinanceTemplatesV77(){ if (!requireAdmin()) return; show(`${c('blue-card-v2 finance-clean-hero-v77', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и финансовых инструментов.</p>`)}${c('', `<button class="btn secondary" onclick="renderFinancialAssistantV77()">← К финансовому помощнику</button>`)}`, 'finance'); }

  function financeHomeCardV77(){
    const open = adminMode();
    return renderMainBlockCard('Финансовый помощник', 'Финансовый модуль, аналитика и готовые шаблоны.', open ? 'доступно администратору' : 'закрыто', open ? 'renderFinancialAssistantV77()' : '', (open ? 'active' : 'soon') + ' compact-card finance-home-card-v77');
  }
  const secondaryBeforeFinanceV77 = window.secondaryBlocksHtmlV40;
  window.secondaryBlocksHtmlV40 = function(){
    let base = typeof secondaryBeforeFinanceV77 === 'function' ? secondaryBeforeFinanceV77() : '<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40"></div>';
    base = base.replace(/<button[\s\S]*?<b>Финансовый помощник<\/b>[\s\S]*?<\/button>/g, '');
  ...
  const bottomBeforeFinanceV77 = window.bottomNav;
  window.bottomNav = function(active){
    if (typeof architectureV41Enabled === 'function' && architectureV41Enabled()){
      function item(key,label,icon,onclick,disabled){ return `<button class="bottom-item ${active===key?'active':''} ${disabled?'is-disabled-v41':''}" ${disabled?'disabled aria-disabled="true"':'onclick="'+onclick+'"'}>${icon}<b>${label}</b>${disabled?'<small>закрыто</small>':''}</button>`; }
      ...
      const financeIcon = `<span class="arch-nav-icon"><svg class="arch-nav-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V9M10 20V4M16 20v-7M22 20H2"/><path d="m4 7 5-3 5 5 6-6"/></svg></span>`;
      return `<nav class="bottom-nav-v2 v41-bottom-nav" aria-label="Основное меню">${item('home','Главная',homeIcon,"safeNavigateV32('renderHome')",false)}${item('finance','Финансовый помощник',financeIcon,'renderFinancialAssistantV77()',!adminMode())}${item('profile','Профиль',profileIcon,"safeNavigateV32('renderProfile')",false)}</nav>`;
    }
    return typeof bottomBeforeFinanceV77 === 'function' ? bottomBeforeFinanceV77(active) : '';
  };
  ...
  window.lockedFinanceSection = window.financeLockedSectionNotice = lockedFinanceSectionV77;
  ...
  window.FINANCE_TRAINER_URL_V77 = FINANCE_TRAINER_URL_V77;
})();
