...
(function installFinancialAssistantV49(){
  ...
  function financeProgressPillV49(){
    const progress = loadFinanceProgressV49();
    const score = Number(progress.section1TestScore || 0);
    const total = Number(progress.section1TestTotal || FINANCE_SECTION1_TEST_V49.length);
    const passed = financeSectionOnePassedV49();
    const percent = total ? Math.round(score / total * 100) : 0;
    if (passed) {
      return `<div class="finance-status-pill passed"><span>Раздел 1 засчитан</span><b>${score}/${total} · ${percent}% · доступ к Разделу 2 открыт</b></div>`;
    }
    if (score > 0) {
      return `<div class="finance-status-pill locked"><span>Итоговая диагностика пока не засчитана</span><b>${score}/${total} · ${percent}% · проходной уровень 80%</b></div>`;
    }
    return `<div class="finance-status-pill locked"><span>Итоговая диагностика не пройдена</span><b>Нужно набрать не менее ${FINANCE_SECTION1_PASS_SCORE_V49}/${FINANCE_SECTION1_TEST_V49.length} правильных ответов</b></div>`;
  }
  ...
  function renderFinancialAssistantV49(){
    ...
    const html = `
      ...
      ${card('', `<h2>Выберите раздел помощника</h2><div class="finance-hub-grid-v49">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>11 разделов, 62 урока, первые 4 урока полностью наполнены текстом, итоговая диагностика стоит в конце первого раздела.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Черновой контур для будущей аналитики бизнеса: ввод фактов, показатели и первичные выводы.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Место для будущих таблиц, шаблонов и инструментов финансового модуля.</p><em>скоро</em></button>
      </div><button class="btn secondary" onclick="renderProfile()">Вернуться в профиль</button>`)}
    `;
    shell(html, 'profile');
  }

  function financeSectionCardHtmlV49(section){
    const open = financeCanOpenSectionV49(section.id);
    const extraBlocks = Number(section.id) === 1 ? 2 : 0;
    const lessonsCount = (section.lessons || []).length + extraBlocks;
    let status = 'откроется после диагностики предыдущего раздела';
    if (Number(section.id) === 1) status = 'доступно';
    else if (open) status = 'открыто';
    return `<button class="finance-section-card-v49 ${open ? '' : 'locked'}" ${open ? `onclick="renderFinanceSection(${Number(section.id)})"` : `onclick="financeLockedSectionNotice()"`}>
      <div><span>Раздел ${Number(section.id)}</span><b>${esc(section.title)}</b><p>${esc(section.description || '')}</p></div>
      <em>${lessonsCount} блоков · ${status}</em>
    </button>`;
  }

  function financeLockedSectionNoticeV49(){
    alert('Этот раздел откроется после прохождения итоговой диагностики предыдущего раздела минимум на 80%.');
  }

  function renderFinanceModuleHomeV49(){
    if (!requireFinanceAdminV49()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Это учебная система, которая собирает финансовое мышление предпринимателя в последовательную архитектуру: выручка, прибыль, деньги, остатки, баланс, метрики, диагностика и решение собственника.</p><p>Модуль состоит из 11 разделов. Каждый следующий раздел открывается после прохождения итоговой диагностики предыдущего раздела минимум на 80%.</p>${financeProgressPillV49()}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>11 разделов модуля</h2><p>Раздел 1 формирует базовую логику: деньги не равны прибыли, поступление не всегда является выручкой, а бизнес нельзя понимать без ОПиУ, ОДДС и баланса. Дальше модуль постепенно раскрывает выручку, прибыльность, безубыточность, деньги, баланс, долг, планирование, метрики, отраслевые финансы и регулярную финансовую систему.</p><div class="finance-section-list-v49">${FINANCE_MODULE_SECTIONS_V49.map(financeSectionCardHtmlV49).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }

  function financeLessonRowHtmlV49(lesson){
    const ready = Number(lesson.id) <= 4;
    const label = ready ? 'полный текст вставлен' : 'структура из общего файла';
    return `<button class="lesson-row-v2 finance-lesson-row-v49" onclick="renderFinanceLesson(${Number(lesson.id)})">
      <div><b>${String(lesson.id).padStart(2,'0')}. ${esc(lesson.title)}</b><p>${esc(label)}</p></div><span>→</span>
    </button>`;
  }
  ...
  function renderFinanceSectionV49(sectionId){
    ...
    const testRow = Number(section.id) === 1
      ? `<button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v49" onclick="startFinanceSection1Test(true)">
          <div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает Раздел 2</p></div><span>→</span>
        </button>`
      : '';
    const trainerRow = Number(section.id) === 1
      ? `<button class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57" onclick="renderFinanceSection1Trainer()">
          <div><b>Финтренажёр раздела 1</b><p>Таблица-тренажёр для дополнительного закрепления: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span>
        </button>`
      : '';

    const sectionIntro = Number(section.id) === 1
      ? 'Этот раздел убирает кассовое мышление. Финансы начинают читаться не по одному остатку денег, а через три экрана: прибыльность, движение денег и финансовое положение.'
      : (section.description || 'Раздел будет раскрыт после прохождения предыдущей диагностики.');

    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-section-hero-v57', `<p class="eyebrow">раздел ${Number(section.id)}</p><h1>${esc(section.title)}</h1><p>${esc(sectionIntro)}</p>${Number(section.id) === 1 ? financeProgressPillV49() : ''}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам</button></div><h2>Блоки раздела</h2><p>Порядок прохождения: сначала уроки, затем итоговая диагностика. Финтренажёр можно открыть после диагностики или использовать как дополнительное закрепление материала.</p><div class="lesson-list-v2">${(section.lessons || []).map(financeLessonRowHtmlV49).join('')}${testRow}${trainerRow}</div>`)}
    `;
    shell(html, 'finance');
  }

  function financeTextBlockHtmlV49(text){
    const lines = String(text || '').split('\n').map(function(line){ return line.trim(); }).filter(Boolean);
    return lines.map(function(line){
      if (/^Слайд\s+\d+\./i.test(line)) return `<h3 class="finance-slide-title-v49">${esc(line)}</h3>`;
      if (/^Общая структура урока$/i.test(line)) return `<h2 class="finance-part-title-v49">${esc(line)}</h2>`;
      if (/^Что показать на слайде$/i.test(line)) return `<p class="finance-mini-head-v49">Что показать на слайде</p>`;
      if (/^Текст под слайдом$/i.test(line)) return `<p class="finance-mini-head-v49">Текст под слайдом</p>`;
      if (/^Итоговая логика урока$/i.test(line)) return `<h2 class="finance-part-title-v49">${esc(line)}</h2>`;
      return `<p>${esc(line)}</p>`;
    }).join('');
  }

  function financeFullLessonHtmlV49(lesson){
    const content = Array.isArray(lesson.fullContent) ? lesson.fullContent : [];
    if (!content.length) return '';
    return `<div class="finance-lesson-text-v49">${content.map(financeTextBlockHtmlV49).join('')}</div>`;
  }

  function financeSummaryLessonHtmlV49(lesson){
    return `<div class="finance-summary-grid-v49">
      <div><span>Задача урока</span><p>${esc(lesson.objective || 'Будет раскрыто позже.')}</p></div>
      <div><span>Ключевое содержание</span><p>${esc(lesson.content || 'Будет раскрыто позже.')}</p></div>
      <div><span>Кейс</span><p>${esc(lesson.case || 'Будет раскрыто позже.')}</p></div>
      <div><span>Результат ученика</span><p>${esc(lesson.result || 'Будет раскрыто позже.')}</p></div>
    </div>`;
  }
  ...
  function renderFinanceLessonV49(lessonId){
    ...
    const full = Array.isArray(lesson.fullContent) && lesson.fullContent.length;
    const html = `
      ${card('blue-card-v2 finance-hero-v49', `<p class="eyebrow">раздел ${Number(lesson.sectionId)} · урок ${Number(lesson.id)}</p><h1>${esc(lesson.title)}</h1><p>${esc(lesson.objective || '')}</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(${Number(lesson.sectionId)})">← К урокам раздела</button></div>${full ? financeFullLessonHtmlV49(lesson) : financeSummaryLessonHtmlV49(lesson)}`)}
    `;
    shell(html, 'profile');
  }
  ...
  function renderFinanceTestQuestionV49(){
    ...
    const selected = state.financeQuizAnswers ? state.financeQuizAnswers[state.financeQuizIndex] : undefined;
    const isLast = state.financeQuizIndex === total - 1;
    const caseHtml = q.case ? `<div class="finance-question-case-v49">${esc(q.case)}</div>` : '';
    const options = (q.options || []).map(function(option, index){
      return `<button class="option-v2 ${Number(selected) === index ? 'selected' : ''}" onclick="selectFinanceTestAnswer(${index})">${String.fromCharCode(65 + index)}. ${esc(option)}</button>`;
    }).join('');
    const html = `
      <div class="nav-panel-v2 nav-panel-v2-three">
        <button class="btn secondary" onclick="renderFinanceSection(1)">К разделу</button>
        <button class="btn secondary" ${state.financeQuizIndex === 0 ? 'disabled' : ''} onclick="prevFinanceTestQuestion()">Назад</button>
        <button class="btn secondary" onclick="renderFinanceModuleHome()">К модулю</button>
      </div>
      <div class="quiz-card-v2 finance-quiz-card-v49">
        <p class="eyebrow">Итоговая диагностика · вопрос ${state.financeQuizIndex + 1}/${total}</p>
        ${caseHtml}
        <h2>${esc(q.q)}</h2>
        <p class="small">Выберите один вариант ответа. Проходной уровень — ${FINANCE_SECTION1_PASS_SCORE_V49} из ${total} правильных ответов.</p>
        ${options}
        ${isLast ? '<p class="small">После выбора ответа тест завершится и покажет результат.</p>' : ''}
      </div>
    `;
    shell(html, 'profile');
  }

  function selectFinanceTestAnswerV49(index){
    state.financeQuizAnswers = state.financeQuizAnswers || {};
    state.financeQuizAnswers[state.financeQuizIndex] = Number(index);
    if (state.financeQuizIndex < FINANCE_SECTION1_TEST_V49.length - 1) {
      state.financeQuizIndex++;
      renderFinanceTestQuestionV49();
    } else {
      finishFinanceSection1TestV49();
    }
  }

  function prevFinanceTestQuestionV49(){
    state.financeQuizIndex = Math.max(0, Number(state.financeQuizIndex || 0) - 1);
    renderFinanceTestQuestionV49();
  }

  function financeReviewHtmlV49(answers){
    return `<div class="quiz-review-v2 finance-review-v49">${FINANCE_SECTION1_TEST_V49.map(function(q, index){
      const userIndex = answers[index] === undefined ? undefined : Number(answers[index]);
      const correctIndex = Number(q.correct || 0);
      const ok = userIndex === correctIndex;
      const critical = Boolean(q.critical) && !ok;
      const userText = userIndex === undefined ? 'нет ответа' : `${String.fromCharCode(65 + userIndex)}. ${q.options[userIndex] || ''}`;
      const correctText = `${String.fromCharCode(65 + correctIndex)}. ${q.options[correctIndex] || ''}`;
      return `<div class="review-row ${ok ? 'ok' : 'bad'} ${critical ? 'critical' : ''}">
        <h3>Вопрос ${index + 1}. ${ok ? 'Верно' : (critical ? 'Критическая ошибка' : 'Нужно повторить')}</h3>
        <p><b>Ваш ответ:</b> ${esc(userText)}</p>
        ${ok ? '' : `<p><b>Правильный ответ:</b> ${esc(correctText)}</p>`}
        <p><b>Почему:</b> ${esc(q.explanation || '')}</p>
      </div>`;
    }).join('')}</div>`;
  }

  function finishFinanceSection1TestV49(){
    const answers = state.financeQuizAnswers || {};
    let score = 0;
    let criticalErrors = 0;
    FINANCE_SECTION1_TEST_V49.forEach(function(q, index){
      const ok = Number(answers[index]) === Number(q.correct);
      if (ok) score++;
      if (!ok && q.critical) criticalErrors++;
    });
    const total = FINANCE_SECTION1_TEST_V49.length;
    const passed = score >= FINANCE_SECTION1_PASS_SCORE_V49;
    const percent = total ? Math.round(score / total * 100) : 0;
    saveFinanceProgressV49({
      section1TestScore: score,
      section1TestTotal: total,
      section1TestPassed: passed,
      section1CriticalErrors: criticalErrors,
      section1TestAt: nowIso()
    });
    const statusTitle = passed ? 'Диагностика пройдена' : 'Диагностика не пройдена';
    const statusText = passed
      ? 'Раздел 1 засчитан. Раздел 2 открыт. Следующие разделы будут открываться по той же логике: после диагностики предыдущего раздела минимум на 80%.'
      : 'Результат ниже 80%. Раздел 2 пока закрыт. Нужно повторить первые 4 урока, разобрать ошибки и пройти диагностику заново.';
    const html = `
      ${card(passed ? 'result-ok-v2' : 'result-bad-v2', `<h1>${statusTitle}</h1><p>Результат: <b>${score}/${total}</b> · <b>${percent}%</b>. Проходной уровень: <b>${FINANCE_SECTION1_PASS_SCORE_V49}/${total}</b>.</p><p>Критических ошибок: <b>${criticalErrors}</b>.</p><p>${statusText}</p><div class="grid-v2"><button class="btn primary" onclick="${passed ? 'renderFinanceModuleHome()' : 'startFinanceSection1Test(true)'}">${passed ? 'К финансовому модулю' : 'Пройти заново'}</button><button class="btn secondary" onclick="renderFinanceSection(1)">К разделу 1</button><button class="btn secondary" onclick="renderFinanceSection1Trainer()">Открыть финтренажёр</button></div>`)}
      ${card('', `<h2>Разбор ответов</h2>${financeReviewHtmlV49(answers)}`)}
    `;
    shell(html, 'finance');
  }
  ...
  function financeHelperHomeCardV49(){
  ...
  const secondaryBeforeFinanceV49 = window.secondaryBlocksHtmlV40;
  window.secondaryBlocksHtmlV40 = function(){
    const base = typeof secondaryBeforeFinanceV49 === 'function' ? secondaryBeforeFinanceV49() : '<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40"></div>';
    const helper = financeHelperHomeCardV49();
    if (base.indexOf('finance-home-card-v49') !== -1) return base;
    return base.replace(/(<div[^>]*architecture-secondary-tracks-v40[^>]*>)/, '$1' + helper);
  };

  window.myBusinessCardHtml = function(){
    if(financeCanOpenAssistantV49()){
      return card('my-business-card', `<p class="eyebrow">мой бизнес</p><h2>Финансовый помощник</h2><p>Финансовый модуль, аналитика и готовые шаблоны. Для учеников блок пока закрыт.</p><button class="btn primary" onclick="renderFinancialAssistant()">Открыть финансовый помощник</button>`);
    }
    return card('my-business-card my-business-card-locked', `<p class="eyebrow">мой бизнес</p><div class="my-business-card-head"><h2>Финансовый помощник</h2><span>закрыто</span></div><p class="small">Раздел находится в подготовке и пока недоступен ученикам.</p>`);
  };

  window.renderFinancialAssistant = renderFinancialAssistantV49;
  window.renderMyBusiness = renderFinancialAssistantV49;
  window.renderFinanceModuleHome = renderFinanceModuleHomeV49;
  window.renderFinanceSection = renderFinanceSectionV49;
  window.renderFinanceLesson = renderFinanceLessonV49;
  window.renderFinanceAnalytics = renderFinanceAnalyticsV49;
  window.renderFinanceTemplates = renderFinanceTemplatesV49;
  window.financeLockedSectionNotice = financeLockedSectionNoticeV49;
  window.startFinanceSection1Test = startFinanceSection1TestV49;
  window.renderFinanceTestQuestion = renderFinanceTestQuestionV49;
  ...
  function financeLessonCoverScreenHtmlV53(lesson, parsed, totalScreens){
    ...
    return `${card('blue-card-v2 finance-hero-v49 finance-lesson-cover-v53', `<p class="eyebrow">раздел ${Number(lesson.sectionId)} · урок ${Number(lesson.id)}</p><h1>${esc(lesson.title)}</h1><p>Структура урока.</p>`)}${card('finance-cover-card-v53', `<h2>Структура урока</h2><div class="finance-cover-grid-v53">${introHtml}</div><div class="finance-screen-counter-v53">Экран 1 из ${Number(totalScreens)}</div>`)}`;
  }
  ...
  function financeCleanLearningCopyV56(value){
    ...
    const replacements = [
      [/Этот урок должен полностью убрать у ученика/gi, 'Основная задача урока — убрать'],
      [/Урок \d+ должен закрепить/gi, 'Этот урок закрепляет'],
      [/Урок \d+ должен сформировать у ученика/gi, 'Этот урок формирует'],
      [/Ученик должен понять/gi, 'Важно зафиксировать'],
      [/ученик должен понять/gi, 'важно зафиксировать'],
      [/Ученик должен уметь/gi, 'После темы важно уметь'],
      [/ученик должен уметь/gi, 'после темы важно уметь'],
      [/Ученик разбирает/gi, 'В кейсе разбирается'],
      [/ученик разбирает/gi, 'в кейсе разбирается'],
      [/Ученик определяет/gi, 'Определяется'],
      [/ученик определяет/gi, 'определяется'],
      [/Ученик видит/gi, 'Формируется понимание того,'],
      [/ученик видит/gi, 'формируется понимание того,'],
    ...
    // Падежи: сначала косвенные формы, потом именительный падеж.
    ...
    out = out.replace(/\bучениках\b/g, 'предпринимателях');
    out = out.replace(/\bученики\b/g, 'предприниматели');
    out = out.replace(/\bУченик\b/g, 'Предприниматель');
    out = out.replace(/\bученик\b/g, 'предприниматель');

    // Ручные страховки от кривых следов старой автозамены.
    out = out.replace(/у предприниматель(\s|$)/gi, 'у предпринимателя$1');
    out = out.replace(/предпринимателя/gi, 'предпринимателя');
    out = out.replace(/предпринимателю/gi, 'предпринимателю');
    out = out.replace(/предпринимателем/gi, 'предпринимателем');
  ...
  function financeLessonCoverScreenHtmlV56(lesson, parsed, totalScreens){
    ...
    return `${card('blue-card-v2 finance-hero-v49 finance-lesson-cover-v53 finance-lesson-cover-v56', `<p class="eyebrow">раздел ${Number(lesson.sectionId)} · урок ${Number(lesson.id)}</p><h1>${esc(lesson.title)}</h1><p>Структура урока перед основной последовательностью слайдов.</p>`)}${card('finance-cover-card-v53 finance-cover-card-v56', `<h2>Титульный лист урока</h2><div class="finance-cover-grid-v53 finance-cover-grid-v56">${grid}</div><div class="finance-screen-counter-v53">Экран 1 из ${Number(totalScreens)}</div>`)} `;
  }

  function financeParagraphsHtmlV56(lines){
    const cleaned = (lines || []).map(function(line){ return financeCleanLearningCopyV56(line); }).filter(Boolean);
    const situationWords = /^(Первая|Вторая|Третья|Четв[её]ртая|Пятая)\s+ситуация\s*[—-]/i;
    const parts = [];
    let buffer = [];
    function flushSituations(){
      if (!buffer.length) return;
      parts.push(`<div class="finance-situation-list-v57">${buffer.map(function(line){
        const m = line.match(/^([^—-]+)[—-]\s*(.*)$/);
        if (m) return `<div><span>${esc(m[1].trim())}</span><p>${esc(m[2].trim())}</p></div>`;
        return `<div><p>${esc(line)}</p></div>`;
      }).join('')}</div>`);
      buffer = [];
    }
    cleaned.forEach(function(line){
      if (situationWords.test(line)) {
        buffer.push(line);
      } else {
        flushSituations();
  ...
  function financeLessonFinalTakeawaysV56(lesson){
    ...
    const map = {
      1: [
        'Деньги на счёте показывают наличие средств, но сами по себе не доказывают прибыль и не объясняют качество бизнеса.',
        'Выручка возникает после передачи ценности клиенту: оказанной услуги, переданного товара или выполненного этапа проекта.',
        'Аванс клиента, кредит, возврат дебиторки и вклад собственника увеличивают деньги, но по-разному влияют на ОПиУ, ОДДС и баланс.',
        'Перед любым выводом по бизнесу сначала определяется финансовый смысл операции: заработок, долг, обязательство, актив или движение старого остатка.',
        'Правильное финансовое мышление начинается с трёх вопросов: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.'
      ],
      2: [
        'ОПиУ отвечает за прибыльность периода: какая выручка заработана, какие расходы относятся к периоду и какой финансовый результат получен.',
        'ОДДС отвечает за движение денег и платёжеспособность: какие поступления и платежи реально произошли.',
        'Баланс показывает активы, обязательства и капитал на дату: где остались деньги, долги, авансы, дебиторка, запасы и оборудование.',
        'Метрики связывают отчёты с управленческими решениями: они показывают не просто цифру, а качество бизнес-модели и зону риска.'
      ],
      3: [
        'Бухгалтерия нужна для документов, корректности, обязательного учёта и отчётности. Это не враг управленки, а дисциплинирующий слой данных.',
        'Налоговый учёт нужен для расчёта обязательств перед государством. Его нельзя автоматически считать управленческой прибылью бизнеса.',
        'Управленческий учёт нужен для решений собственника: прибыльность направлений, деньги, остатки, кассовые риски, план-факт и действия.',
        'Прибыль по отчёту и кассовый риск могут одновременно быть правдой, если они отвечают на разные вопросы и читаются через разные экраны.'
      ],
      4: [
        'Одинаковая выручка не означает одинаковый бизнес: услуги, торговля, производство, проекты, логистика и HoReCa зарабатывают по разным механизмам.',
        'Экономический двигатель показывает, из каких драйверов складываются выручка, маржа, мощность, денежный цикл и повторяемость.',
        'Финансовое ограничение показывает, куда смотреть первым: спрос, маржа, мощность, оборотный капитал, люди, качество или управляемость.',
        'Перед масштабированием нужно понять не только размер выручки, но и то, где бизнес реально создаёт деньги, где теряет маржу и где застревает cash flow.'
      ]
    };
  ...
  window.FINANCE_MODULE_SECTIONS_DATA_V57 = FINANCE_MODULE_SECTIONS_V49;
  window.FINANCE_SECTION1_TEST_TOTAL_V57 = FINANCE_SECTION1_TEST_V49.length;
  window.FINANCE_SECTION1_PASS_SCORE_V57 = FINANCE_SECTION1_PASS_SCORE_V49;
  window.financeProgressKeyV57 = financeProgressKeyV49;
  window.financeLoadProgressV57 = loadFinanceProgressV49;
  window.financeSectionOnePassedV57 = financeSectionOnePassedV49;
...
/* =====================================================
   v54 — Финансовый помощник: блокировка в режиме ученика + компактный экран урока
   ===================================================== */
(function installFinanceStudentLockV54(){
  window.APP_UI_VERSION_V54 = 'v54-finance-student-lock-and-lesson-viewport-20260628';

  function financeAdminModeCanOpenV54(){
    try {
      return typeof isAdminMode === 'function' && isAdminMode() === true;
    } catch(e){
      return false;
    }
  }
  window.financeAdminModeCanOpenV54 = financeAdminModeCanOpenV54;

  // Старые версии v50/v52 проверяли просто факт администратора и могли
  // автоматически переводить просмотр ученика в администрирование. Теперь
  // доступ есть только когда явно включён режим администрирования.
  window.financeAdminCanOpenV50 = financeAdminModeCanOpenV54;
  window.financeAdminCanOpenV52 = financeAdminModeCanOpenV54;
  window.forceAdminModeV52 = function(){ return financeAdminModeCanOpenV54(); };
  window.ensureFinanceAdminModeV50 = function(){ return financeAdminModeCanOpenV54(); };

  function financeSvgV54(){
    return `<span class="arch-nav-icon"><svg class="arch-nav-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V9M10 20V4M16 20v-7M22 20H2"/><path d="m4 7 5-3 5 5 6-6"/></svg></span>`;
  }
  function iconV54(key){
    if (key === 'finance') return financeSvgV54();
    if (typeof architectureNavIconV35 === 'function') return `<span class="arch-nav-icon">${architectureNavIconV35(key)}</span>`;
    if (key === 'home') return '<span>⌂</span>';
    if (key === 'profile') return '<span>○</span>';
    return '<span>•</span>';
  ...
  function financeLockedNoticeV54(){
    alert('Финансовый помощник пока закрыт для учеников. Для проверки материалов включите режим администрирования в профиле.');
  }
  window.financeLockedNoticeV54 = financeLockedNoticeV54;

  function renderFinancialAssistantHubV54(){
    const html = `
      ${card('blue-card-v2 finance-hero-v49', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Открыт режим проверки материалов. Для учеников блок остаётся закрытым до отдельного запуска.</p>`)}
      ${card('', `<h2>Выберите раздел помощника</h2><div class="finance-hub-grid-v49">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>11 разделов, 62 урока, первые 4 урока полностью наполнены текстом, итоговая диагностика стоит в конце первого раздела.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Черновой контур для будущей аналитики бизнеса: ввод фактов, показатели и первичные выводы.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Место для будущих таблиц, шаблонов и инструментов финансового модуля.</p><em>скоро</em></button>
  ...
  function openFinanceAssistantV54(){
    if (!financeAdminModeCanOpenV54()) {
      financeLockedNoticeV54();
      return;
    }
    return renderFinancialAssistantHubV54();
  }
  window.openFinanceAssistantV54 = openFinanceAssistantV54;
  window.openFinanceAssistantV50 = openFinanceAssistantV54;
  window.openFinanceAssistantV52 = openFinanceAssistantV54;
  window.renderFinancialAssistant = openFinanceAssistantV54;
  window.renderMyBusiness = openFinanceAssistantV54;
  try { openFinanceAssistantV50 = openFinanceAssistantV54; } catch(e) {}
  try { openFinanceAssistantV52 = openFinanceAssistantV54; } catch(e) {}
  try { renderFinancialAssistant = openFinanceAssistantV54; } catch(e) {}
  try { renderMyBusiness = openFinanceAssistantV54; } catch(e) {}

  function bottomNavV54(active){
    if (typeof hasVerifiedAccessV32 === 'function' && !hasVerifiedAccessV32()) return '';
    const canFinance = financeAdminModeCanOpenV54();
    const home = `<button class="bottom-item ${active==='home'?'active':''}" onclick="renderHome()">${iconV54('home')}<b>Главная</b></button>`;
    const finance = canFinance
      ? `<button class="bottom-item ${active==='finance'?'active':''} finance-bottom-item-v54" onclick="openFinanceAssistantV54()">${iconV54('finance')}<b>Финансовый помощник</b></button>`
      : `<button class="bottom-item finance-bottom-item-v54 finance-bottom-locked-v54 is-disabled-v41" type="button" disabled aria-disabled="true" title="Раздел пока закрыт">${iconV54('finance')}<b>Финансовый помощник</b><small>закрыто</small></button>`;
    const profile = `<button class="bottom-item ${active==='profile'?'active':''}" onclick="renderProfile()">${iconV54('profile')}<b>Профиль</b></button>`;
    return `<nav class="bottom-nav-v2 v41-bottom-nav v54-bottom-nav" aria-label="Основное меню">${home}${finance}${profile}</nav>`;
  }
  window.bottomNav = bottomNavV54;
  try { bottomNav = bottomNavV54; } catch(e) {}

  function financeHomeCardV54(){
    const canFinance = financeAdminModeCanOpenV54();
    if (canFinance) {
      return `<button data-finance-main-v54="1" class="track-card active compact-card finance-main-card-v54" onclick="openFinanceAssistantV54()"><b>Финансовый помощник</b><p>Финансовый модуль, аналитика и готовые шаблоны. Доступ открыт только в режиме администрирования.</p><em>доступно администратору</em></button>`;
    }
    return `<button data-finance-main-v54="1" class="track-card disabled compact-card finance-main-card-v54" type="button" disabled aria-disabled="true"><b>Финансовый помощник</b><p>Финансовый модуль, аналитика и готовые шаблоны. Для учеников блок пока закрыт.</p><em>закрыто</em></button>`;
  }

  function patchFinanceHomeCardV54(){
    const grid = document.querySelector('.architecture-secondary-tracks-v40');
    if (!grid) return;
    grid.querySelectorAll('[data-finance-main-v49="1"], [data-finance-main-v50="1"], [data-finance-main-v52="1"], [data-finance-main-v54="1"], .finance-home-card-v49, .finance-main-card-v52').forEach(function(node){ node.remove(); });
    grid.insertAdjacentHTML('beforeend', financeHomeCardV54());
  }
  window.patchFinanceHomeCardV54 = patchFinanceHomeCardV54;

  function financeDrawerButtonV54(){
    const canFinance = financeAdminModeCanOpenV54();
    const base = `data-finance-assistant-v54="1" class="finance-drawer-item-v52 finance-drawer-item-v54 ${canFinance ? '' : 'student-locked-v41'}"`;
    if (canFinance) {
      return `<button ${base} onclick="closeAppDrawerV40(); openFinanceAssistantV54()"><span class="app-drawer-number-v40">12</span><span class="app-drawer-copy-v40"><b>Финансовый помощник</b><small>доступно администратору</small></span><span class="app-drawer-arrow-v40">›</span></button>`;
    }
    return `<button ${base} type="button" disabled aria-disabled="true"><span class="app-drawer-number-v40">12</span><span class="app-drawer-copy-v40"><b>Финансовый помощник</b><small>закрыто</small></span><span class="app-drawer-arrow-v40">•</span></button>`;
  }

  function patchFinanceDrawerV54(){
    const list = document.querySelector('.app-drawer-list-v40');
    if (!list) return;
    list.querySelectorAll('[data-finance-assistant-v50="1"], [data-finance-assistant-v52="1"], [data-finance-assistant-v54="1"]').forEach(function(node){ node.remove(); });
    list.insertAdjacentHTML('beforeend', financeDrawerButtonV54());
  }
  window.patchFinanceDrawerV54 = patchFinanceDrawerV54;

  function patchFinanceUiV54(){
    patchFinanceHomeCardV54();
    patchFinanceDrawerV54();
  }
  window.patchFinanceUiV54 = patchFinanceUiV54;

  const secondaryBeforeV54 = window.secondaryBlocksHtmlV40;
  if (typeof secondaryBeforeV54 === 'function') {
    window.secondaryBlocksHtmlV40 = function(){
      const base = secondaryBeforeV54.apply(this, arguments);
      const cleaned = String(base || '').replace(/<button[^>]*(?:data-finance-main-v49="1"|data-finance-main-v50="1"|data-finance-main-v52="1"|data-finance-main-v54="1"|finance-home-card-v49|finance-main-card-v52)[\s\S]*?<\/button>/g, '');
      return cleaned.replace(/(<\/div>\s*)$/, financeHomeCardV54() + '$1');
    };
    try { secondaryBlocksHtmlV40 = window.secondaryBlocksHtmlV40; } catch(e) {}
  }

  const installDrawerBeforeV54 = window.installAppDrawerV40;
  if (typeof installDrawerBeforeV54 === 'function') {
    window.installAppDrawerV40 = function(){
      const result = installDrawerBeforeV54.apply(this, arguments);
      setTimeout(patchFinanceUiV54, 0);
      setTimeout(patchFinanceUiV54, 80);
      return result;
    };
    try { installAppDrawerV40 = window.installAppDrawerV40; } catch(e) {}
  }

  const openDrawerBeforeV54 = window.openAppDrawerV40;
  if (typeof openDrawerBeforeV54 === 'function') {
    window.openAppDrawerV40 = function(){
      const result = openDrawerBeforeV54.apply(this, arguments);
      setTimeout(patchFinanceUiV54, 0);
      setTimeout(patchFinanceUiV54, 80);
      return result;
    };
  ...
  const shellBeforeV54 = window.shell;
  if (typeof shellBeforeV54 === 'function') {
    window.shell = function(content, activeTab){
      const text = String(content || '');
      const financeScreen = text.indexOf('finance-hero-v49') !== -1 || text.indexOf('finance-hub-grid-v49') !== -1 || text.indexOf('finance-section-list-v49') !== -1 || text.indexOf('finance-lesson-card-v49') !== -1 || text.indexOf('finance-slide-card-v53') !== -1 || text.indexOf('finance-cover-card-v53') !== -1;
      const result = shellBeforeV54.call(this, content, financeScreen ? 'finance' : activeTab);
      setTimeout(patchFinanceUiV54, 0);
      setTimeout(patchFinanceUiV54, 120);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }

  const renderHomeBeforeV54 = window.renderHome;
  if (typeof renderHomeBeforeV54 === 'function') {
    window.renderHome = function(){
      const result = renderHomeBeforeV54.apply(this, arguments);
      setTimeout(patchFinanceUiV54, 0);
      setTimeout(patchFinanceUiV54, 120);
      return result;
    };
  ...
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(patchFinanceUiV54, 0);
    setTimeout(patchFinanceUiV54, 300);
  });
})();


/* =====================================================
   v55 — Финансовый помощник: жёсткая блокировка в просмотре ученика,
   пересборка бокового меню и возврат на главную
   ===================================================== */
(function installFinanceHardStudentLockV55(){
  window.APP_UI_VERSION_V55 = 'v55-finance-admin-mode-hard-lock-20260628';

  function financeIsAdminModeV55(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; }
    catch(e) { return false; }
  }
  window.financeIsAdminModeV55 = financeIsAdminModeV55;

  function financeStudentLockedV55(){
    alert('Финансовый помощник пока закрыт для учеников. Проверка материалов доступна только в режиме администрирования.');
    if (typeof renderHome === 'function') renderHome();
  }
  window.financeStudentLockedV55 = financeStudentLockedV55;

  // Перекрываем старые проверки v49/v50/v52/v54: доступ не по факту админ-пользователя,
  // а только при явно включённом режиме администрирования.
  window.financeAdminCanOpenV50 = financeIsAdminModeV55;
  window.financeAdminCanOpenV52 = financeIsAdminModeV55;
  window.financeAdminModeCanOpenV54 = financeIsAdminModeV55;
  window.forceAdminModeV52 = function(){ return financeIsAdminModeV55(); };
  window.ensureFinanceAdminModeV50 = function(){ return financeIsAdminModeV55(); };

  // Добавляем финансовый помощник в общий список закрытых блоков для режима ученика.
  try {
    if (Array.isArray(window.studentLockedBlocksV41) && !window.studentLockedBlocksV41.includes('Финансовый помощник')) {
      window.studentLockedBlocksV41.push('Финансовый помощник');
    }
  } catch(e) {}

  function financeIconV55(){
    return `<span class="arch-nav-icon"><svg class="arch-nav-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V9M10 20V4M16 20v-7M22 20H2"/><path d="m4 7 5-3 5 5 6-6"/></svg></span>`;
  }
  function iconV55(key){
    if (key === 'finance') return financeIconV55();
    if (typeof architectureNavIconV35 === 'function') return `<span class="arch-nav-icon">${architectureNavIconV35(key)}</span>`;
    if (key === 'home') return '<span>⌂</span>';
    if (key === 'profile') return '<span>○</span>';
    return '<span>•</span>';
  ...
  function renderFinanceAssistantHubV55(){
    if (!financeIsAdminModeV55()) return financeStudentLockedV55();
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочий финансовый контур приложения. Здесь собираются учебный финансовый модуль, будущая аналитика бизнеса и готовые шаблоны для практического применения.</p><p>Сейчас раздел открыт только в режиме администрирования для проверки структуры и наполнения перед запуском для учеников.</p>`)}
      ${card('', `<h2>Выберите раздел помощника</h2><div class="finance-hub-grid-v49">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>Последовательная программа из 11 разделов: от базового финансового мышления до отчётности, метрик, отраслевых финансов и регулярной системы управления.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий личный контур показателей бизнеса: факты, отчёты, метрики, диагностика и выводы по данным.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц, тренажёров и инструментов под разные виды бизнеса.</p><em>скоро</em></button>
      </div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  }

  function openFinanceAssistantV55(){
    if (!financeIsAdminModeV55()) return financeStudentLockedV55();
    return renderFinanceAssistantHubV55();
  }

  window.openFinanceAssistantV55 = openFinanceAssistantV55;
  window.openFinanceAssistantV54 = openFinanceAssistantV55;
  window.openFinanceAssistantV52 = openFinanceAssistantV55;
  window.openFinanceAssistantV50 = openFinanceAssistantV55;
  window.renderFinancialAssistant = openFinanceAssistantV55;
  window.renderMyBusiness = openFinanceAssistantV55;
  try { openFinanceAssistantV54 = openFinanceAssistantV55; } catch(e) {}
  try { openFinanceAssistantV52 = openFinanceAssistantV55; } catch(e) {}
  try { openFinanceAssistantV50 = openFinanceAssistantV55; } catch(e) {}
  try { renderFinancialAssistant = openFinanceAssistantV55; } catch(e) {}
  try { renderMyBusiness = openFinanceAssistantV55; } catch(e) {}

  function bottomNavV55(active){
    if (typeof hasVerifiedAccessV32 === 'function' && !hasVerifiedAccessV32()) return '';
    const home = `<button class="bottom-item ${active==='home'?'active':''}" onclick="renderHome()">${iconV55('home')}<b>Главная</b></button>`;
    const finance = financeIsAdminModeV55()
      ? `<button class="bottom-item ${active==='finance'?'active':''} finance-bottom-item-v55" onclick="openFinanceAssistantV55()">${iconV55('finance')}<b>Финансовый помощник</b></button>`
      : `<button class="bottom-item finance-bottom-item-v55 finance-bottom-locked-v55 is-disabled-v41" type="button" disabled aria-disabled="true" title="Раздел пока закрыт">${iconV55('finance')}<b>Финансовый помощник</b><small>закрыто</small></button>`;
    const profile = `<button class="bottom-item ${active==='profile'?'active':''}" onclick="renderProfile()">${iconV55('profile')}<b>Профиль</b></button>`;
    return `<nav class="bottom-nav-v2 v41-bottom-nav v55-bottom-nav" aria-label="Основное меню">${home}${finance}${profile}</nav>`;
  }
  window.bottomNav = bottomNavV55;
  try { bottomNav = bottomNavV55; } catch(e) {}

  function isStudentViewV55(){ return !financeIsAdminModeV55(); }
  function blockCardV55(title, text, adminStatus, action, cls, studentLocked){
    const locked = Boolean(studentLocked) && isStudentViewV55();
    const finalAction = locked ? '' : (action || '');
    const finalStatus = locked ? 'в подготовке' : adminStatus;
    const finalCls = (cls || '') + (locked ? ' student-block-locked-v41' : '');
    return renderMainBlockCard(title, text, finalStatus, finalAction, finalCls);
  }
  function financeHomeCardV55(){
    if (financeIsAdminModeV55()) {
      return `<button data-finance-main-v55="1" class="track-card active compact-card finance-main-card-v55" onclick="openFinanceAssistantV55()"><b>Финансовый помощник</b><p>Финансовый модуль, аналитика и готовые шаблоны. Доступ открыт только в режиме администрирования.</p><em>доступно администратору</em></button>`;
    }
    return `<button data-finance-main-v55="1" class="track-card disabled compact-card finance-main-card-v55 student-block-locked-v41" type="button" disabled aria-disabled="true"><b>Финансовый помощник</b><p>Финансовый модуль, аналитика и готовые шаблоны. Для учеников блок пока закрыт.</p><em>закрыто</em></button>`;
  }

  // Полностью пересобираем вторую сетку блоков, чтобы старые активные карточки финансового помощника
  // больше не могли проскочить через обёртки v49/v52/v54.
  window.secondaryBlocksHtmlV40 = function(){
    const forumOpen = financeIsAdminModeV55() || (typeof forumVisibleInNavigationV38 === 'function' && forumVisibleInNavigationV38());
    const forumStatus = forumOpen ? 'тестирование' : 'в подготовке';
    const forumAction = forumOpen ? 'openForumBlockV40()' : '';
    return `<div class="secondary-track-grid-v22 architecture-secondary-tracks-v40">
      ${blockCardV55('Бизнес-форум','Вопросы по системам, обсуждение практических ситуаций и обмен опытом участников.',forumStatus,forumAction,'compact-card',true)}
      ${blockCardV55('100 книг за 100 дней','Ежедневный челлендж: одна книга, 24 часа, мини-тест, единица освоения и баллы серии.','доступно','renderBookChallenge()','active compact-card',false)}
      ${blockCardV55('Газета','Новости бизнеса и приложения в формате цифровых газетных выпусков.','скоро','renderNewspaperV40()','soon compact-card',true)}
      ${blockCardV55('Предпринимательские статьи','Отдельные материалы, наблюдения, заметки и практические тексты для предпринимателей.','скоро','renderEntrepreneurArticlesV40()','soon compact-card',true)}
      ${blockCardV55('Прямые разборы','Формат разборов бизнес-ситуаций, решений, ограничений и управленческих ошибок.','скоро','renderDirectReviewsV40()','soon compact-card',true)}
      ${blockCardV55('Что посмотреть','Подборки видео, фильмов, лекций и материалов для расширения управленческого кругозора.','скоро','renderWatchV40()','soon compact-card',true)}
      ${blockCardV55('Дополнительные материалы','Отдельные уроки, разборы и материалы, которые дополняют основную библиотеку.','скоро','renderAdditionalMaterials()','soon compact-card',true)}
      ${blockCardV55('VIP уровень','Расширенные разборы, инструменты и закрытые возможности.','в разработке','renderVipV40()','soon compact-card',true)}
      ${financeHomeCardV55()}
    </div>`;
  };
  try { secondaryBlocksHtmlV40 = window.secondaryBlocksHtmlV40; } catch(e) {}

  function drawerItemsV55(){
    const student = isStudentViewV55();
    const lock = function(action){ return student ? '' : action; };
    const lockedStatus = function(status){ return student ? 'в подготовке' : status; };
    return [
      {title:'Я предприниматель',status:'доступно',action:'renderLearning()',locked:false},
      {title:'Нет своего бизнеса',status:lockedStatus('скоро'),action:lock('renderNoBusinessV40()'),locked:student},
      {title:'Я сотрудник',status:lockedStatus('скоро'),action:lock('renderEmployeeRouteV40()'),locked:student},
      {title:'Бизнес-форум',status:lockedStatus('тестирование'),action:lock('openForumBlockV40()'),locked:student},
      {title:'100 книг за 100 дней',status:'доступно',action:'renderBookChallenge()',locked:false},
      {title:'Газета',status:lockedStatus('скоро'),action:lock('renderNewspaperV40()'),locked:student},
      {title:'Предпринимательские статьи',status:lockedStatus('скоро'),action:lock('renderEntrepreneurArticlesV40()'),locked:student},
      {title:'Прямые разборы',status:lockedStatus('скоро'),action:lock('renderDirectReviewsV40()'),locked:student},
      {title:'Что посмотреть',status:lockedStatus('скоро'),action:lock('renderWatchV40()'),locked:student},
      {title:'Дополнительные материалы',status:lockedStatus('скоро'),action:lock('renderAdditionalMaterials()'),locked:student},
      {title:'VIP уровень',status:lockedStatus('в разработке'),action:lock('renderVipV40()'),locked:student},
      {title:'Финансовый помощник',status:financeIsAdminModeV55() ? 'доступно администратору' : 'закрыто',action:financeIsAdminModeV55() ? 'openFinanceAssistantV55()' : '',locked:!financeIsAdminModeV55(),finance:true}
    ];
  }

  function drawerFooterHtmlV55(){
    const adminButton = (typeof isAdminUser === 'function' && isAdminUser())
      ? `<button type="button" class="drawer-admin-v44" onclick="switchAdminModeFromDrawerV55()">${financeIsAdminModeV55() ? 'Просмотр приложения' : 'Администрирование'}</button>`
      : '';
    return `<button type="button" onclick="openSupportFromDrawerV43()">Задать вопрос</button><button type="button" onclick="openIdeaFromDrawerV43()">Предложить идею</button><button type="button" class="drawer-refresh-v43" onclick="refreshAppInformationV43()">Обновить информацию</button>${adminButton}`;
  }

  function appDrawerHtmlV55(){
    return `<div class="app-drawer-overlay-v40" id="app-drawer-overlay-v40" onclick="closeAppDrawerV40()" aria-hidden="true">
      <aside class="app-drawer-v40" role="dialog" aria-modal="true" aria-label="Все разделы приложения" onclick="event.stopPropagation()">
        <div class="app-drawer-head-v40"><div><p>АРХИТЕКТУРА</p><span>Все разделы</span></div><button onclick="closeAppDrawerV40()" aria-label="Закрыть меню">×</button></div>
        <div class="app-drawer-list-v40">${drawerItemsV55().map(function(item,index){
          const n = String(index + 1).padStart(2,'0');
          const attrs = item.locked ? 'type="button" disabled aria-disabled="true"' : `onclick="closeAppDrawerV40(); ${item.action}"`;
          const cls = item.locked ? 'student-locked-v41' : '';
          const arrow = item.locked ? '•' : '›';
          return `<button class="${cls}" ${attrs}><span class="app-drawer-number-v40">${n}</span><span class="app-drawer-copy-v40"><b>${esc(item.title)}</b><small>${esc(item.status)}</small></span><span class="app-drawer-arrow-v40">${arrow}</span></button>`;
        }).join('')}</div>
        <div class="app-drawer-footer-v40">${drawerFooterHtmlV55()}</div>
      </aside>
    </div>`;
  }

  window.switchAdminModeFromDrawerV55 = function(){
    if (typeof closeAppDrawerV40 === 'function') closeAppDrawerV40();
    if (!(typeof isAdminUser === 'function' && isAdminUser())) return;
    if (typeof setAppMode === 'function') setAppMode(financeIsAdminModeV55() ? 'student' : 'admin');
  };
  window.switchAdminModeFromDrawerV44 = window.switchAdminModeFromDrawerV55;

  window.installAppDrawerV40 = function(){
    if (typeof architectureV40Enabled === 'function' && !architectureV40Enabled()) return;
    const shellRoot = document.querySelector('.app-shell-v2');
    ...
    const old = document.getElementById('app-drawer-overlay-v40');
    if (old) old.remove();
    shellRoot.insertAdjacentHTML('beforeend', appDrawerHtmlV55());
  };
  try { installAppDrawerV40 = window.installAppDrawerV40; } catch(e) {}

  window.openAppDrawerV40 = function(){
    window.installAppDrawerV40();
    const overlay = document.getElementById('app-drawer-overlay-v40');
    if (!overlay) return;
    overlay.classList.add('open');
    overlay.setAttribute('aria-hidden','false');
    document.body.classList.add('app-drawer-open-v40');
  ...
  // После переключения режима пересобираем меню с нуля, чтобы не оставался старый активный пункт.
  const setAppModeBeforeV55 = window.setAppMode || (typeof setAppMode === 'function' ? setAppMode : null);
  if (typeof setAppModeBeforeV55 === 'function' && !window.__setAppModeWrappedV55) {
    window.__setAppModeWrappedV55 = true;
    window.setAppMode = function(mode){
      const result = setAppModeBeforeV55.apply(this, arguments);
      try { const old = document.getElementById('app-drawer-overlay-v40'); if (old) old.remove(); } catch(e) {}
      setTimeout(function(){ if (typeof renderHome === 'function') renderHome(); }, 0);
      return result;
    };
    try { setAppMode = window.setAppMode; } catch(e) {}
  }

  function patchFinanceCardsV55(){
    document.querySelectorAll('[data-finance-main-v49="1"], [data-finance-main-v50="1"], [data-finance-main-v52="1"], [data-finance-main-v54="1"], .finance-home-card-v49, .finance-main-card-v52, [data-finance-assistant-v50="1"], [data-finance-assistant-v52="1"], [data-finance-assistant-v54="1"]').forEach(function(node){
      // Удаляем только старые вставки. Новые v55 создаются через secondaryBlocksHtmlV40 и drawerItemsV55.
      node.remove();
    });
  }

  const shellBeforeV55 = window.shell;
  if (typeof shellBeforeV55 === 'function') {
    window.shell = function(content, activeTab){
      const text = String(content || '');
      const financeScreen = text.indexOf('finance-hero-v49') !== -1 || text.indexOf('finance-hub-grid-v49') !== -1 || text.indexOf('finance-section-list-v49') !== -1 || text.indexOf('finance-lesson-card-v49') !== -1 || text.indexOf('finance-slide-card-v53') !== -1 || text.indexOf('finance-cover-card-v53') !== -1;
      const result = shellBeforeV55.call(this, content, financeScreen ? 'finance' : activeTab);
      setTimeout(patchFinanceCardsV55, 0);
      setTimeout(patchFinanceCardsV55, 180);
      setTimeout(patchFinanceCardsV55, 360);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }

  const renderHomeBeforeV55 = window.renderHome;
  if (typeof renderHomeBeforeV55 === 'function') {
    window.renderHome = function(){
      const result = renderHomeBeforeV55.apply(this, arguments);
      setTimeout(function(){
        patchFinanceCardsV55();
        if (typeof installAppDrawerV40 === 'function') installAppDrawerV40();
      }, 0);
      setTimeout(function(){
        patchFinanceCardsV55();
        if (typeof installAppDrawerV40 === 'function') installAppDrawerV40();
...
/* =====================================================
   v57 — Финансовый помощник: редактура текстов, последовательное открытие,
   финтренажёр и нейтральная вводная логика
   ===================================================== */
(function installFinanceEditorialV57(){
  window.APP_UI_VERSION_V57 = 'v57-finance-editorial-unlocks-trainer-20260628';

  const FINANCE_SECTION1_TOTAL_V57 = 30;
  const FINANCE_SECTION1_PASS_V57 = 24;
  const FINANCE_SECTION1_TRAINER_URL_V57 = '#'; // заменить на Google Sheets /copy-ссылку после создания таблицы

  function financeIsAdminModeV57(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; }
    catch(e){ return false; }
  }

  function financeLockedForStudentV57(){
    alert('Финансовый помощник пока закрыт для учеников. Материалы доступны только в режиме администрирования.');
    if (typeof renderHome === 'function') renderHome();
  }

  function financeProgressStorageKeysV57(){
    const keys = [];
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && key.indexOf('architecture_finance_module_progress_v49_') === 0) keys.push(key);
      }
    } catch(e) {}
    return keys;
  }

  function financeLoadProgressV57(){
    let best = {};
    let bestTime = -Infinity;
    financeProgressStorageKeysV57().forEach(function(key){
      try {
        const value = JSON.parse(localStorage.getItem(key) || '{}');
        const time = value.updatedAt ? new Date(value.updatedAt).getTime() : 0;
        if (time >= bestTime) { bestTime = time; best = value || {}; }
      } catch(e) {}
    });
    return best || {};
  }

  function financeSectionPassedV57(sectionId){
    const id = Number(sectionId);
    const progress = financeLoadProgressV57();
    if (id === 1) {
      return Boolean(progress.section1TestPassed) || Number(progress.section1TestScore || 0) >= FINANCE_SECTION1_PASS_V57;
    }
    return Boolean(progress['section' + id + 'TestPassed']);
  }

  function financeCanOpenSectionV57(sectionId){
    const id = Number(sectionId);
    if (id <= 1) return true;
    return financeSectionPassedV57(id - 1);
  }
  window.financeCanOpenSectionV57 = financeCanOpenSectionV57;

  function financeLockedSectionNoticeV57(sectionId){
    const id = Number(sectionId || 2);
    const prev = Math.max(1, id - 1);
    alert('Раздел ' + id + ' откроется после прохождения итоговой диагностики раздела ' + prev + ' минимум на 80%.');
  }
  window.financeLockedSectionNoticeV57 = financeLockedSectionNoticeV57;

  function financeCleanTextV57(value){
    let out = String(value || '');
    if (!out) return '';
    const replacements = [
      [/предпринимателя/g, 'предпринимателя'],
      [/Предпринимателя/g, 'Предпринимателя'],
      [/предпринимателю/g, 'предпринимателю'],
      [/Предпринимателю/g, 'Предпринимателю'],
      [/предпринимателем/g, 'предпринимателем'],
      [/Предпринимателем/g, 'Предпринимателем'],
      ...
      [/урок должен полностью убрать у предпринимателя/gi, 'урок убирает у предпринимателя'],
      [/Финальный смысловой вывод без обращения к ученику\./g, 'Короткая фиксация главной логики урока.'],
      [/Финальный смысловой вывод по теме без обращения к ученику\./g, 'Короткая фиксация главной логики урока.'],
    ];
    replacements.forEach(function(pair){ out = out.replace(pair[0], pair[1]); });
    out = out.replace(/\s+([,.!?;:])/g, '$1');
    return out;
  }
  window.financeCleanTextV57 = financeCleanTextV57;

  function patchTextNodesV57(root){
    const base = root || document;
    const selector = '.finance-slide-card-v56 p, .finance-slide-card-v56 h1, .finance-slide-card-v56 h2, .finance-slide-card-v56 h3, .finance-cover-card-v56 p, .finance-cover-card-v56 span, .finance-final-card-v56 p, .finance-final-grid-v56 p, .finance-main-thought-v56 p, .finance-lesson-callouts-v56 p, .finance-impact-matrix-v56 p, .finance-hero-v49 p, .finance-section-card-v49 p, .finance-section-card-v49 em, .finance-status-pill span, .finance-status-pill b';
    base.querySelectorAll(selector).forEach(function(node){
      const next = financeCleanTextV57(node.textContent);
      if (next && node.textContent !== next) node.textContent = next;
    });
  }

  function splitSituationTextV57(text, mode){
    const labels = mode === 'variant'
      ? ['Первый вариант', 'Второй вариант', 'Третий вариант', 'Четвёртый вариант', 'Пятый вариант']
      : ['Первая ситуация', 'Вторая ситуация', 'Третья ситуация', 'Четвёртая ситуация', 'Пятая ситуация'];
    const result = [];
    labels.forEach(function(label, idx){
      ...
      let chunk = end === -1 ? text.slice(start) : text.slice(start, end);
      chunk = chunk.replace(label + ' —', '').trim();
      chunk = chunk.replace(/\s+/g, ' ').trim();
      // Не забираем длинный общий вывод в пятый пункт.
      if (!nextLabel) {
        chunk = chunk.replace(/\s+Все пять ситуации?[\s\S]*$/i, '').trim();
        chunk = chunk.replace(/\s+Все пять ситуаций[\s\S]*$/i, '').trim();
        chunk = chunk.replace(/\s+Поэтому одинаковый[\s\S]*$/i, '').trim();
      }
      if (chunk) result.push({label: label, text: chunk});
    });
    return result;
  }

  function patchSituationListsV57(){
    document.querySelectorAll('.finance-slide-body-v56').forEach(function(body){
      if (body.dataset.situationPatchedV57 === '1') return;
      const paragraphs = Array.from(body.querySelectorAll('p'));
      if (!paragraphs.length) return;
      const combined = paragraphs.map(function(p){ return financeCleanTextV57(p.textContent); }).join(' ');
      const hasSituation = combined.indexOf('Первая ситуация —') !== -1;
      const hasVariant = combined.indexOf('Первый вариант —') !== -1;
      if (!hasSituation && !hasVariant) return;
      const mode = hasVariant ? 'variant' : 'situation';
      const items = splitSituationTextV57(combined, mode);
      if (items.length < 3) return;
      const intro = combined.split((mode === 'variant' ? 'Первый вариант —' : 'Первая ситуация —'))[0].trim();
      let closing = '';
      const closeMatch = combined.match(/(Все пять ситуаци[ия][\s\S]*$|Поэтому одинаковый приход денег[\s\S]*$)/i);
      if (closeMatch) closing = closeMatch[1].trim();
      const list = `<div class="finance-situation-list-v57">${items.map(function(item){
        return `<div><b>${esc(item.label)}</b><p>${esc(item.text)}</p></div>`;
      }).join('')}</div>`;
      body.innerHTML = '<h3>Объяснение</h3>' + (intro ? `<p>${esc(intro)}</p>` : '') + list + (closing ? `<p>${esc(closing)}</p>` : '');
      body.dataset.situationPatchedV57 = '1';
    });
  }

  function financeSlideTitleTextV57(){
    const h = document.querySelector('.finance-slide-card-v56 h1');
    return h ? String(h.textContent || '').toLowerCase() : '';
  }

  function financeDetailNoteForCurrentSlideV57(){
    const title = financeSlideTitleTextV57();
    if (!title) return '';
    if (title.includes('главный финансовый миф')) return 'Практическое уточнение: остаток на счёте является сигналом ликвидности, но не доказывает заработок. Перед выводом нужно понять источник денег и обязательства, которые появились вместе с поступлением.';
    if (title.includes('три финансовые реальности')) return 'Усиление логики: ОПиУ отвечает за результат периода, ОДДС — за реальные поступления и платежи, баланс — за незавершённые финансовые процессы: долги, авансы, дебиторку, активы и капитал.';
    if (title.includes('аванс')) return 'Практическое уточнение: аванс нельзя считать свободной прибылью. Он должен быть связан с объёмом неоказанных услуг, будущей загрузкой и обязательствами перед клиентами.';
    if (title.includes('кредит')) return 'Практическое уточнение: кредит может временно закрыть кассовый разрыв, но он переносит нагрузку в будущее через проценты и возврат тела долга. Поэтому его нельзя смешивать с операционной выручкой.';
    if (title.includes('дебитор')) return 'Практическое уточнение: дебиторка является активом, но не является деньгами. Чем дольше клиент платит, тем сильнее прибыль отрывается от кассы.';
    if (title.includes('актив') || title.includes('оборуд')) return 'Практическое уточнение: если покупка будет приносить пользу нескольким периодам, её нужно видеть как актив. Иначе месяц покупки станет искусственно слабым, а следующие месяцы — искусственно сильными.';
    if (title.includes('бухгалтер') || title.includes('кассовый риск')) return 'Практическое уточнение: корректная бухгалтерская прибыль не отменяет денежный риск. Для управления нужны сроки оплат, ближайшие платежи и остатки, а не только итоговая прибыль периода.';
    if (title.includes('экономический двигатель')) return 'Практическое уточнение: экономический двигатель нужен для поиска ограничения. Рост выручки без понимания маржи, мощности и денежного цикла может усилить проблему, а не решить её.';
    if (title.includes('услуг')) return 'Практическое уточнение: в услугах важно считать не только визиты и чек, но и выручку на час, загрузку, ФОТ специалиста, отмены и повторяемость.';
    if (title.includes('торгов')) return 'Практическое уточнение: в торговле высокий оборот опасен без контроля валовой маржи, скидок, списаний и скорости превращения товара обратно в деньги.';
    if (title.includes('производ')) return 'Практическое уточнение: в производстве прибыль может застревать в сырье, незавершёнке, готовой продукции и дебиторке. Поэтому важны WIP, брак, мощность и себестоимость.';
    return '';
  }

  function patchSlideDetailNotesV57(){
    const cardNode = document.querySelector('.finance-slide-card-v56');
    if (!cardNode || cardNode.dataset.detailNoteV57 === '1') return;
    const note = financeDetailNoteForCurrentSlideV57();
    if (!note) return;
    const body = cardNode.querySelector('.finance-slide-body-v56');
    if (!body) return;
    body.insertAdjacentHTML('afterend', `<div class="finance-detail-note-v57"><span>Уточнение</span><p>${esc(note)}</p></div>`);
    cardNode.dataset.detailNoteV57 = '1';
  }

  function financeProgressWidgetV57(){
    const p = financeLoadProgressV57();
    const score = Number(p.section1TestScore || 0);
    const total = Number(p.section1TestTotal || FINANCE_SECTION1_TOTAL_V57);
    const passed = financeSectionPassedV57(1);
    const percent = total ? Math.round(score / total * 100) : 0;
    return `<div class="finance-progress-v57 ${passed ? 'passed' : ''}">
      <div><span>Диагностика раздела 1</span><b>${passed ? 'засчитана' : `${score}/${total} · ${percent}%`}</b></div>
      <div class="progress-line"><div style="width:${Math.max(0, Math.min(100, percent))}%"></div></div>
      <p>${passed ? 'Следующий раздел открыт. Дальше каждый раздел будет открываться после теста предыдущего раздела.' : `Для открытия раздела 2 нужно не менее ${FINANCE_SECTION1_PASS_V57} правильных ответов из ${FINANCE_SECTION1_TOTAL_V57}.`}</p>
    </div>`;
  }

  function patchFinanceModuleHomeV57(){
    patchTextNodesV57(document);
    const hero = document.querySelector('.finance-hero-v49');
    if (hero && /Финансовая архитектура бизнеса/i.test(hero.textContent || '')) {
      const h1 = hero.querySelector('h1');
      if (h1) h1.textContent = 'Финансовая архитектура бизнеса';
      const paras = Array.from(hero.querySelectorAll('p')).filter(function(p){ return !p.classList.contains('eyebrow'); });
      if (paras[0]) {
        paras[0].textContent = 'Финансовая архитектура бизнеса — это учебный маршрут, который собирает деньги, выручку, прибыль, остатки, баланс, метрики и решения собственника в одну систему. Задача модуля — не научить заполнять таблицу механически, а сформировать финансовое мышление и подготовить основу для будущей аналитики бизнеса.';
      }
      if (!hero.querySelector('.finance-module-purpose-v57')) {
        hero.insertAdjacentHTML('beforeend', `<div class="finance-module-purpose-v57"><div><b>Как открываются разделы</b><p>Раздел 1 доступен сразу. Каждый следующий раздел открывается после прохождения итоговой диагностики предыдущего раздела минимум на 80%.</p></div><div><b>Что внутри</b><p>11 разделов и 62 урока: от базовой логики денег и прибыли до баланса, ликвидности, метрик, отраслевых финансов и регулярной системы управления.</p></div></div>`);
      }
      const oldPill = hero.querySelector('.finance-status-pill');
      if (oldPill && !hero.querySelector('.finance-progress-v57')) oldPill.outerHTML = financeProgressWidgetV57();
    }

    const sectionListCard = document.querySelector('.finance-section-list-v49')?.closest('.card-v2');
    if (sectionListCard) {
      const h2 = sectionListCard.querySelector('h2');
      if (h2) h2.textContent = 'Карта финансового модуля';
      const firstP = Array.from(sectionListCard.querySelectorAll(':scope > p'))[0];
      if (firstP) firstP.textContent = 'Разделы идут последовательно. Сначала формируется базовая финансовая логика, затем открываются выручка, прибыльность, безубыточность, деньги, баланс, долг, планирование, метрики, отраслевые финансы и система управления.';
    }
    patchFinanceSectionCardsV57();
  }

  function patchFinanceSectionCardsV57(){
    document.querySelectorAll('.finance-section-card-v49').forEach(function(cardNode){
      const span = cardNode.querySelector('span');
      const m = span ? String(span.textContent || '').match(/Раздел\s+(\d+)/i) : null;
      if (!m) return;
      const id = Number(m[1]);
      const em = cardNode.querySelector('em');
      const lessonsText = em ? String(em.textContent || '').replace(/·[\s\S]*$/, '').trim() : '';
      const open = financeCanOpenSectionV57(id);
      cardNode.classList.toggle('locked', !open);
      cardNode.setAttribute('onclick', open ? `renderFinanceSection(${id})` : `financeLockedSectionNoticeV57(${id})`);
      if (em) {
        if (id === 1) em.textContent = `${lessonsText || '5 уроков'} · доступно`;
        else em.textContent = open ? `${lessonsText || ''} · открыто` : `${lessonsText || ''} · после теста раздела ${id - 1}`;
      }
    });
  }

  function patchFinanceSectionPageV57(sectionId){
    patchTextNodesV57(document);
    const id = Number(sectionId || 1);
    const hero = document.querySelector('.finance-hero-v49');
    if (hero && id === 1) {
      const pill = hero.querySelector('.finance-status-pill');
      if (pill && !hero.querySelector('.finance-progress-v57')) pill.outerHTML = financeProgressWidgetV57();
      if (!hero.querySelector('.finance-section-rule-v57')) {
        hero.insertAdjacentHTML('beforeend', `<div class="finance-section-rule-v57"><b>Правило открытия</b><p>После итоговой диагностики раздела 1 при результате 80% откроется раздел 2. Далее такая же логика будет применяться к каждому следующему разделу: тест предыдущего раздела открывает следующий.</p></div>`);
      }
    }
    const testRow = document.querySelector('.finance-test-row-v49');
    if (testRow && !document.querySelector('[data-finance-trainer-row-v57="1"]')) {
      testRow.insertAdjacentHTML('afterend', `<button data-finance-trainer-row-v57="1" class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57" onclick="renderFinanceTrainerSection1()"><div><b>Финтренажёр раздела 1</b><p>Таблица для дополнительного закрепления: операции, ОПиУ, ОДДС, баланс и управленческий вывод</p></div><span>→</span></button>`);
    }
    const sectionCard = document.querySelector('.lesson-list-v2')?.closest('.card-v2');
    if (sectionCard && id === 1) {
      const h2 = sectionCard.querySelector('h2');
      if (h2) h2.textContent = 'Уроки и закрепление раздела';
    }
  }

  function renderFinanceTrainerSection1V57(){
    if (!financeIsAdminModeV57()) return financeLockedForStudentV57();
    const linkButton = FINANCE_SECTION1_TRAINER_URL_V57 && FINANCE_SECTION1_TRAINER_URL_V57 !== '#'
      ? `<a class="btn primary" target="_blank" href="${FINANCE_SECTION1_TRAINER_URL_V57}">Открыть и скопировать таблицу</a>`
      : `<button class="btn primary" onclick="financeTrainerLinkPendingV57()">Ссылка на таблицу будет добавлена</button>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-trainer-hero-v57', `<p class="eyebrow">финтренажёр</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление после первых четырёх уроков. Тренажёр нужен, чтобы отработать финансовый смысл операций: что попадает в ОПиУ, что отражается в ОДДС, что остаётся в балансе и какой вывод должен сделать собственник.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="finance-trainer-steps-v57"><div><b>1. Открыть таблицу</b><p>Используется ссылка на таблицу-тренажёр. После открытия нужно создать свою копию.</p></div><div><b>2. Прочитать инструкцию внутри</b><p>В таблице будет отдельный лист с порядком прохождения и правилами заполнения.</p></div><div><b>3. Пройти задания</b><p>Основной фокус: операции, деньги, выручка, авансы, кредиты, дебиторка, активы, ОПиУ, ОДДС и баланс.</p></div><div><b>4. Вернуться к разделу</b><p>Тренажёр не заменяет итоговую диагностику, а усиливает понимание перед переходом к следующему разделу.</p></div></div>${linkButton}<button class="btn secondary" onclick="renderFinanceSection(1)">← Вернуться к урокам раздела</button>`)}
    `;
    shell(html, 'finance');
  }
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1V57;
  window.financeTrainerLinkPendingV57 = function(){
    alert('Ссылка на таблицу ещё не подключена. Когда таблица будет создана, нужно заменить константу FINANCE_SECTION1_TRAINER_URL_V57 на ссылку формата Google Sheets /copy.');
  };

  function patchFinanceLessonPageV57(){
    patchTextNodesV57(document);
    patchSituationListsV57();
    patchSlideDetailNotesV57();
    const cover = document.querySelector('.finance-lesson-cover-v56');
    if (cover) {
      const p = Array.from(cover.querySelectorAll('p')).find(function(x){ return !x.classList.contains('eyebrow'); });
      if (p) p.textContent = 'Структура урока перед основной последовательностью слайдов.';
    }
    const coverTitle = document.querySelector('.finance-cover-card-v56 h2');
    if (coverTitle) coverTitle.textContent = 'Структура урока';
  }

  const previousRenderFinancialAssistantV57 = window.renderFinancialAssistant;
  function renderFinancialAssistantHubV57(){
    if (!financeIsAdminModeV57()) return financeLockedForStudentV57();
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-assistant-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Финансовый помощник — это контур, который соединяет обучение, будущую аналитику и готовые инструменты. Сейчас внутри открыта проверка учебного финансового модуля, а аналитика и шаблоны подготовлены как отдельные блоки для следующего этапа.</p>`)}
      ${card('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v49">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>Последовательная финансовая архитектура бизнеса: 11 разделов, 62 урока, тесты между разделами и закрепление через тренажёры.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий личный контур данных бизнеса: ввод фактов, отчёты, метрики, диагностика и выводы по финансовому состоянию.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и инструментов: ОПиУ, ОДДС, баланс, платёжный календарь, прогнозы и отраслевые шаблоны.</p><em>скоро</em></button>
      </div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  }

  window.openFinanceAssistantV57 = renderFinancialAssistantHubV57;
  window.openFinanceAssistantV55 = renderFinancialAssistantHubV57;
  window.openFinanceAssistantV54 = renderFinancialAssistantHubV57;
  window.openFinanceAssistantV52 = renderFinancialAssistantHubV57;
  window.openFinanceAssistantV50 = renderFinancialAssistantHubV57;
  window.renderFinancialAssistant = renderFinancialAssistantHubV57;
  window.renderMyBusiness = renderFinancialAssistantHubV57;
  try { renderFinancialAssistant = window.renderFinancialAssistant; } catch(e) {}
  try { renderMyBusiness = window.renderMyBusiness; } catch(e) {}

  const renderFinanceModuleHomeBeforeV57 = window.renderFinanceModuleHome;
  if (typeof renderFinanceModuleHomeBeforeV57 === 'function') {
    window.renderFinanceModuleHome = function(){
      if (!financeIsAdminModeV57()) return financeLockedForStudentV57();
      const result = renderFinanceModuleHomeBeforeV57.apply(this, arguments);
      setTimeout(patchFinanceModuleHomeV57, 0);
      setTimeout(patchFinanceModuleHomeV57, 180);
      return result;
    };
    try { renderFinanceModuleHome = window.renderFinanceModuleHome; } catch(e) {}
  }

  const renderFinanceSectionBeforeV57 = window.renderFinanceSection;
  if (typeof renderFinanceSectionBeforeV57 === 'function') {
    window.renderFinanceSection = function(sectionId){
      if (!financeIsAdminModeV57()) return financeLockedForStudentV57();
      const id = Number(sectionId || 1);
      if (!financeCanOpenSectionV57(id)) return financeLockedSectionNoticeV57(id);
      const result = renderFinanceSectionBeforeV57.apply(this, arguments);
      setTimeout(function(){ patchFinanceSectionPageV57(id); }, 0);
      setTimeout(function(){ patchFinanceSectionPageV57(id); }, 180);
      return result;
    };
    try { renderFinanceSection = window.renderFinanceSection; } catch(e) {}
  }

  const renderFinanceLessonBeforeV57 = window.renderFinanceLesson;
  if (typeof renderFinanceLessonBeforeV57 === 'function') {
    window.renderFinanceLesson = function(){
      if (!financeIsAdminModeV57()) return financeLockedForStudentV57();
      const result = renderFinanceLessonBeforeV57.apply(this, arguments);
      setTimeout(patchFinanceLessonPageV57, 0);
      setTimeout(patchFinanceLessonPageV57, 180);
      return result;
    };
    try { renderFinanceLesson = window.renderFinanceLesson; } catch(e) {}
  }

  const finishFinanceSection1TestBeforeV57 = window.finishFinanceSection1Test;
  if (typeof finishFinanceSection1TestBeforeV57 === 'function') {
    window.finishFinanceSection1Test = function(){
      const result = finishFinanceSection1TestBeforeV57.apply(this, arguments);
      setTimeout(function(){ patchTextNodesV57(document); }, 0);
      return result;
    };
    try { finishFinanceSection1Test = window.finishFinanceSection1Test; } catch(e) {}
  }

  // Обновляем нижнюю навигацию без автоперевода в администрирование.
  const bottomNavBeforeV57 = window.bottomNav;
  window.bottomNav = function(active){
    if (typeof hasVerifiedAccessV32 === 'function' && !hasVerifiedAccessV32()) return '';
    function icon(key){
      if (key === 'finance') return `<span class="arch-nav-icon"><svg class="arch-nav-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V9M10 20V4M16 20v-7M22 20H2"/><path d="m4 7 5-3 5 5 6-6"/></svg></span>`;
      if (typeof architectureNavIconV35 === 'function') return `<span class="arch-nav-icon">${architectureNavIconV35(key)}</span>`;
      return key === 'home' ? '<span>⌂</span>' : '<span>○</span>';
    }
    const home = `<button class="bottom-item ${active==='home'?'active':''}" onclick="renderHome()">${icon('home')}<b>Главная</b></button>`;
    const finance = financeIsAdminModeV57()
      ? `<button class="bottom-item ${active==='finance'?'active':''} finance-bottom-item-v55" onclick="openFinanceAssistantV57()">${icon('finance')}<b>Финансовый помощник</b></button>`
      : `<button class="bottom-item finance-bottom-item-v55 finance-bottom-locked-v55 is-disabled-v41" type="button" disabled aria-disabled="true" title="Раздел пока закрыт">${icon('finance')}<b>Финансовый помощник</b><small>закрыто</small></button>`;
    const profile = `<button class="bottom-item ${active==='profile'?'active':''}" onclick="renderProfile()">${icon('profile')}<b>Профиль</b></button>`;
    return `<nav class="bottom-nav-v2 v41-bottom-nav v55-bottom-nav v57-bottom-nav" aria-label="Основное меню">${home}${finance}${profile}</nav>`;
  };
  try { bottomNav = window.bottomNav; } catch(e) {}

  const shellBeforeV57 = window.shell;
  if (typeof shellBeforeV57 === 'function') {
    window.shell = function(content, activeTab){
      const text = String(content || '');
      const financeScreen = text.indexOf('finance-hero-v49') !== -1 || text.indexOf('finance-slide-card-v56') !== -1 || text.indexOf('finance-cover-card-v56') !== -1 || text.indexOf('finance-trainer-hero-v57') !== -1;
      const result = shellBeforeV57.call(this, content, financeScreen ? 'finance' : activeTab);
      setTimeout(function(){ patchTextNodesV57(document); patchSituationListsV57(); patchSlideDetailNotesV57(); }, 0);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(function(){ patchTextNodesV57(document); patchFinanceModuleHomeV57(); }, 300);
  });
})();


/* =====================================================
   v57 — Финансовый модуль: аудит текста, последовательное открытие,
   финтренажёр раздела 1 и чистая вводная логика
   ===================================================== */
(function installFinanceEditorialAndGateV57(){
  window.APP_UI_VERSION_V57 = 'v57-finance-editorial-gate-trainer-20260628';

  const FINANCE_SECTION1_TRAINER_URL_V57 = window.FINANCE_SECTION1_TRAINER_URL_V57 || '';
  ...
  function financeProgressKeyFallbackV57(){
    try {
      const ids = typeof possibleIds === 'function' ? possibleIds() : [];
      const usernames = typeof possibleUsernames === 'function' ? possibleUsernames() : [];
      return 'architecture_finance_module_progress_v49_' + (ids[0] || usernames[0] || 'local');
    } catch(e) {
      return 'architecture_finance_module_progress_v49_local';
    }
  }

  function financeProgressKeyV57(){
    try { return typeof window.financeProgressKeyV57 === 'function' ? window.financeProgressKeyV57() : financeProgressKeyFallbackV57(); }
    catch(e) { return financeProgressKeyFallbackV57(); }
  }

  function loadFinanceProgressLocalV57(){
    try {
      if (typeof window.financeLoadProgressV57 === 'function') return window.financeLoadProgressV57() || {};
      return JSON.parse(localStorage.getItem(financeProgressKeyV57()) || '{}');
    } catch(e) { return {}; }
  }

  function financeSectionTestPassedV57(sectionId){
    const id = Number(sectionId || 0);
    const p = loadFinanceProgressLocalV57();
    if (id === 1) return Boolean(p.section1TestPassed) || Number(p.section1TestScore || 0) >= 24;
    return Boolean(p['section' + id + 'TestPassed']) || Number(p['section' + id + 'TestScore'] || 0) >= Number(p['section' + id + 'TestPassScore'] || 9999);
  }

  function financeCanOpenSectionV57(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return true;
    if (financeIsAdminModeV57()) return true; // административный предпросмотр не ломает будущую логику ученика
    return financeSectionTestPassedV57(id - 1);
  }

  function financeSectionLearningGateTextV57(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return 'Раздел доступен сразу.';
    return 'Для ученика раздел ' + id + ' открывается после прохождения теста раздела ' + (id - 1) + ' на 80% и выше.';
  }

  function financeSection1ScoreV57(){
    const p = loadFinanceProgressLocalV57();
    ...
    return { score, total, pass, passed };
  }

  function financeProgressPillV57(){
    const s = financeSection1ScoreV57();
    const percent = s.total ? Math.round(s.score / s.total * 100) : 0;
    return `<div class="finance-status-pill finance-status-pill-v57 ${s.passed ? 'passed' : 'locked'}">
      <span>${s.passed ? 'Диагностика раздела 1 пройдена' : 'До открытия раздела 2'}</span>
      <b>${s.passed ? `${s.score}/${s.total} · ${percent}%` : `нужно ${s.pass}/${s.total} · сейчас ${s.score}/${s.total}`}</b>
    </div>`;
  }

  function financeStudentLockedV57(){
    alert('Финансовый помощник пока закрыт для учеников. Проверка материалов доступна только в режиме администрирования.');
    if (typeof renderHome === 'function') renderHome();
  }

  function financeSectionsDataV57(){
    const fromOriginal = Array.isArray(window.FINANCE_MODULE_SECTIONS_DATA_V57) ? window.FINANCE_MODULE_SECTIONS_DATA_V57 : [];
    if (fromOriginal.length) return fromOriginal;
    return [];
  }

  function getFinanceSectionDataV57(id){
    return financeSectionsDataV57().find(function(s){ return Number(s.id) === Number(id); });
  }

  function financeModuleIntroHtmlV57(){
    return `<div class="finance-module-intro-v57">
      <div><b>Что это за модуль</b><p>Финансовая архитектура бизнеса — это пошаговая система, которая учит видеть бизнес не через одну кассу, а через выручку, прибыль, деньги, остатки, обязательства, баланс, метрики и управленческие решения.</p></div>
      <div><b>Как он открывается</b><p>Каждый следующий раздел открывается после итогового теста предыдущего раздела. Проходной уровень — 80% правильных ответов. Такая логика не даёт перейти дальше, пока базовая финансовая связка не закреплена.</p></div>
      <div><b>Как работать с разделами</b><p>Сначала изучаются уроки раздела, затем проходится итоговая диагностика, затем при необходимости используется финтренажёр для дополнительного закрепления материала.</p></div>
    </div>`;
  }

  function renderFinanceAssistantHubV57(){
    if (!financeIsAdminModeV57()) return financeStudentLockedV57();
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Раздел собирает обучение, будущую аналитику и готовые финансовые инструменты. Сейчас он открыт в режиме администрирования для проверки структуры и содержания.</p>`)}
      ${card('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v49 finance-hub-grid-v57">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>Учебная система из 11 разделов: от базового финансового мышления до отчётности, метрик, отраслевых финансов и регулярного управления.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий контур личной аналитики: ввод фактов бизнеса, показатели, динамика и управленческие выводы.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц, тренажёров и финансовых инструментов под разные виды бизнеса.</p><em>скоро</em></button>
      </div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  }

  function financeSectionCardHtmlV57(section){
    const id = Number(section.id);
    const open = financeCanOpenSectionV57(id);
    const lessons = Array.isArray(section.lessons) ? section.lessons : [];
    const testAddon = id === 1 ? 2 : 1; // диагностика + тренажёр в первом разделе, в будущих — диагностика раздела
    const status = id === 1
      ? 'доступно'
      : (financeIsAdminModeV57() ? 'предпросмотр администратора' : (open ? 'доступно' : 'закрыто'));
    const gate = financeSectionLearningGateTextV57(id);
    return `<button class="finance-section-card-v49 finance-section-card-v57 ${open ? '' : 'locked'}" ${open ? `onclick="renderFinanceSection(${id})"` : `onclick="financeLockedSectionNotice(${id})"`}>
      <div><span>Раздел ${id}</span><b>${esc(section.title)}</b><p>${esc(section.description || '')}</p><small>${esc(gate)}</small></div>
      <em>${lessons.length + testAddon} блоков · ${esc(status)}</em>
    </button>`;
  }

  function renderFinanceModuleHomeV57(){
    if (!financeIsAdminModeV57()) return financeStudentLockedV57();
    const sections = financeSectionsDataV57();
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль объясняет, как операции бизнеса превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника.</p>${financeProgressPillV57()}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К помощнику</button></div><h2>Как устроен модуль</h2>${financeModuleIntroHtmlV57()}<h2>Разделы финансовой архитектуры</h2><p>Раздел 1 доступен сразу. Дальше действует последовательная логика: следующий раздел открывается после теста предыдущего раздела на 80% и выше.</p><div class="finance-section-list-v49 finance-section-list-v57">${sections.map(financeSectionCardHtmlV57).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }
  ...
  function financeLockedSectionNoticeV57(sectionId){
    const id = Number(sectionId || 0);
    if (id > 1) alert('Раздел ' + id + ' откроется после прохождения итогового теста раздела ' + (id - 1) + ' на 80% и выше.');
    else alert('Раздел пока закрыт.');
  }

  function financeLessonRowHtmlV57(lesson){
    const id = Number(lesson.id || 0);
    const ready = id >= 1 && id <= 4;
    const label = ready ? 'полный учебный экран' : 'структура урока из общей карты модуля';
    return `<button class="lesson-row-v2 finance-lesson-row-v49 finance-lesson-row-v57" onclick="renderFinanceLesson(${id})">
      <div><b>${String(id).padStart(2,'0')}. ${esc(lesson.title)}</b><p>${esc(label)}</p></div><span>→</span>
    </button>`;
  }

  function financeSection1RowsV57(section){
    const s = financeSection1ScoreV57();
    const testStatus = s.passed ? `пройдено · ${s.score}/${s.total}` : `нужно ${s.pass}/${s.total} · сейчас ${s.score}/${s.total}`;
    const lessons = (section.lessons || []).map(financeLessonRowHtmlV57).join('');
    const testRow = `<button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v49 finance-test-row-v57" onclick="startFinanceSection1Test(true)">
      <div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает следующий раздел · ${esc(testStatus)}</p></div><span>→</span>
    </button>`;
    const trainerRow = `<button class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57" onclick="renderFinanceTrainerSection1()">
      <div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу-тренажёр: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span>
    </button>`;
    return lessons + testRow + trainerRow;
  }

  function renderFinanceSectionV57(sectionId){
    if (!financeIsAdminModeV57()) return financeStudentLockedV57();
    const section = getFinanceSectionDataV57(sectionId);
    if (!section) return renderFinanceModuleHomeV57();
    const id = Number(section.id);
    const canOpen = financeCanOpenSectionV57(id);
    if (!canOpen) return financeLockedSectionNoticeV57(id);

    const gateNote = financeSectionLearningGateTextV57(id);
    const section1Progress = id === 1 ? financeProgressPillV57() : '';
    const rows = id === 1
      ? financeSection1RowsV57(section)
      : (section.lessons || []).map(function(lesson){
          return `<button class="lesson-row-v2 finance-lesson-row-v49 finance-lesson-row-v57" onclick="renderFinanceLessonSummaryV57(${id}, ${Number(lesson.id)})">
            <div><b>${String(lesson.id).padStart(2,'0')}. ${esc(lesson.title)}</b><p>структура урока · подробное раскрытие будет добавляться поэтапно</p></div><span>→</span>
          </button>`;
        }).join('') + `<button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v57 locked" disabled><div><b>Итоговая диагностика раздела ${id}</b><p>будет добавлена после наполнения уроков раздела</p></div><span>•</span></button>`;

    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел ${id}</p><h1>${esc(section.title)}</h1><p>${esc(section.description || '')}</p>${section1Progress}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам</button></div><div class="finance-section-gate-v57"><b>Логика открытия</b><p>${esc(gateNote)}</p></div><h2>Блоки раздела</h2><div class="lesson-list-v2">${rows}</div>`)}
    `;
    shell(html, 'finance');
  }

  function renderFinanceLessonSummaryV57(sectionId, lessonId){
    if (!financeIsAdminModeV57()) return financeStudentLockedV57();
    const section = getFinanceSectionDataV57(sectionId);
    const lesson = section && Array.isArray(section.lessons) ? section.lessons.find(function(l){ return Number(l.id) === Number(lessonId); }) : null;
    if (!lesson) return renderFinanceSectionV57(sectionId);
    const html = `${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел ${Number(sectionId)} · урок ${Number(lessonId)}</p><h1>${esc(lesson.title)}</h1><p>${esc(lesson.objective || '')}</p>`)}${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(${Number(sectionId)})">← К урокам раздела</button></div><div class="finance-summary-grid-v49 finance-summary-grid-v57"><div><span>Ключевое содержание</span><p>${esc(lesson.content || 'Будет раскрыто позже.')}</p></div><div><span>Кейс</span><p>${esc(lesson.case || 'Будет раскрыто позже.')}</p></div><div><span>Основной вывод</span><p>${esc(lesson.result || 'Будет раскрыто позже.')}</p></div><div><span>Статус</span><p>Сейчас это структура урока. Подробный сценарий, слайды, тест и материалы будут добавлены отдельным этапом.</p></div></div>`)} `;
    shell(html, 'finance');
  }

  function renderFinanceTrainerSection1V57(){
    if (!financeIsAdminModeV57()) return financeStudentLockedV57();
    const linkHtml = FINANCE_SECTION1_TRAINER_URL_V57
      ? `<a class="btn primary" href="${esc(FINANCE_SECTION1_TRAINER_URL_V57)}" target="_blank">Открыть таблицу-тренажёр</a><input class="finance-copy-link-v57" readonly value="${esc(FINANCE_SECTION1_TRAINER_URL_V57)}" onclick="this.select(); document.execCommand('copy'); alert('Ссылка скопирована.');">`
      : `<button class="btn secondary" type="button" onclick="alert('Ссылка на таблицу будет добавлена после создания финтренажёра.')">Ссылка на таблицу будет добавлена</button>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финтренажёр</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление материала после итоговой диагностики. Тренажёр помогает руками разложить операции по финансовому смыслу, ОПиУ, ОДДС и балансу.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как пройти</h2><div class="list-clean finance-trainer-instruction-v57"><div><b>1. Открыть таблицу</b><p>Откройте таблицу-тренажёр по ссылке ниже. Если таблица открывается как шаблон, используйте кнопку копирования.</p></div><div><b>2. Скопировать себе</b><p>Создайте личную копию таблицы. Оригинальный шаблон не заполняется: он нужен как чистая учебная форма.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с порядком прохождения, правилами выбора ответов и подсказками.</p></div><div><b>4. Пройти тренажёр</b><p>Разберите операции: где выручка, где поступление денег, где аванс, кредит, дебиторка, актив, обязательство и влияние на отчёты.</p></div></div><div class="finance-trainer-link-v57">${linkHtml}</div><p class="small">Финтренажёр не заменяет итоговую диагностику. Он нужен для дополнительного закрепления и подготовки к следующим разделам.</p>`)}
    `;
    shell(html, 'finance');
  }

  function financeFixTextV57(value){
    let out = String(value || '');
    out = out.replace(/предпринимателя/g, 'предпринимателя');
    out = out.replace(/у предприниматель(?![а-яё])/gi, function(m){ return m[0] === 'у' ? 'у предпринимателя' : 'У предпринимателя'; });
    out = out.replace(/для предприниматель(?![а-яё])/gi, 'для предпринимателя');
    out = out.replace(/к предприниматель(?![а-яё])/gi, 'к предпринимателю');
    out = out.replace(/с предприниматель(?![а-яё])/gi, 'с предпринимателем');
  ...
  function financeWalkTextNodesV57(root){
    if (!root) return;
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null);
    const nodes = [];
    while (walker.nextNode()) nodes.push(walker.currentNode);
    nodes.forEach(function(node){
      const next = financeFixTextV57(node.nodeValue);
      if (next !== node.nodeValue) node.nodeValue = next;
    });
  }

  function financeExtraContextForSlideV57(title){
    const t = String(title || '').toLowerCase();
    if (t.includes('сквозной кейс')) return '<b>Карта пяти ситуаций</b><p>1) заработанная выручка; 2) аванс клиента; 3) кредит; 4) возврат дебиторки; 5) вклад собственника. Сумма на счёте может быть одинаковой, но управленческий смысл, влияние на ОПиУ, ОДДС и баланс будут разными.</p>';
    if (t.includes('аванс')) return '<b>Дополнительное пояснение</b><p>Аванс опасен тем, что визуально усиливает кассу. Но пока услуга не оказана или товар не передан, это не свободная прибыль, а будущая обязанность бизнеса.</p>';
    if (t.includes('кредит')) return '<b>Дополнительное пояснение</b><p>Кредит можно использовать для прохождения кассового разрыва или роста, но нельзя оценивать как заработок. Он улучшает деньги сегодня и создаёт нагрузку на будущий денежный поток.</p>';
    if (t.includes('дебитор')) return '<b>Дополнительное пояснение</b><p>Дебиторка объясняет, почему прибыль может быть уже признана, а денег ещё нет. Чем дольше клиент не платит, тем сильнее прибыль застревает вне кассы.</p>';
    if (t.includes('оборуд') || t.includes('актив')) return '<b>Дополнительное пояснение</b><p>Актив не исчезает в момент покупки. Деньги меняют форму: вместо денег появляется ресурс, который должен приносить пользу в следующих периодах.</p>';
    if (t.includes('бухгалтер') || t.includes('кассовый риск')) return '<b>Дополнительное пояснение</b><p>Корректная бухгалтерская прибыль и реальный кассовый риск могут существовать одновременно. Спор снимается не выбором одного отчёта, а связкой ОПиУ, ОДДС и баланса.</p>';
    if (t.includes('услуг')) return '<b>Дополнительное пояснение</b><p>В услугах главный ресурс — время и компетенция специалиста. Поэтому высокая загрузка без маржи не является здоровым ростом.</p>';
    if (t.includes('торгов')) return '<b>Дополнительное пояснение</b><p>В торговле выручка может выглядеть высокой, но деньги оказываются заморожены в товаре. Поэтому оборот нужно читать вместе с маржей и оборачиваемостью.</p>';
    if (t.includes('производ')) return '<b>Дополнительное пояснение</b><p>В производстве прибыль зависит не только от продаж, но и от сырья, WIP, брака, мощности и сроков оплаты. Деньги часто застревают ещё до момента реализации.</p>';
    return '';
  }

  function financePolishCurrentLessonDomV57(){
    const root = document.querySelector('.content-v2');
    if (!root) return;
    financeWalkTextNodesV57(root);

    const slideCard = root.querySelector('.finance-slide-card-v56');
    if (slideCard && !slideCard.querySelector('.finance-extra-context-v57')) {
      const title = slideCard.querySelector('h1') ? slideCard.querySelector('h1').textContent : '';
      const extra = financeExtraContextForSlideV57(title);
      if (extra) {
        const target = slideCard.querySelector('.finance-lesson-callouts-v56') || slideCard.querySelector('.finance-slide-body-v56');
        if (target) target.insertAdjacentHTML('afterend', `<div class="finance-extra-context-v57">${extra}</div>`);
      }
    }
  }

  const renderFinanceLessonBeforeV57 = window.renderFinanceLesson;
  if (typeof renderFinanceLessonBeforeV57 === 'function' && !window.__renderFinanceLessonWrappedV57) {
    window.__renderFinanceLessonWrappedV57 = true;
    window.renderFinanceLesson = function(){
      const result = renderFinanceLessonBeforeV57.apply(this, arguments);
      setTimeout(financePolishCurrentLessonDomV57, 0);
      setTimeout(financePolishCurrentLessonDomV57, 120);
      return result;
    };
    try { renderFinanceLesson = window.renderFinanceLesson; } catch(e) {}
  }

  window.renderFinanceModuleHome = renderFinanceModuleHomeV57;
  window.renderFinanceSection = renderFinanceSectionV57;
  window.renderFinanceLessonSummaryV57 = renderFinanceLessonSummaryV57;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1V57;
  window.financeLockedSectionNotice = financeLockedSectionNoticeV57;
  window.renderFinancialAssistant = renderFinanceAssistantHubV57;
  window.openFinanceAssistantV55 = renderFinanceAssistantHubV57;
  window.openFinanceAssistantV54 = renderFinanceAssistantHubV57;
  window.openFinanceAssistantV52 = renderFinanceAssistantHubV57;
  window.openFinanceAssistantV50 = renderFinanceAssistantHubV57;
  try { renderFinanceModuleHome = window.renderFinanceModuleHome; } catch(e) {}
  try { renderFinanceSection = window.renderFinanceSection; } catch(e) {}
  try { renderFinanceTrainerSection1 = window.renderFinanceTrainerSection1; } catch(e) {}
  ...
  const shellBeforeV57 = window.shell;
  if (typeof shellBeforeV57 === 'function' && !window.__shellWrappedFinanceV57) {
    window.__shellWrappedFinanceV57 = true;
    window.shell = function(content, activeTab){
      const result = shellBeforeV57.call(this, content, activeTab);
      setTimeout(financePolishCurrentLessonDomV57, 0);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
...
/* =====================================================
   v57 — Финансовый помощник: последовательный доступ, тренажёр, редакторский аудит уроков
   ===================================================== */
(function installFinanceEditorialAndUnlockV57(){
  window.APP_UI_VERSION_V57 = 'v57-finance-unlock-trainer-editorial-audit-20260628';

  const FINANCE_SECTION_PASS_PERCENT_V57 = 80;
  const FINANCE_SECTION1_TOTAL_V57 = 30;
  const FINANCE_SECTION1_PASS_SCORE_V57 = 24;
  const FINANCE_SECTION1_TRAINER_URL_V57 = '#';

  const FINANCE_SECTIONS_V57 = [
    { id:1, title:'Финансовое мышление собственника', description:'Базовая логика: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс, управленческий учёт и экономический двигатель бизнеса.', lessons:5 },
    { id:2, title:'Выручка, продажи и unit-экономика', description:'Как возникает выручка, из каких драйверов она состоит и почему продажа сама по себе не гарантирует прибыльность.', lessons:6 },
    { id:3, title:'ОПиУ и прибыльность', description:'Себестоимость, маржа, расходы, EBITDA, чистая прибыль и ошибки, которые искажают финансовый результат.', lessons:7 },
    { id:4, title:'Безубыточность, масштабирование и операционный рычаг', description:'Минимальный объём продаж для выживания, запас прочности и эффект постоянных расходов при росте или падении бизнеса.', lessons:3 },
    { id:5, title:'ОДДС, деньги и ликвидность', description:'Почему прибыль не равна деньгам, где возникают кассовые разрывы и как управлять платёжеспособностью.', lessons:6 },
    { id:6, title:'Баланс и остатки', description:'Активы, обязательства, капитал, авансы, основные средства и мосты остатков как основа управленческой картины.', lessons:7 },
    { id:7, title:'Долг, налоги, ФОТ и собственник', description:'Кредиты, проценты, налоги, команда и личные деньги собственника — крупные блоки, которые меняют прибыль и деньги.', lessons:4 },
    { id:8, title:'Планирование, прогнозирование и инвестиции', description:'План-факт, драйверное бюджетирование, сценарии, стресс-тесты, инвестиции и источники финансирования роста.', lessons:5 },
    { id:9, title:'Метрики, диагностика и дашборд', description:'Система показателей, диагностика бизнеса и экран собственника, где цифры превращаются в решения.', lessons:6 },
    { id:10, title:'Отраслевые финансы', description:'Почему финансы услуг, торговли, производства, проектов, логистики и HoReCa считаются по-разному.', lessons:6 },
    { id:11, title:'Финансовая система и управление', description:'Регулярная финансовая архитектура: правила учёта, статьи, закрытие месяца, роли, контроль и стадии бизнеса.', lessons:8 }
  ];

  function financeEscV57(value){
    if (typeof esc === 'function') return esc(value);
    return String(value ?? '').replace(/[&<>'"]/g, function(c){ return ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'})[c]; });
  }
  function financeAdminModeV57(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; } catch(e){ return false; }
  }
  function financeRequireAdminV57(){
    if (financeAdminModeV57()) return true;
    alert('Финансовый помощник пока закрыт для учеников. Для проверки материалов включите режим администрирования.');
    return false;
  }
  function financeUserSuffixV57(){
    try {
      const ids = typeof possibleIds === 'function' ? possibleIds() : [];
      const usernames = typeof possibleUsernames === 'function' ? possibleUsernames() : [];
      return ids[0] || usernames[0] || 'local';
  ...
  function financeProgressKeyV57(){ return 'architecture_finance_module_progress_v49_' + financeUserSuffixV57(); }
  function loadFinanceProgressV57(){
    try { return JSON.parse(localStorage.getItem(financeProgressKeyV57()) || '{}'); } catch(e){ return {}; }
  }
  function sectionTestPassedV57(sectionId){
    const id = Number(sectionId);
    const p = loadFinanceProgressV57();
    if (id === 1) return Boolean(p.section1TestPassed) || Number(p.section1TestScore || 0) >= FINANCE_SECTION1_PASS_SCORE_V57;
    return Boolean(p['section' + id + 'TestPassed']) || Number(p['section' + id + 'TestScore'] || 0) >= Number(p['section' + id + 'PassScore'] || 999999);
  }
  function financeCanOpenSectionV57(sectionId){
    const id = Number(sectionId);
    if (id <= 1) return true;
    return sectionTestPassedV57(id - 1);
  }
  function financeSectionStatusV57(section){
    const id = Number(section.id);
    if (id === 1) return 'доступно';
    if (financeCanOpenSectionV57(id)) return 'открыто после теста раздела ' + (id - 1);
    return 'закрыто до теста раздела ' + (id - 1);
  }
  function financeLockedSectionNoticeV57(sectionId){
    const id = Number(sectionId || 2);
    const prev = Math.max(1, id - 1);
  ...
  function financeSection1ProgressBoxV57(){
    const p = loadFinanceProgressV57();
    const score = Math.max(0, Number(p.section1TestScore || 0));
    const total = Number(p.section1TestTotal || FINANCE_SECTION1_TOTAL_V57);
    const pct = total ? Math.max(0, Math.min(100, Math.round(score / total * 100))) : 0;
    const passed = sectionTestPassedV57(1);
    return `<div class="finance-unlock-progress-v57 ${passed ? 'passed' : 'locked'}">
      <div class="finance-unlock-head-v57"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : 'не пройдена'}</b></div>
      <div class="finance-unlock-bar-v57"><div style="width:${pct}%"></div></div>
      <p>${score}/${total} правильных ответов · нужно ${FINANCE_SECTION1_PASS_SCORE_V57}/${FINANCE_SECTION1_TOTAL_V57} (${FINANCE_SECTION_PASS_PERCENT_V57}%) для открытия раздела 2.</p>
    </div>`;
  }

  function renderFinancialAssistantHubV57(){
    if (!financeRequireAdminV57()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Финансовая архитектура бизнеса показывает, как операции превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника. Это не просто набор таблиц, а учебная система, которая постепенно собирает финансовое мышление и готовит к работе с инструментами.</p>`)}
      ${card('', `<h2>Что внутри</h2><div class="finance-hub-grid-v49 finance-hub-grid-v57">
        <button class="finance-hub-card-v49 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>11 разделов: от базового различения денег, выручки и прибыли до баланса, ликвидности, метрик, отраслевых финансов и регулярной системы управления.</p><em>открыть</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий контур личных показателей бизнеса: факты, отчёты, динамика, риски и управленческие выводы.</p><em>каркас</em></button>
        <button class="finance-hub-card-v49" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц, тренажёров и финансовых инструментов для закрепления материала и применения в бизнесе.</p><em>готовится</em></button>
      </div><div class="finance-module-principles-v57"><b>Логика открытия</b><p>Каждый следующий раздел открывается только после прохождения итоговой диагностики предыдущего раздела минимум на 80%. Так сохраняется порядок: сначала база, потом прибыльность, деньги, баланс, метрики и система управления.</p></div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  }
  window.openFinanceAssistantV57 = renderFinancialAssistantHubV57;
  ...
  window.renderFinancialAssistant = renderFinancialAssistantHubV57;
  window.renderMyBusiness = renderFinancialAssistantHubV57;
  try { openFinanceAssistantV55 = renderFinancialAssistantHubV57; } catch(e) {}
  try { renderFinancialAssistant = renderFinancialAssistantHubV57; } catch(e) {}
  try { renderMyBusiness = renderFinancialAssistantHubV57; } catch(e) {}

  function financeSectionCardHtmlV57(section){
    const open = financeCanOpenSectionV57(section.id);
    const status = financeSectionStatusV57(section);
    const click = open ? `renderFinanceSection(${Number(section.id)})` : `financeLockedSectionNotice(${Number(section.id)})`;
    return `<button class="finance-section-card-v49 finance-section-card-v57 ${open ? '' : 'locked'}" onclick="${click}">
      <div><span>Раздел ${Number(section.id)}</span><b>${financeEscV57(section.title)}</b><p>${financeEscV57(section.description)}</p></div>
      <em>${Number(section.lessons)} уроков · ${financeEscV57(status)}</em>
    </button>`;
  }
  ...
  function renderFinanceModuleHomeV57(){
    if (!financeRequireAdminV57()) return;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль выстроен как последовательная лестница финансового мышления. Сначала фиксируется базовая логика: деньги не равны прибыли, выручка не равна поступлению, баланс показывает активы и обязательства. Далее открываются разделы про выручку, прибыльность, безубыточность, деньги, баланс, долг, планирование, метрики, отраслевые финансы и регулярную систему управления.</p>${financeSection1ProgressBoxV57()}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>11 разделов модуля</h2><p>Разделы открываются последовательно: следующий раздел становится доступен после итоговой диагностики предыдущего раздела минимум на 80%.</p><div class="finance-section-list-v49">${FINANCE_SECTIONS_V57.map(financeSectionCardHtmlV57).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }
  const renderFinanceModuleHomeBeforeV57 = window.renderFinanceModuleHome;
  window.renderFinanceModuleHome = renderFinanceModuleHomeV57;
  try { renderFinanceModuleHome = renderFinanceModuleHomeV57; } catch(e) {}

  function patchFinanceSectionDomV57(sectionId){
    if (Number(sectionId) === 1) {
      const oldPill = document.querySelector('.finance-status-pill');
      if (oldPill) oldPill.outerHTML = financeSection1ProgressBoxV57();
      const list = document.querySelector('.lesson-list-v2');
      if (list && !list.querySelector('[data-finance-trainer-v57="1"]')) {
        const test = list.querySelector('.finance-test-row-v49');
        const row = `<button data-finance-trainer-v57="1" class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57" onclick="renderFinanceSection1Trainer()"><div><b>Финтренажёр раздела 1</b><p>Таблица для дополнительного закрепления: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
        if (test) test.insertAdjacentHTML('afterend', row); else list.insertAdjacentHTML('beforeend', row);
      }
      const hero = document.querySelector('.finance-hero-v49');
      if (hero) {
        const para = hero.querySelector('p:not(.eyebrow)');
        if (para) para.textContent = 'Раздел формирует базовую логику: предприниматель перестаёт смотреть только на кассу и начинает видеть бизнес как систему финансовых потоков, остатков, обязательств и решений.';
      }
    }
  }
  ...
  window.renderFinanceSection = function(sectionId){
    if (!financeRequireAdminV57()) return;
    if (!financeCanOpenSectionV57(sectionId)) return financeLockedSectionNoticeV57(sectionId);
    if (typeof renderFinanceSectionBeforeV57 === 'function') {
      const result = renderFinanceSectionBeforeV57.apply(this, arguments);
      setTimeout(function(){ patchFinanceSectionDomV57(sectionId); }, 0);
      setTimeout(function(){ patchFinanceSectionDomV57(sectionId); }, 120);
      return result;
    }
    return renderFinanceModuleHomeV57();
  };
  try { renderFinanceSection = window.renderFinanceSection; } catch(e) {}

  window.renderFinanceSection1Trainer = function(){
    if (!financeRequireAdminV57()) return;
    const hasUrl = FINANCE_SECTION1_TRAINER_URL_V57 && FINANCE_SECTION1_TRAINER_URL_V57 !== '#';
    const button = hasUrl
      ? `<a class="btn primary" href="${FINANCE_SECTION1_TRAINER_URL_V57}" target="_blank">Открыть и скопировать таблицу</a>`
      : `<button class="btn primary" onclick="alert('Ссылка на таблицу финтренажёра будет добавлена после загрузки шаблона.')">Ссылка на таблицу будет добавлена</button>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление после итоговой диагностики. Тренажёр нужен для отработки финансового смысла операций: где выручка, где деньги, где актив, где обязательство и как это влияет на ОПиУ, ОДДС и баланс.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean finance-trainer-steps-v57"><div><b>1. Открыть таблицу</b><p>После подключения шаблона здесь будет ссылка на Google Sheets с тренажёром.</p></div><div><b>2. Сделать копию</b><p>Нужно открыть таблицу и создать собственную копию, чтобы ответы и расчёты не смешивались с исходным шаблоном.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения, обозначениями и порядком работы.</p></div><div><b>4. Пройти задания</b><p>Задача — классифицировать операции, проверить влияние на ОПиУ, ОДДС и баланс, затем сформулировать управленческий вывод.</p></div></div>${button}<button class="btn secondary" onclick="renderFinanceModuleHome()">← К финансовому модулю</button>`)}
    `;
    shell(html, 'finance');
  };

  function replaceTextNodesV57(root){
    if (!root) return;
    const replacements = [
      [/предпринимателя/g, 'предпринимателя'],
      [/предпринимателю/g, 'предпринимателю'],
      [/предпринимателем/g, 'предпринимателем'],
      [/у предприниматель(?![а-яё])/gi, 'у предпринимателя'],
      [/для предприниматель(?![а-яё])/gi, 'для предпринимателя'],
      [/к предприниматель(?![а-яё])/gi, 'к предпринимателю'],
      [/Этот урок должен полностью убрать у предпринимателя/gi, 'Этот урок убирает у предпринимателя'],
  ...
  function lessonIdFromFinanceScreenV57(){
    const eyebrow = document.querySelector('.finance-hero-v49 .eyebrow, .finance-slide-card-v56 .eyebrow, .finance-final-card-v56 .eyebrow');
    const bodyText = document.body ? document.body.textContent || '' : '';
    const m = bodyText.match(/раздел\s+1\s+·\s+урок\s+(\d+)/i) || bodyText.match(/урок\s+(\d+)/i);
    return m ? Number(m[1]) : 0;
  }
  function patchCoverV57(){
    const grid = document.querySelector('.finance-cover-grid-v56');
    if (!grid) return;
    const hero = document.querySelector('.finance-lesson-cover-v56');
    const title = hero ? (hero.querySelector('h1')?.textContent || '') : (document.querySelector('.finance-hero-v49 h1')?.textContent || '');
    const meta = {
      'Финансовая карта бизнеса': {
        slides:'22.',
        format:'Теоретическое объяснение, схема финансовой карты, сквозной кейс одного месяца, разбор через ОПиУ, ОДДС, баланс и метрики.',
        task:'Показать бизнес как систему, где операции превращаются в выручку, деньги — в денежный поток, остатки — в баланс, а метрики — в решения.',
        output:'Финансы бизнеса перестают выглядеть как одна таблица кассы. Одна операция начинает читаться через прибыльность, движение денег, остатки и управленческие метрики.'
      },
      'Управленческий учёт против бухгалтерии': {
        slides:'22.',
        format:'Сравнительные схемы, разбор бухгалтерского, налогового и управленческого контуров, кейс расхождения прибыли и кассового риска.',
        task:'Отделить управленческую финансовую систему собственника от бухгалтерского и налогового учёта.',
        output:'Бухгалтерия, налоги и управленческий учёт перестают смешиваться. Каждый контур получает свою задачу, а собственник видит, какие данные нужны для решений.'
      },
      'Экономика бизнес-модели': {
        slides:'32.',
        format:'Теория, отраслевые схемы, формулы драйверов, сравнительный кейс и диагностика экономического двигателя бизнеса.',
        task:'Показать, за счёт чего конкретный бизнес зарабатывает и где у него возникает главное финансовое ограничение.',
        output:'Бизнес начинает читаться через экономический двигатель: выручку, маржу, мощность, оборачиваемость, повторяемость и главное ограничение модели.'
      },
      'Деньги, прибыль и выручка: почему предприниматель ошибается': {
        slides:'18.',
        format:'Теоретическое объяснение, разбор финансовых понятий, сквозной кейс, сравнение влияния на ОПиУ, ОДДС и баланс.',
        task:'Разрушить базовый финансовый миф предпринимателя: деньги на счёте не равны прибыли, а поступление денег не всегда является выручкой.',
        output:'Предприниматель перестаёт оценивать бизнес только по остатку денег и разделяет три финансовые реальности: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.'
      }
    };
    const chosen = Object.keys(meta).find(function(key){ return title.indexOf(key) !== -1; });
    if (!chosen) return;
    const rowData = [
      ['Количество слайдов', meta[chosen].slides],
      ['Формат урока', meta[chosen].format],
      ['Главная задача', meta[chosen].task],
      ['Основной вывод', meta[chosen].output]
    ];
    grid.innerHTML = rowData.map(function(row){ return `<div class="finance-cover-line-v53 finance-cover-line-v56"><span>${financeEscV57(row[0])}</span><p>${financeEscV57(row[1])}</p></div>`; }).join('');
    const intro = document.querySelector('.finance-lesson-cover-v56 > p:not(.eyebrow)');
    if (intro) intro.textContent = 'Структура урока перед основной последовательностью слайдов.';
  }

  function patchCaseSituationsV57(){
    const cardNode = document.querySelector('.finance-slide-card-v56');
    if (!cardNode) return;
    const title = cardNode.querySelector('h1')?.textContent || '';
    const body = cardNode.querySelector('.finance-slide-body-v56');
    if (!body || !/Сквозной кейс/i.test(title) || body.querySelector('.finance-situation-list-v57')) return;
    body.innerHTML = `<h3>Объяснение</h3>
      <p>На счёт бизнеса пришла одна и та же сумма. В банковской выписке это выглядит одинаково: деньги увеличились. Но для управления бизнесом важен не только факт поступления, а причина, по которой деньги пришли.</p>
      <div class="finance-situation-list-v57">
        <div><b>1. Оплата за уже оказанную услугу или проданный товар.</b><p>Это может быть выручкой текущего периода. В ОПиУ появляется выручка, в ОДДС — операционное поступление, в балансе увеличиваются деньги.</p></div>
        <div><b>2. Аванс клиента.</b><p>Деньги уже пришли, но выручка ещё не заработана полностью. В балансе появляется обязательство: бизнес должен оказать услугу, поставить товар или выполнить обещание.</p></div>
        <div><b>3. Кредит.</b><p>Деньги пришли как финансирование, а не как заработок. В ОПиУ выручка не возникает. В ОДДС появляется финансовое поступление, а в балансе одновременно растут деньги и долг.</p></div>
        <div><b>4. Возврат дебиторки.</b><p>Клиент оплатил старый долг. Деньги пришли сейчас, но выручка могла быть признана раньше. В балансе дебиторка уменьшается, деньги увеличиваются.</p></div>
        <div><b>5. Вклад собственника.</b><p>Касса усилилась за счёт владельца, а не за счёт клиента. Это поддерживает бизнес, но не является выручкой и не доказывает прибыльность модели.</p></div>
      </div>
      <p>Вывод простой: одно поступление нельзя называть выручкой, пока не определён его финансовый смысл и влияние на ОПиУ, ОДДС и баланс.</p>`;
  }

  function checkTextForSlideV57(titleText){
    const t = String(titleText || '').toLowerCase();
    if (t.includes('аванс')) return 'Проверка: какая часть денег уже заработана, какая часть остаётся обязательством, сколько услуг ещё нужно оказать.';
    if (t.includes('кредит')) return 'Проверка: где в балансе отражён долг, какие проценты будут расходом и когда тело кредита начнёт давить на денежный поток.';
    if (t.includes('дебитор')) return 'Проверка: когда клиент должен оплатить, сколько денег зависло в дебиторке и создаёт ли это кассовый разрыв.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Проверка: что стало активом, какой срок использования, как платёж связан с будущей амортизацией.';
    if (t.includes('расход') && t.includes('плат')) return 'Проверка: операция уменьшила прибыль, деньги или оба показателя одновременно.';
    if (t.includes('бухгалтер')) return 'Проверка: какой вопрос решает бухгалтерский контур, а какой вопрос должен решать управленческий учёт.';
    if (t.includes('экономический двигатель') || t.includes('бизнес-модели')) return 'Проверка: за счёт чего бизнес зарабатывает, где возникает маржа и что ограничивает рост.';
    if (t.includes('услуг')) return 'Проверка: часы специалистов, загрузка, выручка на час, ФОТ и повторные визиты.';
    if (t.includes('торгов')) return 'Проверка: валовая маржа, товарный остаток, скидки, списания и скорость оборачиваемости.';
    if (t.includes('производ')) return 'Проверка: сырьё, WIP, готовая продукция, брак, мощность и сроки оплаты клиентов.';
    return 'Проверка: какой финансовый смысл у операции, где она отражается и какое решение должен принять собственник.';
  }

  function patchLessonCheckBoxV57(){
    const cardNode = document.querySelector('.finance-slide-card-v56');
    if (!cardNode || cardNode.querySelector('.finance-check-v57')) return;
    const title = cardNode.querySelector('h1')?.textContent || '';
    const callouts = cardNode.querySelector('.finance-lesson-callouts-v56');
    const html = `<div class="finance-check-v57"><span>Проверка перед выводом</span><p>${financeEscV57(checkTextForSlideV57(title))}</p></div>`;
    if (callouts) callouts.insertAdjacentHTML('afterend', html);
  }

  const renderFinanceLessonBeforeV57 = window.renderFinanceLesson;
  window.renderFinanceLesson = function(lessonId, screenIndex){
    if (typeof renderFinanceLessonBeforeV57 === 'function') {
      const result = renderFinanceLessonBeforeV57.apply(this, arguments);
      setTimeout(function(){
        replaceTextNodesV57(document.querySelector('.content-v2'));
        patchCoverV57();
        patchCaseSituationsV57();
        patchLessonCheckBoxV57();
      }, 0);
      setTimeout(function(){
        replaceTextNodesV57(document.querySelector('.content-v2'));
        patchCoverV57();
  ...
  if (typeof shellBeforeV57 === 'function') {
    window.shell = function(content, activeTab){
      const result = shellBeforeV57.call(this, content, activeTab);
      setTimeout(function(){ replaceTextNodesV57(document.querySelector('.content-v2')); }, 0);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }
...
/* =====================================================
   v57 — Финансовый модуль: аудит текстов, логика открытия разделов,
   финтренажёр раздела 1 и чистые инструкции
   ===================================================== */
(function installFinanceContentAuditV57(){
  window.APP_UI_VERSION_V57 = 'v60-finance-route-final-20260628';

  window.FINANCE_SECTION1_TRAINER_URL_V57 = window.FINANCE_SECTION1_TRAINER_URL_V57 || '';

  window.copyFinanceTrainerLinkV57 = function(){
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    if (!url || url === '#') {
      alert('Ссылка на таблицу финтренажёра ещё не подключена. После загрузки шаблона нужно вставить URL в FINANCE_SECTION1_TRAINER_URL_V57.');
      return;
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url).then(function(){ alert('Ссылка на финтренажёр скопирована.'); }).catch(function(){ prompt('Скопируйте ссылку на финтренажёр:', url); });
    } else {
      prompt('Скопируйте ссылку на финтренажёр:', url);
    }
  };

  window.openFinanceTrainerLinkV57 = function(){
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    if (!url || url === '#') {
      alert('Ссылка на таблицу финтренажёра ещё не подключена.');
      return;
    }
  ...
  window.renderFinanceSection1Trainer = function(){
    if (typeof financeIsAdminModeV55 === 'function' && !financeIsAdminModeV55()) {
      if (typeof financeStudentLockedV55 === 'function') return financeStudentLockedV55();
      alert('Финансовый помощник пока закрыт для учеников.');
      return;
    }
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    const hasUrl = Boolean(url && url !== '#');
    const linkBlock = hasUrl
      ? `<div class="finance-trainer-link-box-v57"><span>Ссылка на таблицу</span><p>${esc(url)}</p></div><div class="grid-v2"><button class="btn primary" onclick="openFinanceTrainerLinkV57()">Открыть таблицу</button><button class="btn secondary" onclick="copyFinanceTrainerLinkV57()">Скопировать ссылку</button></div>`
      : `<div class="finance-trainer-link-box-v57 muted"><span>Ссылка на таблицу</span><p>Будет подключена после загрузки шаблона финтренажёра.</p></div><button class="btn secondary" onclick="copyFinanceTrainerLinkV57()">Проверить подключение ссылки</button>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-trainer-hero-v57', `<p class="eyebrow">раздел 1</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление темы после уроков и итоговой диагностики. Тренажёр нужен для отработки главной логики: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean finance-trainer-instruction-v57"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями по цветам, статусам и проверке.</p></div><div><b>4. Пройти задания</b><p>Последовательно разберите операции, один месяц через три отчёта и итоговый кейс. Цель — не угадать, а закрепить финансовую классификацию.</p></div></div>${linkBlock}<button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}
    `;
    shell(html, 'finance');
  };
})();
...
/* =====================================================
   v58 — Финансовый модуль: финальная чистка текстов и тренажёр с копируемой ссылкой
   ===================================================== */
(function installFinanceFinalTextAuditV58(){
  window.APP_UI_VERSION_V58 = 'v60-finance-route-final-20260628';
  window.FINANCE_SECTION1_TRAINER_URL_V58 = window.FINANCE_SECTION1_TRAINER_URL_V58 || '';

  function financeAdminV58(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; }
    catch(e){ return false; }
  }

  function cleanFinanceTextV58(value){
    let out = String(value || '');
    if (!out) return '';
    const rules = [
      [/предпринимателя/g, 'предпринимателя'],
      ...
      [/предпринимателем/g, 'предпринимателем'],
      [/Предпринимателем/g, 'Предпринимателем'],
      [/у предприниматель(?=\s|,|\.|;|:|!|\?)/gi, 'у предпринимателя'],
      [/для предприниматель(?=\s|,|\.|;|:|!|\?)/gi, 'для предпринимателя'],
      [/к предприниматель(?=\s|,|\.|;|:|!|\?)/gi, 'к предпринимателю'],
      [/Этот урок должен полностью убрать у предпринимателя привычку/gi, 'Этот урок убирает привычку предпринимателя'],
      [/урок должен полностью убрать у предпринимателя/gi, 'урок убирает у предпринимателя'],
      [/Финальный смысловой вывод без обращения к ученику\./g, 'Короткая фиксация главной логики урока.'],
      [/Финальный смысловой вывод по теме без обращения к ученику\./g, 'Короткая фиксация главной логики урока.']
    ];
    rules.forEach(function(pair){ out = out.replace(pair[0], pair[1]); });
    return out.replace(/\s+([,.!?;:])/g, '$1').trim();
  }

  function cleanFinanceDomV58(){
    document.querySelectorAll('.finance-slide-card-v56 p, .finance-slide-card-v56 h1, .finance-slide-card-v56 h2, .finance-slide-card-v56 h3, .finance-cover-card-v56 p, .finance-cover-card-v56 span, .finance-final-grid-v56 p, .finance-main-thought-v56 p, .finance-lesson-callouts-v56 p, .finance-impact-matrix-v56 p, .finance-hero-v49 p, .finance-section-card-v49 p, .finance-section-card-v49 em, .finance-status-pill span, .finance-status-pill b').forEach(function(node){
      const next = cleanFinanceTextV58(node.textContent);
      if (next && next !== node.textContent) node.textContent = next;
    });
  }

  function trainerUrlV58(){
    return String(window.FINANCE_SECTION1_TRAINER_URL_V58 || '').trim();
  }

  window.copyFinanceTrainerUrlV58 = function(){
    const url = trainerUrlV58();
    if (!url || url === '#') { alert('Ссылка на таблицу ещё не добавлена.'); return; }
    if (navigator.clipboard && navigator.clipboard.writeText) {
  ...
  window.renderFinanceTrainerSection1 = function(){
    if (!financeAdminV58()) {
      alert('Финтренажёр пока закрыт для учеников. Проверка доступна только в режиме администрирования.');
      if (typeof renderHome === 'function') renderHome();
      return;
    }
    const url = trainerUrlV58();
    const linkBlock = url && url !== '#'
      ? `<div class="finance-trainer-link-v58"><input value="${esc(url)}" readonly><button class="btn secondary" onclick="copyFinanceTrainerUrlV58()">Скопировать ссылку</button><a class="btn primary" target="_blank" href="${esc(url)}">Открыть таблицу</a></div>`
      : `<div class="finance-trainer-link-v58 empty"><p>Ссылка на таблицу ещё не подключена. После создания Google Sheets нужно вставить ссылку в переменную <b>FINANCE_SECTION1_TRAINER_URL_V58</b>.</p><button class="btn secondary" disabled>Ссылка будет добавлена</button></div>`;
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-trainer-hero-v57', `<p class="eyebrow">раздел 1 · закрепление</p><h1>Финтренажёр раздела 1</h1><p>Дополнительная практика после первых четырёх уроков. Тренажёр закрепляет финансовый смысл операций: деньги, выручка, прибыль, аванс, кредит, дебиторка, актив, обязательство, ОПиУ, ОДДС и баланс.</p>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="finance-trainer-steps-v57"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на таблицу-тренажёр после её подключения.</p></div><div><b>2. Создать копию</b><p>В Google Sheets выберите «Файл → Создать копию», чтобы работать в собственной версии.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и подсказками.</p></div><div><b>4. Пройти задания</b><p>Разберите операции и их влияние на ОПиУ, ОДДС и баланс. Это закрепление, а не замена итоговой диагностики.</p></div></div>${linkBlock}<button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам модуля</button>`)}
    `;
    shell(html, 'finance');
  };
  window.renderFinanceTrainerSection1V58 = window.renderFinanceTrainerSection1;

  const shellBeforeV58 = window.shell;
  if (typeof shellBeforeV58 === 'function' && !window.__shellWrappedFinanceV58) {
    window.__shellWrappedFinanceV58 = true;
    window.shell = function(content, activeTab){
      const result = shellBeforeV58.call(this, content, activeTab);
      setTimeout(cleanFinanceDomV58, 0);
      setTimeout(cleanFinanceDomV58, 180);
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(cleanFinanceDomV58, 300);
  });
})();

/* =====================================================
   v59 — Финансовый модуль: прямой переход к урокам раздела
   Исправляет ситуацию, когда на экране модуля есть описание,
   но пользователь не видит явного перехода к урокам.
   ===================================================== */
(function installFinanceModuleDirectOpenV59(){
  window.APP_UI_VERSION_V59 = 'v60-finance-route-final-20260628';

  function financeAdminModeV59(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; }
    catch(e){ return false; }
  }

  function financeEscV59(value){
    if (typeof esc === 'function') return esc(value);
  ...
  function financeSectionsV59(){
    const fromMain = Array.isArray(window.FINANCE_MODULE_SECTIONS_DATA_V57) ? window.FINANCE_MODULE_SECTIONS_DATA_V57 : [];
  ...
  function financeSectionByIdV59(sectionId){
    return financeSectionsV59().find(function(section){ return Number(section.id) === Number(sectionId); }) || null;
  }

  function financeLoadProgressV59(){
    try {
      if (typeof window.financeLoadProgressV57 === 'function') return window.financeLoadProgressV57() || {};
    } catch(e) {}
    try {
  ...
  function financeSectionOnePassedV59(){
    const p = financeLoadProgressV59();
    const total = Number(p.section1TestTotal || window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
    const pass = Number(window.FINANCE_SECTION1_PASS_SCORE_V57 || Math.ceil(total * 0.8));
    const score = Number(p.section1TestScore || 0);
  ...
  function financeCanOpenSectionV59(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return true;
    // В режиме администрирования владелец может просматривать структуру всех разделов.
    if (financeAdminModeV59()) return true;
    // Для будущего ученического доступа: каждый раздел открывается после теста предыдущего.
    if (id === 2) return financeSectionOnePassedV59();
    return false;
  }

  function financeRequireAdminV59(){
    if (financeAdminModeV59()) return true;
    alert('Финансовый помощник пока закрыт для учеников. Просмотр доступен только в режиме администрирования.');
    if (typeof renderHome === 'function') renderHome();
    return false;
  }

  function financeProgressBoxV59(){
    const p = financeLoadProgressV59();
    const total = Number(p.section1TestTotal || window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
    const pass = Number(window.FINANCE_SECTION1_PASS_SCORE_V57 || Math.ceil(total * 0.8));
    const score = Math.max(0, Math.min(total, Number(p.section1TestScore || 0)));
    const pct = total ? Math.round(score / total * 100) : 0;
    const passed = financeSectionOnePassedV59();
    return `<div class="finance-progress-v59 ${passed ? 'passed' : 'locked'}">
      <div class="finance-progress-head-v59"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : `${score}/${total} · ${pct}%`}</b></div>
      <div class="progress-line"><div style="width:${Math.max(0, Math.min(100, pct))}%"></div></div>
      <p>${passed ? 'Раздел 2 открыт. Дальше логика такая же: следующий раздел открывается после теста предыдущего.' : `Для открытия раздела 2 нужно пройти диагностику минимум на 80%: ${pass}/${total} правильных ответов.`}</p>
    </div>`;
  }

  function financeSectionTileV59(section){
    const id = Number(section.id || 0);
    const lessonsCount = Array.isArray(section.lessons) ? section.lessons.length : Number(section.lessons || 0);
    const extra = id === 1 ? 2 : 0; // диагностика + финтренажёр
    const countText = id === 1 ? `${lessonsCount + extra} блоков` : `${lessonsCount || 'структура'} уроков`;
    const open = financeCanOpenSectionV59(id);
    const status = id === 1 ? 'доступно' : (open ? 'доступно в администрировании' : `после теста раздела ${id - 1}`);
    const action = open ? `renderFinanceSection(${id})` : `financeLockedSectionNotice(${id})`;
    return `<section class="finance-section-tile-v59 ${open ? 'open' : 'locked'}">
      <div class="finance-section-tile-head-v59"><span>Раздел ${id}</span><em>${financeEscV59(status)}</em></div>
      <h3>${financeEscV59(section.title || '')}</h3>
      <p>${financeEscV59(section.description || '')}</p>
      <div class="finance-section-tile-bottom-v59"><b>${financeEscV59(countText)}</b><button class="btn ${open ? 'primary' : 'secondary'}" onclick="${action}">${open ? 'Открыть уроки' : 'Закрыто'}</button></div>
    </section>`;
  }

  function financeModuleMapV59(){
    return `<div class="finance-module-map-v57 finance-module-map-v59">
      <div><b>01</b><p>База: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс и управленческий смысл операции.</p></div>
      <div><b>02–04</b><p>Выручка, unit-экономика, прибыльность, безубыточность и операционный рычаг.</p></div>
      <div><b>05–07</b><p>Деньги, ликвидность, баланс, остатки, долг, налоги, ФОТ и собственник.</p></div>
      <div><b>08–11</b><p>Планирование, метрики, отраслевые финансы и регулярная система управления.</p></div>
    </div>`;
  }

  function renderFinanceModuleHomeV59(){
    if (!financeRequireAdminV59()) return;
    const sections = financeSectionsV59();
    const first = financeSectionByIdV59(1) || sections[0] || {};
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57 finance-hero-v59', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Финансовая архитектура показывает, как бизнес зарабатывает, как движутся деньги, где появляются остатки, почему нужен баланс и как метрики превращаются в управленческие решения.</p><p class="small">Разделы идут последовательно. Каждый следующий раздел открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>${financeProgressBoxV59()}`)}
      ${card('finance-quick-open-v59', `<h2>Быстрый переход</h2><p>Первый раздел доступен сразу. Нажмите кнопку ниже, чтобы перейти к урокам раздела, итоговой диагностике и финтренажёру.</p><button class="btn primary" onclick="renderFinanceSection(1)">Открыть уроки раздела 1</button>`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>Карта финансового модуля</h2>${financeModuleMapV59()}<h2>11 разделов</h2><div class="finance-section-list-v59">${sections.map(financeSectionTileV59).join('')}</div>`)}
    `;
    shell(html, 'finance');
  }

  function financeLessonRowV59(lesson, index){
    const id = Number(lesson.id || index + 1);
    const full = Array.isArray(lesson.fullContent) && lesson.fullContent.length;
    const label = full ? 'полный текст и учебные экраны' : 'структура урока из общей карты модуля';
    return `<button class="lesson-row-v2 finance-lesson-row-v49 finance-lesson-row-v59" onclick="renderFinanceLesson(${id},0)"><div><b>${String(id).padStart(2,'0')}. ${financeEscV59(lesson.title || 'Урок')}</b><p>${financeEscV59(label)}</p></div><span>→</span></button>`;
  }

  function financeSectionOneExtrasV59(){
    const p = financeLoadProgressV59();
    const score = Number(p.section1TestScore || 0);
    const total = Number(p.section1TestTotal || window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
    const passed = financeSectionOnePassedV59();
    const testLabel = passed ? `пройдено · ${score}/${total}` : `30 вопросов · проходной уровень 80%`;
    return `
      <button class="lesson-row-v2 finance-lesson-row-v49 finance-test-row-v49 finance-test-row-v59" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>${financeEscV59(testLabel)} · открывает раздел 2</p></div><span>→</span></button>
      <button class="lesson-row-v2 finance-lesson-row-v49 finance-trainer-row-v57 finance-trainer-row-v59" onclick="renderFinanceTrainerSection1()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
  }

  function renderFinanceSectionV59(sectionId){
    if (!financeRequireAdminV59()) return;
    const id = Number(sectionId || 1);
    const section = financeSectionByIdV59(id);
    if (!section) return renderFinanceModuleHomeV59();
    if (!financeCanOpenSectionV59(id)) return financeLockedSectionNoticeV59(id);
    const lessons = Array.isArray(section.lessons) ? section.lessons : [];
    const rows = lessons.length
      ? lessons.map(financeLessonRowV59).join('')
      : `<div class="finance-empty-v59"><b>Материалы раздела будут раскрыты позже</b><p>Сейчас в общей структуре есть назначение раздела и список тем. Подробные уроки будут добавляться после утверждения контента.</p></div>`;
    const extras = id === 1 ? financeSectionOneExtrasV59() : '';
    const html = `
      ${card('blue-card-v2 finance-hero-v49 finance-hero-v57 finance-section-hero-v59', `<p class="eyebrow">раздел ${id}</p><h1>${financeEscV59(section.title || '')}</h1><p>${financeEscV59(section.description || '')}</p>${id === 1 ? financeProgressBoxV59() : `<p class="small">Административный предпросмотр. Для ученика раздел откроется после прохождения диагностики предыдущего раздела.</p>`}`)}
      ${card('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К разделам</button><button class="btn secondary" onclick="renderFinancialAssistant()">К помощнику</button></div><h2>${id === 1 ? 'Уроки и закрепление раздела' : 'Уроки раздела'}</h2><p>${id === 1 ? 'Порядок: пройти 4 урока, затем итоговую диагностику. Финтренажёр нужен для дополнительного закрепления материала.' : 'Раздел открыт в режиме администрирования для проверки структуры.'}</p><div class="lesson-list-v2">${rows}${extras}</div>`)}
    `;
    shell(html, 'finance');
  }

  function financeLockedSectionNoticeV59(sectionId){
    const id = Number(sectionId || 2);
    const prev = Math.max(1, id - 1);
    alert('Раздел ' + id + ' откроется после прохождения итоговой диагностики раздела ' + prev + ' минимум на 80%.');
  }

  // Жёсткая финальная привязка: последние функции побеждают старые v49–v58-обёртки.
  window.renderFinanceModuleHome = renderFinanceModuleHomeV59;
  window.renderFinanceSection = renderFinanceSectionV59;
  window.financeLockedSectionNotice = financeLockedSectionNoticeV59;
  window.financeCanOpenSectionV59 = financeCanOpenSectionV59;
...
/* =====================================================
   v60 — Финансовый помощник: финальный маршрут Финансовый модуль → Раздел → Уроки
   Исправляет: раздел 1 теперь всегда открывает список уроков, диагностику и финтренажёр.
   ===================================================== */
(function installFinanceRouteV60(){
  window.APP_UI_VERSION_V60 = 'v60-finance-route-section-lessons-20260628';

  const PASS_SCORE_V60 = 24;
  const TOTAL_SCORE_V60 = 30;
  const FINANCE_SECTIONS_V60 = [
    { id:1, title:'Финансовое мышление собственника', description:'Формирует базовую логику: предприниматель перестаёт смотреть только на кассу и начинает видеть бизнес как систему финансовых потоков, остатков, обязательств и решений.', lessons:[
      { id:1, title:'Деньги, прибыль и выручка: почему предприниматель ошибается' },
      { id:2, title:'Финансовая карта бизнеса' },
      { id:3, title:'Управленческий учёт против бухгалтерии' },
      { id:4, title:'Экономика бизнес-модели' }
    ]},
    { id:2, title:'Выручка, продажи и unit-экономика', description:'Показывает, как возникает выручка, из каких драйверов она состоит и почему продажа сама по себе ещё не гарантирует прибыльность.', count:6 },
    { id:3, title:'ОПиУ и прибыльность', description:'Разбирает прибыльность бизнеса: себестоимость, маржу, расходы, EBITDA, чистую прибыль и ошибки, которые искажают финансовый результат.', count:7 },
    { id:4, title:'Безубыточность, масштабирование и операционный рычаг', description:'Объясняет минимальный объём продаж для выживания, запас прочности и эффект постоянных расходов при росте или падении бизнеса.', count:3 },
    { id:5, title:'ОДДС, деньги и ликвидность', description:'Показывает, почему прибыль не равна деньгам, где возникают кассовые разрывы и как управлять платёжеспособностью.', count:6 },
    { id:6, title:'Баланс и остатки', description:'Раскрывает активы, обязательства, капитал, авансы, основные средства и мосты остатков как основу настоящей управленческой картины.', count:7 },
    { id:7, title:'Долг, налоги, ФОТ и собственник', description:'Разбирает кредиты, проценты, налоги, команду и личные деньги собственника — блоки, которые часто искажают прибыль и деньги.', count:4 },
    { id:8, title:'Планирование, прогнозирование и инвестиции', description:'Учит смотреть вперёд: план-факт, драйверное бюджетирование, сценарии, стресс-тесты, инвестиции и источники финансирования роста.', count:5 },
    { id:9, title:'Метрики, диагностика и дашборд', description:'Даёт систему показателей, диагностику бизнеса и логику экрана собственника, чтобы цифры превращались в решения.', count:6 },
    { id:10, title:'Отраслевые финансы', description:'Показывает, что финансы услуг, торговли, производства, проектов, логистики и HoReCa устроены по-разному.', count:6 },
    { id:11, title:'Финансовая система и управление', description:'Собирает финансовую архитектуру в регулярную систему управления: политика учёта, статьи, закрытие месяца, роли, контроль и стадии бизнеса.', count:8 }
  ];

  function feV60(value){
    if (typeof esc === 'function') return esc(value);
    return String(value ?? '').replace(/[&<>'"]/g, function(c){ return ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'})[c]; });
  }
  function financeAdminModeV60(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; } catch(e){ return false; }
  }
  function requireFinanceAdminV60(){
    if (financeAdminModeV60()) return true;
    alert('Финансовый помощник пока закрыт для учеников. Проверка материалов доступна только в режиме администрирования.');
    if (typeof renderHome === 'function') renderHome();
    return false;
  }
  function userSuffixV60(){
    try {
      const ids = typeof possibleIds === 'function' ? possibleIds() : [];
  ...
  function progressKeyV60(){ return 'architecture_finance_module_progress_v49_' + userSuffixV60(); }
  function loadProgressV60(){ try { return JSON.parse(localStorage.getItem(progressKeyV60()) || '{}') || {}; } catch(e){ return {}; } }
  function section1PassedV60(){
    const p = loadProgressV60();
    const total = Number(p.section1TestTotal || TOTAL_SCORE_V60);
    const pass = Number(p.section1TestPassScore || Math.ceil(total * 0.8) || PASS_SCORE_V60);
    const score = Number(p.section1TestScore || 0);
    return Boolean(p.section1TestPassed) || score >= pass;
  }
  function canOpenSectionV60(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return true;
    if (financeAdminModeV60()) return true;
    if (id === 2) return section1PassedV60();
    const p = loadProgressV60();
    return Boolean(p['section' + (id - 1) + 'TestPassed']);
  }
  function sectionByIdV60(sectionId){ return FINANCE_SECTIONS_V60.find(function(s){ return Number(s.id) === Number(sectionId); }) || null; }

  function progressBoxV60(){
    const p = loadProgressV60();
    const total = Number(p.section1TestTotal || TOTAL_SCORE_V60);
    const pass = Number(p.section1TestPassScore || Math.ceil(total * 0.8) || PASS_SCORE_V60);
    const score = Math.max(0, Math.min(total, Number(p.section1TestScore || 0)));
    const percent = total ? Math.round(score / total * 100) : 0;
    const passed = Boolean(p.section1TestPassed) || score >= pass;
    return `<div class="finance-progress-v60 ${passed ? 'passed' : 'locked'}"><div class="finance-progress-head-v60"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : score + '/' + total + ' · ' + percent + '%'}</b></div><div class="progress-line"><div style="width:${Math.max(0, Math.min(100, percent))}%"></div></div><p>${passed ? 'Раздел 2 открыт. Следующие разделы открываются по такой же логике: после диагностики предыдущего раздела.' : 'Для открытия раздела 2 нужно набрать минимум ' + pass + '/' + total + ' правильных ответов.'}</p></div>`;
  }

  function renderFinancialAssistantV60(){
    if (!requireFinanceAdminV60()) return;
    const html = `${card('blue-card-v2 finance-hero-v49 finance-hub-hero-v60', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Это рабочая зона финансового блока: обучение, будущая аналитика бизнеса и готовые инструменты. Сейчас раздел открыт только в режиме администрирования для проверки наполнения.</p>`)}${card('', `<h2>Что открыть</h2><div class="finance-hub-grid-v60"><button class="finance-hub-card-v60 active" onclick="renderFinanceModuleHome()"><b>Финансовый модуль</b><p>Учебная архитектура из 11 разделов: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс, метрики и управление.</p><em>открыть</em></button><button class="finance-hub-card-v60" onclick="renderFinanceAnalytics()"><b>Моя аналитика</b><p>Будущий личный контур для фактов бизнеса, показателей, рисков и управленческих выводов.</p><em>каркас</em></button><button class="finance-hub-card-v60" onclick="renderFinanceTemplates()"><b>Готовые шаблоны</b><p>Место для таблиц, тренажёров и финансовых инструментов, которые будут подключаться к разделам.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← На главную</button>`)}`;
    shell(html, 'finance');
  }

  function moduleMapV60(){
    return `<div class="finance-module-map-v60"><div><b>01</b><p>Финансовое мышление: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс.</p></div><div><b>02–04</b><p>Выручка, unit-экономика, прибыльность, безубыточность и масштабирование.</p></div><div><b>05–07</b><p>Деньги, ликвидность, баланс, остатки, долг, налоги и ФОТ.</p></div><div><b>08–11</b><p>Планирование, метрики, отраслевые финансы и регулярная система управления.</p></div></div>`;
  }
  function learnerGateTextV60(sectionId){
    const id = Number(sectionId || 0);
    if (id <= 1) return 'Раздел доступен сразу.';
    return 'Открывается после итоговой диагностики раздела ' + (id - 1) + ' минимум на 80%.';
  }
  function sectionCardV60(section){
    const id = Number(section.id || 0);
    const open = canOpenSectionV60(id);
    const blocks = id === 1 ? 6 : ((Number(section.count || 0) || 0) + 1);
    const status = id === 1 ? 'доступно' : (open ? 'предпросмотр администратора' : 'закрыто');
    const action = open ? `renderFinanceSection(${id})` : `financeLockedSectionNotice(${id})`;
    return `<section class="finance-section-card-v60 ${open ? 'open' : 'locked'}"><div class="finance-section-card-top-v60"><span>Раздел ${id}</span><em>${feV60(status)}</em></div><h3>${feV60(section.title)}</h3><p>${feV60(section.description)}</p><small>${feV60(learnerGateTextV60(id))}</small><div class="finance-section-card-bottom-v60"><b>${blocks} блоков</b><button class="btn ${open ? 'primary' : 'secondary'}" onclick="${action}">${open ? 'Открыть уроки' : 'Закрыто'}</button></div></section>`;
  }

  function renderFinanceModuleHomeV60(){
    if (!requireFinanceAdminV60()) return;
    const html = `${card('blue-card-v2 finance-hero-v49 finance-module-hero-v60', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Финансовый модуль показывает, как бизнес зарабатывает, как движутся деньги, как появляются остатки, почему нужен баланс и как метрики превращаются в управленческие решения.</p><p class="small">Логика открытия находится внутри финансового модуля: каждый следующий раздел открывается после итоговой диагностики предыдущего раздела.</p>${progressBoxV60()}`)}${card('finance-quick-open-v60', `<h2>Быстрый переход</h2><p>Первый раздел доступен сразу. Здесь находятся четыре урока, итоговая диагностика и финтренажёр.</p><button class="btn primary" onclick="renderFinanceSection(1)">Открыть уроки раздела 1</button>`)}${card('', `<div class="finance-toolbar-v60"><button class="btn secondary" onclick="renderFinancialAssistant()">← К финансовому помощнику</button></div><h2>Карта финансового модуля</h2>${moduleMapV60()}<h2>11 разделов</h2><p>Нажмите кнопку «Открыть уроки» внутри нужного раздела.</p><div class="finance-section-grid-v60">${FINANCE_SECTIONS_V60.map(sectionCardV60).join('')}</div>`)}`;
    shell(html, 'finance');
  }

  function lessonRowV60(lesson){
    const id = Number(lesson.id || 0);
    return `<button class="lesson-row-v2 finance-lesson-row-v60" onclick="renderFinanceLesson(${id},0)"><div><b>${String(id).padStart(2,'0')}. ${feV60(lesson.title)}</b><p>полный учебный экран</p></div><span>→</span></button>`;
  }
  function sectionOneExtrasV60(){
    const p = loadProgressV60();
    const total = Number(p.section1TestTotal || TOTAL_SCORE_V60);
    const score = Number(p.section1TestScore || 0);
    const testText = section1PassedV60() ? 'пройдено · ' + score + '/' + total : '30 вопросов · проходной уровень 80%';
    return `<button class="lesson-row-v2 finance-lesson-row-v60 finance-test-row-v60" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>${feV60(testText)} · открывает раздел 2</p></div><span>→</span></button><button class="lesson-row-v2 finance-lesson-row-v60 finance-trainer-row-v60" onclick="renderFinanceTrainerSection1()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операция → финансовый смысл → ОПиУ → ОДДС → баланс.</p></div><span>→</span></button>`;
  }
  function renderFinanceSectionV60(sectionId){
    if (!requireFinanceAdminV60()) return;
    const id = Number(sectionId || 1);
    const section = sectionByIdV60(id);
    if (!section) return renderFinanceModuleHomeV60();
    if (!canOpenSectionV60(id)) return financeLockedSectionNoticeV60(id);
    const isFirst = id === 1;
    const rows = isFirst
      ? (section.lessons || []).map(lessonRowV60).join('') + sectionOneExtrasV60()
      : `<div class="finance-empty-v60"><b>Раздел открыт для административного предпросмотра</b><p>Полные уроки этого раздела будут добавляться после утверждения содержания. Сейчас зафиксирована тема раздела и место в общей финансовой архитектуре.</p></div><button class="lesson-row-v2 finance-lesson-row-v60 locked" type="button" disabled><div><b>Итоговая диагностика раздела ${id}</b><p>будет добавлена после наполнения уроков раздела</p></div><span>•</span></button>`;
    const html = `${card('blue-card-v2 finance-hero-v49 finance-section-hero-v60', `<p class="eyebrow">раздел ${id}</p><h1>${feV60(section.title)}</h1><p>${feV60(section.description)}</p>${isFirst ? progressBoxV60() : '<p class="small">Административный предпросмотр. В учебной логике раздел откроется после диагностики предыдущего раздела.</p>'}`)}${card('', `<div class="finance-toolbar-v60"><button class="btn secondary" onclick="renderFinanceModuleHome()">← К карте модуля</button><button class="btn secondary" onclick="renderFinancialAssistant()">К помощнику</button></div><h2>${isFirst ? 'Уроки и закрепление раздела' : 'Материалы раздела'}</h2><p>${isFirst ? 'Маршрут раздела: 4 урока → итоговая диагностика → финтренажёр.' : 'Этот раздел пока работает как структура для будущего наполнения.'}</p><div class="lesson-list-v2 finance-lesson-list-v60">${rows}</div>`)}`;
    shell(html, 'finance');
  }

  function renderFinanceTrainerSection1V60(){
    if (!requireFinanceAdminV60()) return;
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V60 || window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    const linkBlock = url && url !== '#'
      ? `<div class="finance-trainer-link-box-v57"><span>Ссылка на таблицу</span><p>${feV60(url)}</p></div><div class="grid-v2"><a class="btn primary" href="${feV60(url)}" target="_blank">Открыть таблицу</a><button class="btn secondary" onclick="navigator.clipboard && navigator.clipboard.writeText ? navigator.clipboard.writeText('${String(url).replace(/'/g,"\\'")}').then(()=>alert('Ссылка скопирована.')) : prompt('Скопируйте ссылку:', '${String(url).replace(/'/g,"\\'")}')">Скопировать ссылку</button></div>`
      : `<div class="finance-trainer-link-box-v57 muted"><span>Ссылка на таблицу</span><p>Ссылка будет подключена после создания финтренажёра.</p></div><button class="btn secondary" disabled>Ссылка будет добавлена</button>`;
    const html = `${card('blue-card-v2 finance-hero-v49 finance-trainer-hero-v60', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление темы после уроков и итоговой диагностики. Тренажёр нужен для отработки логики: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}${card('', `<div class="finance-toolbar-v60"><button class="btn secondary" onclick="renderFinanceSection(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean finance-trainer-instruction-v57"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями.</p></div><div><b>4. Пройти задания</b><p>Классифицируйте операции и проверьте влияние на ОПиУ, ОДДС и баланс.</p></div></div>${linkBlock}<button class="btn secondary" onclick="renderFinanceModuleHome()">← К финансовому модулю</button>`)}`;
    shell(html, 'finance');
  }

  window.renderFinancialAssistant = renderFinancialAssistantV60;
  window.renderMyBusiness = renderFinancialAssistantV60;
  window.openFinanceAssistantV60 = renderFinancialAssistantV60;
  window.openFinanceAssistantV57 = renderFinancialAssistantV60;
  window.openFinanceAssistantV55 = renderFinancialAssistantV60;
  window.openFinanceAssistantV54 = renderFinancialAssistantV60;
  window.openFinanceAssistantV52 = renderFinancialAssistantV60;
  window.openFinanceAssistantV50 = renderFinancialAssistantV60;
  window.renderFinanceModuleHome = renderFinanceModuleHomeV60;
  window.renderFinanceSection = renderFinanceSectionV60;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1V60;
  window.renderFinanceSection1Trainer = renderFinanceTrainerSection1V60;
  window.financeLockedSectionNotice = financeLockedSectionNoticeV60;
  window.financeCanOpenSectionV60 = canOpenSectionV60;
  try { renderFinancialAssistant = window.renderFinancialAssistant; } catch(e) {}
  try { renderMyBusiness = window.renderMyBusiness; } catch(e) {}
  try { openFinanceAssistantV55 = window.openFinanceAssistantV55; } catch(e) {}
  try { renderFinanceModuleHome = window.renderFinanceModuleHome; } catch(e) {}
  try { renderFinanceSection = window.renderFinanceSection; } catch(e) {}
  try { renderFinanceTrainerSection1 = window.renderFinanceTrainerSection1; } catch(e) {}
  try { renderFinanceSection1Trainer = window.renderFinanceSection1Trainer; } catch(e) {}
  try { financeLockedSectionNotice = window.financeLockedSectionNotice; } catch(e) {}
})();
...
/* =====================================================
   v62 — FINAL: Финансовый помощник без лишней логики на входе,
   Финансовый модуль с прямым переходом в уроки раздела 1.
   ===================================================== */
(function installFinanceRouteHardFinalV62(){
  window.APP_UI_VERSION_V62 = 'v63-finance-module-clean-route-20260628';
  const SECTIONS = [{"id":1,"title":"Финансовое мышление собственника","description":"Формирует базовую логику: предприниматель перестаёт смотреть только на кассу и начинает видеть бизнес как систему финансовых потоков, остатков, обязательств и решений.","lessons":[{"id":1,"title":"Деньги, прибыль и выручка: почему предприниматель ошибается","objective":"Разрушить главный финансовый миф: деньги на счёте не равны прибыли, а поступление не всегда является выручкой.","content":"Деньги, выручка, поступления, расходы, платежи, активы, кредиты, авансы клиентов. Разница между фактом денег и фактом заработка.","case":"На счёт пришла одна сумма. В кейсе сравнивается, как меняется влияние на ОПиУ, ОДДС и баланс, если это выручка, аванс клиента, кредит, возврат дебиторки или вклад собственника.","result":"Предприниматель перестаёт оценивать бизнес только по остатку денег и разделяет три финансовые реальности: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.","status":"ready"},{"id":2,"title":"Финансовая карта бизнеса","objective":"Показать бизнес как систему: операции превращаются в выручку, деньги превращаются в денежный поток, остатки формируют баланс, метрики ведут к решениям.","content":"Операционный контур, денежный контур, балансовый контур, контур метрик и управленческих решений.","case":"Один и тот же месяц бизнеса разбирается через три экрана: ОПиУ, ОДДС и баланс. Видно, почему каждый экран отвечает на свой вопрос.","result":"Финансы бизнеса нельзя свести к одной таблице кассы: одна операция должна читаться через ОПиУ, ОДДС, баланс, метрики и управленческое решение.","status":"ready"},{"id":3,"title":"Управленческий учёт против бухгалтерии","objective":"Отделить управленческую финансовую систему собственника от бухгалтерского учёта для государства.","content":"Бухгалтерский учёт, налоговый учёт, управленческий учёт, управленческая отчётность, внутренние правила, логическая честность данных.","case":"Бухгалтер показывает прибыль по правилам учёта, а собственник видит кассовый риск. Разбирается, почему оба взгляда могут быть правильными, но решают разные задачи.","result":"Бухгалтерия, налоговый учёт и управленческий учёт решают разные задачи. Управленческая система нужна, чтобы видеть прибыль, деньги, остатки, обязательства, риски и решения собственника.","status":"ready"},{"id":4,"title":"Экономика бизнес-модели","objective":"Научить видеть, за счёт чего конкретный бизнес зарабатывает и где у него возникает финансовое ограничение.","content":"Драйверы выручки, маржи, загрузки, оборачиваемости, мощности, проектных этапов и повторных продаж. Отличия услуг, торговли, производства, проектов, логистики и HoReCa.","case":"Сравниваются салон услуг, магазин и производство при одинаковой выручке. Разбирается, почему финансовая логика, ограничения и решения у них разные.","result":"Бизнес начинает читаться через экономический двигатель: выручку, маржу, мощность, оборачиваемость, повторяемость и главное финансовое ограничение модели.","status":"ready"}]},{"id":2,"title":"Выручка, продажи и unit-экономика","description":"Показывает, как возникает выручка, из каких драйверов она состоит и почему продажа сама по себе ещё не гарантирует прибыльность.","lessons":[{"id":5,"title":"Выручка: когда бизнес действительно заработал","objective":"Объяснить момент признания выручки простым предпринимательским языком.","content":"Оказанная услуга, отгруженный товар, выполненный этап проекта, подписка, абонемент, сертификат, предоплата, рассрочка.","case":"Клиент оплатил годовой абонемент. Определяется, какая часть является деньгами, какая обязательством, а какая выручкой текущего месяца.","result":"Предприниматель не смешивает деньги и заработанную выручку.","status":"summary"},{"id":6,"title":"Unit-экономика","objective":"Показать экономику одной единицы бизнеса: клиента, визита, заказа, SKU, часа, проекта, рейса или посадки.","content":"CAC, LTV, средний чек, маржа на единицу, contribution margin, payback CAC, частота покупок, повторные продажи.","case":"Бизнес привлекает клиента за 1 500 ₽. Предприниматель считает, окупается ли клиент при одном визите, трёх визитах и годовом цикле.","result":"Формируется понимание, где бизнес зарабатывает на уровне единицы, а где теряет деньги.","status":"summary"},{"id":7,"title":"Средний чек, конверсия и объём","objective":"Разложить выручку на управляемые драйверы, а не смотреть на неё как на одно число.","content":"Поток, лиды, заявки, трафик, конверсия, средний чек, повторяемость, объём продаж, частота покупок.","case":"Выручка упала на 15%. Определяется, что именно просело: поток, конверсия, чек или повторные продажи.","result":"Формируется переход управлять причинами выручки, а не только итоговой суммой.","status":"summary"},{"id":8,"title":"Ценообразование","objective":"Показать цену как главный финансовый рычаг, а не просто маркетинговое решение.","content":"Цена от себестоимости, цена от ценности, цена от рынка, цена от загрузки, минимальная цена, ценовые пакеты, абонементы, динамическая цена.","case":"Бизнес снижает цену на 15% ради спроса. Предприниматель считает, насколько должен вырасти объём, чтобы прибыль не упала.","result":"Формируется понимание финансовые последствия изменения цены.","status":"summary"},{"id":9,"title":"Скидки, акции и промо","objective":"Показать, что скидка бьёт по марже сильнее, чем кажется предпринимателю.","content":"Скидка, промо, акция, купон, бесплатная услуга, бонус, компенсация скидки объёмом, промо-экономика.","case":"Акция увеличила продажи на 30%, но прибыль снизилась. Предприниматель объясняет, почему рост выручки может разрушить маржу.","result":"Предприниматель считает акции не по обороту, а по дополнительной прибыли.","status":"summary"},{"id":10,"title":"Продуктовая линейка, ассортимент и портфель услуг","objective":"Научить видеть продукты и услуги по ролям: привлечение, маржа, повторные продажи, касса, обязательства, нагрузка.","content":"Продуктовый портфель, SKU, категории, услуги, пакеты, абонементы, маржинальность, оборачиваемость, лид-магниты, флагманские продукты.","case":"В линейке есть популярная услуга с низкой маржой и редкая услуга с высокой маржой. Предприниматель решает, что масштабировать и что менять.","result":"Предприниматель управляет не только продажами, но и составом продуктовой модели.","status":"summary"}]},{"id":3,"title":"ОПиУ и прибыльность","description":"Разбирает прибыльность бизнеса: себестоимость, маржу, расходы, EBITDA, чистую прибыль и ошибки, которые искажают финансовый результат.","lessons":[{"id":11,"title":"Себестоимость","objective":"Объяснить, какие затраты напрямую связаны с созданием услуги, продажей товара, выпуском продукта или выполнением проекта.","content":"Прямые материалы, прямой труд, расходники, закупочная стоимость, доставка, производственные накладные, субподряд, потери и списания.","case":"Одинаковая выручка в услугах, торговле и производстве даёт разную валовую прибыль. Определяется, какие затраты относятся в себестоимость.","result":"Формируется понимание основу валовой прибыли и не смешивает прямые и общие расходы.","status":"summary"},{"id":12,"title":"Постоянные, переменные и ступенчатые расходы","objective":"Показать поведение расходов при изменении объёма бизнеса.","content":"Переменные, постоянные, полупеременные, ступенчатые, разовые, расходы роста и расходы поддержания.","case":"Бизнес открывает вторую точку: аренда и администратор появляются скачком. Определяется, почему прибыль временно проседает.","result":"Формируется понимание, какие расходы растут вместе с объёмом, а какие создают новый уровень обязательств.","status":"summary"},{"id":13,"title":"Валовая прибыль и валовая маржа","objective":"Научить читать качество основной бизнес-модели через валовую прибыль.","content":"Валовая прибыль, валовая маржа, прямые расходы, маржа по продуктам, маржа по категориям, маржа по направлениям.","case":"Выручка выросла, но валовая маржа упала. Предприниматель ищет причины: скидки, рост себестоимости, слабая линейка или изменение структуры продаж.","result":"Формируется видение, приносит ли основная деятельность достаточный запас для покрытия остальных расходов.","status":"summary"},{"id":14,"title":"Contribution margin","objective":"Показать вклад каждой продажи в покрытие постоянных расходов и прибыль.","content":"Contribution margin, переменные расходы, вклад на покрытие, маржинальный доход, связь с безубыточностью и рекламой.","case":"Маркетинговая кампания даёт продажи, но с низким contribution margin. Определяется, масштабировать её или остановить.","result":"Предприниматель оценивает продажи по их реальному вкладу, а не только по обороту.","status":"summary"},{"id":15,"title":"Операционные расходы","objective":"Разделить расходы по функциям, чтобы собственник видел, что именно съедает прибыль.","content":"Маркетинговые, коммерческие, административные, управленческие, производственные и прочие операционные расходы.","case":"Расходы выросли на 25%. Предприниматель раскладывает рост по функциям и определяет, где причина ухудшения EBITDA.","result":"Предприниматель перестаёт складывать всё в одну категорию и начинает видеть структуру расходов.","status":"summary"},{"id":16,"title":"EBITDA, EBIT и чистая прибыль","objective":"Объяснить уровни прибыли и управленческий смысл каждого уровня.","content":"EBITDA, амортизация, EBIT, проценты, налоги, чистая прибыль, операционная сила бизнеса.","case":"Два бизнеса имеют одинаковую чистую прибыль, но разную EBITDA и долговую нагрузку. Предприниматель сравнивает качество моделей.","result":"Формируется понимание, почему один показатель прибыли не может описать весь бизнес.","status":"summary"},{"id":17,"title":"Почему прибыль может быть иллюзией","objective":"Показать ошибки, из-за которых предприниматель получает красивую, но ложную прибыль.","content":"Авансы как выручка, кредит как доход, тело долга как расход, capex как расход месяца, отсутствие амортизации, налогов, начислений и закрытия месяца.","case":"Бизнес показывает прибыль, но баланс и деньги не сходятся. Предприниматель находит искажения.","result":"Формируется понимание, что прибыль без правил учёта и контроля может быть опасной иллюзией.","status":"summary"}]},{"id":4,"title":"Безубыточность, масштабирование и операционный рычаг","description":"Объясняет минимальный объём продаж для выживания, запас прочности и эффект постоянных расходов при росте или падении бизнеса.","lessons":[{"id":18,"title":"Точка безубыточности","objective":"Научить считать минимальный объём, при котором бизнес выходит в ноль.","content":"Постоянные расходы, contribution margin, минимальная выручка, минимальное количество клиентов, визитов, заказов, рейсов или проектов.","case":"Салону нужно покрыть 700 000 ₽ постоянных расходов. Предприниматель считает нужное число визитов при разной марже и среднем чеке.","result":"Формируется понимание минимальный уровень продаж, ниже которого бизнес убыточен.","status":"summary"},{"id":19,"title":"Запас финансовой прочности","objective":"Показать, насколько бизнес устойчив к падению выручки.","content":"Margin of safety, выручка фактическая, выручка безубыточности, запас прочности, устойчивость к снижению спроса.","case":"Выручка бизнеса 2 млн ₽, точка безубыточности 1,5 млн ₽. Предприниматель считает допустимое падение и риск модели.","result":"Формируется видение, насколько бизнес защищён от просадки.","status":"summary"},{"id":20,"title":"Операционный рычаг","objective":"Показать, как постоянные расходы усиливают рост прибыли и усиливают убытки при падении выручки.","content":"Операционный рычаг, структура расходов, масштабирование, риск высокой фиксированной базы, чувствительность прибыли к выручке.","case":"Два бизнеса растут на 20%, но прибыль одного растёт на 60%, а другого на 15%. Предприниматель объясняет эффект структуры расходов.","result":"Формируется понимание риск и силу масштабирования.","status":"summary"}]},{"id":5,"title":"ДДС, деньги и ликвидность","description":"Показывает, почему прибыль не равна деньгам, где возникают кассовые разрывы и как управлять платёжеспособностью.","lessons":[{"id":21,"title":"ДДС: как движутся деньги","objective":"Собрать логику отчёта движения денежных средств.","content":"Операционный денежный поток, инвестиционный денежный поток, финансовый денежный поток, деньги на начало, деньги на конец, чистое изменение денег.","case":"За месяц пришли деньги от клиентов, куплено оборудование и погашен кредит. Предприниматель распределяет движения по OCF, CFI и CFF.","result":"Формируется понимание, почему деньги двигаются не так, как прибыль.","status":"summary"},{"id":22,"title":"Почему прибыль есть, а денег нет","objective":"Разобрать ключевой предпринимательский парадокс: бизнес прибыльный, но касса пустая.","content":"Дебиторка, запасы, авансы поставщикам, capex, погашение долга, налоги прошлых периодов, сезонность, рассрочки.","case":"Прибыль 300 000 ₽, деньги снизились на 200 000 ₽. Предприниматель находит причины через рабочий капитал, capex и долг.","result":"Предприниматель умеет объяснять расхождение прибыли и денег.","status":"summary"},{"id":23,"title":"Рабочий капитал","objective":"Показать место, где прибыль превращается или не превращается в деньги.","content":"Дебиторка, кредиторка, запасы, авансы клиентов, авансы поставщикам, налоги к уплате, ФОТ к выплате.","case":"Бизнес прибыльный, но деньги застряли в дебиторке и запасах. Определяется, какие остатки давят на cash flow.","result":"Формируется понимание финансовую роль остатков в обороте.","status":"summary"},{"id":24,"title":"Cash Conversion Cycle","objective":"Показать, сколько дней деньги застревают в операционном цикле.","content":"DSO, DIO, DPO, cash conversion cycle, сроки оплаты клиентов, сроки оплаты поставщиков, оборачиваемость запасов.","case":"Магазин платит поставщику раньше, чем продаёт товар и получает деньги. Предприниматель считает, сколько дней бизнес финансирует цикл.","result":"Формируется видение скорость превращения вложений в деньги.","status":"summary"},{"id":25,"title":"Платёжный календарь и кассовый разрыв","objective":"Научить управлять краткосрочной ликвидностью.","content":"Поступления, обязательные платежи, переносимые платежи, минимальный остаток, 91-дневный прогноз, дни риска.","case":"Через 12 дней нужно выплатить зарплату и аренду, а крупная оплата клиента ожидается позже. Предприниматель строит логику решения кассового разрыва.","result":"Предприниматель умеет заранее видеть опасные дни по деньгам.","status":"summary"},{"id":26,"title":"Финансовая безопасность и резервы","objective":"Показать, зачем бизнесу денежная подушка и резервы под обязательства.","content":"Резерв ФОТ, аренды, налогов, долга, сезонности, ремонта, падения спроса, минимальный денежный остаток.","case":"Бизнес работает без резерва и попадает в кассовый разрыв при задержке оплат. Определяется нужный минимальный запас денег.","result":"Формируется понимание, что свободные деньги и безопасно доступные деньги - не одно и то же.","status":"summary"}]},{"id":6,"title":"Баланс и остатки","description":"Раскрывает активы, обязательства, капитал, авансы, основные средства и мосты остатков как основу настоящей управленческой картины.","lessons":[{"id":27,"title":"Баланс: активы, обязательства и капитал","objective":"Объяснить баланс предпринимательским языком.","content":"Активы, обязательства, капитал, балансовое равенство, финансовое положение, снимок бизнеса на дату.","case":"У бизнеса есть деньги на счёте, но большие долги и авансы клиентов. Определяется, почему бизнес не так силён, как кажется.","result":"Формируется понимание, что баланс показывает реальную финансовую конструкцию бизнеса.","status":"summary"},{"id":28,"title":"Активы","objective":"Разобрать, что бизнес контролирует и что может приносить будущую пользу.","content":"Деньги, дебиторка, запасы, авансы поставщикам, основные средства, НДС к возмещению, прочие активы.","case":"Купили оборудование и товарный запас. Определяется, почему это не просто расходы месяца.","result":"Предприниматель отличает актив от расхода.","status":"summary"},{"id":29,"title":"Обязательства","objective":"Показать, что бизнес должен клиентам, сотрудникам, поставщикам, государству и кредиторам.","content":"Кредиторка, авансы клиентов, налоги, ФОТ, кредиты, проценты, НДС к уплате, прочие обязательства.","case":"Бизнес получил предоплату и отсрочку от поставщика. Определяется, какие обязательства появились.","result":"Формируется видение будущие выплаты и обязанности, а не только текущую кассу.","status":"summary"},{"id":30,"title":"Капитал собственника","objective":"Объяснить, что реально остаётся собственнику внутри бизнеса.","content":"Взносы, изъятия, накопленная прибыль, дивиденды, капитал, стоимость внутри бизнеса.","case":"Бизнес показывает прибыль, но собственник регулярно забирает деньги. Предприниматель анализирует влияние изъятий на капитал и устойчивость.","result":"Формируется понимание разницу между прибылью бизнеса и деньгами, которые можно забрать.","status":"summary"},{"id":31,"title":"Основные средства, capex и амортизация","objective":"Показать разницу между крупной покупкой, расходом и амортизацией.","content":"Capex, основные средства, дата ввода, срок полезного использования, амортизация, остаточная стоимость, выбытие.","case":"Купили оборудование за 500 000 ₽. Предприниматель показывает влияние на ОДДС, ОПиУ и баланс.","result":"Предприниматель не списывает инвестиции в расход одного месяца.","status":"summary"},{"id":32,"title":"Авансы клиентов и предоплаты","objective":"Разобрать обязательства, возникающие при оплате до оказания услуги или поставки товара.","content":"Абонементы, сертификаты, подписки, предоплаты, неоказанные услуги, признание выручки, обязательства перед клиентом.","case":"Продали абонементы на 800 000 ₽ и оказали услуг только на 200 000 ₽. Предприниматель распределяет деньги, выручку и обязательства.","result":"Формируется понимание, почему авансовые продажи могут улучшить кассу, но не прибыль.","status":"summary"},{"id":33,"title":"Мосты остатков","objective":"Научить видеть каждый остаток как движение от начала к концу периода.","content":"Остаток на начало, увеличение, уменьшение, корректировка, остаток на конец. Мосты ДЗ, КЗ, запасов, авансов, ОС, долга, налогов, ФОТ.","case":"Дебиторка выросла за месяц. Предприниматель восстанавливает мост: было, начислили, оплатили, осталось.","result":"Формируется понимание, что баланс должен собираться через логику движения, а не вводиться произвольно.","status":"summary"}]},{"id":7,"title":"Долг, налоги, ФОТ и собственник","description":"Разбирает крупные блоки, которые часто искажают прибыль и деньги: кредиты, проценты, налоги, команда и личные деньги собственника.","lessons":[{"id":34,"title":"Долг, проценты и кредитная нагрузка","objective":"Показать, как долг влияет на деньги, прибыль, баланс и риск.","content":"Тело долга, проценты, график платежей, Debt/EBITDA, interest coverage, DSCR, краткосрочный и долгосрочный долг.","case":"Бизнес берёт кредит на развитие. Предприниматель разделяет получение денег, проценты, погашение тела и остаток долга.","result":"Предприниматель не путает долг с доходом, а погашение тела долга с расходом.","status":"summary"},{"id":35,"title":"Налоги в управленческой системе","objective":"Объяснить налоги как управленческий блок: начисления, оплаты и обязательства.","content":"Начисленные налоги, уплаченные налоги, налоги к уплате, НДС, налог на прибыль, зарплатные налоги, налоговый резерв.","case":"Налог начислен в одном месяце, а уплачен в другом. Предприниматель показывает влияние на ОПиУ, ОДДС и баланс.","result":"Формируется понимание, что налоги нельзя вести только по факту оплаты.","status":"summary"},{"id":36,"title":"ФОТ и экономика команды","objective":"Показать фонд оплаты труда как одну из ключевых финансовых систем бизнеса.","content":"Оклад, переменная часть, премии, взносы, производственный ФОТ, коммерческий ФОТ, административный ФОТ, управленческий ФОТ, ФОТ/выручка.","case":"Выручка растёт, но ФОТ растёт быстрее. Предприниматель анализирует влияние команды на маржу и EBITDA.","result":"Формируется понимание, как команда влияет на себестоимость, расходы и масштабируемость.","status":"summary"},{"id":37,"title":"Деньги бизнеса и деньги собственника","objective":"Разделить личные финансы собственника и финансы компании.","content":"Зарплата собственника, дивиденды, изъятия, вклад собственника, займ собственника, свободный денежный поток, безопасное изъятие.","case":"Собственник забирает деньги из кассы каждый месяц. Определяется, как это отражается в ОДДС, балансе и устойчивости.","result":"Формируется понимание, сколько можно забирать без разрушения бизнеса.","status":"summary"}]},{"id":8,"title":"Планирование, прогнозирование и инвестиции","description":"Учит смотреть вперёд: план-факт, драйверное бюджетирование, сценарии, стресс-тесты, инвестиции и источники финансирования роста.","lessons":[{"id":38,"title":"План-факт","objective":"Научить сравнивать ожидания и реальность по прибыли, деньгам, остаткам и метрикам.","content":"План выручки, расходов, прибыли, денег, баланса, отклонения, причины отклонений, потоковые и остаточные показатели.","case":"План по выручке выполнен, а прибыль нет. Предприниматель раскладывает отклонение на цену, объём, маржу и расходы.","result":"Предприниматель умеет не просто видеть отклонение, а искать его причину.","status":"summary"},{"id":39,"title":"Бюджетирование по драйверам","objective":"Показать, что планировать нужно причины цифр, а не только итоговые суммы.","content":"Драйверы выручки, конверсии, среднего чека, загрузки, объёма, мощности, ставок, рейсов, гостей, этапов проекта.","case":"Услуги планируют выручку через заявки, конверсию, визиты и чек. Предприниматель строит логику плана без абстрактной суммы сверху.","result":"Формируется понимание, как строится реалистичный бюджет бизнеса.","status":"summary"},{"id":40,"title":"Сценарии и стресс-тесты","objective":"Научить проверять устойчивость бизнеса при изменении внешних и внутренних условий.","content":"Базовый, оптимистичный, пессимистичный, кризисный сценарии. Падение спроса, рост расходов, задержка оплат, снижение маржи, рост долга.","case":"Выручка падает на 20%, ФОТ не снижается, оплата клиентов задерживается. Предприниматель оценивает, когда наступит кассовый риск.","result":"Предприниматель умеет смотреть не только на план, но и на опасные варианты будущего.","status":"summary"},{"id":41,"title":"Инвестиционные решения","objective":"Показать, как считать решения о развитии бизнеса.","content":"Открытие точки, покупка оборудования, найм, ремонт, запуск направления, payback, ROI, NPV на понятном уровне, IRR на понятном уровне, cash impact.","case":"Бизнес хочет купить оборудование. Предприниматель оценивает окупаемость, влияние на кассу, риски и сценарии.","result":"Предприниматель принимает инвестиционные решения не по желанию, а через финансовую логику.","status":"summary"},{"id":42,"title":"Финансирование роста","objective":"Разобрать источники денег для развития и цену каждого источника.","content":"Реинвестирование прибыли, кредит, займ собственника, инвестор, лизинг, отсрочка поставщика, предоплата клиента, факторинг.","case":"Есть три варианта финансирования новой точки: кредит, инвестор или собственная прибыль. Предприниматель сравнивает цену, риск и контроль.","result":"Формируется понимание, что деньги для роста всегда имеют стоимость и последствия.","status":"summary"}]},{"id":9,"title":"Метрики, диагностика и дашборд","description":"Даёт систему показателей, диагностику бизнеса и логику экрана собственника, чтобы цифры превращались в решения.","lessons":[{"id":43,"title":"Универсальные финансовые метрики","objective":"Собрать базовый набор показателей, которые нужны почти любому бизнесу.","content":"Выручка, валовая прибыль, валовая маржа, EBITDA, EBITDA margin, чистая прибыль, OCF, Net CF, current ratio, quick ratio, NWC, Debt/EBITDA, DSCR, ROA, ROE, ROI.","case":"Предприниматель получает набор показателей по бизнесу и определяет, какие из них говорят о прибыли, какие о деньгах, какие об устойчивости.","result":"Предприниматель умеет читать финансовое здоровье бизнеса через набор метрик.","status":"summary"},{"id":44,"title":"Маркетинговые и клиентские метрики","objective":"Показать связь маркетинга, клиентов и финансового результата.","content":"CAC, LTV, LTV/CAC, ROMI, ROAS, conversion, retention, churn, повторные покупки, частота покупок, средний чек, когортный анализ на простом уровне.","case":"ROAS высокий, но прибыль слабая. Определяется, почему маркетинговая метрика может обманывать без маржи и LTV.","result":"Предприниматель оценивает маркетинг через деньги, а не только через заявки.","status":"summary"},{"id":45,"title":"Операционные метрики","objective":"Показать связь процессов с финансовым результатом.","content":"Загрузка, производительность, выручка на сотрудника, выручка на час, оборачиваемость, срок выполнения заказа, брак, простои, мощность, утилизация ресурсов.","case":"В салоне высокая загрузка, но прибыль слабая. Предприниматель проверяет чек, ФОТ, маржу, повторные продажи и потери времени.","result":"Формируется понимание, что операционные показатели должны объяснять финансовый результат.","status":"summary"},{"id":46,"title":"Метрики, которые обманывают","objective":"Научить не принимать решения по красивым, но неполным показателям.","content":"Рост выручки при падении маржи, хороший ROAS при плохой прибыли, EBITDA при отрицательном cash flow, хороший current ratio при мёртвых запасах, завышенный LTV, неполный CAC.","case":"Бизнес показывает рост всех верхних метрик, но денег становится меньше. Предприниматель выявляет, какие показатели создают ложное ощущение успеха.","result":"Предприниматель читает метрики критически и в связке друг с другом.","status":"summary"},{"id":47,"title":"Финансовая диагностика бизнеса","objective":"Дать карту поиска проблем по цифрам.","content":"Проблемы с деньгами, прибылью, маржей, ростом, устойчивостью, долгом, управляемостью и качеством данных.","case":"У бизнеса просела чистая прибыль. Предприниматель по диагностической карте проверяет выручку, маржу, расходы, ФОТ, налоги, долг и разовые факторы.","result":"Предприниматель умеет превращать отчёты в расследование причин.","status":"summary"},{"id":48,"title":"Дашборд собственника","objective":"Показать, какой экран нужен собственнику для регулярного контроля бизнеса.","content":"Главные показатели, красные зоны, период, план-факт, прибыль, деньги, баланс, риск, метрики, качество данных.","case":"Формируется видение перегруженный дашборд из 80 метрик и собирает версию из 12–15 ключевых показателей.","result":"Формируется понимание, что дашборд нужен для решений, а не для красоты.","status":"summary"}]},{"id":10,"title":"Отраслевые финансы","description":"Показывает, что финансы услуг, торговли, производства, проектов, логистики и HoReCa устроены по-разному.","lessons":[{"id":49,"title":"Финансы бизнеса услуг","objective":"Разобрать финансовую механику услуг.","content":"Заявки, записи, визиты, загрузка, специалисты, ФОТ, абонементы, повторные визиты, выручка на час, маржа услуги.","case":"Салон услуг растёт по выручке, но не по прибыли. Предприниматель ищет проблему в загрузке, чеке, ФОТ, линейке и повторных визитах.","result":"Формируется понимание, как считать и анализировать сервисный бизнес.","status":"summary"},{"id":50,"title":"Финансы торговли","objective":"Разобрать финансовую механику розничной и оптовой торговли.","content":"Товарный остаток, закупка, наценка, маржа, оборачиваемость, скидки, списания, категории, SKU, GMROI.","case":"Магазин имеет высокую выручку, но деньги заморожены в товаре. Предприниматель анализирует оборачиваемость и маржу.","result":"Формируется понимание, что торговля живёт через маржу и оборот запасов.","status":"summary"},{"id":51,"title":"Финансы производства","objective":"Разобрать финансовую механику производственного бизнеса.","content":"Сырьё, незавершённое производство, готовая продукция, мощность, себестоимость выпуска, брак, производственные накладные, план производства.","case":"Производство увеличило выпуск, но деньги ухудшились. Предприниматель анализирует запасы, WIP, оплату поставщикам и сроки реализации.","result":"Формируется понимание, как производство связывает мощность, себестоимость, запасы и деньги.","status":"summary"},{"id":52,"title":"Финансы строительства и проектного бизнеса","objective":"Разобрать проектную финансовую модель.","content":"Этапы, смета, процент выполнения, авансы, WIP, субподряд, overbilling, underbilling, проектный cash gap.","case":"Проект прибыльный по смете, но требует финансирования до оплаты заказчика. Предприниматель анализирует проектный cash gap.","result":"Формируется понимание, почему проектный бизнес может быть прибыльным и одновременно кассово тяжёлым.","status":"summary"},{"id":53,"title":"Финансы логистики","objective":"Разобрать финансовую механику логистики и перевозок.","content":"Рейсы, километры, топливо, водители, ремонт, маржа рейса, загрузка транспорта, cost per km, выручка на машину.","case":"Перевозчик увеличил число рейсов, но прибыль не выросла. Предприниматель анализирует топливо, пустые пробеги, ремонт и ставки.","result":"Формируется понимание логистику через рейс, километр, загрузку и маржу маршрута.","status":"summary"},{"id":54,"title":"Финансы HoReCa","objective":"Разобрать финансовую механику ресторанов, кафе и гостиничных элементов.","content":"Food cost, labor cost, prime cost, посадка, средний чек, оборачиваемость столов, списания, меню-инжиниринг, сезонность.","case":"Кафе имеет полную посадку, но низкую прибыль. Предприниматель ищет проблему в food cost, labor cost, списаниях и меню.","result":"Формируется понимание HoReCa через маржу меню, труд, загрузку и списания.","status":"summary"}]},{"id":11,"title":"Финансовая система и управление","description":"Собирает всю финансовую архитектуру в регулярную систему управления: политика учёта, статьи, закрытие месяца, роли, контроль и стадии бизнеса.","lessons":[{"id":55,"title":"Управленческая учётная политика","objective":"Показать, что финансовая система требует заранее установленных правил.","content":"Как признавать выручку, что считать себестоимостью, как относить расходы, как учитывать авансы, ОС, долги, ФОТ и закрытие месяца.","case":"Два менеджера по-разному классифицируют один и тот же расход. Определяется, почему нужна управленческая политика.","result":"Формируется понимание, что без правил данные становятся несравнимыми.","status":"summary"},{"id":56,"title":"Структура статей и справочников","objective":"Научить строить понятные справочники доходов, расходов, ОДДС, активов, обязательств и аналитик.","content":"Статьи доходов, расходов, ОДДС, активов, обязательств, центры ответственности, направления, проекты, филиалы.","case":"В бизнесе 300 статей расходов и половина операций попадает в прочее. Предприниматель пересобирает справочник до управляемой структуры.","result":"Формируется понимание, как избежать хаоса в финансовой классификации.","status":"summary"},{"id":57,"title":"Закрытие месяца","objective":"Показать закрытие месяца как регулярный управленческий ритуал.","content":"Сверка денег, выручки, расходов, ФОТ, налогов, остатков, авансов, долга, ОС и баланса.","case":"Месяц не закрыт, но собственник уже смотрит прибыль. Определяется, почему выводам нельзя доверять.","result":"Формируется понимание, что отчёты имеют смысл только после закрытия периода.","status":"summary"},{"id":58,"title":"Финансовый календарь собственника","objective":"Определить, что предприниматель должен смотреть ежедневно, еженедельно, ежемесячно, ежеквартально и ежегодно.","content":"Ежедневный cash control, еженедельный план-факт, ежемесячное закрытие, квартальные решения, годовой бюджет.","case":"Собственник смотрит ОПиУ каждый день, но не видит кассовый разрыв. Предприниматель распределяет контроль по правильным периодам.","result":"Предприниматель получает ритм финансового управления.","status":"summary"},{"id":59,"title":"Внутренний контроль и качество данных","objective":"Показать, как защищать финансовую систему от ошибок, двойного учёта и недостоверных данных.","content":"Сверка кассы, банка, расходов, остатков, прав доступа, двойной учёт, QA, ошибки ввода, ответственность за данные.","case":"В кассе не сходится остаток, а расходы задвоены. Определяется, какие проверки должны были это поймать.","result":"Формируется понимание, что плохие данные опаснее отсутствия отчёта.","status":"summary"},{"id":60,"title":"Финансовые роли в бизнесе","objective":"Разобрать, кто за что отвечает в финансовой системе компании.","content":"Собственник, руководитель, администратор, бухгалтер, финансовый менеджер, операционный директор, маркетолог, руководитель продаж.","case":"Все считают, что за деньги отвечает бухгалтер. Предприниматель распределяет ответственность между ролями.","result":"Формируется понимание, какие финансовые функции можно делегировать, а какие собственник должен контролировать лично.","status":"summary"},{"id":61,"title":"Финансовое управление по стадиям бизнеса","objective":"Показать, что финансовые приоритеты меняются в зависимости от стадии компании.","content":"Старт, первые продажи, стабилизация, рост, масштабирование, несколько точек, кризис, подготовка к продаже.","case":"Бизнес масштабируется второй точкой и теряет cash flow. Определяется, какие метрики и риски становятся главными на этой стадии.","result":"Формируется видение, что один и тот же показатель по-разному читается на разных этапах бизнеса.","status":"summary"},{"id":62,"title":"Итоговая финансовая архитектура бизнеса","objective":"Собрать весь модуль в единую систему финансового управления.","content":"Выручка, расходы, прибыль, деньги, баланс, остатки, метрики, план, прогноз, контроль, решения, отраслевые особенности.","case":"Итоговый кейс: у бизнеса есть данные по продажам, деньгам, расходам, остаткам и плану. Предприниматель собирает управленческую картину и формулирует решения.","result":"Формируется понимание бизнес как финансовую архитектуру, а не набор разрозненных цифр.","status":"summary"}]}];
  const PASS = 24;
  const TOTAL = 30;
  const previousFinanceLesson = window.renderFinanceLesson;
  ...
  function s(html, active){ return typeof shell === 'function' ? shell(html, active || 'finance') : ((document.getElementById('app') || {}).innerHTML = html); }
  function adminMode(){ return typeof isAdminMode === 'function' && isAdminMode(); }
  function locked(){ s(c('result-bad-v2 finance-locked-v62', `<p class="eyebrow">финансовый помощник</p><h1>Раздел закрыт</h1><p>Финансовый помощник пока закрыт для режима просмотра ученика. Материалы открываются только в режиме администрирования.</p><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`), 'finance'); }
  function requireAdmin(){ if (adminMode()) return true; locked(); return false; }
  function section(id){ return SECTIONS.find(x => Number(x.id) === Number(id)) || null; }
  function lesson(sec, id){ return sec && Array.isArray(sec.lessons) ? (sec.lessons.find(x => Number(x.id) === Number(id)) || null) : null; }
  function progress(){
    const list = [];
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i) || '';
        if (key.indexOf('architecture_finance_module_progress_v49_') !== -1) {
          try { const v = JSON.parse(localStorage.getItem(key) || '{}'); v.__key = key; list.push(v); } catch(err) {}
        }
      }
    } catch(err) {}
    if (!list.length) return {};
    list.sort((a,b) => (new Date(b.updatedAt || b.section1TestAt || 0).getTime() || 0) - (new Date(a.updatedAt || a.section1TestAt || 0).getTime() || 0));
    return list[0] || {};
  }
  function sectionPassed(id){
    const p = progress();
    id = Number(id);
    if (id === 1) return Boolean(p.section1TestPassed) || Number(p.section1TestScore || 0) >= PASS;
    return Boolean(p['section' + id + 'TestPassed']);
  }
  function canOpen(id){ id = Number(id); if (adminMode()) return true; if (id === 1) return true; return sectionPassed(id - 1); }
  function statusText(id){ id = Number(id); if (id === 1) return 'доступно'; if (adminMode()) return 'предпросмотр администратора'; return canOpen(id) ? 'доступно' : 'закрыто'; }
  function progressBox(){
    const p = progress();
    const total = Number(p.section1TestTotal || TOTAL) || TOTAL;
    const score = Math.max(0, Math.min(total, Number(p.section1TestScore || 0)));
    const pct = total ? Math.round(score / total * 100) : 0;
    const passed = sectionPassed(1);
    return `<div class="finance-progress-v62 ${passed ? 'passed' : 'locked'}"><div class="finance-progress-head-v62"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : `${score}/${total} · ${pct}%`}</b></div><div class="progress-line"><div style="width:${Math.max(0, Math.min(100, pct))}%"></div></div><p>${passed ? 'Раздел 2 открыт в учебной логике.' : `Для открытия раздела 2 нужно пройти диагностику минимум на 80%: ${PASS}/${TOTAL} правильных ответов.`}</p></div>`;
  }

  function renderFinancialAssistant62(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения. Здесь отдельно собраны обучение, будущая аналитика бизнеса и готовые финансовые шаблоны.</p>`)}${c('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v62"><button class="finance-hub-card-v62 active" onclick="renderFinanceModuleHome62()"><b>Финансовый модуль</b><p>Учебная программа по финансовой архитектуре бизнеса: разделы, уроки, диагностики и тренажёры.</p><em>открыть</em></button><button class="finance-hub-card-v62" onclick="renderFinanceAnalytics62()"><b>Моя аналитика</b><p>Будущий контур личных показателей: факты, отчёты, динамика, риски и выводы.</p><em>каркас</em></button><button class="finance-hub-card-v62" onclick="renderFinanceTemplates62()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и инструментов: ОПиУ, ОДДС, баланс, календарь и прогнозы.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}`, 'finance');
  }

  function moduleMap(){ return `<div class="finance-module-map-v62"><div><b>01</b><p>База: деньги, выручка, прибыль, ОПиУ, ОДДС, баланс и смысл операции.</p></div><div><b>02–04</b><p>Выручка, unit-экономика, прибыльность, безубыточность и операционный рычаг.</p></div><div><b>05–07</b><p>Деньги, ликвидность, баланс, остатки, долг, налоги, ФОТ и собственник.</p></div><div><b>08–11</b><p>Планирование, метрики, отраслевые финансы и регулярная система управления.</p></div></div>`; }
  function sectionCard(sec){
    const id = Number(sec.id);
    const open = canOpen(id);
    const lessons = Array.isArray(sec.lessons) ? sec.lessons.length : 0;
    const action = open ? `renderFinanceSection62(${id})` : `financeLockedSectionNotice62(${id})`;
    return `<button type="button" class="finance-section-tile-v62 ${open ? 'open' : 'locked'}" onclick="${action}"><div class="finance-section-tile-head-v62"><span>Раздел ${id}</span><em>${e(statusText(id))}</em></div><h3>${e(sec.title || '')}</h3><p>${e(sec.description || '')}</p><div class="finance-section-tile-bottom-v62"><b>${id === 1 ? lessons + 2 : lessons} блоков</b><span>${open ? 'Открыть уроки →' : 'Закрыто'}</span></div></button>`;
  }
  function renderFinanceModuleHome62(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль показывает, как операции бизнеса превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника.</p><p class="small">Внутри финансового модуля разделы идут последовательно: следующий раздел открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>`)}${c('finance-quick-open-v62', `<h2>Быстрый переход</h2><p>Первый раздел доступен сразу. Здесь находятся уроки, итоговая диагностика и финтренажёр.</p><button class="btn primary" onclick="renderFinanceSection62(1)">Открыть уроки раздела 1</button>`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant62()">← К финансовому помощнику</button></div><h2>Карта финансового модуля</h2>${moduleMap()}<h2>11 разделов</h2><p class="small">Нажимается вся карточка раздела. Раздел 1 сразу открывает список уроков и закрепление.</p><div class="finance-section-list-v62">${SECTIONS.map(sectionCard).join('')}</div>`)}`, 'finance');
  }

  function lessonRow(sec, les, idx){
    const secId = Number(sec.id), id = Number(les.id || idx + 1);
    const full = secId === 1 && id >= 1 && id <= 4;
    const action = full ? `renderFinanceLesson62(${id},0)` : `renderFinanceLessonSummary62(${secId},${id})`;
    const label = full ? 'полный учебный экран' : 'структура урока из общей карты модуля';
    return `<button class="lesson-row-v2 finance-lesson-row-v62" onclick="${action}"><div><b>${String(id).padStart(2,'0')}. ${e(les.title || 'Урок')}</b><p>${e(label)}</p></div><span>→</span></button>`;
  }
  function sectionExtras(id){
    if (Number(id) !== 1) return `<button class="lesson-row-v2 locked" disabled><div><b>Итоговая диагностика раздела ${Number(id)}</b><p>будет добавлена после полного наполнения уроков раздела</p></div><span>•</span></button>`;
    return `<button class="lesson-row-v2 finance-test-row-v62" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает раздел 2</p></div><span>→</span></button><button class="lesson-row-v2 finance-trainer-row-v62" onclick="renderFinanceTrainerSection1_62()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
  }
  function renderFinanceSection62(id){
    if (!requireAdmin()) return;
    id = Number(id || 1);
    const sec = section(id);
    if (!sec) return renderFinanceModuleHome62();
    if (!canOpen(id)) return financeLockedSectionNotice62(id);
    const rows = (Array.isArray(sec.lessons) ? sec.lessons : []).map((les, idx) => lessonRow(sec, les, idx)).join('');
    s(`${c('blue-card-v2 finance-section-hero-v62', `<p class="eyebrow">раздел ${id}</p><h1>${e(sec.title || '')}</h1><p>${e(sec.description || '')}</p>${id === 1 ? progressBox() : '<p class="small">Административный предпросмотр структуры раздела.</p>'}`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome62()">← К разделам</button><button class="btn secondary" onclick="renderFinancialAssistant62()">К помощнику</button></div><h2>${id === 1 ? 'Уроки и закрепление раздела' : 'Уроки раздела'}</h2><p>${id === 1 ? 'Порядок работы: 4 урока → итоговая диагностика → финтренажёр для дополнительного закрепления.' : 'Раздел открыт в режиме администрирования для проверки структуры.'}</p><div class="lesson-list-v2 finance-lesson-list-v62">${rows}${sectionExtras(id)}</div>`)}`, 'finance');
  }
  function renderFinanceLesson62(id, screenIndex){
    if (!requireAdmin()) return;
    if (typeof previousFinanceLesson === 'function') return previousFinanceLesson.call(this, Number(id), Number(screenIndex || 0));
    alert('Полный экран урока не найден в текущей сборке.');
  }
  function renderFinanceLessonSummary62(sectionId, lessonId){
    if (!requireAdmin()) return;
    const sec = section(sectionId), les = lesson(sec, lessonId);
    if (!sec || !les) return renderFinanceSection62(sectionId);
    s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">раздел ${Number(sectionId)} · урок ${Number(lessonId)}</p><h1>${e(les.title || '')}</h1><p>${e(les.objective || '')}</p>`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection62(${Number(sectionId)})">← К урокам раздела</button></div><div class="finance-summary-grid-v62"><div><span>Ключевое содержание</span><p>${e(les.content || 'Будет раскрыто позже.')}</p></div><div><span>Кейс</span><p>${e(les.case || 'Будет раскрыто позже.')}</p></div><div><span>Основной вывод</span><p>${e(les.result || 'Будет раскрыто позже.')}</p></div><div><span>Статус</span><p>Сейчас это структура урока. Полное раскрытие будет добавлено отдельным этапом.</p></div></div>`)}`, 'finance');
  }
  function renderFinanceTrainerSection1_62(){
    if (!requireAdmin()) return;
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V62 || window.FINANCE_SECTION1_TRAINER_URL_V61 || window.FINANCE_SECTION1_TRAINER_URL_V60 || window.FINANCE_SECTION1_TRAINER_URL_V58 || window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    const link = url && url !== '#' ? `<div class="finance-trainer-link-box-v57"><span>Ссылка на таблицу</span><p>${e(url)}</p></div><a class="btn primary" href="${e(url)}" target="_blank">Открыть таблицу</a>` : `<div class="finance-trainer-link-box-v57 muted"><span>Ссылка на таблицу</span><p>Ссылка будет подключена после создания финтренажёра.</p></div><button class="btn secondary" disabled>Ссылка будет добавлена</button>`;
    s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceSection62(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями.</p></div><div><b>4. Пройти задания</b><p>Классифицируйте операции и проверьте влияние на ОПиУ, ОДДС и баланс.</p></div></div>${link}<button class="btn secondary" onclick="renderFinanceSection62(1)">← Вернуться к урокам раздела</button>`)}`, 'finance');
  }
  function renderFinanceAnalytics62(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Будущий контур личных показателей бизнеса.</p>`)}${c('', `<p>Раздел пока оставлен как каркас. Его нужно подключать после утверждения финансового ядра.</p><button class="btn secondary" onclick="renderFinancialAssistant62()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function renderFinanceTemplates62(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v62', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и инструментов.</p>`)}${c('', `<div class="list-clean"><div><b>ОПиУ</b><p>Шаблоны отчёта о прибыли и убытках.</p></div><div><b>ОДДС</b><p>Шаблоны движения денежных средств и платёжного календаря.</p></div><div><b>Баланс</b><p>Шаблоны остатков, активов и обязательств.</p></div></div><button class="btn secondary" onclick="renderFinancialAssistant62()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function financeLockedSectionNotice62(id){ alert('Раздел ' + Number(id || 2) + ' откроется после прохождения итоговой диагностики предыдущего раздела минимум на 80%.'); }

  Object.assign(window, { renderFinancialAssistant62, renderFinanceModuleHome62, renderFinanceSection62, renderFinanceLesson62, renderFinanceLessonSummary62, renderFinanceTrainerSection1_62, renderFinanceAnalytics62, renderFinanceTemplates62, financeLockedSectionNotice62 });
  window.renderFinancialAssistant = renderFinancialAssistant62;
  window.renderMyBusiness = renderFinancialAssistant62;
  window.openFinanceAssistantV62 = renderFinancialAssistant62;
  window.openFinanceAssistantV61 = renderFinancialAssistant62;
  window.openFinanceAssistantV60 = renderFinancialAssistant62;
  window.openFinanceAssistantV57 = renderFinancialAssistant62;
  window.openFinanceAssistantV55 = renderFinancialAssistant62;
  window.openFinanceAssistantV54 = renderFinancialAssistant62;
  window.openFinanceAssistantV52 = renderFinancialAssistant62;
  window.openFinanceAssistantV50 = renderFinancialAssistant62;
  window.renderFinanceModuleHome = renderFinanceModuleHome62;
  window.renderFinanceSection = renderFinanceSection62;
  window.renderFinanceLesson = renderFinanceLesson62;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1_62;
  window.renderFinanceSection1Trainer = renderFinanceTrainerSection1_62;
  window.financeLockedSectionNotice = financeLockedSectionNotice62;
  try { renderFinancialAssistant = renderFinancialAssistant62; } catch(e) {}
  try { renderMyBusiness = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV62 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV61 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV60 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV57 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV55 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV54 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV52 = renderFinancialAssistant62; } catch(e) {}
  try { openFinanceAssistantV50 = renderFinancialAssistant62; } catch(e) {}
  try { renderFinanceModuleHome = renderFinanceModuleHome62; } catch(e) {}
  try { renderFinanceSection = renderFinanceSection62; } catch(e) {}
  try { renderFinanceLesson = renderFinanceLesson62; } catch(e) {}
  try { renderFinanceTrainerSection1 = renderFinanceTrainerSection1_62; } catch(e) {}
  try { renderFinanceSection1Trainer = renderFinanceTrainerSection1_62; } catch(e) {}
  try { financeLockedSectionNotice = financeLockedSectionNotice62; } catch(e) {}
})();

/* =====================================================
   v63 — Финансовый модуль: чистый маршрут без быстрого перехода,
   без карты модуля, без главных экранов-глав внутри уроков.
   ===================================================== */
(function installFinanceCleanRouteV63(){
  window.APP_UI_VERSION_V63 = 'v63-finance-module-clean-route-20260628';
  const PASS = Number(window.FINANCE_SECTION1_PASS_SCORE_V57 || 24);
  const TOTAL = Number(window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
  ...
  function s(html, active){ return typeof shell === 'function' ? shell(html, active || 'finance') : ((document.getElementById('app') || {}).innerHTML = html); }
  function adminMode(){ return typeof isAdminMode === 'function' && isAdminMode(); }
  function locked(){ s(c('result-bad-v2 finance-locked-v63', `<p class="eyebrow">финансовый помощник</p><h1>Раздел закрыт</h1><p>Финансовый помощник пока закрыт для режима просмотра ученика. Проверка материалов доступна только в режиме администрирования.</p><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`), 'finance'); }
  function requireAdmin(){ if (adminMode()) return true; locked(); return false; }
  function sections(){ return Array.isArray(window.FINANCE_MODULE_SECTIONS_DATA_V57) ? window.FINANCE_MODULE_SECTIONS_DATA_V57 : []; }
  function section(id){ return sections().find(function(x){ return Number(x.id) === Number(id); }) || null; }
  function lesson(sec, id){ return sec && Array.isArray(sec.lessons) ? (sec.lessons.find(function(x){ return Number(x.id) === Number(id); }) || null) : null; }
  ...
  function progress(){
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i) || '';
        if (key.indexOf('architecture_finance_module_progress_v49_') !== -1) {
          try { const v = JSON.parse(localStorage.getItem(key) || '{}'); list.push(v); } catch(err) {}
        }
      }
    } catch(err) {}
    list.sort(function(a,b){ return (new Date(b.updatedAt || b.section1TestAt || 0).getTime() || 0) - (new Date(a.updatedAt || a.section1TestAt || 0).getTime() || 0); });
    return list[0] || {};
  }
  function sectionPassed(id){
    const p = progress();
  ...
  function canOpen(id){
    id = Number(id);
    if (id === 1) return true;
    return sectionPassed(id - 1);
  }
  function statusText(id){
    id = Number(id);
    if (id === 1) return 'доступно';
    return canOpen(id) ? 'доступно' : 'закрыто';
  }
  function progressBox(){
    const p = progress();
    const total = Number(p.section1TestTotal || TOTAL) || TOTAL;
    ...
    const pct = total ? Math.round(score / total * 100) : 0;
    const passed = sectionPassed(1);
    return `<div class="finance-progress-v63 ${passed ? 'passed' : 'locked'}"><div class="finance-progress-head-v63"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : `${score}/${total} · ${pct}%`}</b></div><div class="progress-line"><div style="width:${Math.max(0, Math.min(100, pct))}%"></div></div><p>${passed ? 'Раздел 2 открыт.' : `Для открытия раздела 2 нужно пройти диагностику раздела 1 минимум на 80%: ${PASS}/${TOTAL} правильных ответов.`}</p></div>`;
  }
  ...
  function clean(value){
    ...
    if (!out) return '';
    out = out.replace(/\s+/g, ' ');
    out = out.replace(/[ОO]{2,}\s*ДДС/g, 'ОДДС');
    out = out.replace(/[ОO]+\s+ДДС/g, 'ОДДС');
    out = out.replace(/(^|[^А-Яа-яA-Za-z0-9])ДДС(?=$|[^А-Яа-яA-Za-z0-9])/g, '$1ОДДС');
    out = out.replace(/[ОO]{2,}\s*ДДС/g, 'ОДДС');
    out = out.replace(/ОПУ/g, 'ОПиУ');
    out = out.replace(/ООПиУ/g, 'ОПиУ');
    out = out.replace(/ОПиУ,\s*ОО?ДДС/g, 'ОПиУ, ОДДС');
    out = out.replace(/Результат ученика\s*:/gi, 'Основной вывод:');
    out = out.replace(/ученика/g, 'предпринимателя');
    out = out.replace(/Ученика/g, 'Предпринимателя');
    out = out.replace(/ученику/g, 'предпринимателю');
    ...
    out = out.replace(/ученик/g, 'предприниматель');
    out = out.replace(/Ученик/g, 'Предприниматель');
    out = out.replace(/у предприниматель(?![а-яё])/gi, 'у предпринимателя');
    out = out.replace(/предпринимателя/g, 'предпринимателя');
    out = out.replace(/предпринимателю/g, 'предпринимателю');
    out = out.replace(/предпринимателем/g, 'предпринимателем');
    out = out.replace(/Предпринимателя/g, 'Предпринимателя');
    out = out.replace(/Предпринимателю/g, 'Предпринимателю');
    out = out.replace(/Предпринимателем/g, 'Предпринимателем');
    out = out.replace(/Этот урок должен полностью убрать у предпринимателя привычку/gi, 'Основной результат урока — убрать привычку');
    out = out.replace(/Предприниматель должен понять, что/gi, 'Важно понимать, что');
    out = out.replace(/предприниматель должен понять, что/gi, 'важно понимать, что');
    out = out.replace(/Предприниматель должен/g, 'Важно');
    out = out.replace(/предприниматель должен/g, 'важно');
    out = out.replace(/Сквозной кейс на\s*1\s*000\s*000\s*₽/gi, 'Сквозной кейс одной суммы на счёте');
    out = out.replace(/сквозной кейс на\s*1\s*000\s*000\s*₽/gi, 'сквозной кейс');
    out = out.replace(/Формируется понимание основу/g, 'Формируется понимание основы');
    out = out.replace(/Формируется понимание финансовые/g, 'Формируется понимание финансовых');
    out = out.replace(/Формируется понимание логистику/g, 'Формируется понимание логистики');
    out = out.replace(/Формируется понимание бизнес как/g, 'Формируется понимание бизнеса как');
    out = out.replace(/Формируется переход управлять/g, 'Формируется переход к управлению');
    out = out.replace(/Формируется понимание, какие финансовые последствия/g, 'Формируется понимание финансовых последствий');
    out = out.replace(/Формируется понимание финансовые последствия/g, 'Формируется понимание финансовых последствий');
    out = out.replace(/Формируется понимание, как избежать/g, 'Формируется понимание того, как избежать');
    return out.trim();
  }

  function flattenFullContent(les){
    const arr = Array.isArray(les && les.fullContent) ? les.fullContent : [];
    const lines = [];
  ...
  function parseLesson(les){
    const lines = flattenFullContent(les);
    const intro = [];
    const slides = [];
    const finalLines = [];
    let mode = 'intro';
    ...
    lines.forEach(function(line){
      ...
      if (/^Итоговая логика урока$/i.test(line)) {
        mode = 'final'; current = null; skipVisual = false; textStarted = false; return;
      }
      if (mode === 'intro') {
        if (/^Урок\s+\d+\./i.test(line)) return;
        if (/^Общая структура урока$/i.test(line)) return;
        if (/^Название урока\s*:/i.test(line)) return;
        if (/^Рекомендуемая длительность/i.test(line)) return;
        if (/^Тест после урока/i.test(line)) return;
        if (/^Практическая таблица/i.test(line)) return;
        if (/^Задача урока\s*:/i.test(line)) line = line.replace(/^Задача урока\s*:/i, 'Главная задача:');
        if (/^Главная задача урока\s*:/i.test(line)) line = line.replace(/^Главная задача урока\s*:/i, 'Главная задача:');
        if (/^Результат ученика\s*:/i.test(line) || /^Основной вывод\s*:/i.test(line)) line = line.replace(/^Результат ученика\s*:/i, 'Основной вывод:');
        if (/^Количество слайдов\s*:/i.test(line)) return; // ниже ставим фактическое количество
        if (/^Формат урока\s*:/i.test(line) && Number(les.id) === 1) {
          line = 'Формат урока: теоретическое объяснение, разбор финансовых понятий, сквозной кейс, сравнение влияния на ОПиУ, ОДДС и баланс.';
        }
        intro.push(clean(line));
        return;
      }
      if (mode === 'slide') {
        if (/^Что показать на слайде$/i.test(line)) { skipVisual = true; textStarted = false; return; }
        if (/^Текст под слайдом$/i.test(line)) { skipVisual = false; textStarted = true; return; }
        if (skipVisual && !textStarted) return;
        if (!current) return;
        current.text.push(clean(line));
        return;
      }
      if (mode === 'final') finalLines.push(clean(line));
    });
    intro.unshift('Количество слайдов: ' + slides.length + '.');
    if (!intro.some(x => /^Формат урока\s*:/i.test(x))) intro.push('Формат урока: теоретическое объяснение, кейсы, типовые ошибки и управленческие выводы.');
    if (!intro.some(x => /^Главная задача\s*:/i.test(x))) intro.push('Главная задача: раскрыть финансовую логику темы и убрать типовые ошибки в управлении бизнесом.');
    if (!intro.some(x => /^Основной вывод\s*:/i.test(x))) intro.push('Основной вывод: ' + clean(les.result || 'Формируется управленческое понимание темы.'));
    return { intro, slides, finalLines };
  }

  function slideTitle(title){ return clean(String(title || '').replace(/^Слайд\s+\d+\.\s*/i, '')); }
  function firstSentence(text){
    const t = clean(text || '');
    const m = t.match(/^(.{30,220}?[.!?])\s/);
    return m ? m[1] : (t.length > 190 ? t.slice(0, 190).replace(/\s+\S*$/, '') + '…' : t);
  }
  function slideError(slide){
    const title = slideTitle(slide.title).toLowerCase();
    const text = clean((slide.text || []).join(' ')).toLowerCase();
    if (/аванс|абонемент|предоплат/.test(title + ' ' + text)) return 'Считать полученный аванс свободной прибылью и потратить деньги до оказания услуги.';
    if (/кредит|долг/.test(title + ' ' + text)) return 'Называть кредит выручкой и принимать рост денег за рост результата бизнеса.';
    if (/дебитор/.test(title + ' ' + text)) return 'Считать оплату старого долга новой выручкой или забывать, что прибыль ещё не стала деньгами.';
    if (/оборудован|актив|capex|основн/.test(title + ' ' + text)) return 'Списывать крупную покупку как расход одного месяца и искажать прибыль периода.';
    if (/плат[её]ж|расход/.test(title + ' ' + text)) return 'Путать факт оплаты с расходом периода и из-за этого неверно читать прибыль.';
    if (/бухгалтер|налог|управлен/.test(title + ' ' + text)) return 'Использовать бухгалтерскую отчётность как единственный инструмент управления.';
    if (/выручк/.test(title + ' ' + text)) return 'Оценивать бизнес по верхней строке, не проверяя маржу, деньги и обязательства.';
    if (/баланс|остат/.test(title + ' ' + text)) return 'Смотреть только движение денег и не видеть активы, долги, авансы и незакрытые остатки.';
    if (/экономик|двигател|модель|огранич/.test(title + ' ' + text)) return 'Сравнивать разные бизнесы по одной выручке и игнорировать разную механику заработка.';
    return 'Делать вывод по одной цифре без проверки её финансового смысла и связи с отчётами.';
  }
  function slideConclusion(slide){
    const title = slideTitle(slide.title).toLowerCase();
    const text = clean((slide.text || []).join(' ')).toLowerCase();
    if (/аванс|абонемент|предоплат/.test(title + ' ' + text)) return 'Авансы нужно отделять от заработанной выручки и контролировать как обязательства перед клиентами.';
    if (/кредит|долг/.test(title + ' ' + text)) return 'Кредит улучшает кассу сегодня, но создаёт долг и будущую нагрузку на денежный поток.';
    if (/дебитор/.test(title + ' ' + text)) return 'Дебиторка объясняет, почему бизнес мог заработать, но ещё не получить деньги.';
    if (/оборудован|актив|capex|основн/.test(title + ' ' + text)) return 'Крупные покупки нужно разделять на платёж, актив в балансе и будущий расход через амортизацию.';
    if (/бухгалтер|налог|управлен/.test(title + ' ' + text)) return 'Бухгалтерия нужна для корректности, а управленческий учёт — для решений, рисков и контроля бизнеса.';
    if (/модель|экономик|двигател|огранич/.test(title + ' ' + text)) return 'Управление начинается с понимания двигателя модели: драйверов выручки, маржи, мощности, денег и ограничений.';
    return 'Правильный вывод появляется только после связи операции с ОПиУ, ОДДС, балансом и управленческим решением.';
  }
  function reportMatrix(title){
    const t = slideTitle(title).toLowerCase();
    let rows = null;
    if (/настоящая выручка|вариант\s*1/.test(t)) rows = ['ОПиУ: появляется выручка периода.', 'ОДДС: появляется операционное поступление.', 'Баланс: растут деньги; прибыль влияет на капитал.'];
    else if (/аванс|вариант\s*2/.test(t)) rows = ['ОПиУ: выручка признаётся только по мере оказания услуги.', 'ОДДС: деньги уже поступили.', 'Баланс: растут деньги и обязательство перед клиентом.'];
    else if (/кредит|вариант\s*3/.test(t)) rows = ['ОПиУ: выручки нет.', 'ОДДС: финансовое поступление.', 'Баланс: растут деньги и долг.'];
    else if (/дебитор|вариант\s*4/.test(t)) rows = ['ОПиУ: новой выручки нет, она была признана раньше.', 'ОДДС: поступают деньги.', 'Баланс: деньги растут, дебиторка уменьшается.'];
    else if (/вклад собственника|вариант\s*5/.test(t)) rows = ['ОПиУ: выручки нет.', 'ОДДС: финансовое поступление от собственника.', 'Баланс: растут деньги и капитал либо долг перед собственником.'];
    else if (/оборудован|актив/.test(t)) rows = ['ОПиУ: расход появляется постепенно через амортизацию.', 'ОДДС: деньги уходят сразу.', 'Баланс: появляется актив.'];
    if (!rows) return '';
    return `<div class="finance-impact-matrix-v63"><b>Влияние на отчёты</b>${rows.map(function(x){ return `<p>${e(clean(x))}</p>`; }).join('')}</div>`;
  }
  function lessonFinalTakeaways(les, parsed){
    const id = Number(les.id);
    const map = {
      1: [
        'Деньги на счёте показывают кассу, но не доказывают прибыль бизнеса.',
        'Поступление может быть выручкой, авансом, кредитом, возвратом дебиторки или вкладом собственника.',
        'Пять ситуаций нужно читать без разрыва: выручка → аванс клиента → кредит → возврат дебиторки → вклад собственника.',
        'Каждая ситуация по-разному влияет на ОПиУ, ОДДС и баланс, хотя банковский остаток увеличивается одинаково.'
      ],
      2: [
        'Финансовая карта связывает операции, деньги, остатки, метрики и решения.',
        'ОПиУ показывает прибыльность, ОДДС — движение денег, баланс — финансовое положение.',
        'Касса не объясняет авансы, дебиторку, активы, долги и обязательства.',
        'Один месяц бизнеса нужно читать через несколько экранов, а не через одну таблицу движения денег.'
      ],
      3: [
        'Бухгалтерский, налоговый и управленческий учёт решают разные задачи.',
        'Бухгалтерская прибыль может быть корректной, а кассовый риск — реальным.',
        'Управленческая система нужна, чтобы видеть прибыль, деньги, остатки, обязательства и решения.',
        'Спор “бухгалтер против собственника” заменяется связкой ОПиУ, ОДДС, баланса и платёжного календаря.'
      ],
      4: [
        'Одинаковая выручка не означает одинаковую экономику бизнеса.',
        'Услуги, торговля, производство, проекты, логистика и HoReCa имеют разные драйверы, маржу и ограничения.',
        'Экономический двигатель показывает, где бизнес создаёт деньги, где теряет маржу и где упирается в потолок.',
        'Перед ростом нужно понять ограничение модели: спрос, мощность, маржа, оборотный капитал, люди или качество.'
      ]
    };
    return map[id] || (parsed.finalLines && parsed.finalLines.length ? parsed.finalLines : [clean(les.result || '')]);
  }

  function coverHtml(les, parsed, total){
    const lines = parsed.intro.map(function(line){
      const m = clean(line).match(/^([^:]{3,80}):\s*(.*)$/);
      if (m) return `<div class="finance-cover-line-v63"><span>${e(m[1])}</span><p>${e(m[2])}</p></div>`;
      return `<div class="finance-cover-line-v63 wide"><p>${e(line)}</p></div>`;
    }).join('');
    return `${c('blue-card-v2 finance-lesson-cover-v63', `<p class="eyebrow">раздел 1 · урок ${Number(les.id)}</p><h1>${e(clean(les.title || ''))}</h1><p>Титульная структура урока перед основной последовательностью слайдов.</p>`)}${c('finance-cover-card-v63', `<h2>Структура урока</h2><div class="finance-cover-grid-v63">${lines}</div><div class="finance-screen-counter-v63">Экран 1 из ${total}</div>`)}`;
  }
  function slideHtml(les, slide, slideIndex, slidesTotal, screenIndex, total){
    const text = (slide.text || []).map(function(line){ return `<p>${e(clean(line))}</p>`; }).join('');
    const thought = firstSentence((slide.text || [])[0] || slideTitle(slide.title));
    return `${c('finance-slide-card-v63', `<p class="eyebrow">слайд ${slideIndex} из ${slidesTotal}</p><h1>${e(slideTitle(slide.title))}</h1><div class="finance-image-placeholder-v63"><div><b>Место для изображения</b><p>Изображение будет подключено после генерации визуалов.</p></div></div><div class="finance-learning-block-v63 main"><b>Главная мысль</b><p>${e(thought)}</p></div><div class="finance-slide-body-v63">${text || '<p>Текст слайда будет добавлен после редакторской проверки.</p>'}</div>${reportMatrix(slide.title)}<div class="finance-learning-block-v63 error"><b>Типовая ошибка</b><p>${e(slideError(slide))}</p></div><div class="finance-learning-block-v63 conclusion"><b>Управленческий вывод</b><p>${e(slideConclusion(slide))}</p></div><div class="finance-screen-counter-v63">Экран ${screenIndex + 1} из ${total}</div>`)}`;
  }
  function finalHtml(les, parsed, screenIndex, total){
    const items = lessonFinalTakeaways(les, parsed).map(function(line, idx){ return `<div><b>${String(idx + 1).padStart(2,'0')}</b><p>${e(clean(line))}</p></div>`; }).join('');
    const situations = Number(les.id) === 1 ? `<div class="finance-situation-flow-v63"><b>Пять ситуаций одной суммы</b><span>выручка</span><span>аванс клиента</span><span>кредит</span><span>возврат дебиторки</span><span>вклад собственника</span></div>` : '';
    return `${c('blue-card-v2 finance-final-card-v63', `<p class="eyebrow">итоговая логика</p><h1>Итоговая логика урока</h1><p>Финальный смысловой вывод без обращения к ученику.</p>`)}${c('', `${situations}<div class="finance-final-grid-v63">${items}</div><div class="finance-screen-counter-v63">Экран ${screenIndex + 1} из ${total}</div>`)}`;
  }
  function navHtml(les, index, total){
    const prevDisabled = index <= 0;
    const isLast = index >= total - 1;
    const prevAction = `renderFinanceLesson63(${Number(les.id)}, ${Math.max(0, index - 1)})`;
    const nextAction = isLast ? `renderFinanceSection63(1)` : `renderFinanceLesson63(${Number(les.id)}, ${index + 1})`;
    const nextText = isLast ? 'К урокам раздела' : 'Далее';
    return `<div class="nav-panel-v2 nav-panel-v2-three finance-lesson-nav-v63"><button class="btn secondary" onclick="renderFinanceSection63(1)">К разделу</button><button class="btn secondary" ${prevDisabled ? 'disabled' : ''} onclick="${prevAction}">Назад</button><button class="btn primary" onclick="${nextAction}">${nextText}</button></div>`;
  }

  function renderFinancialAssistant63(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения. Здесь отдельно собраны обучение, будущая аналитика бизнеса и готовые финансовые шаблоны.</p>`)}${c('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v63"><button class="finance-hub-card-v63 active" onclick="renderFinanceModuleHome63()"><b>Финансовый модуль</b><p>Разделы, уроки, диагностики и тренажёры по финансовой архитектуре бизнеса.</p><em>открыть</em></button><button class="finance-hub-card-v63" onclick="renderFinanceAnalytics63()"><b>Моя аналитика</b><p>Будущий контур личных показателей: факты, отчёты, динамика, риски и выводы.</p><em>каркас</em></button><button class="finance-hub-card-v63" onclick="renderFinanceTemplates63()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и инструментов: ОПиУ, ОДДС, баланс, календарь и прогнозы.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}`, 'finance');
  }
  function sectionCard(sec){
    const id = Number(sec.id);
    const open = canOpen(id);
    const lessonsCount = Array.isArray(sec.lessons) ? sec.lessons.length : 0;
    const blocks = id === 1 ? lessonsCount + 2 : lessonsCount;
    const action = open ? `renderFinanceSection63(${id})` : `financeLockedSectionNotice63(${id})`;
    return `<button type="button" class="finance-section-tile-v63 ${open ? 'open' : 'locked'}" onclick="${action}"><div class="finance-section-tile-head-v63"><span>Раздел ${id}</span><em>${e(statusText(id))}</em></div><h3>${e(clean(sec.title || ''))}</h3><p>${e(clean(sec.description || ''))}</p><div class="finance-section-tile-bottom-v63"><b>${blocks} блоков</b><span>${open ? 'Перейти в раздел →' : 'После диагностики предыдущего раздела'}</span></div></button>`;
  }
  function renderFinanceModuleHome63(){
    if (!requireAdmin()) return;
    const list = sections().map(sectionCard).join('');
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль показывает, как операции бизнеса превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника.</p><p class="small">Разделы идут последовательно: каждый следующий раздел открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>`)}${c('', `<div class="finance-toolbar-v63"><button class="btn secondary" onclick="renderFinancialAssistant63()">← К финансовому помощнику</button></div><h2>11 разделов</h2><p class="small">Нажимается вся карточка раздела. Раздел 1 доступен сразу.</p><div class="finance-section-list-v63">${list}</div>`)}`, 'finance');
  }
  function lessonRow(sec, les, idx){
    const secId = Number(sec.id), id = Number(les.id || idx + 1);
    const full = secId === 1 && id >= 1 && id <= 4 && Array.isArray(les.fullContent) && les.fullContent.length;
    const action = full ? `renderFinanceLesson63(${id},0)` : `renderFinanceLessonSummary63(${secId},${id})`;
    const label = full ? 'полный учебный экран' : 'структура урока из общей карты модуля';
    return `<button class="lesson-row-v2 finance-lesson-row-v63" onclick="${action}"><div><b>${String(id).padStart(2,'0')}. ${e(clean(les.title || 'Урок'))}</b><p>${e(label)}</p></div><span>→</span></button>`;
  }
  function sectionExtras(id){
    if (Number(id) !== 1) return `<button class="lesson-row-v2 locked" disabled><div><b>Итоговая диагностика раздела ${Number(id)}</b><p>будет добавлена после полного наполнения уроков раздела</p></div><span>•</span></button>`;
    return `<button class="lesson-row-v2 finance-test-row-v63" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает раздел 2</p></div><span>→</span></button><button class="lesson-row-v2 finance-trainer-row-v63" onclick="renderFinanceTrainerSection1_63()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
  }
  function renderFinanceSection63(id){
    if (!requireAdmin()) return;
    id = Number(id || 1);
    const sec = section(id);
    if (!sec) return renderFinanceModuleHome63();
    if (!canOpen(id)) return financeLockedSectionNotice63(id);
    const rows = (Array.isArray(sec.lessons) ? sec.lessons : []).map(function(les, idx){ return lessonRow(sec, les, idx); }).join('');
    s(`${c('blue-card-v2 finance-section-hero-v63', `<p class="eyebrow">раздел ${id}</p><h1>${e(clean(sec.title || ''))}</h1><p>${e(clean(sec.description || ''))}</p>${id === 1 ? progressBox() : ''}`)}${c('', `<div class="finance-toolbar-v63"><button class="btn secondary" onclick="renderFinanceModuleHome63()">← К разделам</button><button class="btn secondary" onclick="renderFinancialAssistant63()">К помощнику</button></div><h2>${id === 1 ? 'Уроки и закрепление раздела' : 'Уроки раздела'}</h2><p>${id === 1 ? 'Порядок работы: 4 урока → итоговая диагностика → финтренажёр для дополнительного закрепления.' : 'Материалы раздела будут наполняться после прохождения предыдущей диагностики.'}</p><div class="lesson-list-v2 finance-lesson-list-v63">${rows}${sectionExtras(id)}</div>`)}`, 'finance');
  }
  function renderFinanceLesson63(lessonId, screenIndex){
    if (!requireAdmin()) return;
    const sec = section(1);
    const les = lesson(sec, lessonId);
    if (!les) return renderFinanceSection63(1);
    const parsed = parseLesson(les);
    const screens = [{type:'cover'}].concat(parsed.slides.map(function(slide, idx){ return {type:'slide', slide:slide, idx:idx}; }));
    screens.push({type:'final'});
    const total = screens.length;
    const index = Math.max(0, Math.min(Number(screenIndex || 0), total - 1));
    const current = screens[index];
    let body = '';
    if (current.type === 'cover') body = coverHtml(les, parsed, total);
    if (current.type === 'slide') body = slideHtml(les, current.slide, current.idx + 1, parsed.slides.length, index, total);
    if (current.type === 'final') body = finalHtml(les, parsed, index, total);
    s(`${navHtml(les, index, total)}${body}`, 'finance');
  }
  function renderFinanceLessonSummary63(sectionId, lessonId){
    if (!requireAdmin()) return;
    const sec = section(sectionId), les = lesson(sec, lessonId);
    if (!sec || !les) return renderFinanceSection63(sectionId);
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">раздел ${Number(sectionId)} · урок ${Number(lessonId)}</p><h1>${e(clean(les.title || ''))}</h1><p>${e(clean(les.objective || ''))}</p>`)}${c('', `<div class="finance-toolbar-v63"><button class="btn secondary" onclick="renderFinanceSection63(${Number(sectionId)})">← К урокам раздела</button></div><div class="finance-summary-grid-v63"><div><span>Ключевое содержание</span><p>${e(clean(les.content || 'Будет раскрыто позже.'))}</p></div><div><span>Кейс</span><p>${e(clean(les.case || 'Будет раскрыто позже.'))}</p></div><div><span>Основной вывод</span><p>${e(clean(les.result || 'Будет раскрыто позже.'))}</p></div><div><span>Статус</span><p>Сейчас это структура урока. Полное раскрытие будет добавлено отдельным этапом.</p></div></div>`)}`, 'finance');
  }
  function renderFinanceTrainerSection1_63(){
    if (!requireAdmin()) return;
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V63 || window.FINANCE_SECTION1_TRAINER_URL_V62 || window.FINANCE_SECTION1_TRAINER_URL_V61 || window.FINANCE_SECTION1_TRAINER_URL_V60 || window.FINANCE_SECTION1_TRAINER_URL_V58 || window.FINANCE_SECTION1_TRAINER_URL_V57 || '').trim();
    const link = url && url !== '#' ? `<div class="finance-trainer-link-box-v57"><span>Ссылка на таблицу</span><p>${e(url)}</p></div><a class="btn primary" href="${e(url)}" target="_blank">Открыть таблицу</a>` : `<div class="finance-trainer-link-box-v57 muted"><span>Ссылка на таблицу</span><p>Ссылка будет подключена после создания финтренажёра.</p></div><button class="btn secondary" disabled>Ссылка будет добавлена</button>`;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}${c('', `<div class="finance-toolbar-v63"><button class="btn secondary" onclick="renderFinanceSection63(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями.</p></div><div><b>4. Пройти задания</b><p>Классифицируйте операции и проверьте влияние на ОПиУ, ОДДС и баланс.</p></div></div>${link}<button class="btn secondary" onclick="renderFinanceSection63(1)">← Вернуться к урокам раздела</button>`)}`, 'finance');
  }
  function renderFinanceAnalytics63(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Будущий контур личных показателей бизнеса.</p>`)}${c('', `<p>Раздел пока оставлен как каркас. Его нужно подключать после утверждения финансового ядра.</p><button class="btn secondary" onclick="renderFinancialAssistant63()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function renderFinanceTemplates63(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и инструментов.</p>`)}${c('', `<div class="list-clean"><div><b>ОПиУ</b><p>Шаблоны отчёта о прибыли и убытках.</p></div><div><b>ОДДС</b><p>Шаблоны движения денежных средств и платёжного календаря.</p></div><div><b>Баланс</b><p>Шаблоны остатков, активов и обязательств.</p></div></div><button class="btn secondary" onclick="renderFinancialAssistant63()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function financeLockedSectionNotice63(id){ alert('Раздел ' + Number(id || 2) + ' откроется после прохождения итоговой диагностики предыдущего раздела минимум на 80%.'); }

  Object.assign(window, {
    renderFinancialAssistant63, renderFinanceModuleHome63, renderFinanceSection63,
    renderFinanceLesson63, renderFinanceLessonSummary63, renderFinanceTrainerSection1_63,
    renderFinanceAnalytics63, renderFinanceTemplates63, financeLockedSectionNotice63
  });
  window.renderFinancialAssistant = renderFinancialAssistant63;
  window.renderFinancialAssistant62 = renderFinancialAssistant63;
  window.renderFinancialAssistant61 = renderFinancialAssistant63;
  window.renderFinancialAssistant60 = renderFinancialAssistant63;
  window.renderFinanceAnalytics62 = renderFinanceAnalytics63;
  window.renderFinanceTemplates62 = renderFinanceTemplates63;
  window.renderMyBusiness = renderFinancialAssistant63;
  window.openFinanceAssistantV63 = renderFinancialAssistant63;
  window.openFinanceAssistantV62 = renderFinancialAssistant63;
  window.openFinanceAssistantV61 = renderFinancialAssistant63;
  window.openFinanceAssistantV60 = renderFinancialAssistant63;
  window.openFinanceAssistantV57 = renderFinancialAssistant63;
  window.openFinanceAssistantV55 = renderFinancialAssistant63;
  window.openFinanceAssistantV54 = renderFinancialAssistant63;
  window.openFinanceAssistantV52 = renderFinancialAssistant63;
  window.openFinanceAssistantV50 = renderFinancialAssistant63;
  window.renderFinanceModuleHome = renderFinanceModuleHome63;
  window.renderFinanceModuleHome62 = renderFinanceModuleHome63;
  window.renderFinanceModuleHome61 = renderFinanceModuleHome63;
  window.renderFinanceModuleHome60 = renderFinanceModuleHome63;
  window.renderFinanceSection = renderFinanceSection63;
  window.renderFinanceSection62 = renderFinanceSection63;
  window.renderFinanceSection61 = renderFinanceSection63;
  window.renderFinanceSection60 = renderFinanceSection63;
  window.renderFinanceLesson = renderFinanceLesson63;
  window.renderFinanceLesson62 = renderFinanceLesson63;
  window.renderFinanceLesson61 = renderFinanceLesson63;
  window.renderFinanceLesson60 = renderFinanceLesson63;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1_63;
  window.renderFinanceSection1Trainer = renderFinanceTrainerSection1_63;
  window.financeLockedSectionNotice = financeLockedSectionNotice63;
  try { renderFinancialAssistant = renderFinancialAssistant63; } catch(e) {}
  try { renderMyBusiness = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV63 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV62 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV61 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV60 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV57 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV55 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV54 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV52 = renderFinancialAssistant63; } catch(e) {}
  try { openFinanceAssistantV50 = renderFinancialAssistant63; } catch(e) {}
  try { renderFinanceModuleHome = renderFinanceModuleHome63; } catch(e) {}
  try { renderFinanceSection = renderFinanceSection63; } catch(e) {}
  try { renderFinanceLesson = renderFinanceLesson63; } catch(e) {}
  try { financeLockedSectionNotice = financeLockedSectionNotice63; } catch(e) {}
})();


/* =====================================================
   v64 — final active finance route: no quick jump, no module map.
   Active route: Financial Assistant → Financial Module → Section → Lessons.
   ===================================================== */
(function installFinanceFinalActiveV64(){
  window.APP_UI_VERSION_V64 = 'v64-finance-route-simple-active-20260628';

  function c(cls, html){ return typeof card === 'function' ? card(cls, html) : `<section class="card-v2 ${cls||''}">${html}</section>`; }
  function s(html){ if (typeof shell === 'function') shell(html, 'finance'); }
  function esc64(value){ return typeof esc === 'function' ? esc(value) : String(value ?? '').replace(/[&<>'"]/g, function(ch){ return ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'})[ch]; }); }
  function clean64(value){
    ...
    out = out.replace(/предпринимателю/gi, 'предпринимателю');
    out = out.replace(/предпринимателем/gi, 'предпринимателем');
    out = out.replace(/у предприниматель(?=\s|,|\.|;|:|!|\?)/gi, 'у предпринимателя');
    out = out.replace(/Формируется понимание финансовые/gi, 'Формируется понимание финансовых');
    out = out.replace(/Формируется понимание основу/gi, 'Формируется понимание основы');
    out = out.replace(/Формируется понимание логистику/gi, 'Формируется понимание логистики');
    out = out.replace(/Формируется понимание бизнес как/gi, 'Формируется понимание бизнеса как');
    return out.replace(/\s+([,.!?;:])/g, '$1').trim();
  }
  function e(value){ return esc64(clean64(value)); }
  function isAdmin(){ try { return typeof isAdminMode === 'function' && isAdminMode() === true; } catch(err){ return false; } }
  ...
  function sections(){ return Array.isArray(window.FINANCE_MODULE_SECTIONS_DATA_V57) ? window.FINANCE_MODULE_SECTIONS_DATA_V57 : []; }
  function section(id){ return sections().find(function(sec){ return Number(sec.id) === Number(id); }) || null; }
  function userKey(){
    try { const ids = typeof possibleIds === 'function' ? possibleIds() : []; const names = typeof possibleUsernames === 'function' ? possibleUsernames() : []; return ids[0] || names[0] || 'local'; } catch(err){ return 'local'; }
  }
  function progressKey(){ return 'architecture_finance_module_progress_v49_' + userKey(); }
  function progress(){ try { return JSON.parse(localStorage.getItem(progressKey()) || '{}') || {}; } catch(err){ return {}; } }
  function passScore(){ const total = Number(progress().section1TestTotal || window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30); return Number(window.FINANCE_SECTION1_PASS_SCORE_V57 || Math.ceil(total * 0.8) || 24); }
  function sectionPassed(id){
    id = Number(id || 0);
    const p = progress();
    if (id === 1) return Boolean(p.section1TestPassed) || Number(p.section1TestScore || 0) >= passScore();
    return Boolean(p['section' + id + 'TestPassed']);
  }
  function canOpenSection(id){ id = Number(id || 0); if (id === 1) return true; return sectionPassed(id - 1); }
  function progressBox(){
    const p = progress();
    const total = Number(p.section1TestTotal || window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
    const score = Math.max(0, Math.min(total, Number(p.section1TestScore || 0)));
    ...
    const percent = total ? Math.round(score / total * 100) : 0;
    const passed = sectionPassed(1);
    return `<div class="finance-progress-box-v63 ${passed ? 'passed' : ''}"><span>Итоговая диагностика раздела 1</span><b>${score}/${total} · ${percent}%</b><p>${passed ? 'Раздел 2 открыт.' : 'Для открытия раздела 2 нужно минимум ' + pass + '/' + total + ' правильных ответов.'}</p></div>`;
  }
  function renderFinancialAssistant64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения. Здесь отдельно собраны учебный модуль, будущая личная аналитика и готовые финансовые шаблоны.</p>`)}${c('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v63"><button class="finance-hub-card-v62 active" onclick="renderFinanceModuleHome64()"><b>Финансовый модуль</b><p>Последовательная учебная программа по финансовой архитектуре бизнеса.</p><em>открыть</em></button><button class="finance-hub-card-v62" onclick="renderFinanceAnalytics64()"><b>Моя аналитика</b><p>Будущий контур личных показателей, отчётов, динамики, рисков и управленческих выводов.</p><em>каркас</em></button><button class="finance-hub-card-v62" onclick="renderFinanceTemplates64()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и инструментов: ОПиУ, ОДДС, баланс, календарь и прогнозы.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}`);
  }
  function sectionCard(sec){
    const id = Number(sec.id || 0);
    const open = canOpenSection(id);
    const lessonCount = Array.isArray(sec.lessons) ? sec.lessons.length : 0;
    const blocks = id === 1 ? lessonCount + 2 : lessonCount;
    const status = id === 1 ? 'доступно' : (open ? 'доступно' : 'закрыто');
    const action = open ? `renderFinanceSection64(${id})` : `financeLockedSectionNotice64(${id})`;
    return `<button class="finance-section-card-v63 ${open ? 'active' : 'locked'}" onclick="${action}"><div class="finance-section-card-number-v63">${String(id).padStart(2,'0')}</div><div><b>${e(sec.title || '')}</b><p>${e(sec.description || '')}</p><div class="finance-section-card-bottom-v63"><span>${blocks} блоков</span><em>${e(status)}</em></div></div></button>`;
  }
  function renderFinanceModuleHome64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль показывает, как операции бизнеса превращаются в выручку, прибыль, деньги, остатки, баланс, метрики и решения собственника.</p><p class="small">Разделы идут последовательно: следующий раздел открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button></div><h2>Разделы</h2><div class="finance-section-list-v63">${sections().map(sectionCard).join('')}</div>`)}`);
  }
  function lessonRow(sec, les, idx){
    const sectionId = Number(sec.id || 0), id = Number(les.id || idx + 1);
    const full = sectionId === 1 && id >= 1 && id <= 4;
    const action = full ? `renderFinanceLesson64(${id},0)` : `renderFinanceLessonSummary64(${sectionId},${id})`;
    return `<button class="lesson-row-v2 finance-lesson-row-v63" onclick="${action}"><div><b>${String(id).padStart(2,'0')}. ${e(les.title || 'Урок')}</b><p>${full ? 'полный учебный урок' : 'структура урока из общей карты модуля'}</p></div><span>→</span></button>`;
  }
  function extrasForSection(id){
    if (Number(id) !== 1) return `<button class="lesson-row-v2 locked" disabled><div><b>Итоговая диагностика раздела ${Number(id)}</b><p>будет добавлена после полного наполнения уроков раздела</p></div><span>•</span></button>`;
    return `<button class="lesson-row-v2 finance-test-row-v63" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает раздел 2</p></div><span>→</span></button><button class="lesson-row-v2 finance-trainer-row-v63" onclick="renderFinanceTrainerSection1_64()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
  }
  function renderFinanceSection64(id){
    if (!requireAdmin()) return;
    id = Number(id || 1);
    const sec = section(id);
    if (!sec) return renderFinanceModuleHome64();
    if (!canOpenSection(id)) return financeLockedSectionNotice64(id);
    const rows = (Array.isArray(sec.lessons) ? sec.lessons : []).map(function(les, idx){ return lessonRow(sec, les, idx); }).join('');
    s(`${c('blue-card-v2 finance-section-hero-v63', `<p class="eyebrow">раздел ${id}</p><h1>${e(sec.title || '')}</h1><p>${e(sec.description || '')}</p>${id === 1 ? progressBox() : '<p class="small">Структура раздела подготовлена. Полное наполнение будет добавляться отдельным этапом.</p>'}`)}${c('', `<div class="finance-toolbar-v49"><button class="btn secondary" onclick="renderFinanceModuleHome64()">← К разделам</button><button class="btn secondary" onclick="renderFinancialAssistant64()">К помощнику</button></div><h2>${id === 1 ? 'Уроки и закрепление раздела 1' : 'Уроки раздела'}</h2><div class="lesson-list-v2 finance-lesson-list-v63">${rows}${extrasForSection(id)}</div>`)}`);
  }
  function renderFinanceLesson64(id, screenIndex){
    if (typeof window.renderFinanceLesson63 === 'function') return window.renderFinanceLesson63(Number(id), Number(screenIndex || 0));
    if (typeof window.renderFinanceLessonV61 === 'function') return window.renderFinanceLessonV61(Number(id), Number(screenIndex || 0));
    alert('Полный экран урока не найден.');
  }
  function renderFinanceLessonSummary64(sectionId, lessonId){
    if (typeof window.renderFinanceLessonSummary63 === 'function') return window.renderFinanceLessonSummary63(Number(sectionId), Number(lessonId));
    if (typeof window.renderFinanceLessonSummaryV61 === 'function') return window.renderFinanceLessonSummaryV61(Number(sectionId), Number(lessonId));
    return renderFinanceSection64(sectionId);
  }
  function renderFinanceTrainerSection1_64(){
    if (typeof window.renderFinanceTrainerSection1_63 === 'function') return window.renderFinanceTrainerSection1_63();
    if (typeof window.renderFinanceSection1Trainer === 'function') return window.renderFinanceSection1Trainer();
  }
  function renderFinanceAnalytics64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Будущий контур личных показателей бизнеса.</p>`)}${c('', `<p>Раздел пока оставлен как каркас. Его нужно подключать после утверждения финансового ядра.</p><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button>`)}`);
  }
  function renderFinanceTemplates64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v63', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и инструментов.</p>`)}${c('', `<div class="list-clean"><div><b>ОПиУ</b><p>Шаблоны отчёта о прибыли и убытках.</p></div><div><b>ОДДС</b><p>Шаблоны движения денежных средств и платёжного календаря.</p></div><div><b>Баланс</b><p>Шаблоны остатков, активов и обязательств.</p></div></div><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button>`)}`);
  }
  function financeLockedSectionNotice64(id){
    id = Number(id || 2);
    alert('Раздел ' + id + ' откроется после прохождения итоговой диагностики раздела ' + Math.max(1, id - 1) + ' минимум на 80%.');
  }

  Object.assign(window, { renderFinancialAssistant64, renderFinanceModuleHome64, renderFinanceSection64, renderFinanceLesson64, renderFinanceLessonSummary64, renderFinanceTrainerSection1_64, renderFinanceAnalytics64, renderFinanceTemplates64, financeLockedSectionNotice64 });
  window.renderFinancialAssistant = renderFinancialAssistant64;
  window.renderMyBusiness = renderFinancialAssistant64;
  ['V64','V63','V62','V61','V60','V57','V55','V54','V52','V50'].forEach(function(suffix){ window['openFinanceAssistant' + suffix] = renderFinancialAssistant64; });
  window.renderFinanceModuleHome = renderFinanceModuleHome64;
  window.renderFinanceSection = renderFinanceSection64;
  window.renderFinanceLesson = renderFinanceLesson64;
  window.renderFinanceLessonSummary = renderFinanceLessonSummary64;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1_64;
  window.renderFinanceSection1Trainer = renderFinanceTrainerSection1_64;
  window.financeLockedSectionNotice = financeLockedSectionNotice64;
  try { renderFinancialAssistant = renderFinancialAssistant64; } catch(err) {}
  try { renderMyBusiness = renderFinancialAssistant64; } catch(err) {}
  try { renderFinanceModuleHome = renderFinanceModuleHome64; } catch(err) {}
  try { renderFinanceSection = renderFinanceSection64; } catch(err) {}
  try { renderFinanceLesson = renderFinanceLesson64; } catch(err) {}
  try { renderFinanceSection1Trainer = renderFinanceTrainerSection1_64; } catch(err) {}
})();

/* =====================================================
   v64 — Финансовый модуль: финальная чистка маршрута и экранов
   Без карты, без быстрого перехода, без экранов-глав, без повторного ОДДС.
   ===================================================== */
(function installFinanceCleanFinalV64(){
  window.APP_UI_VERSION_V64 = 'v64-finance-clean-final-20260628';
  const oldShellV64 = window.shell;
  const PASS = Number(window.FINANCE_SECTION1_PASS_SCORE_V57 || 24);
  const TOTAL = Number(window.FINANCE_SECTION1_TEST_TOTAL_V57 || 30);
  ...
  function c(cls, html){ return typeof card === 'function' ? card(cls || '', html || '') : `<section class="card-v2 ${cls || ''}">${html || ''}</section>`; }
  function s(html, active){ return typeof shell === 'function' ? shell(html, active || 'finance') : ((document.getElementById('app') || {}).innerHTML = html); }
  function adminMode(){ try { return typeof isAdminMode === 'function' && isAdminMode() === true; } catch(err){ return false; } }
  function requireAdmin(){
    if (adminMode()) return true;
    s(c('result-bad-v2 finance-locked-v64', `<p class="eyebrow">финансовый помощник</p><h1>Раздел закрыт</h1><p>Финансовый помощник пока закрыт для режима просмотра ученика. Проверка материалов доступна только в режиме администрирования.</p><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`), 'finance');
    return false;
  }
  function sections(){ return Array.isArray(window.FINANCE_MODULE_SECTIONS_DATA_V57) ? window.FINANCE_MODULE_SECTIONS_DATA_V57 : []; }
  function section(id){ return sections().find(function(x){ return Number(x.id) === Number(id); }) || null; }
  ...
  function progress(){
    ...
    try {
      ...
      for (let i=0;i<localStorage.length;i++){
        const key = localStorage.key(i) || '';
        if (key.indexOf('architecture_finance_module_progress_v49_') !== -1) best = JSON.parse(localStorage.getItem(key) || '{}');
      }
      return best;
    } catch(err){ return {}; }
  }
  ...
  function canOpen(id){ id = Number(id); return id <= 1 || sectionPassed(id - 1); }
  function statusText(id){ id = Number(id); return id === 1 ? 'доступно' : (canOpen(id) ? 'открыто' : 'закрыто до диагностики раздела ' + (id - 1)); }
  ...
  function clean(value){
    ...
    out = out.replace(/ОПУ/g, 'ОПиУ');
    out = out.replace(/предпринимателя/g, 'предпринимателя').replace(/Предпринимательа/g, 'Предпринимателя');
    out = out.replace(/предпринимателю/g, 'предпринимателю').replace(/Предпринимательу/g, 'Предпринимателю');
    out = out.replace(/предпринимателем/g, 'предпринимателем').replace(/Предпринимательом/g, 'Предпринимателем');
    out = out.replace(/у предприниматель(?=\s|,|\.|;|:|!|\?|$)/gi, function(m){ return m[0] === 'у' ? 'у предпринимателя' : 'У предпринимателя'; });
    out = out.replace(/для предприниматель(?=\s|,|\.|;|:|!|\?|$)/gi, 'для предпринимателя');
    out = out.replace(/к предприниматель(?=\s|,|\.|;|:|!|\?|$)/gi, 'к предпринимателю');
    out = out.replace(/с предприниматель(?=\s|,|\.|;|:|!|\?|$)/gi, 'с предпринимателем');
    out = out.replace(/Результат ученика\s*:/gi, 'Основной вывод:');
    out = out.replace(/Ученик\s+переста[её]т/gi, 'Предприниматель перестаёт');
    out = out.replace(/ученик\s+переста[её]т/gi, 'предприниматель перестаёт');
    out = out.replace(/Ученик\s+понимает/gi, 'Формируется понимание');
    out = out.replace(/ученик\s+понимает/gi, 'формируется понимание');
    out = out.replace(/Ученик\s+видит/gi, 'Формируется видение');
    out = out.replace(/ученик\s+видит/gi, 'формируется видение');
    out = out.replace(/Ученик\s+разбирает/gi, 'Разбирается');
    out = out.replace(/ученик\s+разбирает/gi, 'разбирается');
    out = out.replace(/Формируется понимание основу/g, 'Формируется понимание основы');
    out = out.replace(/Формируется понимание финансовые/g, 'Формируется понимание финансовых');
    out = out.replace(/Финальный смысловой вывод без обращения к ученику\.?/g, 'Короткая фиксация главной логики урока.');
    out = out.replace(/\s+([,.!?;:])/g, '$1');
    return out.trim();
  }
  function splitSentences(text){ return clean(text).split(/(?<=[.!?])\s+/).filter(Boolean); }
  function parseLesson(les){
    const raw = Array.isArray(les.fullContent) ? les.fullContent : [];
    const slides = [];
    let current = null;
    let mode = 'intro';
    raw.forEach(function(item){
      ...
      if (!line) return;
      const m = line.match(/^Слайд\s+(\d+)\.\s*(.+)$/i);
      if (m) { current = {num:Number(m[1]), title:m[2], text:[]}; slides.push(current); mode = 'after-title'; return; }
      if (!current) return;
      if (/^Что показать на слайде$/i.test(line)) { mode = 'visual'; return; }
      if (/^Текст под слайдом$/i.test(line)) { mode = 'text'; return; }
      if (mode === 'visual') return;
      if (/^Показать\s+/i.test(line) && mode !== 'text') return;
      current.text.push(line);
    });
    return slides;
  }
  function lessonFormat(les){
    const map = {
      1:'теоретическое объяснение, разбор финансовых понятий, сквозной кейс и сравнение влияния операции на ОПиУ, ОДДС и баланс.',
      2:'теоретическое объяснение, системная схема финансовой карты, сквозной кейс одного месяца и разбор через ОПиУ, ОДДС, баланс и метрики.',
      3:'теоретическое объяснение, сравнительные схемы, кейс конфликта отчётов и разбор различий между бухгалтерией, налоговым и управленческим учётом.',
      4:'теоретическое объяснение, отраслевые схемы, формулы драйверов, сравнительный кейс и диагностика экономического двигателя бизнеса.'
    };
    return map[Number(les.id)] || 'теоретическое объяснение, кейс и управленческий вывод.';
  }
  function lessonOutput(les){
    const map = {
      1:'Предприниматель перестаёт оценивать бизнес только по остатку денег и начинает разделять три финансовые реальности: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.',
      2:'Финансы бизнеса нельзя сводить к одной таблице кассы: каждая операция должна читаться через ОПиУ, ОДДС, баланс, метрики и управленческое решение.',
      3:'Бухгалтерский, налоговый и управленческий контуры не смешиваются: бухгалтерия отвечает за корректность, налоговый учёт — за обязательства, управленческий учёт — за решения собственника.',
      4:'Бизнес оценивается не только по выручке, а через экономический двигатель: драйверы выручки, маржу, мощность, денежный цикл, повторяемость и главное финансовое ограничение.'
    };
    return map[Number(les.id)] || clean(les.result || '');
  }
  function coverHtml(les, slides, total){
    const rows = [
      ['Количество слайдов', slides.length + '.'],
      ['Формат урока', lessonFormat(les)],
      ['Главная задача', clean(les.objective || '')],
      ['Основной вывод', lessonOutput(les)]
    ].map(function(row){ return `<div class="finance-cover-line-v64"><span>${e(row[0])}</span><p>${e(row[1])}</p></div>`; }).join('');
    return `${c('blue-card-v2 finance-lesson-cover-v64', `<p class="eyebrow">раздел 1 · урок ${Number(les.id)}</p><h1>${e(clean(les.title || ''))}</h1><p>Титульная структура урока перед основной последовательностью слайдов.</p>`)}${c('finance-cover-card-v64', `<h2>Структура урока</h2><div class="finance-cover-grid-v64">${rows}</div><div class="finance-screen-counter-v64">Экран 1 из ${total}</div>`)}`;
  }
  function firstSentence(text){ return splitSentences(text || '')[0] || ''; }
  function mainThought(les, slide){
    const t = clean((slide.title || '') + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (t.includes('аванс')) return 'Аванс улучшает деньги сегодня, но создаёт обязательство на будущее.';
    if (t.includes('кредит')) return 'Кредит увеличивает деньги, но не создаёт выручку и прибыль.';
    if (t.includes('дебитор')) return 'Дебиторка означает: выручка уже есть, но деньги ещё не пришли.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Покупка не всегда является расходом периода: иногда деньги превращаются в актив.';
    if (Number(les.id) === 4) return 'Экономический двигатель показывает, за счёт чего бизнес зарабатывает и где ограничен.';
    if (Number(les.id) === 3) return 'Бухгалтерия и управленческий учёт отвечают на разные вопросы.';
    if (Number(les.id) === 2) return 'Финансовая карта связывает операцию, деньги, остатки, отчёты, метрики и решение.';
    return firstSentence(slide.text && slide.text[0]) || 'Сначала определяется финансовый смысл операции, затем её влияние на отчёты.';
  }
  function slideError(les, slide){
    const t = clean((slide.title || '') + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (t.includes('аванс')) return 'Принять аванс за свободную прибыль и потратить деньги до оказания услуги.';
    if (t.includes('кредит')) return 'Записать кредит как доход и принять рост долга за рост бизнеса.';
    if (t.includes('дебитор')) return 'Посчитать возврат старого долга новой выручкой текущего месяца.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Списать крупную покупку в расход одного месяца и исказить прибыль.';
    if (Number(les.id) === 4) return 'Сравнивать разные бизнесы только по выручке и не видеть ограничение модели.';
    if (Number(les.id) === 3) return 'Спорить, какой отчёт правильный, вместо разделения задач отчётов.';
    return 'Сделать вывод по одной цифре и не проверить остальные финансовые экраны.';
  }
  function slideConclusion(les, slide){
    const t = clean((slide.title || '') + ' ' + (slide.text || []).join(' ')).toLowerCase();
    if (t.includes('аванс')) return 'Авансы нужно отдельно видеть в ОДДС и балансе: деньги пришли, но обязательство осталось.';
    if (t.includes('кредит')) return 'Кредит читается через финансовый поток, долг, проценты и будущую нагрузку, а не через выручку.';
    if (t.includes('дебитор')) return 'Дебиторку нужно контролировать как мост между заработанной выручкой и реальными деньгами.';
    if (t.includes('оборуд') || t.includes('актив')) return 'Крупные покупки нужно разделять на денежный платёж, актив баланса и будущую амортизацию.';
    if (Number(les.id) === 4) return 'Перед решением о росте нужно определить двигатель модели, главное ограничение и метрику контроля.';
    if (Number(les.id) === 3) return 'Управленческая система нужна, чтобы прибыль, деньги, остатки и риски объясняли ситуацию вместе.';
    if (Number(les.id) === 2) return 'Одна операция должна сохранять смысл в ОПиУ, ОДДС, балансе и метриках.';
    return 'Перед выводом нужно ответить: заработал ли бизнес, пришли ли деньги, что изменилось в активах и обязательствах.';
  }
  function paragraphs(lines){
    const cleanLines = (lines || []).map(clean).filter(Boolean);
    const situation = /^(Перв(ая|ый)|Втор(ая|ой)|Третья|Третий|Четв[её]рт(ая|ый)|Пят(ая|ый))\s+(ситуация|вариант)\s*[—-]/i;
    const parts = [];
    let buf = [];
    function flush(){
      if (!buf.length) return;
      parts.push(`<div class="finance-situation-list-v64">${buf.map(function(line){
        const m = line.match(/^([^—-]+)[—-]\s*(.*)$/);
        return m ? `<div><span>${e(m[1].trim())}</span><p>${e(m[2].trim())}</p></div>` : `<div><p>${e(line)}</p></div>`;
      }).join('')}</div>`);
      buf = [];
    }
    cleanLines.forEach(function(line){
      if (situation.test(line)) buf.push(line);
      else { flush(); parts.push(`<p>${e(line)}</p>`); }
    });
    flush();
    return parts.join('');
  }
  function matrix(slide){
    const t = clean((slide.title || '') + ' ' + (slide.text || []).join(' ')).toLowerCase();
    let rows = null;
    if (t.includes('кредит')) rows = ['ОПиУ: выручка не возникает. Проценты позже могут стать расходом.', 'ОДДС: финансовое поступление сейчас и будущие платежи по долгу.', 'Баланс: деньги увеличиваются, долг увеличивается.'];
    else if (t.includes('аванс') || t.includes('абонемент') || t.includes('сертификат') || t.includes('предоплат')) rows = ['ОПиУ: выручка признаётся по мере оказания услуги или поставки товара.', 'ОДДС: деньги поступили сейчас.', 'Баланс: деньги увеличиваются, обязательство перед клиентом увеличивается.'];
    else if (t.includes('дебитор')) rows = ['ОПиУ: выручка уже признана.', 'ОДДС: поступления пока нет; при оплате будет денежный приток.', 'Баланс: дебиторка увеличивается; при оплате деньги растут, дебиторка снижается.'];
    else if (t.includes('оборуд') || t.includes('актив') || t.includes('capex')) rows = ['ОПиУ: расход появляется постепенно через амортизацию.', 'ОДДС: инвестиционный платёж уменьшает деньги сразу.', 'Баланс: деньги уменьшаются, актив увеличивается.'];
    if (!rows) return '';
    return `<div class="finance-impact-matrix-v64">${rows.map(function(row){ const parts = row.split(':'); return `<div><span>${e(parts[0])}</span><p>${e(clean(parts.slice(1).join(':').trim()))}</p></div>`; }).join('')}</div>`;
  }
  function slideHtml(les, slide, idx, slidesTotal, screenIndex, total){
    return `${c('finance-slide-card-v64', `<p class="eyebrow">слайд ${idx} из ${slidesTotal}</p><h1>${e(clean(slide.title || ''))}</h1><div class="finance-image-placeholder-v64"><div><b>Место для изображения</b><p>Изображение будет подключено после генерации визуалов.</p></div></div><div class="finance-learning-block-v64 main"><b>Главная мысль</b><p>${e(mainThought(les, slide))}</p></div><div class="finance-slide-body-v64"><h3>Объяснение</h3>${paragraphs(slide.text)}</div><div class="finance-learning-block-v64 error"><b>Типовая ошибка</b><p>${e(slideError(les, slide))}</p></div><div class="finance-learning-block-v64 conclusion"><b>Управленческий вывод</b><p>${e(slideConclusion(les, slide))}</p></div>${matrix(slide)}<div class="finance-screen-counter-v64">Экран ${screenIndex + 1} из ${total}</div>`)}`;
  }
  function finalTakeaways(les){
    const map = {
      1:['Деньги на счёте показывают кассу, но не доказывают прибыль бизнеса.','Поступление может быть выручкой, авансом, кредитом, возвратом дебиторки или вкладом собственника.','Пять ситуаций читаются одной связкой: выручка → аванс клиента → кредит → возврат дебиторки → вклад собственника.','Каждая ситуация по-разному влияет на ОПиУ, ОДДС и баланс, хотя банковский остаток увеличивается одинаково.'],
      2:['Финансовая карта связывает операции, деньги, остатки, метрики и решения.','ОПиУ показывает прибыльность, ОДДС — движение денег, баланс — финансовое положение.','Касса не объясняет авансы, дебиторку, активы, долги и обязательства.','Один месяц бизнеса нужно читать через несколько экранов, а не через одну таблицу движения денег.'],
      3:['Бухгалтерский, налоговый и управленческий учёт решают разные задачи.','Бухгалтерская прибыль может быть корректной, а кассовый риск — реальным.','Управленческая система нужна, чтобы видеть прибыль, деньги, остатки, обязательства и решения.','Спор “бухгалтер против собственника” заменяется связкой ОПиУ, ОДДС, баланса и платёжного календаря.'],
      4:['Одинаковая выручка не означает одинаковую экономику бизнеса.','Услуги, торговля, производство, проекты, логистика и HoReCa имеют разные драйверы, маржу и ограничения.','Экономический двигатель показывает, где бизнес создаёт деньги, где теряет маржу и где упирается в потолок.','Перед ростом нужно понять ограничение модели: спрос, мощность, маржа, оборотный капитал, люди или качество.']
    };
    return map[Number(les.id)] || [lessonOutput(les)];
  }
  function finalHtml(les, screenIndex, total){
    const cards = finalTakeaways(les).map(function(line, i){ return `<div><b>${String(i+1).padStart(2,'0')}</b><p>${e(clean(line))}</p></div>`; }).join('');
    const situations = Number(les.id) === 1 ? `<div class="finance-situation-flow-v64"><b>Пять ситуаций одной суммы</b><span>выручка</span><span>аванс клиента</span><span>кредит</span><span>возврат дебиторки</span><span>вклад собственника</span></div>` : '';
    return `${c('blue-card-v2 finance-final-card-v64', `<p class="eyebrow">итоговая логика</p><h1>Итоговая логика урока</h1><p>Короткая фиксация главной логики урока.</p>`)}${c('', `${situations}<div class="finance-final-grid-v64">${cards}</div><div class="finance-screen-counter-v64">Экран ${screenIndex + 1} из ${total}</div>`)}`;
  }
  function nav(les, index, total){
    const last = index >= total - 1;
    return `<div class="nav-panel-v2 nav-panel-v2-three finance-lesson-nav-v64"><button class="btn secondary" onclick="renderFinanceSection64(1)">К разделу</button><button class="btn secondary" ${index<=0?'disabled':''} onclick="renderFinanceLesson64(${Number(les.id)},${Math.max(0,index-1)})">Назад</button><button class="btn primary" onclick="${last ? 'renderFinanceSection64(1)' : `renderFinanceLesson64(${Number(les.id)},${index+1})`}">${last ? 'К урокам раздела' : 'Далее'}</button></div>`;
  }
  function renderFinanceLesson64(lessonId, screenIndex){
    if (!requireAdmin()) return;
    const sec = section(1), les = lesson(sec, lessonId);
    if (!les) return renderFinanceSection64(1);
    const slides = parseLesson(les);
    const total = slides.length + 2;
    const index = Math.max(0, Math.min(Number(screenIndex || 0), total - 1));
    let body = '';
    if (index === 0) body = coverHtml(les, slides, total);
    else if (index === total - 1) body = finalHtml(les, index, total);
    else body = slideHtml(les, slides[index-1], index, slides.length, index, total);
    s(nav(les,index,total) + body, 'finance');
  }
  function progressBox(){
    const p = progress();
    const score = Math.max(0, Number(p.section1TestScore || 0));
    const total = Math.max(1, Number(p.section1TestTotal || TOTAL));
    const pct = Math.max(0, Math.min(100, Math.round(score / total * 100)));
    const passed = sectionPassed(1);
    return `<div class="finance-progress-v64 ${passed?'passed':'locked'}"><div class="finance-progress-head-v64"><span>Итоговая диагностика раздела 1</span><b>${passed ? 'засчитана' : pct + '%'}</b></div><div class="progress-line"><div style="width:${pct}%"></div></div><p>${score}/${total} правильных ответов. Для открытия раздела 2 нужно ${PASS}/${TOTAL} правильных ответов.</p></div>`;
  }
  function renderFinancialAssistant64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v64', `<p class="eyebrow">финансовый помощник</p><h1>Финансовый помощник</h1><p>Рабочая зона финансового контура приложения. Здесь отдельно собраны обучение, будущая аналитика бизнеса и готовые финансовые шаблоны.</p>`)}${c('', `<h2>Выберите блок</h2><div class="finance-hub-grid-v64"><button class="finance-hub-card-v64 active" onclick="renderFinanceModuleHome64()"><b>Финансовый модуль</b><p>Разделы, уроки, диагностики и тренажёры по финансовой архитектуре бизнеса.</p><em>открыть</em></button><button class="finance-hub-card-v64" onclick="renderFinanceAnalytics64()"><b>Моя аналитика</b><p>Будущий контур личных показателей: факты, отчёты, динамика, риски и выводы.</p><em>каркас</em></button><button class="finance-hub-card-v64" onclick="renderFinanceTemplates64()"><b>Готовые шаблоны</b><p>Будущая библиотека таблиц и инструментов: ОПиУ, ОДДС, баланс, календарь и прогнозы.</p><em>готовится</em></button></div><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>`)}`, 'finance');
  }
  function sectionCard(sec){
    const id = Number(sec.id), open = canOpen(id);
    const blocks = id === 1 ? 6 : (Array.isArray(sec.lessons) ? sec.lessons.length : 0);
    const action = open ? `renderFinanceSection64(${id})` : `financeLockedSectionNotice64(${id})`;
    return `<button type="button" class="finance-section-card-v64 ${open?'open':'locked'}" onclick="${action}"><div class="finance-section-card-head-v64"><span>Раздел ${id}</span><em>${e(statusText(id))}</em></div><h3>${e(clean(sec.title || ''))}</h3><p>${e(clean(sec.description || ''))}</p><div class="finance-section-card-bottom-v64"><b>${blocks} ${id===1?'блоков':'уроков'}</b><span>${open ? '→' : '•'}</span></div></button>`;
  }
  function renderFinanceModuleHome64(){
    if (!requireAdmin()) return;
    s(`${c('blue-card-v2 finance-hero-v64', `<p class="eyebrow">финансовый модуль</p><h1>Финансовая архитектура бизнеса</h1><p>Модуль выстроен как последовательная лестница: сначала различаются деньги, выручка и прибыль, затем собираются ОПиУ, ОДДС, баланс, ликвидность, метрики, отраслевые финансы и регулярная система управления.</p><p class="small">Внутри финансового модуля каждый следующий раздел открывается после итоговой диагностики предыдущего раздела минимум на 80%.</p>`)}${c('', `<div class="finance-toolbar-v64"><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button></div><h2>11 разделов</h2><p class="small">Нажимается вся карточка раздела. Раздел 1 доступен сразу.</p><div class="finance-section-list-v64">${sections().map(sectionCard).join('')}</div>`)}`, 'finance');
  }
  function renderFinanceSection64(id){
    if (!requireAdmin()) return;
    id = Number(id || 1);
    const sec = section(id);
    if (!sec) return renderFinanceModuleHome64();
    if (!canOpen(id)) return financeLockedSectionNotice64(id);
    if (id !== 1) {
      const rows = (Array.isArray(sec.lessons) ? sec.lessons : []).map(function(les, idx){ return `<button class="lesson-row-v2 locked" disabled><div><b>${String(Number(les.id || idx+1)).padStart(2,'0')}. ${e(clean(les.title || 'Урок'))}</b><p>полное наполнение будет добавлено позже</p></div><span>•</span></button>`; }).join('');
      s(`${c('blue-card-v2 finance-section-hero-v64', `<p class="eyebrow">раздел ${id}</p><h1>${e(clean(sec.title || ''))}</h1><p>${e(clean(sec.description || ''))}</p>`)}${c('', `<div class="finance-toolbar-v64"><button class="btn secondary" onclick="renderFinanceModuleHome64()">← К разделам</button></div><h2>Уроки раздела</h2><div class="lesson-list-v2 finance-lesson-list-v64">${rows}</div>`)}`, 'finance');
      return;
    }
    const rows = (Array.isArray(sec.lessons) ? sec.lessons.slice(0,4) : []).map(function(les, idx){ return `<button class="lesson-row-v2 finance-lesson-row-v64" onclick="renderFinanceLesson64(${Number(les.id || idx+1)},0)"><div><b>${String(Number(les.id || idx+1)).padStart(2,'0')}. ${e(clean(les.title || 'Урок'))}</b><p>полный учебный экран</p></div><span>→</span></button>`; }).join('');
    const extras = `<button class="lesson-row-v2 finance-test-row-v64" onclick="startFinanceSection1Test(true)"><div><b>Итоговая диагностика раздела 1</b><p>30 вопросов · проходной уровень 80% · открывает раздел 2</p></div><span>→</span></button><button class="lesson-row-v2 finance-trainer-row-v64" onclick="renderFinanceTrainerSection1_64()"><div><b>Финтренажёр раздела 1</b><p>Дополнительное закрепление через таблицу: операции, ОПиУ, ОДДС, баланс и управленческий вывод.</p></div><span>→</span></button>`;
    s(`${c('blue-card-v2 finance-section-hero-v64', `<p class="eyebrow">раздел 1</p><h1>${e(clean(sec.title || ''))}</h1><p>${e(clean(sec.description || ''))}</p>${progressBox()}`)}${c('', `<div class="finance-toolbar-v64"><button class="btn secondary" onclick="renderFinanceModuleHome64()">← К разделам</button><button class="btn secondary" onclick="renderFinancialAssistant64()">К помощнику</button></div><h2>Уроки и закрепление раздела</h2><p>Порядок работы: 4 урока → итоговая диагностика → финтренажёр для дополнительного закрепления.</p><div class="lesson-list-v2 finance-lesson-list-v64">${rows}${extras}</div>`)}`, 'finance');
  }
  function renderFinanceTrainerSection1_64(){
    if (!requireAdmin()) return;
    const url = String(window.FINANCE_SECTION1_TRAINER_URL_V64 || window.FINANCE_SECTION1_TRAINER_URL_V63 || window.FINANCE_SECTION1_TRAINER_URL_V62 || window.FINANCE_SECTION1_TRAINER_URL_V58 || '').trim();
    const link = url && url !== '#' ? `<a class="btn primary" href="${e(url)}" target="_blank">Открыть таблицу</a>` : `<button class="btn secondary" disabled>Ссылка будет добавлена</button>`;
    s(`${c('blue-card-v2 finance-hero-v64', `<p class="eyebrow">раздел 1 · практика</p><h1>Финтренажёр раздела 1</h1><p>Дополнительное закрепление: операция → финансовый смысл → ОПиУ → ОДДС → баланс → управленческий вывод.</p>`)}${c('', `<div class="finance-toolbar-v64"><button class="btn secondary" onclick="renderFinanceSection64(1)">← К разделу 1</button></div><h2>Как проходить</h2><div class="list-clean"><div><b>1. Открыть таблицу</b><p>Перейдите по ссылке на Google Sheets / Excel-шаблон финтренажёра.</p></div><div><b>2. Создать копию</b><p>Скопируйте таблицу себе, чтобы ответы и расчёты сохранялись в личной версии файла.</p></div><div><b>3. Прочитать инструкцию внутри</b><p>Внутри таблицы будет отдельный лист с правилами прохождения и пояснениями.</p></div><div><b>4. Пройти задания</b><p>Классифицируйте операции и проверьте влияние на ОПиУ, ОДДС и баланс.</p></div></div>${link}<button class="btn secondary" onclick="renderFinanceSection64(1)">← Вернуться к урокам раздела</button>`)}`, 'finance');
  }
  function renderFinanceAnalytics64(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v64', `<p class="eyebrow">моя аналитика</p><h1>Моя аналитика</h1><p>Будущий контур личных показателей бизнеса.</p>`)}${c('', `<p>Раздел пока оставлен как каркас. Его нужно подключать после утверждения финансового ядра.</p><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function renderFinanceTemplates64(){ if (!requireAdmin()) return; s(`${c('blue-card-v2 finance-hero-v64', `<p class="eyebrow">готовые шаблоны</p><h1>Готовые шаблоны</h1><p>Будущая библиотека таблиц и инструментов.</p>`)}${c('', `<div class="list-clean"><div><b>ОПиУ</b><p>Шаблоны отчёта о прибыли и убытках.</p></div><div><b>ОДДС</b><p>Шаблоны движения денежных средств и платёжного календаря.</p></div><div><b>Баланс</b><p>Шаблоны остатков, активов и обязательств.</p></div></div><button class="btn secondary" onclick="renderFinancialAssistant64()">← К финансовому помощнику</button>`)}`, 'finance'); }
  function financeLockedSectionNotice64(id){ alert('Раздел ' + Number(id || 2) + ' откроется после прохождения итоговой диагностики предыдущего раздела минимум на 80%.'); }
  function cleanDom(){
    try {
      document.querySelectorAll('.finance-quick-open-v59,.finance-quick-open-v62,.finance-module-map-v62,.finance-chapter-card-v56').forEach(function(el){ el.remove(); });
      const root = document.getElementById('app');
      if (!root) return;
      const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null);
      const nodes = [];
      while (walker.nextNode()) nodes.push(walker.currentNode);
      nodes.forEach(function(n){ const next = clean(n.nodeValue); if (next && next !== n.nodeValue) n.nodeValue = next; });
    } catch(err) {}
  }
  if (typeof oldShellV64 === 'function' && !window.__financeShellV64) {
    window.__financeShellV64 = true;
    window.shell = function(content, activeTab){
      const result = oldShellV64.call(this, content, activeTab);
      [0,120,260,520,900].forEach(function(ms){ setTimeout(cleanDom, ms); });
      return result;
    };
    try { shell = window.shell; } catch(err) {}
  }
  Object.assign(window, { renderFinancialAssistant64, renderFinanceModuleHome64, renderFinanceSection64, renderFinanceLesson64, renderFinanceTrainerSection1_64, renderFinanceAnalytics64, renderFinanceTemplates64, financeLockedSectionNotice64, financeCleanDomV64:cleanDom });
  window.renderFinancialAssistant = renderFinancialAssistant64;
  window.renderMyBusiness = renderFinancialAssistant64;
  window.openFinanceAssistantV64 = renderFinancialAssistant64;
  window.openFinanceAssistantV63 = renderFinancialAssistant64;
  window.openFinanceAssistantV62 = renderFinancialAssistant64;
  window.openFinanceAssistantV61 = renderFinancialAssistant64;
  window.openFinanceAssistantV60 = renderFinancialAssistant64;
  window.openFinanceAssistantV57 = renderFinancialAssistant64;
  window.openFinanceAssistantV55 = renderFinancialAssistant64;
  window.openFinanceAssistantV54 = renderFinancialAssistant64;
  window.openFinanceAssistantV52 = renderFinancialAssistant64;
  window.openFinanceAssistantV50 = renderFinancialAssistant64;
  window.renderFinanceModuleHome = renderFinanceModuleHome64;
  window.renderFinanceModuleHome63 = renderFinanceModuleHome64;
  window.renderFinanceSection = renderFinanceSection64;
  window.renderFinanceSection63 = renderFinanceSection64;
  window.renderFinanceLesson = renderFinanceLesson64;
  window.renderFinanceLesson63 = renderFinanceLesson64;
  window.renderFinanceTrainerSection1 = renderFinanceTrainerSection1_64;
  window.renderFinanceSection1Trainer = renderFinanceTrainerSection1_64;
  window.renderFinanceAnalytics = renderFinanceAnalytics64;
  window.renderFinanceTemplates = renderFinanceTemplates64;
  window.financeLockedSectionNotice = financeLockedSectionNotice64;
  try { renderFinancialAssistant = renderFinancialAssistant64; } catch(err) {}
  try { renderMyBusiness = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV64 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV63 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV62 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV61 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV60 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV57 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV55 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV54 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV52 = renderFinancialAssistant64; } catch(err) {}
  try { openFinanceAssistantV50 = renderFinancialAssistant64; } catch(err) {}
  try { renderFinanceModuleHome = renderFinanceModuleHome64; } catch(err) {}
  try { renderFinanceSection = renderFinanceSection64; } catch(err) {}
  try { renderFinanceLesson = renderFinanceLesson64; } catch(err) {}
  try { financeLockedSectionNotice = financeLockedSectionNotice64; } catch(err) {}
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(cleanDom, 400); });
})();


/* =====================================================
   v65 — вход по умолчанию как ученик + финальная защита от автоперехода в администрирование
   ===================================================== */
(function installDefaultStudentModeV65(){
  window.APP_UI_VERSION_V65 = 'v65-default-student-mode-finance-images-20260628';

  function adminModeActiveV65(){
    try { return typeof isAdminMode === 'function' && isAdminMode() === true; }
    catch(e){ return false; }
  }

  // Новый вход в приложение всегда начинается с просмотра как ученик.
  // Администратор включает администрирование вручную через профиль/боковое меню.
  try {
    if (typeof state !== 'undefined') state.appMode = 'student';
    localStorage.setItem('lego_app_mode', 'student');
  } catch(e) {}

  // Старые слои v50/v52 не имеют права переключать режим автоматически.
  window.ensureFinanceAdminModeV50 = function(){ return adminModeActiveV65(); };
  window.forceAdminModeV52 = function(){ return adminModeActiveV65(); };
  window.financeAdminCanOpenV50 = adminModeActiveV65;
  window.financeAdminCanOpenV52 = adminModeActiveV65;
  window.financeAdminModeCanOpenV54 = adminModeActiveV65;

  // Чистка текстовых артефактов после отрисовки.
  function cleanTextV65(value){
    let out = String(value || '');
    out = out.replace(/почему предприниматель ошибается/gi, 'почему предприниматель ошибается');
    out = out.replace(/предпринимателя/gi, 'предпринимателя');
    out = out.replace(/предпринимателю/gi, 'предпринимателю');
    out = out.replace(/предпринимательом/gi, 'предпринимателем');
    out = out.replace(/у предприниматель(?=\s|,|\.|;|:|!|\?)/gi, 'у предпринимателя');
    out = out.replace(/ОПУ/g, 'ОПиУ');
    out = out.replace(/О+\s*ДДС/g, 'ОДДС');
    out = out.replace(/ОО+ДДС/g, 'ОДДС');
    out = out.replace(/предпринимателя должен/gi, 'предприниматель должен');
    out = out.replace(/предпринимателя может/gi, 'предприниматель может');
    out = out.replace(/предпринимателя начинает/gi, 'предприниматель начинает');
    out = out.replace(/почему предпринимателя ошибается/gi, 'почему предприниматель ошибается');
    return out;
  }

  function cleanDomV65(){
    try {
      const root = document.getElementById('app');
      if (!root) return;
      const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null);
      ...
      nodes.forEach(function(n){
        const next = cleanTextV65(n.nodeValue);
        if (next !== n.nodeValue) n.nodeValue = next;
      });
    } catch(e) {}
  }

  const oldShellV65 = window.shell;
  if (typeof oldShellV65 === 'function' && !window.__financeShellV65) {
    window.__financeShellV65 = true;
    window.shell = function(content, activeTab){
      const result = oldShellV65.call(this, content, activeTab);
      [0,80,200,500].forEach(function(ms){ setTimeout(cleanDomV65, ms); });
      return result;
    };
    try { shell = window.shell; } catch(e) {}
  }

  // Финансовый помощник открывается только если режим администрирования уже включён вручную.
  const financeRender = window.renderFinancialAssistant64 || window.renderFinancialAssistant63 || window.renderFinancialAssistant;
  window.openFinanceAssistantV65 = function(){
    if (!adminModeActiveV65()) {
      if (typeof shell === 'function' && typeof card === 'function') {
        shell(card('result-bad-v2 finance-locked-v65', '<p class="eyebrow">финансовый помощник</p><h1>Раздел закрыт</h1><p>Финансовый помощник пока закрыт в режиме просмотра ученика. Для проверки материалов нужно вручную включить режим администрирования.</p><button class="btn secondary" onclick="renderHome()">← Вернуться на главную</button>'), 'finance');
      } else {
        alert('Финансовый помощник пока закрыт в режиме просмотра ученика. Включите режим администрирования вручную.');
      }
      return;
    }
    return typeof financeRender === 'function' ? financeRender() : null;
  };

  window.renderFinancialAssistant = window.openFinanceAssistantV65;
  window.renderMyBusiness = window.openFinanceAssistantV65;
  ['V64','V63','V62','V61','V60','V57','V55','V54','V52','V50'].forEach(function(suffix){
    window['openFinanceAssistant' + suffix] = window.openFinanceAssistantV65;
  });
  try { renderFinancialAssistant = window.openFinanceAssistantV65; } catch(e) {}
  try { renderMyBusiness = window.openFinanceAssistantV65; } catch(e) {}

  window.financeSection1ImageCountV65 = {
    lesson1: 18,
    lesson2: 22,
    lesson3: 22,
    lesson4: 32,
    lessonSlidesTotal: 94,
    optionalTitleScreens: 4,
    optionalFinalScreens: 4
  };
})();

/* v66 final marker */
window.APP_UI_VERSION_V66 = 'v66-student-first-exact-finance-image-plan-20260628';
try { window.financeSection1ImageCountV66 = window.financeSection1ImageCountV65; } catch(e) {}
...
/* =====================================================
   v68 — вход всегда в режиме ученика
   Администратор включается только вручную через меню.
   ===================================================== */
(function installDefaultStudentEntryV68(){
  window.APP_UI_VERSION_V68 = 'v68-default-student-entry-section1-image-count-20260628';
  try { localStorage.setItem('lego_app_mode', 'student'); } catch(e) {}
})();


/* =====================================================
   v68 — контрольный план изображений Раздела 1
   ===================================================== */
(function installFinanceImagePlanV68(){
  window.financeSection1ImagePlanV68 = {
    lesson1: { title: 'Деньги, прибыль и выручка: почему предприниматель ошибается', slides: 18 },
    lesson2: { title: 'Финансовая карта бизнеса', slides: 22 },
    lesson3: { title: 'Управленческий учёт против бухгалтерии', slides: 22 },
    lesson4: { title: 'Экономика бизнес-модели', slides: 32 },
    requiredSlideImages: 94,
    optionalTitleImages: 4,
    optionalFinalImages: 4,
    note: 'Обязательные изображения считаются по слайдам уроков. Титульные и итоговые экраны можно оставить текстовыми либо оформить позже как отдельные обложки.'
  };
})();


/* =====================================================
   v68 — финальная фиксация входа и счётчик изображений Раздела 1
   ===================================================== */
(function installStudentFirstAndImageCountV68(){
  window.APP_UI_VERSION_V68 = 'v68-student-first-fix-section1-image-count-20260628';
  // Новый запуск всегда начинается как просмотр приложения учеником.
  try {
    if (typeof state !== 'undefined') state.appMode = 'student';
    localStorage.setItem('lego_app_mode', 'student');
  } catch(e) {}
  window.financeSection1ImageCountV68 = {
    lesson1: 18,
    lesson2: 22,
    lesson3: 22,
    lesson4: 32,
    requiredSlideImages: 94,
    optionalLessonCoverImages: 4,
    optionalSectionCoverImage: 1,
    optionalTotalWithLessonCovers: 98,
    optionalTotalWithSectionAndLessonCovers: 99
  };
})();
