...
--
-- This migration is deliberately additive. It does not change the locked
-- Telegram-code issuer foundation, public.users, existing entitlements, or
-- any production data. The private worker resolves the trusted Main user only
-- through the separate service-only resolver migration at processing time;
...
ALTER FUNCTION public.architecture_set_finance_access_desired_internal(uuid, uuid, bytea, text, text, text)
OWNER TO postgres;
...
ALTER FUNCTION public.architecture_claim_finance_access_outbox_internal(uuid, text, integer)
OWNER TO postgres;
...
BEGIN
  ...
  FOREACH v_function IN ARRAY ARRAY[
    'public.architecture_set_finance_access_desired_internal(uuid,uuid,bytea,text,text,text)'::regprocedure,
    'public.architecture_claim_finance_access_outbox_internal(uuid,text,integer)'::regprocedure,
...
REVOKE ALL ON FUNCTION public.architecture_set_finance_access_desired_internal(uuid, uuid, bytea, text, text, text)
FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.architecture_claim_finance_access_outbox_internal(uuid, text, integer)
FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.architecture_finish_finance_access_outbox_internal(uuid, uuid, text, text)
FROM PUBLIC, anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.architecture_set_finance_access_desired_internal(uuid, uuid, bytea, text, text, text)
TO service_role;
GRANT EXECUTE ON FUNCTION public.architecture_claim_finance_access_outbox_internal(uuid, text, integer)
TO service_role;
...
BEGIN
  ...
  IF EXISTS (
    WITH expected(function_name, identity_arguments, grantee_name, privilege_type, is_grantable) AS (
      VALUES
        ('architecture_set_finance_access_desired_internal', 'uuid, uuid, bytea, text, text, text', 'service_role', 'EXECUTE', false),
        ('architecture_claim_finance_access_outbox_internal', 'uuid, text, integer', 'service_role', 'EXECUTE', false),
