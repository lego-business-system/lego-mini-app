...
Браузерная реализация находится в [`../../finance-pilot/architecture-finance.js`](../../finance-pilot/architecture-finance.js) и публикуется только через отдельную staging-only сборку [`../../scripts/build-finance-pilot.mjs`](../../scripts/build-finance-pilot.mjs). Artifact не содержит полный Main `app.js`, forum/business assets или production URL. Он хранит `initData`, UUID и показанный код только в памяти, отправляет запрос без cookie/Authorization, повторяет те же байты не дольше 55 секунд и очищает код при уходе с экрана. Production root entrypoint и его `index.html` пилотом не изменяются.
...
`finance-sync-entitlements` — приватный, выключенный по умолчанию worker. Он принимает только точный `POST {}` для ограниченного batch либо canonical `POST {"event_id":"<UUIDv4>"}` для адресной доставки одного операторского события. Browser `Origin`, cookie и Authorization запрещены; обязателен отдельный заголовок `x-architecture-sync-trigger`. Этот trigger secret не используется для HMAC-запроса в Finance и не должен совпадать ни с одним другим секретом.
...
Outbox не хранит raw Telegram ID. Setter требует optimistic `expected_version`, отдельный STABLE status RPC позволяет проверить outcome без повторной mutation. После outbox-миграции service role не может обойти аудит прямым вызовом legacy entitlement upsert. Финансовые записи Finance при revoke не удаляются: закрываются доступ, активные коды и устройства. Все worker/endpoint gates в примерах имеют значение `disabled`.
