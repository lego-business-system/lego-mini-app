...
Локальные тесты проверяют неизменность и структуру SQL, включая adversarial-подмены, но не считаются доказательством исполнения PostgreSQL. Первый реальный запуск допускается только в одноразовом main staging на той же major-версии PostgreSQL; успешный commit транзакции и повторный catalog audit — отдельный go/no-go. Retention request/replay rows остаётся нерешённым продуктовым вопросом в runbook и этой миграцией не задаётся.
