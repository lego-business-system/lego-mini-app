...
function parseTargetEvent(body: Uint8Array): string | null {
  const source = decoder.decode(body);
  if (source === "{}") return null;
  const match = /^\{"event_id":"([0-9a-f-]+)"\}$/u.exec(source);
...
export async function handleFinanceSyncRequest(request: Request): Promise<Response> {
  try {
    ...
    const targetEventId = parseTargetEvent(body);
    ...
    const maximumEvents = targetEventId === null ? config.batchSize : 1;
    for (let index = 0; index < maximumEvents; index += 1) {
      timeoutWithinDeadline(config.totalDeadline, 250);
      const claimToken = crypto.randomUUID();
      const event = await claimEvent(config, claimToken, targetEventId);
      if (event === null) break;
      counts.claimed += 1;
