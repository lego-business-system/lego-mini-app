...
Foundation behavior smoke выполняется сразу после foundation и до outbox:
только в этой точке service-role legacy upsert ещё является частью старого
контракта. Затем outbox отзывает этот ACL, и финальный combined smoke отдельно
доказывает, что service role больше не может обойти desired-state путь.
...
- desired-state/outbox: доверенный Main user, последовательные версии,
  optimistic expected-version, отдельный STABLE status, идемпотентный
  set/targeted-claim/finish, lease, retry/backoff и `dead_letter`;
- resolver: точное преобразование подтверждённого `bigint` Telegram ID в строку
  без передачи идентификатора в outbox;
- отсутствие прямого доступа к таблицам у `service_role`, запрет обхода outbox
  через legacy upsert и отказ вызова RPC у `authenticated`;
