...
BEGIN
  ...
  IF EXISTS (
    WITH expected(function_name, identity_arguments, grantee_name, grantor_name, privilege_type, is_grantable) AS (
      VALUES
        ('architecture_set_finance_access_desired_internal', 'uuid, uuid, bytea, text, text, text', 'service_role', 'postgres', 'EXECUTE', false),
        ('architecture_claim_finance_access_outbox_internal', 'uuid, text, integer', 'service_role', 'postgres', 'EXECUTE', false),
