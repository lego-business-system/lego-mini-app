...
test("entitlement outbox is a separate pinned additive migration", () => {
  ...
  assert.match(outboxMigration, /p_expected_version bigint/);
  assert.match(outboxMigration, /v_desired\.version IS DISTINCT FROM p_expected_version[\s\S]*?version_conflict/);
  assert.match(outboxMigration, /CREATE FUNCTION public\.architecture_get_finance_access_status_internal\([\s\S]*?STABLE/);
  assert.match(outboxMigration, /p_event_id IS NULL OR candidate\.event_id = p_event_id/);
  ...
  assert.match(outboxMigration, /REVOKE ALL ON FUNCTION public\.architecture_upsert_product_entitlement_internal\([\s\S]*?FROM service_role/);
});
...
test("subject resolver is service-only and preserves bigint identity as text", () => {
  assert.equal(
    ...
    "1896a473adb43411c94da47226e2ed4b070043b152ebf92df903dee1073f338c",
  );
  ...
  assert.match(
    resolverMigration,
    /architecture_claim_finance_access_outbox_internal\(uuid,text,integer,uuid\)/,
  );
  assert.match(
    resolverMigration,
...
test("harness inputs are regular files and runner fails closed", () => {
  ...
  assert.ok(
    runScript.indexOf('apply_file "$harness_dir/behavior_smoke.sql"') <
      runScript.indexOf('apply_file "$outbox_migration"'),
    "foundation behavior must execute before outbox ACL hardening",
  );
  assert.doesNotMatch(
    runScript,
...
test("outbox behavior covers ordered delivery, audit, retry and dead letter", () => {
  ...
  assert.match(outboxBehaviorSmoke, /stale expected version changed desired state/);
  assert.match(outboxBehaviorSmoke, /read-only desired-state status was not exact/);
  assert.match(outboxBehaviorSmoke, /targeted claim bypassed an earlier user version/);
  ...
  assert.match(runScript, /service_role unexpectedly bypassed the outbox through legacy upsert/);
});
...
test("external postflight and fingerprint cover semantic catalog state", () => {
  ...
  assert.match(fingerprint, /architecture_get_finance_access_status_internal/);
