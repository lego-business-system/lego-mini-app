...
test("dependencies are exact, examples contain no secrets and frontend is fail-closed", () => {
  ...
  assert.match(pilotTemplate, /finance-pilot-config\.js[\s\S]*architecture-finance\.js/);
  assert.doesNotMatch(pilotTemplate, /(?:^|["'/])app\.js|forum\.js|business-architecture/);
  assert.match(financePilotShell, /source\.environment !== "staging"/);
  assert.match(financePilotShell, /root\.location\.origin !== publicOrigin/);
  assert.doesNotMatch(financePilotShell, /soxtekhspohkddpdidvp/);
