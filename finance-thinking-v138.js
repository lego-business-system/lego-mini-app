...
(function(){
  "use strict";
  const RELEASE="finance-thinking-v138-20260724-r2";
  const FILES=[
  ...
  async function start(){
    const parts=await Promise.all(FILES.map(async path=>{
      ...
      if(!response.ok)throw new Error("Не удалось загрузить финансовый модуль: "+path+" ("+response.status+")");
  ...
  window.FinanceThinkingV138Ready=start().catch(error=>{
